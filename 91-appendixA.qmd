...
# Statistical Background {#sec-appendixA}
...
## Basic statistical terms {#sec-appendix-stat-terms}
...
### Standard deviation and variance {#sec-appendix-sd-variance}
...
The *five-number summary* consists of five summary statistics: the minimum, the first quartile AKA 25th percentile, the second quartile AKA median or 50th percentile, the third quartile AKA 75th percentile, and the maximum. The five-number summary of a variable is used when constructing boxplots, as seen in @sec-boxplots.
...
Recall from @sec-viz that we can visualize the distribution of a numerical variable using binning in a histogram and that we can visualize the distribution of a categorical variable using a barplot.
...
Let's next discuss one particular kind of distribution: \index{distribution!normal} *normal distributions*. Such bell-shaped distributions are defined by two values: (1) the *mean* $\mu$ ("mu") which locates the center of the distribution and (2) the *standard deviation* $\sigma$ ("sigma") which determines the variation of the distribution. In @fig-normal-curves2, we plot three normal distributions where:
...
```{r fig-normal-curves2, fig.alt="Three normal density curves overlaid: a tall narrow one centered at 5 (small SD), a wider one also at 5 (larger SD), and a tall narrow one shifted to 15 (different mean, small SD).", echo=FALSE, fig.cap="Three normal distributions.", purl=FALSE, out.width="90%"}
all_points <- tibble(
...
```{r fig-normal-rule-of-thumb, fig.alt="Standard normal curve with vertical lines at -3, -2, -1, 0, 1, 2, 3 standard deviations, each segment shaded and labeled with the corresponding area — the empirical 68-95-99.7 rule.", echo=FALSE, fig.cap="Rules of thumb about areas under normal curves.", purl=FALSE, out.width = "80%"}
...
```{r normal-curve-shaded-3, fig.alt="Standard normal density curve from z = -4 to 4. The central region between z = -2 and z = 2 is shaded grey to highlight the middle 95% of the distribution, with the two tail regions outside left unshaded; an annotation marks the upper boundary as `q`."}
...
## log10 transformations {#sec-appendix-log10-transformations}
...
Log transformations allow us to focus on changes in *orders of magnitude*. In other words, they allow us to focus on *multiplicative changes* instead of *additive ones*. Let's illustrate this idea in @tbl-logten with examples of prices of consumer goods in 2019 US dollars. 
...
Let's make some remarks about log10 transformations based on @tbl-logten:
