...
* *Are you curious about the publishing of this book? Then, read the "About this book" section for more information on the open-source technology.*
...
We present a map of your upcoming journey in @fig-moderndive-figure. 


```{r fig-moderndive-figure, fig.alt="Flowchart showing the book's structure: Getting started with data, then Data science with tidyverse (visualization, wrangling, tidy data), Statistical modeling with moderndive (simple and multiple regression), and Statistical inference with infer (sampling, confidence intervals, hypothesis testing, inference for regression), concluding with telling your story with data.", fig.align="center", fig.cap="*ModernDive* flowchart.", echo=FALSE, purl=FALSE, out.width="100%"}
include_graphics("images/flowcharts/flowchart/flowchart.002.png")
```

You'll first get started with data in [Chapter 1 (Getting Started with Data in R)](01-getting-started.html#sec-getting-started) where you'll learn about the difference between R and RStudio, start coding in R, install and load your first R packages, and explore your first dataset: all domestic departure `flights` from a New York City airport in 2023. Then you'll cover the following three portions of this book (Parts 2 and 4 are combined into a single portion):

1. Data science with `tidyverse`. You'll assemble your data science toolbox using `tidyverse` packages. In particular, you'll
    + [Chapter 2 (Data Visualization)](02-visualization.html#sec-viz): Visualize data using the `ggplot2` package.
    + [Chapter 3 (Data Wrangling)](03-wrangling.html#sec-wrangling): Wrangle data using the `dplyr` package.
    + [Chapter 4 (Data Importing and Tidy Data)](04-tidy.html#sec-tidy): Learn about the concept of "tidy" data as a standardized data input and output format for all packages in the `tidyverse`. Furthermore, you'll learn how to import spreadsheet files into R using the `readr` package.
1. Statistical/Data modeling with `moderndive`. Using these data science tools and helper functions from the `moderndive` package, you'll fit your first data models. In particular, you'll
    + [Chapter 5 (Simple Linear Regression)](05-regression.html#sec-regression): Discover basic regression models with only one explanatory variable.
    + [Chapter 6 (Multiple Regression)](06-multiple-regression.html#sec-multiple-regression): Examine multiple regression models with more than one explanatory variable. 
1. Statistical inference with `infer`. Once again using your newly acquired data science tools, you'll unpack statistical inference using the `infer` package. In particular, you'll:
    + [Chapter 7 (Sampling)](07-sampling.html#sec-sampling): Learn about the role that sampling variability plays in statistical inference and the role that sample size plays in this sampling variability.
    + [Chapter 8 (Estimation, Confidence Intervals, and Bootstrapping)](08-confidence-intervals.html#sec-confidence-intervals): Construct confidence intervals using bootstrapping.
    + [Chapter 9 (Hypothesis Testing)](09-hypothesis-testing.html#sec-hypothesis-testing): Conduct hypothesis tests using permutation.
1. Statistical/Data modeling with `moderndive` (revisited): Armed with your understanding of statistical inference, you'll revisit and review the models you've constructed in [Chapter 5 (Simple Linear Regression)](05-regression.html#sec-regression) and [Chapter 6 (Multiple Regression)](06-multiple-regression.html#sec-multiple-regression). In particular, you'll:
    + [Chapter 10 (Inference for Regression)](10-inference-for-regression.html#sec-inference-for-regression): Interpret confidence intervals and hypothesis tests in a regression setting.

We'll end with a discussion on what it means to "tell your story with data" in [Chapter 11 (Tell Your Story with Data)](11-tell-your-story-with-data.html#sec-thinking-with-data) by presenting example case studies.^[Note that you'll see different versions of the word "ModernDive" in this book:
...
### What we hope you will learn from this book {.unnumbered}
...
These sub-fields are summarized in what Garrett Grolemund \index{Grolemund, Garrett} and Hadley Wickham \index{Wickham, Hadley} have previously termed the ["data/science pipeline"](http://r4ds.had.co.nz/explore-intro.html) in @fig-pipeline-figure.

```{r fig-pipeline-figure, fig.alt="Diagram of the data science pipeline as a cycle: Import → Tidy → Understand (which loops through Transform, Visualize, Model) → Communicate, with Program supporting every stage.", fig.align="center", fig.cap="Data/science pipeline.", echo=FALSE, purl=FALSE, out.width="100%"}
...
### Final note for students {.unnumbered}
...
## Introduction for instructors {.unnumbered}
...
### Why did we write this book? {.unnumbered}
...
1. **Focus on the entire data/science research pipeline**
    + We believe that the entirety of Grolemund and Wickham's [data/science pipeline](http://r4ds.had.co.nz/introduction.html) \index{data science pipeline} as seen in @fig-pipeline-figure should be taught.
...
## Connect and contribute {.unnumbered}
...
## About this book {#sec-about-the-book .unnumbered}

This book was written using the [quarto](https://quarto.org/) system. This system simplifies book publishing by having all content written in plain text that can be converted into different output formats. The source code of ModernDive is available on GitHub:

* **Latest online version**
    + Errata and updates will be made to the online text. They will be tracked on GitHub via the NEWS.md file [here](https://github.com/moderndive/ModernDive_book/blob/v2/NEWS.md).
    + Available at <https://moderndive.com/v2/>
    + The online version has continued to evolve since Version 2.0.0 went to print. Here is a summary of the larger additions, with full details in the NEWS.md file linked above:
        - **Rebuilt with Quarto:** The online edition now uses the Quarto publishing system, bringing dark mode, a click-to-enlarge lightbox for figures, function names in code blocks that link to their package documentation, and hover previews for citations.
        - **Run R in your browser:** Interactive code cells powered by webR appear throughout the chapters and Appendix B. Click *Run Code* to execute R right on the page — no installation needed — then edit and re-run to experiment. The companion data packages load in the browser as well, so `library()` calls work just as they do in RStudio.
        - **Quick checks:** 115 multiple-choice questions (ten to twelve per chapter) placed right before each Conclusion, with collapsible answers. Wrong-answer options deliberately reflect common student misconceptions, and each answer explains why the tempting wrong choices are wrong.
        - **End-of-chapter exercises:** Over 500 new exercises across the eleven chapters, each marked by difficulty (★ warm-up, ★★ standard, ★★★ critical thinking) and organized by the book sections they cover. Most code-based exercises include their own in-browser sandbox.
        - **New datasets:** Five datasets introduced gradually from Chapter 1 onward for the exercises: modern Olympic athletes ([`olympicAthletes`](https://cran.r-project.org/package=olympicAthletes)), Rick Steves' Europe episodes ([`steves`](https://cran.r-project.org/package=steves)), confirmed exoplanets ([`exoplanetdata`](https://cran.r-project.org/package=exoplanetdata)), Holocene volcanoes ([`volcanoes`](https://cran.r-project.org/package=volcanoes)), and `bob_ross` from the `fivethirtyeight` package.
...
## Versions of R packages used {.unnumbered}
...
```{r preface-load-internal, message=FALSE, echo=FALSE, purl=FALSE}
...
```{r preface-install-package, eval=FALSE}
