...
include_graphics("images/SEATAC_temperatures.png")
```
...
Based on the scatterplot visualization, there seems to be a weak negative relationship between age and teaching score. As age increases, the teaching score seems to decrease slightly. 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Why is it that our 33 groups of friends did not all have the same numbers of balls that were red out of 50, and hence different proportions red?
  ...
  **Solution**:  
  ...
  Because each group drew a different random sample of 50 balls from the bowl. This chance variation from sample to sample is *sampling variation*: it caused the number of red balls, and hence the proportion red, to differ from group to group. 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What would performing a census in our bowl activity correspond to? Why did we not perform a census?
  ...
  **Solution**: 
  ...
  Performing a census in our bowl activity corresponds to going through all of the bowl's balls and counting the number that are red. We did not perform a census because doing so would be time-consuming and tedious; the whole point of sampling is to avoid an exhaustive count. 
...
By thoroughly mixing the bowl's contents before each use of the shovel, so that every ball had an equal chance of being sampled. 

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Why is it important that sampling be done *at random*?

**Solution**: 

So that the sample is unbiased and representative of the population. Only then can results based on the sample *generalize* to the population, making the point estimate a "good guess" of the unknown population parameter. 

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What are we *inferring* about the bowl based on the samples using the shovel?

**Solution**: 

We are *inferring* the value of the population proportion $p$ of the bowl's balls that are red, using the sample proportion $\widehat{p}$ of the shovel's balls that are red as our estimate. 
...
Accuracy describes how "on target" our estimates are: whether the estimates are centered at the true value of the population parameter, rather than systematically off to one side. Precision describes how "consistent" our estimates are: how little they vary from sample to sample. An estimate can be precise without being accurate (consistently missing in the same direction), and accurate without being precise (centered at the right value but highly variable). 

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** How do we ensure that an estimate is *accurate*? How do we ensure that an estimate is *precise*?

**Solution**:

We ensure that an estimate is *accurate* by sampling at random: random sampling is unbiased, so the estimates are centered at the true value of the population parameter. We ensure that an estimate is *precise* by using as large a sample size as we can afford: as the sample size increases, the standard error decreases, and thus the estimates vary less from sample to sample. 
...
The 1000 repeated samples were a simulation used to study (1) the effect of *sampling variation* on our estimates and (2) the effect of sample size on that sampling variation. In real life we only get one sample and never observe the sampling distribution, so these simulations built our understanding of how much a point estimate based on a single sample can be expected to vary. 
...
A *sampling distribution* is constructed by taking many samples from the population, whereas a *bootstrap distribution* is constructed by resampling *with replacement* from a *single* sample, with each resample of the same size as the original sample. The bootstrap distribution will likely have a similar shape and spread as the sampling distribution, making it a good approximation for computing the standard error, but it is centered at the original sample's statistic rather than at the true value of the population parameter.
...
*Most* values lie between 1990 and 2000. 
...
The $p$-value is the probability of observing a difference in sample promotion rates as large as or larger than the 29.2% difference we observed in favor of résumés with male names, *assuming the null hypothesis is true*. In other words, it quantifies how surprising our observed difference would be in a hypothesized universe where there is no difference in promotion rates. 
...
From the faceted histogram, we can see the full shape of each genre's `rating` distribution — for example any skewness, multiple modes, or gaps — whereas the boxplot only shows the five-number summary and outliers. 
...
The $p$-value is the probability of observing a difference in sample mean ratings between action and romance movies as extreme as or more extreme than the difference we observed (in either direction, since $H_A$ is two-sided), *assuming the null hypothesis is true* that action and romance movies have the same mean rating on IMDb. 
...
```{r non-linear2, fig.cap="Scatterplot of teaching score over age.", echo=FALSE, fig.height=3.3}
set.seed(76)
evals_ch5 %>%
  ...
  expand_limits(y = 10)
```
The relationship between `score` and `age` appears to be weakly negative and roughly linear, though with a large amount of scatter about the line. 
...
The residuals appear roughly normally distributed, though with a slight left skew.
...
```{r numxplot62, echo=FALSE, warning=FALSE, fig.cap="Plot of residuals over age."}
