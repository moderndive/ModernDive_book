...
ggplot(gapminder2007, aes(x = continent, y = gdpPercap)) +
  geom_boxplot() +
  labs(x = "Continent", y = "GPD per capita",
...
Failing to prove the defendent is guilty does not equal to proving that the defendant is innocent. There will always be the possibility of making errors in the trial. 
...
The smaller $\alpha$ of 0.01 will lead to a more libral hypothesis testing procedure, because the required p-value for reject the null hypothesis $H_0$ is smaller. 
...
* 95th percentile: Starting from the left of Figure \@ref(fig:normal-rule-of-thumb), since 0.15% + 2.35% + 13.5% + 34% + 34% + 13.5% = 97.5%, the 97.5th percentile is z = +2. However, this is in standard units. Thus we need the value in the normal distribution that is two standard deviations higher than the mean: $\mu + 2 \cdot\sigma = 6 + 2 \cdot 3 = 12$.
