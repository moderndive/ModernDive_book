...
action_romance <- movies %>%
  ...
  # filter(Action + Romance == 1) %>%
  mutate(genre = ifelse(Action == 1, "Action", "Romance"))
```

Note above:

1. In the `filter()` command that `(Action == 1 | Romance == 1)` picks out the movies that are either action or romance while the `!(Action == 1 & Romance == 1)` leaves out the movies that are classified as both. Alternatively, as only one of `Action` and `Romance` can be 1, the sum of these two variables for those movies of either genre will be 1. Therefore, you can also use the commented-out filter to pick out the movies.
2. In the `mutate()` we use `ifelse()` to create a new variable `genre` whose values are either `"Action"` or `"Romance"`.
