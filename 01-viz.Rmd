...
At the beginning of this chapter and all subsequent chapters, we'll always have a list of packages you should have installed and loaded. Let's load all the packages needed for this chapter (this assumes you've already installed them). Read Chapter \@ref(packages) for information on how to install and load R packages.
...
There are other components of the Grammar of Graphics we can control.  As you start to delve deeper into the Grammar of Graphics, you'll start to encounter these topics more and more often. In this book however we'll only work with the first two; the other components are left to a more advanced text.
...
In Figure \@ref(fig:noalpha) we see that a positive relationship exists between `dep_delay` and `arr_delay`: as departure delays increase, arrival delays tend to also increase.  We also note that the majority of points fall near the point (0, 0).  There is a large mass of points clustered there. Let's break down this keeping in mind our discussion in Chapter \@ref(grammarofgraphics):
...
This one aspect of creating barplots using `ggplot2` that causes the most initial confusion: when the categorical variable you want to plot is not pre-tabulated in your data frame you need to use `geom_bar`, but if the categorical variable is pre-tabulted and stored in a variable, you need to use `geom_col` and explicitly map this variable to the `y` aesthetic.
...
In this table, the counts of the carriers are pre-tabulated. To create a barchart using the data frame `flights_table`, we use `geom_col` and map the `y` aesthetic to the variable `n`. Compare this barplot using `geom_col` in Figure \@ref(fig:flightscol) with the earlier barplot using `geom_bar` in Figure \@ref(fig:flightsbar). They are identical.
...
We begin by getting the names of the airports in NYC that were included in the `flights` dataset. Here, we preview the `inner_join` function from Chapter \@ref(wrangling). This function will join the data frame `flights` with the data frame `airports` by match rows that have the same airport code. However, in `flights` the airport code is included in the `origin` variable whereas in `airports` the airport code is included in the `faa` variable. We will revisit such examples in Chapter \@ref(joins) on joining data sets.
