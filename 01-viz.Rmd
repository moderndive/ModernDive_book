...
There are other components of the Grammar of Graphics we can control. As you start getting more familiar with the Grammar of Graphics and `ggplot` you'll need to learn about these topics, but for now, they can be mostly left to a more intemediate level textbook.
...
There are other attributes that can be customized in graphics, including the plot title, axes labels, and visual themes for the plot. In general, the Grammar of Graphics allows for a high degree of customization and also a consistent framework for easy updating/modification of plots.
...
For our purposes, we will be limiting consideration to five different types of graphs (Note: in this text we use the terms "graphs", "plots", and "charts" interchangeably).  We term these five named graphs the **5NG**:
...
Scatter-plots display the relationship between two continuous variables and is among the most commonly used plot because they can provide an immediate way to see the trend in one variable versus another.  However, if you try to create a scatter-plot where either one of the two variables is not quantitative, you will get strange results.  Be careful!
...
**`r paste0("(LC", chap, ".", (lc), ")")`: Does the `temp` variable in the `weather` data set have a lot of variability? Why do you say that?**
...
Both histograms and boxplots represent ways to visualize the variability of continuous variables. Another common task is to present the distribution of a categorical variable. This is a simpler task, focused on how many elements from the data fall into different categories of the categorical variable.
