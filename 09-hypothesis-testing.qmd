---
description: &desc "Conduct permutation-based hypothesis tests with the infer workflow and learn to interpret p-values without the common traps."
open-graph:
  description: *desc
twitter-card:
  description: *desc
...
```{r lc-inline-setup-09, include=FALSE, purl=FALSE}
...
We have studied confidence intervals in @sec-confidence-intervals. We now introduce hypothesis testing, another widely used method for statistical inference. A claim is made about a value or characteristic of the population and then a random sample is used to infer about the plausibility of this claim or hypothesis. For example, in @sec-ht-activity, we use data collected from Spotify to investigate whether metal music is more popular than deep-house music.

Many of the relevant concepts, ideas, and we have already introduced many of the necessary concepts to understand hypothesis testing in @sec-sampling and @sec-confidence-intervals. We can now expand further on these ideas and provide a general framework for understanding hypothesis tests. By understanding this general framework, you will be able to adapt it to many different scenarios.
...
In @sec-tying-CI-hypo we review confidence intervals and introduce hypothesis tests for one-sample problems; in particular, for the mean $\mu$. We use both theory-based and simulation-based approaches, and we provide some justification why we consider it a better idea to carefully unpack the simulation-based approach for hypothesis testing in the context of two-sample problems. We also show the direct link between confidence intervals and hypothesis tests. In @sec-ht-activity we introduce the activity that motivates the simulation-based approach for two-sample problems, data collected from Spotify to investigate whether metal music is more popular than deep-house music. In @sec-understanding-ht, @sec-ht-infer, and @sec-ht-interpretation we explain, conduct, and interpret hypothesis tests, respectively, using the simulation-based approach of permutation. We introduce a case study in @sec-ht-case-study, and in @sec-nhst-conclusion we conclude with a discussion of the theory-based approach for two-sample problems and some additional remarks.
...
## Needed packages {.unnumbered #sec-nhst-packages}
...
## Tying confidence intervals to hypothesis testing {#sec-tying-CI-hypo}

In @sec-confidence-intervals, we used a random sample to construct an
...
Hypothesis testing takes advantage of similar tools but the nature and the goal 
...
### The one-sample hypothesis test for the population mean {#sec-one-sample-hyp}

Let's continue working with the population mean, $\mu$. 
In @sec-confidence-intervals, we used a random sample to construct a 
...
#### The theory-based approach {.unnumbered}
...
We recall that due to the Central Limit Theorem described in @sec-central-limit-theorem, the sample mean weight of almonds, $\overline X$, 
...
$s/\sqrt{n}$. As presented in @sec-t-distribution-CI, the $t$-test 
statistic
...
in this code next. @tbl-hypo-test-1 presents these values.
...
```{r tbl-hypo-test-1, fig.cap="The t test statistic.", echo=FALSE, purl=FALSE}
...
The shaded regions on both tails of the $t$ distribution in @fig-t-curve-hypo represent the probability of these extreme values.

```{r fig-t-curve-hypo, fig.alt="Standard t-curve with both tails shaded beyond the observed test statistic, illustrating the rejection regions for a two-tailed hypothesis test.", echo=FALSE, fig.height=ifelse(is_latex_output(), 1.5, 4), fig.width=3, purl=FALSE, fig.cap="The tails of a t curve for this hypothesis test."}
...
only if each roll is a "one," and you score 100 otherwise. There is only a 1 in 
...
#### The simulation-based approach {.unnumbered}
...
For example, in @sec-t-distribution-CI we calculated the 95% confidence interval for the almonds' mean weight, $\mu$, using the sample `almonds_sample_100`. The theory-based approach was given by
...
## Music popularity activity {#sec-ht-activity}
...
The `moderndive` package contains the data on the songs by genre in the `spotify_by_genre` data frame. There are six genres selected in that data (`country`, `deep-house`, `dubstep`, `hip-hop`, `metal`, and `rock`). You will have the opportunity to explore relationships with the other genres and popularity in the *Learning checks*. Let's explore this data by focusing on just `metal` and `deep-house` by looking at 12 randomly selected rows and our columns of interest in @tbl-twelve-spotify. Note that we also group our selection so that each of the four possible groupings of `track_genre` and `popular_or_not` are selected.
...
```{r tbl-twelve-spotify, echo=FALSE}
spotify_metal_deephouse <- spotify_by_genre |> 
  filter(track_genre %in% c("metal", "deep-house")) |> 
...
The `track_genre` variable indicates what genre the song is classified under, the `artists` and `track_name` columns provide additional information about the track by providing the artist and the name of the song, `popularity` is the metric mentioned earlier given by Spotify, and `popular_or_not` is a categorical representation of the `popularity` column with any value above 50 (the 75th percentile of `popularity`) referring to `popular` and anything at or below 50 as `not popular`. (The decision made by the authors to call a song "popular" if it is above the 75th percentile (3rd quartile) of `popularity` is arbitrary and could be changed to any other value.)

Let's perform an exploratory data analysis of the relationship between the two categorical variables `track_genre` and `popular_or_not`. Recall that we saw in @sec-two-categ-barplot that one way we can visualize such a relationship is by using a stacked barplot. 
...
```{r fig-spotify-genre-barplot, fig.alt="Stacked-proportion barplot showing the share of popular vs not-popular songs in two music genres (metal and deep house).", echo=FALSE, fig.cap="Barplot relating genre to popularity.", fig.height=ifelse(knitr::is_latex_output(), 1.6, 4), purl=FALSE}
...
```{r tbl-spotify-52-sample, echo=FALSE}
spotify_52_original |> 
  select(-track_id) |> 
  head(10) |> 
...
```{r tbl-spotify-shuffled-52-sample, echo=FALSE}
spotify_52_shuffled |> 
  select(-track_id) |> 
  head(10) |> 
...
Observe in @tbl-spotify-shuffled-52-sample that the `popular_or_not` column how the `popular` and `not popular` results are now listed in a different order. Some of the original `popular` now are `not popular`, some of the `not popular` are `popular`, and others are the same as the original. 

Again, such random shuffling of the `popular_or_not` label only makes sense in our hypothesized universe of no difference in popularity between genres. Is there a tactile way for us to understand what is going on with this shuffling? One way would be by using a standard deck of 52 playing cards, which we display in @fig-deck-of-cards.

```{r fig-deck-of-cards, fig.alt="Photograph of a standard 52-card deck spread out, used as an analogy for permutation testing.", echo=FALSE, fig.cap="Standard deck of 52 playing cards.", purl=FALSE, out.width="60%"}
include_graphics("images/shutterstock/shutterstock_670789453.jpg")
```

Since we started with equal sample sizes of 1000 songs for each genre, we can think about splitting the deck in half to have 26 cards in two piles (one for `metal` and another for `deep-house`). After shuffling these 52 cards as seen in @fig-shuffling, we split the deck equally into the two piles of 26 cards each. Then, we can flip the cards over one-by-one, assigning "popular" for each red card and "not popular" for each black card keeping a tally of how many of each genre are popular.

```{r fig-shuffling, fig.alt="Photograph of a deck of cards being shuffled, illustrating the permutation step in the infer workflow.", echo=FALSE, fig.cap="Shuffling a deck of cards.", purl=FALSE, out.width="50%"}
include_graphics("images/shutterstock/shutterstock_128283971.jpg")
```

Let's repeat the same exploratory data analysis we did for the original `spotify_metal_deephouse` data on our `spotify_52_original` and `spotify_52_shuffled` data frames. Let's create a barplot visualizing the relationship between `track_genre` and the new shuffled `popular_or_not` variable, and compare this to the original un-shuffled version in @fig-spotify-genre-barplot-permuted.
...
ggplot(spotify_52_shuffled, aes(x = track_genre, fill = popular_or_not)) +
  ...
  labs(x = "Genre of track")
```
```{r fig-spotify-genre-barplot-permuted, fig.alt="Two side-by-side stacked barplots: the observed relationship between genre and popularity (left) and the same data after randomly shuffling the popularity labels (right). The shuffled version shows no clear difference between genres.", fig.cap="Barplots of relationship of genre with `popular or not' (left) and shuffled `popular or not' (right).", fig.height=ifelse(knitr::is_latex_output(), 2.6, 4), echo=FALSE, purl=FALSE, message=FALSE}
...
$$\widehat{p}_{m} -\widehat{p}_{d} = `r p_metal_popular` - `r p_deephouse_popular` = `r observed_test_statistic`.$$ 
...
Much like the terminology, notation, and definitions relating to sampling you saw in @sec-sampling-framework, there are a lot of terminology, notation, and definitions related to hypothesis testing as well. Some of this was introduced in @sec-tying-CI-hypo. Learning these may seem like a very daunting task at first. However, with practice, practice, and more practice, anyone can master them. 

First, a **hypothesis** \index{hypothesis testing!hypothesis} is a statement about the value of an unknown population parameter. In our genre popularity activity, our population parameter of interest is the difference in population proportions $p_{m} - p_{d}$. Hypothesis tests can involve any of the population parameters in @tbl-table-ch8-c of the five inference scenarios we will cover in this book and also more advanced types we will not cover here.
...
Observe how the alternative hypothesis $H_A$ is written $p_{m} - p_{d} > 0$. Since we have chosen this particular formulation, the one-sided test becomes *right-sided* because we are looking for a difference that is greater than zero as evidence to reject the null hypothesis. Had we opted for a two-sided alternative, we would have set $p_{m} - p_{d} \neq 0$. We work here with the right-sided test and will present an example of a two-sided test in @sec-ht-case-study.

Third, a **test statistic** \index{hypothesis testing!test statistic} is a *point estimate/sample statistic* formula used for hypothesis testing. Note that a sample statistic is merely a summary statistic based on a sample of observations. Recall we saw in @sec-summarize that a summary statistic takes in many values and returns only one. Here, the samples would be the $n_m = `r n_songs_genre`$ metal songs and the $n_d = `r n_songs_genre`$ deep house songs. Hence, the point estimate of interest is the difference in sample proportions $\widehat{p}_{m} - \widehat{p}_{d}$. 
...
**The $p$-value is not the probability that $H_0$ is true.** It's the probability of seeing data this extreme *if $H_0$ were true*. Two related traps to avoid:

- **"$p < 0.05$ means the alternative hypothesis is true."** It means the data would be unusual under $H_0$ — that's evidence against $H_0$, not proof of $H_a$.
- **"Failing to reject $H_0$ means $H_0$ is true."** It means we don't have enough evidence to rule it out. Absence of evidence is not evidence of absence.
:::
...
Seventh and lastly, in many hypothesis testing procedures, it is commonly recommended to set the **significance level** \index{hypothesis testing!significance level} of the test beforehand.  It is denoted by $\alpha$. Please review our discussion of $\alpha$ in @sec-one-sample-hyp when we discussed the theory-based approach. For now, it is sufficient to recall that if the $p$-value is less than or equal to $\alpha$, we reject the null hypothesis $H_0$. 

Alternatively, if the $p$-value is greater than  $\alpha$, we would "fail to reject $H_0$." Note the latter statement is not quite the same as saying we "accept $H_0$." This distinction is rather subtle and not immediately obvious. So we will revisit it later in @sec-ht-interpretation.

While different fields tend to use different values of $\alpha$, some commonly used values for $\alpha$ are 0.1, 0.01, and 0.05; with 0.05 being the choice people often make without putting much thought into it. We will talk more about $\alpha$ significance levels in @sec-ht-interpretation, but first let's fully conduct a hypothesis test corresponding to our genre popularity activity using the `infer` package.

## Conducting hypothesis tests {#sec-ht-infer}

In @sec-bootstrap-process, we showed you how to construct confidence intervals. We first illustrated how to do this using `dplyr` data wrangling verbs and the `rep_sample_n()` function from @sec-sampling-simulation which we used as a virtual shovel. In particular, we constructed confidence intervals by resampling with replacement by setting the `replace = TRUE` argument to the `rep_sample_n()` function. 

We then showed you how to perform the same task using the `infer` package workflow. While both workflows resulted in the same bootstrap distribution from which we can construct confidence intervals, the `infer` package workflow emphasizes each of the steps in the overall process in @fig-infer-ci. It does so using function names that are intuitively named with verbs:
...
```{r fig-infer-ci, fig.alt="Workflow diagram for confidence intervals using infer: specify → generate(bootstrap) → calculate → get_ci.", echo=FALSE, fig.cap="Confidence intervals with the infer package.", purl=FALSE, out.width="90%"}
include_graphics("images/flowcharts/infer/visualize.png")
```

In this section, we will now show you how to seamlessly modify the previously seen `infer` code for constructing confidence intervals to conduct hypothesis tests. You will notice that the basic outline of the workflow is almost identical, except for an additional `hypothesize()` step between the `specify()` and `generate()` steps, as can be seen in @fig-inferht.

```{r fig-inferht, fig.alt="Workflow diagram for hypothesis tests using infer: specify → hypothesize → generate(permute) → calculate → get_p_value.", echo=FALSE, fig.cap="Hypothesis testing with the infer package.", purl=FALSE, out.width="90%"}
...
Furthermore, we will use a pre-specified significance level $\alpha$ = `r alpha` for this hypothesis test. Please read the discussion about $\alpha$ in @sec-one-sample-hyp or later on in @sec-ht-interpretation. 


### `infer` package workflow {#sec-infer-workflow-ht}

#### 1. `specify` variables {.unnumbered}
...
Again, notice how the `spotify_metal_deephouse` data itself does not change, but the `Response: popular_or_not (factor)` and `Explanatory: track_genre (factor)` *meta-data* do. This is similar to how the `group_by()` verb from `dplyr` does not change the data, but only adds "grouping" meta-data, as we saw in @sec-groupby. We also now focus on only the two columns of interest in the data for our problem at hand with `popular_or_not` and `track_genre`.

#### 2. `hypothesize` the null {.unnumbered}

In order to conduct hypothesis tests using the `infer` workflow, we need a new step not present for confidence intervals: \index{R packages!infer!hypothesize()} `hypothesize()`. Recall from @sec-understanding-ht that our hypothesis test was
...
Where do the terms `"point"` and `"independence"` come from? These are two technical statistical terms. The term "point" relates to the fact that for a single group of observations, you will test the value of a single point. Going back to the almonds example from @sec-confidence-intervals, say we wanted to test if the mean weight of all chocolate-covered almonds was equal to 3.6 grams or not. We would be testing the value of a "point" $\mu$, the mean weight in grams of *all* chocolate-covered almonds, as follows
...
#### 3. `generate` replicates {.unnumbered}
...
#### 5. `visualize` the p-value {.unnumbered}
...
We start by visualizing the *null distribution* of our `r n_reps` values of $\widehat{p}_{m} - \widehat{p}_{d}$ using `visualize()` \index{R packages!infer!visualize()} in @fig-null-distribution-infer. Recall that these are values of the difference in popularity rates assuming $H_0$ is true. This corresponds to being in our hypothesized universe of no difference in genre popularity.

```{r fig-null-distribution-infer, fig.alt="Histogram of the null distribution of the difference in proportions, generated by 1000 permutations under the null of independence.", fig.show="hold", fig.cap="Null distribution.", fig.height=ifelse(knitr::is_latex_output(), 1.8, 4)}
...
On the other hand, had our alternative hypothesis $H_A$ been the other possible one-sided alternative $p_{m} - p_{d} < 0$, suggesting popularity in favor of deep house songs, we would have set `direction = "left"`.  Had our alternative hypothesis $H_A$ been two-sided $p_{m} - p_{d} \neq 0$, suggesting a difference in popularity in either direction, we would have set `direction = "both"`.

```{r fig-null-distribution-infer-2, fig.alt="Same null distribution histogram with the area beyond the observed test statistic shaded, that shaded area is the p-value.", fig.cap="Shaded histogram to show $p$-value."}
visualize(null_distribution, bins = 25) + 
...
Using this `bootstrap_distribution`, let's first compute the percentile-based confidence intervals, as we did in @sec-bootstrap-process:
...
Using our shorthand interpretation for 90% confidence intervals, we are 90% "confident" that the true difference in population proportions $p_{m} - p_{d}$ is between (`r percentile_ci[["lower_ci"]] |> round(4)`, `r percentile_ci[["upper_ci"]] |> round(4)`). Let's visualize `bootstrap_distribution` and this percentile-based 90% confidence interval for $p_{m} - p_{d}$ in @fig-bootstrap-distribution-two-prop-percentile.

```{r hypothesis-testing-viz-dist, eval=FALSE}
visualize(bootstrap_distribution) + 
  shade_confidence_interval(endpoints = percentile_ci)
```
```{r fig-bootstrap-distribution-two-prop-percentile, fig.alt="Bootstrap distribution of the difference in two proportions with the central 90% region shaded for the percentile-method confidence interval.", echo=FALSE, fig.show="hold", fig.cap="Percentile-based 90\\% confidence interval.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.2, 4)}
if (is_html_output()) {
  visualize(bootstrap_distribution) +
    shade_confidence_interval(endpoints = percentile_ci)
} else {
...
Since the bootstrap distribution appears to be roughly normally shaped, we can also use the standard error method as we did in @sec-bootstrap-process. In this case, we must specify the `point_estimate` argument as the observed difference in popularity rates `r observed_test_statistic` = `r observed_test_statistic*100`% saved in `obs_diff_prop`. This value acts as the center of the confidence interval.
...
Let's visualize `bootstrap_distribution` again, but now the standard error based 95% confidence interval for $p_{m} - p_{d}$ in @fig-bootstrap-distribution-two-prop-se. Again, notice how the value 0 is not included in our confidence interval, again suggesting that $p_{m}$ and $p_{d}$ are truly different!

```{r hypothesis-testing-viz-dist-alt, eval=FALSE}
visualize(bootstrap_distribution) + 
shade_confidence_interval(endpoints = se_ci)
```
```{r fig-bootstrap-distribution-two-prop-se, fig.alt="Bootstrap distribution of the difference in two proportions with a 95% confidence interval marked using the standard-error method.", eval=FALSE, echo=FALSE, fig.show="hold", fig.cap="Standard error-based 95\\% confidence interval.", purl=FALSE, fig.height=3.4}
if (is_html_output()) {
visualize(bootstrap_distribution) +
shade_confidence_interval(endpoints = se_ci) #+
...
```{r lc-sol-09-01, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 1))
```
...
```{r lc-sol-09-02, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 2))
```
...
```{r lc-sol-09-03, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 3))
```
...
```{r fig-htdowney, fig.alt="Allen Downey's flowchart titled 'There is only one test': decision tree showing how nearly every classical hypothesis test reduces to the same simulation-based logic.", echo=FALSE, fig.cap="Allen Downey's hypothesis testing framework.", purl=FALSE, out.width=ifelse(knitr::is_latex_output(), "85%", "100%")}
include_graphics("images/copyright/there_is_only_one_test.png")
```

Notice its similarity with the "hypothesis testing with `infer`" diagram you saw in @fig-inferht. That is because the `infer` package was explicitly designed to match the "There is only one test" framework. So if you can understand the framework, you can easily generalize these ideas for all hypothesis-testing scenarios. Whether for population proportions $p$, population means $\mu$, differences in population proportions $p_1 - p_2$, differences in population means $\mu_1 - \mu_2$, and as you will see in @sec-inference-for-regression on inference for regression, population regression slopes $\beta_1$ as well. In fact, it applies more generally even than just these examples to more complicated hypothesis tests and test statistics as well.
...
```{r lc-sol-09-04, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 4))
```
...
So going back to our songs activity in @sec-ht-infer, recall that our hypothesis test was $H_0: p_{m} - p_{d} = 0$ versus $H_A: p_{m} - p_{d} > 0$ and that we used a pre-specified significance level of $\alpha$ = `r alpha`.  We found a $p$-value of `r pull(p_value)`. Since the $p$-value was smaller than $\alpha$ = `r alpha`, we rejected $H_0$. In other words, we found needed levels of evidence in this particular sample to say that $H_0$ is false at the $\alpha$ = `r alpha` significance level. We also state this conclusion using non-statistical language: we found enough evidence in this data to suggest that there was a difference in the popularity of our two genres of music.
...
```{r fig-trial-errors-table, fig.alt="Two-by-two table illustrating Type I and Type II errors in criminal trials: rows are the verdict (guilty/not guilty), columns are the truth (innocent/guilty).", echo=FALSE, fig.cap="Type I and Type II errors in criminal trials.", purl=FALSE, out.width="95%"}
include_graphics("images/gt_error_table_v2.png")
```

Thus, a Type I error corresponds to incorrectly putting a truly innocent person in jail, whereas a Type II error corresponds to letting a truly guilty person go free. Let's show the corresponding table in @fig-hypo-test-errors for hypothesis tests.
...
```{r fig-hypo-test-errors, fig.alt="Same two-by-two table structure but for hypothesis tests: rows are the decision (reject H0 / fail to reject), columns are reality (H0 true / H0 false).", echo=FALSE, fig.cap="Type I and Type II errors in hypothesis tests.", purl=FALSE, out.width="95%"}
...
- A Type I Error\index{hypothesis testing!Type I Error} is rejecting the null hypothesis when it is true. The probability of a Type I Error occurring is $\alpha$, the *significance level*, which we defined in @sec-one-sample-hyp and in @sec-understanding-ht
...
```{r lc-sol-09-08, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 8))
```

:::

## Case study: are action or romance movies rated higher? {#sec-ht-case-study}
...
### IMDb ratings data {#sec-imdb-data}
...
Let's perform an exploratory data analysis of this data. Recall from @sec-geomboxplot that a boxplot is a visualization we can use to show the relationship between a numerical and a categorical variable. Another option you saw in @sec-facets would be to use a faceted histogram. However, in the interest of brevity, let's only present the boxplot in @fig-action-romance-boxplot.

```{r fig-action-romance-boxplot, fig.alt="Side-by-side boxplots of IMDb ratings for Action movies vs Romance movies. Romance has a slightly higher median; spreads are similar.", fig.cap="Boxplot of IMDb rating vs. genre.", fig.height=ifelse(knitr::is_latex_output(), 4, 4)}
ggplot(data = movies_sample, aes(x = genre, y = rating)) +
  geom_boxplot() +
  labs(y = "IMDb rating")
```

Eyeballing @fig-action-romance-boxplot, romance movies have a higher median rating. Do we have reason to believe, however, that there is a *significant* difference between the mean `rating` for action movies compared to romance movies?  It is hard to say just based on this plot. The boxplot does show that the median sample rating is higher for romance movies. 
...
Since this sample was randomly taken from the population `movies`, it is representative of all romance and action movies on IMDb. Thus, any analysis and results based on `movies_sample` can generalize to the entire population. What are the relevant *population parameter* and *point estimates*? We introduce the fourth sampling scenario in @tbl-summarytable-ch10. 

```{r tbl-summarytable-ch10, echo=FALSE, message=FALSE, purl=FALSE}
# The following Google Doc is published to CSV and loaded using read_csv():
# https://docs.google.com/spreadsheets/d/1QkOpnBGqOXGyJjwqx1T2O5G5D72wWGfWlPyufOgtkk4/edit#gid=0
...
So, whereas the sampling bowl exercise in @sec-sampling-activity concerned *proportions*, the almonds exercise in @sec-revisit-almond-bootstrap concerned *means*, the case study on whether yawning is contagious in @sec-case-study-two-prop-ci and the music genre activity in @sec-ht-activity concerned *differences in proportions*, we are now concerned with *differences in means*. 
...
We set the null hypothesis $H_0: \mu_a - \mu_r = 0$ by using the `hypothesize()` function. Since we have two samples, action and romance movies, we set `null` to be `"independence"` as we described in @sec-ht-infer.
...
```{r fig-null-distribution-movies-2, fig.alt="Null distribution of the difference in mean ratings (Action minus Romance) generated by permutation, with the observed test statistic marked and the right-tail p-value shaded.", echo=FALSE, fig.cap="Null distribution, observed test statistic, and $p$-value.", fig.height=ifelse(knitr::is_latex_output(), 1.8, 4), purl=FALSE}
if (is_html_output()) {
  visualize(null_distribution_movies, bins = 10) +
    shade_p_value(obs_stat = obs_diff_means, direction = "both")
} else {
...
```{r lc-sol-09-09, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 9))
```
...
```{r lc-sol-09-10, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 10))
```
...
```{r lc-sol-09-11, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 11))
```
...
```{r lc-sol-09-12, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 12))
```
...
```{r lc-sol-09-13, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 13))
```
...
```{r lc-sol-09-14, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 14))
```
...
```{r lc-sol-09-15, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(9, 15))
```
...
Twelve questions to assess your understanding. Several are designed around common misconceptions — read each option carefully before peeking at the answer. The last two prompts anchor the abstract framework in this chapter's two worked examples: the music-popularity activity (§ 9.2) and the IMDb case study (§ 9.6).

**Q9-1.** A small *p*-value (e.g., $p = 0.001$) means:

a. The chance the next study will replicate this result is high
b. The alternative hypothesis is proven
c. The null hypothesis is true with probability 0.001
d. The data would be unusual if the null hypothesis were true

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** $p$-values measure how surprising the data is *under* $H_0$, not the probability that $H_0$ is true. A small $p$-value is evidence against $H_0$, but it doesn't *prove* $H_a$ and doesn't tell you anything about the magnitude of the effect.
:::

**Q9-2.** A *Type I error* occurs when:

a. You reject a true null hypothesis
b. You measure something incorrectly
c. You fail to reject a false null hypothesis
d. You correctly reject the null hypothesis

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** Type I = rejecting $H_0$ when it's actually true, i.e. a "false positive". Its probability is the significance level $\alpha$ (commonly 0.05). Type II (option c) = failing to reject $H_0$ when it's actually false.
:::

**Q9-3.** A simulation-based hypothesis test computes the $p$-value as "share of *null* statistics at least as extreme as the observed one." For a **two-sided** alternative $H_A: \mu \neq \mu_0$, "as extreme as" means:

a. At least as far *below* the null center as the observed value, ignoring the upper tail
b. Exactly equal to the observed value and no other simulated statistic
c. At least as far *above* the null center as the observed value, ignoring the lower tail
d. At least as far from the null center as the observed value, in *either* direction

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** Options (a) and (c) count extremity in only one tail, so they describe one-sided tests; (b) would make the $p$-value essentially zero for any continuous statistic. A two-sided alternative cares about extremity in *both* tails, equally surprising whether the observed sample mean lands far above or far below $\mu_0$. The one-sided variants ($H_A: \mu > \mu_0$ or $H_A: \mu < \mu_0$) only count extremity in the *predicted* direction. When the observed statistic falls in that predicted direction, the resulting $p$-value is roughly half what a two-sided test would give on the same data, but when it falls the *other* way, the one-sided $p$-value is large (close to 1), not half. The choice of one- vs two-sided must be made *before* looking at the data.
:::

**Q9-4.** A hypothesis test has $\alpha = 0.05$ and you obtain $p = 0.04$. The correct conclusion:

a. $H_0$ is false with 96% probability
b. Reject $H_0$; the data are unusual under $H_0$
c. Accept $H_a$ (the alternative is proven)
d. The result will replicate the same way in the next study

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Be careful with wording. We *reject* $H_0$, not "accept $H_a$". And $p = 0.04$ doesn't say anything about replicability or the magnitude of any difference, only about how surprising the data are if $H_0$ were exactly true.
:::

**Q9-5.** Section 9.1 ties confidence intervals to hypothesis tests. A 95% CI for a parameter $\theta$ is $(2.1, 5.8)$. Which value of $\theta_0$ would the corresponding **two-sided** test fail to reject at $\alpha = 0.05$?
...
::: {.callout-tip collapse="true" title="Show answer"}
**(c)** The CI-test duality says: at level $\alpha$, a two-sided test rejects $H_0: \theta = \theta_0$ if and only if $\theta_0$ falls **outside** the corresponding $1 - \alpha$ CI. Since 4 lies *inside* the CI $(2.1, 5.8)$, the test fails to reject; the other three values lie outside and would be rejected. Useful shortcut: once you have a CI, you've effectively pre-computed the test for *every* candidate $\theta_0$ at once.
:::

**Q9-6.** Increasing the **sample size** in a study primarily affects:

a. The significance level $\alpha$
b. The probability of Type I error
c. The probability of Type II error
d. Neither error rate; only the size of the point estimate

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Larger $n$ *lowers* the Type II error rate. Recall from Chapter 7 that larger samples produce narrower sampling distributions. With less sampling variability, a real difference from $H_0$ is easier to detect, so the test fails to reject a false $H_0$ less often, that is, a lower Type II error rate. Type I error is set by the chosen $\alpha$, not by $n$.
:::

**Q9-7.** A research team reports a "statistically significant" result with $p = 0.03$ and $\alpha = 0.05$. Which of the following is the BEST interpretation of what the test has shown?

a. The alternative hypothesis $H_A$ has been shown to be exactly true for the population
b. The probability that the null hypothesis is true is 0.03
c. The data are unlikely enough under $H_0$ to reject $H_0$ at $\alpha = 0.05$
d. The null hypothesis has been proven definitively false

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Hypothesis testing is *binary*: reject or fail to reject. A small $p$-value says the observed data would be rare if $H_0$ were exactly true, so by the decision rule we reject $H_0$. What the test *doesn't* tell you is how far off from $H_0$ the truth actually is; the gap could be tiny or large. (CIs from Chapter 8 are the tool for answering "how much".)
:::

**Q9-8.** A permutation test for "is there a difference between groups" works by:

a. Using the t-distribution to compute the p-value directly
b. Computing the observed difference once and comparing it to zero
c. Resampling the rows with replacement and recomputing the difference each time
d. Repeatedly shuffling the group labels and recomputing the difference

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** Shuffling labels destroys any real association between group and outcome, so it simulates the world where $H_0$ (no association) is true. The distribution of test statistics across many shuffles *is* the empirical null distribution, against which the observed difference is compared. Option (c) describes the bootstrap, which resamples *with replacement* and is the Chapter 8 tool for CIs, not a null distribution.
:::

**Q9-9.** You run 20 *independent* hypothesis tests at $\alpha = 0.05$, and ALL 20 null hypotheses are actually TRUE. How many "significant" results do you expect?
...
::: {.callout-tip collapse="true" title="Show answer"}
**(a)** $\alpha = 0.05$ means the chapter's decision rule wrongly rejects $H_0$, a Type I error, 5% of the time when $H_0$ is in fact true. Run 20 independent tests at $\alpha = 0.05$ and you expect $20 \times 0.05 = 1$ false rejection on average, even when nothing real is happening. Takeaway: looking at lots of tests in parallel inflates the chance you'll see at least one "significant" result by chance alone.
:::

**Q9-10.** Two studies report different hypothesis-test outcomes: Study A rejects $H_0$ with $p = 0.001$. Study B *fails* to reject $H_0$ with $p = 0.40$. What's the BEST way to interpret Study B's result?

a. There's not enough evidence in the data to reject $H_0$
b. Study B contradicts Study A and one of them is wrong
c. The data must come from a population where $H_0$ holds exactly
d. The null hypothesis has been proven true

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** Hypothesis testing is asymmetric: a small $p$-value gives you evidence to *reject* $H_0$, but a large $p$-value only tells you the data are **consistent** with $H_0$ — not that $H_0$ is true. The truth could still be a small step away from $H_0$ in a direction the sample didn't have enough resolution to detect. "Fail to reject" and "accept" are different conclusions, always use "fail to reject" in your write-up.
:::

**Q9-11.** In Section 9.2, the metal-vs-deep-house comparison shuffled the `popular_or_not` values across the rows of `spotify_metal_deephouse` and recomputed the difference in the proportion of popular tracks each time. What is the **shuffling** simulating?

a. The population distribution of popularity for *all* Spotify songs
b. The world in which $H_0$ is true and `track_genre` is unrelated to `popular_or_not`
c. The act of collecting a brand-new random sample of metal and deep-house tracks from Spotify
d. A bootstrap of the original sample, with replacement

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Shuffling breaks any real association between the labels and the response. Under the null hypothesis "metal and deep-house songs have the same proportion of popular tracks," the genre label is just an arbitrary tag; the rows would be statistically interchangeable. Each shuffled replicate gives you one plausible difference-in-proportions if the null were true; collecting many of them builds the *null distribution* against which the observed difference is compared. Bootstrap (option c) uses *replacement* and a different setup, that's the Chapter 8 mechanism for CIs, not the Chapter 9 mechanism for testing.
:::

**Q9-12.** Section 9.6's IMDb case study asks "Are action or romance movies rated higher on average?" After running the two-sample permutation test on `movies_sample`, you obtain $p \approx 0.004$ for $H_0: \mu_{\text{action}} = \mu_{\text{romance}}$ versus a two-sided alternative. Suppose that *before seeing the data* you had pre-specified $\alpha = 0.05$, a less conservative level than the chapter's 0.001. The MOST defensible conclusion is:

a. The $p$-value of 0.004 means there is a 99.6 % chance that romance movies are rated higher
b. Reject $H_0$ at $\alpha = 0.05$; the evidence points to a difference in mean rating
c. The IMDb user population gives romance movies higher ratings *because* of the genre
d. The test proves romance movies are objectively better than action movies

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** At the pre-specified $\alpha = 0.05$, a $p$-value of about 0.004 is small enough to reject the null that the two population means are equal — that's exactly the decision the test was designed to make. What it establishes is evidence that the *mean rating* differs between action and romance movies in the broader IMDb population, and nothing more. It does **not** speak to causality (option c — IMDb raters self-select, so the rating gap could reflect *who* watches and rates each genre rather than the genre itself), it does **not** make a value judgment (option d), and it is **not** "the probability $H_0$ is true" (option a — a $p$-value is computed *assuming* $H_0$ is true, not as a probability statement about $H_0$). Note the contrast with the chapter itself: Section 9.6 pre-specified the much stricter $\alpha = 0.001$, and since $0.004 > 0.001$ it *fails to reject* $H_0$ there. Same data, same $p$-value, different decision: the conclusion depends on the significance level chosen *before* seeing the data.
:::

::: {.callout-tip title="Chapter cheatsheet"}

The `infer` workflow for hypothesis tests — chain these with `|>`:

| Step | Function | Quick example |
|---|---|---|
| 1. specify | `specify(formula, success = "...")` | `specify(popular_or_not ~ track_genre, success = "popular")` |
| 2. hypothesize | `hypothesize(null = "independence")` | for one-sample means: `null = "point", mu = 3.6` |
| 3. generate | `generate(reps = 1000, type = "permute")` | for point nulls: `type = "bootstrap"` |
| 4. calculate | `calculate(stat = "diff in props", order = c("a","b"))` | `"diff in means"`, `"prop"`, `"mean"`, ... |
| 5. visualize / p-value | `visualize() + shade_p_value(obs_stat, direction)` then `get_p_value(...)` | `direction = "right" / "left" / "two-sided"` |

Other null and generation types exist for other scenarios (see `?hypothesize` and `?generate`).

:::

## Exercises {.unnumbered #sec-ht-exercises}

The end-of-chapter exercises mix three datasets:

* `bob_ross` from `fivethirtyeight` (403 episodes × 71 binary indicators).
* `volcanoes` and `eruptions` from `volcanoes`.
* `olympic_athletes` from `olympicAthletes`.

Solutions are available to instructors separately.
...
Observe that the formula for $\text{SE}(\bar{x}_a - \bar{x}_r)$ has the sample sizes $n_a$ and $n_r$ in them. So as the sample sizes increase, the standard error goes down. We have seen this characteristic for one-sample problems in @sec-sampling-variation and @sec-random-variable-sample-mean when describing the sample distribution of the sample mean or the sample proportion and in @sec-theory-based-CI when discussing the sample size effect on confidence intervals. 
...
Let's compute the $t$-test statistic. Recall the summary statistics we computed during our exploratory data analysis in @sec-imdb-data.
...
1. This is met since the distributions of the ratings in our samples of $n_a$ = `r n_action` action movies and $n_r$ = `r n_romance` romance movies do not seem to be highly skewed and therefore are somewhat symmetric.
...
However, let's first construct an exploratory visualization as we suggested earlier. Since `air_time` is numerical and `carrier` is categorical, a boxplot can display the relationship between these two variables, which we display in @fig-ha-as-flights-boxplot.

```{r fig-ha-as-flights-boxplot, fig.alt="Side-by-side boxplots of air time (minutes) for Hawaiian and Alaska Airlines flights leaving NYC in 2023. Hawaiian flights are dramatically longer.", fig.cap="Air time for Hawaiian and Alaska Airlines flights departing NYC in 2023.", fig.height=ifelse(knitr::is_latex_output(), 2.8, 4)}
ggplot(data = flights_sample, mapping = aes(x = carrier, y = air_time)) +
  geom_boxplot() +
  labs(x = "Carrier", y = "Air Time")
```

This is what we like to call "no PhD in Statistics needed" moments. You do not have to be an expert in statistics to know that Alaska Airlines and Hawaiian Airlines have *notably* different air times. The two boxplots do not even overlap! Constructing a confidence interval or conducting a hypothesis test would frankly not provide much more insight than @fig-ha-as-flights-boxplot. 
...
On top of the many common misunderstandings about hypothesis testing and $p$-values we listed in @sec-ht-interpretation, another unfortunate consequence of the expanded use of $p$-values and hypothesis testing is a phenomenon known as "p-hacking." \index{p-hacking} p-hacking is the act of "cherry-picking" only results that are "statistically significant" while dismissing those that are not, even if at the expense of the scientific ideas. There are lots of articles written recently about misunderstandings and the problems with $p$-values. We encourage you to check some of them out:
...
We conclude with the `infer` pipeline for hypothesis testing in @fig-infer-workflow-ht. 

```{r fig-infer-workflow-ht, fig.alt="Detailed workflow diagram of the infer hypothesis-testing pipeline: specify → hypothesize → generate → calculate → visualize / get_p_value.", out.width="100%", echo=FALSE, fig.cap="infer package workflow for hypothesis testing.", purl=FALSE}
include_graphics("images/flowcharts/infer/ht_diagram_trimmed.png")
```

Now that we have armed ourselves with an understanding of confidence intervals from @sec-confidence-intervals and hypothesis tests from this chapter, we will now study inference for regression in the upcoming @sec-inference-for-regression.
