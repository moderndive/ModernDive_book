...
a. You reject a true null hypothesis (a "false positive")
...
::: {.callout-tip collapse="true" title="Show answer"}
**(a)** Type I = rejecting $H_0$ when it's actually true. Probability is the significance level $\alpha$ (commonly 0.05). Type II = failing to reject $H_0$ when it's actually false.
:::
...
a. Less than the observed value only
b. Equal to the observed value exactly
c. Greater than the observed value only
d. **At least as far from the null center as the observed value, in *either* direction** (so statistics that are unusually high *or* unusually low both count)
...
c. **The probability of Type II error (lower with larger $n$)**
d. Nothing

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Recall from Chapter 7 that larger samples produce narrower sampling distributions. With less sampling variability, a real difference from $H_0$ is easier to detect, so the test fails to reject a false $H_0$ less often — that is, a lower Type II error rate. Type I error is set by the chosen $\alpha$, not by $n$.
:::
...
b. Computing the difference once and stopping there
c. Bootstrapping the data with replacement
d. **Repeatedly shuffling the group labels and recomputing the difference, building an empirical null distribution**

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** Shuffling labels destroys any real association between group and outcome — so it simulates the world where $H_0$ (no association) is true. The distribution of test statistics across many shuffles IS the null distribution.
:::
...
a. **There's not enough evidence in the data to reject $H_0$ — failing to reject is NOT the same as accepting $H_0$ as true**
...
a. The $p$-value of 0.004 means there is a 99.6 % chance romance is better than action
b. **Since $p \approx 0.004 < 0.05$, reject $H_0$: there is evidence that the *mean rating* differs between action and romance movies in the broader IMDb population — but the test does not tell us *why* the difference exists**
c. The IMDb user population gives romance movies higher ratings *because* of the genre
d. Romance movies are objectively better than action movies
