...
**EX9.4 (★★)** Bob Ross painted a `tree` in *most* episodes. Test $H_0: p = 0.5$ vs $H_A: p > 0.5$ for the true proportion of episodes featuring a tree, using `bob_ross` as the entire population (or treat it as one large sample). Use `infer`'s simulation workflow.
...
**EX9.7 (★★★)** Repeat the test for `mountain`, `cabin`, and `barn`. Which subjects appear in significantly *more* than half of episodes?
...
### Two-sample hypothesis tests (music popularity, permutation) {.unnumbered}
...
### Understanding hypothesis tests (vocabulary) {.unnumbered}
...
(d) the **null distribution**
(e) the **p-value**
(f) the decision to **reject** (or fail to reject) $H_0$

### Conducting hypothesis tests (`infer` workflow) {.unnumbered}
...
**EX9.22 (★★★)** Conduct a **theory-based** two-sample t-test on Athletics Height for M vs F using `t.test()`. Compare its p-value to your simulation-based test from EX9.10.

```{webr-r}
# your code here
```

### Interpreting hypothesis tests (errors and alpha) {.unnumbered}
...
**EX9.27 (★★★)** **Multiple testing**: if you tested 20 elements in `bob_ross` against $H_0: p = 0.5$ at $\alpha = 0.05$, how many "significant" results would you expect *under the null*? What fix should you apply?
...
**EX9.30 (★★)** What's the difference between **statistical significance** and **practical significance**? Give a volcano-themed example where they disagree.
