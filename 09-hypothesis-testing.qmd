...
**Q3.** "Failing to reject the null hypothesis" means:

a. The null hypothesis is true
b. The alternative hypothesis is false
c. The data did not provide enough evidence to rule out the null
d. The test was poorly designed

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Absence of evidence is not evidence of absence. It might also mean the sample was too small to detect a real effect (low statistical power).
:::
...
::: {.callout-tip collapse="true" title="Show answer"}
**(a)** Be careful with wording. We *reject* $H_0$, not "accept $H_a$". And p = 0.04 says nothing about the size of the effect.
:::

**Q5.** A p-value of 0.45 means:

a. 45% probability $H_0$ is true
b. **The data are NOT unusual under $H_0$ — fail to reject $H_0$**
c. The alternative is wrong
d. The test was poorly designed

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** A large p-value just means the observed data is consistent with what we'd expect if $H_0$ were true — not that $H_0$ is true. The proper conclusion is "fail to reject", not "accept".
:::

**Q6.** Increasing the **sample size** in a study primarily affects:

a. The probability of Type I error
b. **The probability of Type II error (lower with larger $n$ → higher statistical power)**
c. The significance level $\alpha$
d. Nothing

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Larger $n$ → tighter sampling distribution → easier to detect real differences from $H_0$ → lower Type II error rate (higher *power*). Type I error is set by the chosen $\alpha$, not by $n$.
:::

**Q7.** A research team reports a "statistically significant" result with $p = 0.03$ and $\alpha = 0.05$. The BEST interpretation:

a. The effect is large
b. The probability the null is true is 0.03
c. **The data are unlikely under $H_0$ — but the effect could still be small or unimportant in practice**
d. The null is definitively false

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Statistical significance ≠ practical importance. A small p-value with a tiny effect (especially with a huge sample) tells you the effect is *real* but doesn't tell you it *matters*. Always report and interpret effect sizes alongside p-values.
:::
...
::: {.callout-tip collapse="true" title="Show answer"}
**(a)** $\alpha = 0.05$ means a 5% false-positive rate per test. Run 20 tests, get ~1 false positive on average — even when nothing is going on. This is the **multiple testing problem**, and it's why pre-registration and corrections like Bonferroni exist.
:::

**Q10.** A test concludes "we reject $H_0$" with $p = 0.001$. A reviewer asks: "but is the effect IMPORTANT?" To answer, you need:

a. A different test
b. **The effect SIZE plus contextual judgment about what magnitude matters in this domain — not just the p-value**
c. $p < 0.05$
d. A bigger $n$

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Significance answers "is there an effect at all?" Effect size answers "how big?" — and "importance" requires domain knowledge to interpret that magnitude. With huge samples, even trivial differences become statistically significant; with tiny samples, important effects may be missed.
:::
...
### One-sample hypothesis test for a proportion {.unnumbered}

**EX9.1 (★★)** Bob Ross painted a `tree` in *most* episodes. Test $H_0: p = 0.5$ vs $H_A: p > 0.5$ for the true proportion of episodes featuring a tree, using `bob_ross` as the entire population (or treat it as one large sample). Use `infer`'s simulation workflow.
...
**EX9.2 (★★)** Visualize the null distribution and shade the rejection region.
...
**EX9.3 (★★)** Interpret the p-value in context: what does it tell you about the data under the assumption that exactly half the episodes have a tree?

**EX9.4 (★★★)** Repeat the test for `mountain`, `cabin`, and `barn`. Which subjects appear in significantly *more* than half of episodes?

```{webr-r}
# your code here
```

### One-sample hypothesis test for a mean {.unnumbered}

**EX9.5 (★★)** Test $H_0: \mu = 1500\,\text{m}$ vs $H_A: \mu \neq 1500$ for the population mean elevation of `volcanoes`. Use bootstrap-based simulation via `infer` (treat the full table as one big sample).

```{webr-r}
# your code here
```

**EX9.6 (★★)** What is the *observed* sample mean? What is the p-value?

```{webr-r}
# your code here
```

**EX9.7 (★★★)** Now test $H_0: \mu = 1500$ vs $H_A: \mu > 1500$ (one-sided). How does the p-value change?

```{webr-r}
# your code here
```

### Two-sample hypothesis tests {.unnumbered}

**EX9.8 (★★★)** Filter `olympic_athletes` to `Sport == "Athletics"` and drop missing `Height`. Test $H_0: \mu_M = \mu_F$ vs $H_A: \mu_M > \mu_F$ using the **permutation** workflow in `infer`.
...
**EX9.9 (★★)** Visualize `null_diff` with `obs_diff` shaded.
...
**EX9.10 (★★★)** Interpret the result. Is the observed difference *consistent with* random shuffling under $H_0$?

**EX9.11 (★★★)** Repeat the test for `Sport == "Athletics"` for `Weight` (M vs F). Compare the p-value to the Height test.

```{webr-r}
# your code here
```

**EX9.12 (★★★)** Test whether mean **eruption VEI** differs between Asia and South America (in `volcanoes::eruptions`, after joining `volcanoes` for `region`).

```{webr-r}
# your code here
```

### Two-proportion test {.unnumbered}

**EX9.13 (★★★)** Did Bob paint **mountains** at a different rate than **cabins** across episodes? Test $H_0: p_\text{mountain} = p_\text{cabin}$. Hint: pivot to long form so each row is "(episode, element, present)" then test difference in proportions.
...
get_p_value(null_p, obs_stat = obs_p, direction = "two-sided")
```

**EX9.14 (★★★)** What does the p-value let you *conclude*? What does it *not* let you conclude?

**EX9.15 (★★★)** Did Bob paint a `cabin` more or less often when there was also a `barn`? Frame and test (independence between cabin and barn presence).

```{webr-r}
# your code here
```

### Linking CIs and tests {.unnumbered}

**EX9.16 (★★★)** Build a 95% CI for $\mu_M - \mu_F$ for `Athletics` Height. Does the CI contain 0? Does this match the conclusion from your hypothesis test?

```{webr-r}
# your code here
```

**EX9.17 (★★★)** Why are CIs and tests *equivalent* when (i) the test is two-sided and (ii) the confidence level matches $1 - \alpha$? Explain in 2 sentences.

### Errors and alpha {.unnumbered}

**EX9.18 (★★★)** What is a **Type I error** in the context of EX9.8? What is a **Type II error**?

**EX9.19 (★★★)** Why does choosing $\alpha = 0.001$ instead of $0.05$ make Type I errors *less* common but Type II errors *more* common? Use volcanoes example to motivate.

**EX9.20 (★★★)** Imagine a colleague says "we got $p = 0.03$, so the alternative is **definitely** true." Push back in 2–3 sentences using the chapter's framing.

### Misuses and critical thinking {.unnumbered}

**EX9.21 (★★★)** Compute p-values for the proportion test in EX9.1 at sample sizes $n = 50$, $200$, and $1000$ (treating successive subsamples as the data). What happens to the p-value, and what does that tell you about $p$-value's relationship to sample size?

```{webr-r}
# your code here
```

**EX9.22 (★★★)** A peer interprets "$p = 0.06$" as "we found *no effect*." Refute in 2 sentences.

**EX9.23 (★★★)** **Multiple testing**: if you tested 20 elements in `bob_ross` against $H_0: p = 0.5$ at $\alpha = 0.05$, how many "significant" results would you expect *under the null*? What fix should you apply?

**EX9.24 (★★★)** Practice writing a complete **statistical conclusion** for the Athletics Height test: the null, alternative, statistic, p-value, decision at $\alpha = 0.05$, and a plain-English interpretation.

**EX9.25 (★★★)** Build a chart that visualizes the **null distribution + observed statistic** for any one of the tests above, clearly highlighting the rejection region.

```{webr-r}
# your code here
```

**EX9.26 (★★★)** Pick a non-test that *looks* like a hypothesis test but is actually inappropriate (e.g., asking "is the population mean exactly 1500 m?" when the data is the entire population). Explain why hypothesis testing is unnecessary.

**EX9.27 (★★)** What's the difference between **statistical significance** and **practical significance**? Give a volcano-themed example where they disagree.

**EX9.28 (★★★)** Why does the chapter say "there is only one test"? What's the unifying framework?

**EX9.29 (★★★)** Conduct a **theory-based** two-sample t-test on Athletics Height for M vs F using `t.test()`. Compare its p-value to your simulation-based test from EX9.8.

```{webr-r}
# your code here
```

**EX9.30 (★★★)** Why might a theory-based and simulation-based test disagree? Under what conditions do they agree closely?

**EX9.31 (★★★)** Open exploration: pose a yes/no question of your choice using one of the three datasets, set up the hypothesis test, run it via `infer`, and interpret in plain language.

```{webr-r}
# your exploration here
```

**EX9.32 (★★★)** Do **larger volcanoes** in `volcanoes` (above median elevation) have **more eruptions** in `eruptions`? Frame as a hypothesis test on the mean number of eruptions per volcano.
