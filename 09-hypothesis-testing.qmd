...
::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Larger $n$ → tighter sampling distribution → easier for the test to *detect* real differences from $H_0$ → lower Type II error rate (fewer "fail to reject" results when $H_0$ is actually false). Type I error is set by the chosen $\alpha$, not by $n$.
:::
...
b. **The world in which $H_0$ is true — `track_genre` has no relationship with `popularity`, so the two groups are interchangeable labels**
...
|---|---|---|
| 1. specify | `specify(formula, success = "...")` | `specify(popular ~ genre, success = "yes")` |
| 2. hypothesize | `hypothesize(null = "independence")` | also `null = "point", p = 0.5` |
| 3. generate | `generate(reps = 1000, type = "permute")` | also `type = "bootstrap"`, `"draw"` |
...
library(readr)
# olympicAthletes and volcanoes are GitHub-only; read their datasets from the v2 raw mirror.
# Exercises here reference olympic_athletes, eruptions, and volcanoes.
olympic_athletes <- read_csv(
  "https://raw.githubusercontent.com/moderndive/ModernDive_book/v2/data/olympic_athletes.csv.gz",
