...
a. The null hypothesis is true with probability 0.001
b. The data would be unusual if the null hypothesis were true
c. The alternative hypothesis is proven
d. The chance the next study will replicate this result is high

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** $p$-values measure how surprising the data is *under* $H_0$, not the probability that $H_0$ is true. A small $p$-value is evidence against $H_0$, but it doesn't *prove* $H_a$ and doesn't tell you anything about the magnitude of the effect.
:::

**Q9-2.** A *Type I error* occurs when:

a. You correctly reject the null hypothesis
b. You reject a true null hypothesis (a "false positive")
c. You fail to reject a false null hypothesis
d. You measure something incorrectly

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Type I = rejecting $H_0$ when it's actually true. Probability is the significance level $\alpha$ (commonly 0.05). Type II = failing to reject $H_0$ when it's actually false.
:::
...
b. **The probability of Type II error (lower with larger $n$)**
c. The significance level $\alpha$
d. Nothing

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Recall from Chapter 7 that larger samples produce narrower sampling distributions. With less sampling variability, a real difference from $H_0$ is easier to detect, so the test fails to reject a false $H_0$ less often — that is, a lower Type II error rate. Type I error is set by the chosen $\alpha$, not by $n$.
:::
...
a. The truth is exactly $H_A$
b. The probability the null is true is 0.03
c. **The data are unlikely under $H_0$ — small enough to reject $H_0$ by the chapter's decision rule, but the test doesn't tell us *by how much* the truth differs from $H_0$**
d. The null is definitively false
...
a. Computing the difference once and stopping there
b. **Repeatedly shuffling the group labels and recomputing the difference, building an empirical null distribution**
c. Bootstrapping the data with replacement
d. Using the t-distribution

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Shuffling labels destroys any real association between group and outcome — so it simulates the world where $H_0$ (no association) is true. The distribution of test statistics across many shuffles IS the null distribution.
:::
...
a. The act of collecting a brand-new random sample of metal and deep-house tracks from Spotify
b. **The world in which $H_0$ is true — `track_genre` has no relationship with `popular_or_not`, so the group labels are interchangeable**
c. A bootstrap of the original sample, with replacement
d. The population distribution of popularity for *all* Spotify songs
...
a. Romance movies are objectively better than action movies
b. The IMDb user population gives romance movies higher ratings *because* of the genre
c. **Since $p \approx 0.004 < 0.05$, reject $H_0$: there is evidence that the *mean rating* differs between action and romance movies in the broader IMDb population — but the test does not tell us *why* the difference exists**
d. The $p$-value of 0.004 means there is a 99.6 % chance romance is better than action

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** At the pre-specified $\alpha = 0.05$, a $p$-value of about 0.004 is small enough to reject the null that the two population means are equal — that's exactly the decision the test was designed to make. It does **not** speak to causality (option b — IMDb raters self-select, so the rating gap could reflect *who* watches and rates each genre rather than the genre itself), it does **not** make a value judgment (option a), and it is **not** "the probability $H_0$ is true" (option d — a $p$-value is computed *assuming* $H_0$ is true, not as a probability statement about $H_0$). Note the contrast with the chapter itself: Section 9.6 pre-specified the much stricter $\alpha = 0.001$, and since $0.004 > 0.001$ it *fails to reject* $H_0$ there. Same data, same $p$-value, different decision — the conclusion depends on the significance level chosen *before* seeing the data.
:::
