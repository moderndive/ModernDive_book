...
```{r t-curve-hypo, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 1.5, 4), fig.width=3, purl=FALSE, fig.cap="The tails of a t curve for this hypothesis test."}
...
```{r spotify-genre-barplot, echo=FALSE, fig.cap="Barplot relating genre to popularity.", fig.height=ifelse(knitr::knitr::is_latex_output(), 1.6, 4), purl=FALSE}
...
ggplot(spotify_52_shuffled, aes(x = track_genre, fill = popular_or_not)) +
  ...
  labs(x = "Genre of track")
```
```{r spotify-genre-barplot-permuted, fig.cap="Barplots of relationship of genre with `popular or not' (left) and shuffled `popular or not' (right).", fig.height=ifelse(knitr::knitr::is_latex_output(), 2.6, 4), echo=FALSE, purl=FALSE, message=FALSE}
...
```{r null-distribution-infer, fig.show="hold", fig.cap="Null distribution.", fig.height=ifelse(knitr::knitr::is_latex_output(), 1.8, 4)}
...
```
```{r bootstrap-distribution-two-prop-percentile, echo=FALSE, fig.show="hold", fig.cap="Percentile-based 90\\% confidence interval.", purl=FALSE, fig.height=ifelse(knitr::knitr::is_latex_output(), 3.2, 4)}
if (knitr::is_html_output()) {
  visualize(bootstrap_distribution) +
    shade_confidence_interval(endpoints = percentile_ci)
} else {
...
```{r htdowney, echo=FALSE, fig.cap="Allen Downey's hypothesis testing framework.", purl=FALSE, out.width=ifelse(knitr::knitr::is_latex_output(), "85%", "100%")}
...
```{r action-romance-boxplot, fig.cap="Boxplot of IMDb rating vs. genre.", fig.height=ifelse(knitr::knitr::is_latex_output(), 4, 4)}
...
```{r null-distribution-movies-2, echo=FALSE, fig.cap="Null distribution, observed test statistic, and $p$-value.", fig.height=ifelse(knitr::knitr::is_latex_output(), 1.8, 4), purl=FALSE}
if (knitr::is_html_output()) {
  visualize(null_distribution_movies, bins = 10) +
    shade_p_value(obs_stat = obs_diff_means, direction = "both")
} else {
...
```{r ha-as-flights-boxplot, fig.cap="Air time for Hawaiian and Alaska Airlines flights departing NYC in 2023.", fig.height=ifelse(knitr::knitr::is_latex_output(), 2.8, 4)}
