...
hub_nav_html <- function(at_root = TRUE) {
  ...
  paste(c(
    ...
    # Umami Cloud analytics — paste the website ID to activate; empty = off.
    # KEEP THE ID IN SYNC with instructor-solutions/_includes/pagefind-search.html
    # and instructor-solutions/slides/moderndive-slides.js (three render paths,
    # one Umami website).
    '<script>',
    '  (function () {',
    ...
    '      }',
    '    };',
    '    if (!UMAMI_WEBSITE_ID || window.__mdUmami) return;',
    '    window.__mdUmami = true;',
    '    var s = document.createElement("script");',
    '    s.defer = true;',
    '    s.src = "https://cloud.umami.is/script.js";',
    '    s.setAttribute("data-website-id", UMAMI_WEBSITE_ID);',
    '    s.setAttribute("data-domains", "moderndive-instructor-resources.netlify.app,moderndive.com");',
    ...
    '  // Pagefind result URLs are absolute from the indexed site root; rewrite',
    '  // them page-relative (via this page\'s known prefix) so links stay inside',
    '  // the hub when it is proxied under a path prefix (moderndive.com/v2/...).',
    '  window.addEventListener("DOMContentLoaded", function() {',
    '    if (typeof PagefindUI !== "undefined") {',
    sprintf('      var reroot = function (u) { return "%s" + String(u).replace(/^\\//, ""); };', prefix),
    '      new PagefindUI({ element: "#insthub-pagefind-search",',
    '        showSubResults: true, resetStyles: false,',
    '        processResult: function (result) {',
    '          if (result && result.url) result.url = reroot(result.url);',
    '          if (result && result.sub_results) {',
    '            result.sub_results.forEach(function (s) { if (s.url) s.url = reroot(s.url); });',
    '          }',
    '          return result;',
    '        },',
    ...
    # Each dropdown alphabetized for findability — match _quarto.yml order.
