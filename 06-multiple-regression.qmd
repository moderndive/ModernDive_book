---
description: &desc "Extend linear regression to multiple predictors, distinguishing parallel-slopes models from interaction models."
open-graph:
  description: *desc
twitter-card:
  description: *desc
...
::: {.callout-tip collapse="true" title="Try it interactively"}
```{webr-r}
# Fit interaction model and get the coefficients
model_int <- lm(fert_rate ~ life_exp * income, data = UN_data_ch6)
coef(model_int)
```
:::
...
website](https://moderndive.com/v2/extras/regression-plane-ISLR2.html) to open an
...
a. R has built-in regularization
b. **Least squares is free to pick a coefficient near zero for the new predictor — equivalent to ignoring it — so the new model can always at least match the old one's fit**
c. The data is invariant
...
b. **When the slopes don't visibly differ across groups in the EDA plot, and the fitted lines for both models look nearly indistinguishable when overlaid on the data** — the simpler model is doing about as well with fewer moving parts
...
# Pre-define the two planet subsets used throughout this chapter's exercises.
# EX 6.3 / 6.15 ask students to construct them; pre-defining keeps later
# exercises (4, 5, 16, 21, 37, 39, 44–46, 53) runnable independently.
...
# Pre-define `UN_data_ch6` — chapter narrative defines it in an inline
# {webr-r} cell; pre-defining ensures later cells work standalone.
UN_data_ch6 <- un_member_states_2024 |>
  select(country,
         life_expectancy_2022,
         fertility_rate_2022,
