...
```{r lc-inline-setup-06, include=FALSE, purl=FALSE}
...
```{r fig-numxcatxplot1, fig.alt="Scatterplot of life expectancy (x) vs fertility rate (y), with points colored by income group (low, lower-middle, upper-middle, high). Different income groups occupy different regions of the plot.", echo=FALSE, fig.cap="Colored scatterplot of life expectancy and fertility rate.", fig.height=ifelse(knitr::is_latex_output(), 3.2, 4), purl=FALSE, message=FALSE}
if (is_html_output()) {
  ggplot(UN_data_ch6, aes(x = life_exp, y = fert_rate, color = income)) +
    geom_point() +
    labs(x = "Life Expectancy", y = "Fertility Rate", color = "Income group") +
...
D_3 = \left\{
\begin{array}{ll}
1 & \text{if the UN member state has upper middle income}\phantom{a} \\
0 & \text{otherwise}\end{array}
\right.
$$
$$
...
UN member state has `Upper middle income`, then its dummy variables
...
$$\hat y_i = \widehat{\text{fert rate}}_i = b_0 + b_2 D_{2i} + b_3 D_{3i} + b_4 D_{4i}$$
...
`Upper middle income` level, then $D_1 = D_2 = D_4 = 0$ and $D_3 = 1$ so
the linear regression becomes:

$$\hat y = \widehat{\text{fert rate}} = b_0 + b_3$$ The average
fertility rate for an `Upper middle income` country is $b_0 + b_3$.
Observe that $b_3$ is an *offset* for fertility rate between the
baseline level and the `Upper middle income` level. The same logic
...
in average fertility rate due to changes in life expectancy. This is the
...
The linear regression shows how the average fertility rate is affected
...
both `x1` and `x2` be either categorical or numerical, and we need to
...
```{r lc-sol-06-01, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 1))
```
...
```{r lc-sol-06-02, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 2))
```
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Which statement about the use of dummy variables in regression models is correct?
  ...
  -   B. Dummy variables are used to represent categorical variables with at least two levels.
  -   C. Dummy variables always decrease the R-squared value.
  -   D. Dummy variables are unnecessary in regression models.

```{r lc-sol-06-03, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 3))
```
...
in a given country are the same regardless of the income group of this
country.
...
`x1` and `x2` be either categorical or numerical; therefore, we
...
```{r lc-sol-06-04, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 4))
```
...
```{r lc-sol-06-05, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 5))
```
...
```{r lc-sol-06-06, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 6))
```
...
```{r lc-sol-06-07, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 7))
```
...
&= -385.179 + 0.264 \cdot x_1 - 7.663 \cdot x_2
\end{aligned}
...
```{r lc-sol-06-08, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 8))
```
...
```{r lc-sol-06-09, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 9))
```
...
```{r lc-sol-06-10, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 10))
```
...
```{r lc-sol-06-11, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 11))
```
...
```{r lc-sol-06-12, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(6, 12))
```
...
$$
\begin{aligned}
\widehat{y} = \widehat{\text{debt}} &= -385.179 + 0.264 \cdot x_1 - 7.663 \cdot x_2
\end{aligned}
...
a. Only one regression line is fit to all the data
b. The numerical and categorical predictors are independent of each other
c. Each category gets its own intercept with a common slope
d. Each category gets its own slope and intercept
...
a. Whether the slope of `y` on `x` differs by `group`
b. The intercept for the reference category of `group`
c. The total variance of `y` in the data
d. Whether `group` on its own predicts `y`
...
a. The model must be misspecified
b. A coefficient can never flip sign when you add a predictor
c. R has a bug in its `lm()` implementation
d. The single-predictor model omitted a confounder
...
a. Adding the `income` coefficient
b. Holding `income` constant
c. Multiplied by the `income` coefficient
d. Ignoring `income` entirely
...
a. The model is broken and silently dropped a level
b. R silently errored when encoding the third level
c. `low` is the *reference level* absorbed into the intercept
d. Drop the categorical predictor and refit the model without it
...
a. R automatically applies built-in regularization to any new predictor
b. Setting the new coefficient to zero reproduces the old fit
c. R's compiler optimizes the extra predictor away
d. The residuals are computed from the same data
...
a. R returns an error for correlated predictors
b. $R^2$ automatically drops to 0 when predictors are correlated
c. The two slopes should have come out equal once both variables are in the model
d. One is the simple-regression slope and the other the partial slope
...
a. The second syntax is deprecated
b. The second multiplies the `x1` and `x2` columns together
c. Only the second includes an *interaction* term
d. The two formulas are identical
...
a. Never; the interaction model is always more accurate
b. When the fitted lines look nearly identical
c. Always; the simpler model is better
d. Whenever you have a lot of data
...
a. Price rises twice as fast with size in NORTH
b. NORTH has twice as much data as SOUTH
c. NORTH and SOUTH have equal size slopes
d. NORTH houses are twice as large as SOUTH houses
