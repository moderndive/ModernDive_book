...
```{r multiple-regression-setup, include=FALSE}
...
```{r multiple-regression-load-packages-alt, echo=FALSE, message=FALSE}
library(tidyverse)
library(moderndive)
library(ISLR2)
```

```{r multiple-regression-load-internal, message=FALSE, echo=FALSE}
...
```{r multiple-regression-dynamic-text, echo=FALSE}
...
```{r numxcatxplot1, echo=FALSE, fig.cap="Colored scatterplot of life expectancy and fertility rate.", fig.height=ifelse(knitr::is_latex_output(), 3.2, 4), message=FALSE}
if (is_html_output()) {
  ggplot(UN_data_ch6, aes(x = life_exp, y = fert_rate, color = income)) +
    geom_point() +
    labs(x = "Life Expectancy", y = "Fertility Rate", color = "Income group") +
...
```{r multiple-regression-create-lifeExp_model, echo=FALSE}
...
We can match the coefficients with the values computed in Table \@ref(tab:regtable-interaction): the fitted
...
```{r numxcatx-parallel, echo=FALSE, fig.cap="Parallel slopes model of fertility rate with life expectancy and income.", fig.height=ifelse(knitr::is_latex_output(), 2.7, 4)}
...
```{r regtable-parallel1, echo=FALSE}
# Fit regression model:
...
In this model without interactions presented in Table 
\@ref(tab:regtable-parallel1), the slope is the same for all the
...
```{r numxcatx-comparison, fig.width=8, echo=FALSE, fig.cap="Comparison of interaction and parallel slopes models.", message=FALSE}
...
`model_int`, and are displayed in Table \@ref(tab:regtable-interaction1):

```{r regtable-interaction1, echo=FALSE}
# Fit regression model:
model_int <- lm(fert_rate ~ income + life_exp + income:life_exp , data = UN_data_ch6)
b_int <- round(coef(model_int),2)
# Get the coefficients of the model
...
```{r fitted-values, echo=FALSE, fig.cap="Fitted values for two new countries.", fig.height=ifelse(knitr::is_latex_output(), 4.7, 5), message=FALSE}
newpoints <- 
  get_regression_points(model_int, newdata = slice(UN_data_ch6, c(41, 102)))
...
```{r multiple-regression-create-n_credit_ch6, echo=FALSE}
...
```{r 2numxplot1, echo=FALSE, fig.cap="Relationship between credit card debt and credit limit/income.", fig.height=ifelse(knitr::is_latex_output(), 3.2, 4), message=FALSE}
...
```{r 3D-scatterplot, echo=FALSE, fig.cap="3D scatterplot and regression plane.", out.width="60%"}
knitr::include_graphics("images/ISLR2-credit_card_balance_regression_plane.png")
```
...
```{r multiple-regression-create-model_lm, eval=FALSE, echo=FALSE}
...
```{r multiple-regression-alt2-dup3, echo=FALSE, eval=FALSE}
...
```{r multiple-regression-create-debt_model1, echo=FALSE}
# Fit regression model:
...
```{r multiple-regression-conditional-text, echo=FALSE, results="asis"}
if (is_latex_output()) {
...
```{r multiple-regression-demo-code-v2, echo=FALSE, results="asis"}
...
in Figure \@ref(fig:part3)!

Also in Chapter \@ref(inference-for-regression), we revisit the
...
```{r part3, echo=FALSE, fig.cap="(ref:flowchart-partiii)", out.width="65%"}
