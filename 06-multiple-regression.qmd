...
```{r multiple-regression-setup, include=FALSE, purl=FALSE}
...
In @sec-regression, we studied simple linear regression as a
...
explanatory $x$ variable that was either numerical in @sec-model1 or categorical in @sec-model2.
...
## Needed packages {#sec-mult-reg-packages .unnumbered}

If needed, read @sec-packages for information on how to
install and load R packages.

```{r multiple-regression-load-packages, eval=FALSE}
library(tidyverse)
library(moderndive)
library(ISLR2)
```

```{r multiple-regression-load-packages-alt, echo=FALSE, message=FALSE, purl=TRUE}
library(tidyverse)
library(moderndive)
library(ISLR2)
```

```{r multiple-regression-load-internal, message=FALSE, echo=FALSE, purl=FALSE}
...
## One numerical and one categorical explanatory variable {#sec-model4}

We continue using the UN member states dataset introduced in @sec-model1. Recall that we studied the relationship between the
...
2.  Two explanatory variables:
      a. A numerical explanatory variable $x_1$, the life expectancy.
      b. A categorical explanatory variable $x_2$, the income group.
...
chosen in @sec-regression. We also set the `income` variable
to be a `factor` so that its levels show up in the expected order.

```{r multiple-regression-create-UN_data_ch6}
...
```{r multiple-regression-glimpse-UN_data_ch6}
glimpse(UN_data_ch6)
```
...
```{r multiple-regression-dynamic-text, echo=FALSE, purl=FALSE}
# This code is used for dynamic non-static in-line text output purposes
n_UN_data_ch6 <- UN_data_ch6 |> nrow()
```

We also display a random sample of 10 rows of the `r n_UN_data_ch6` rows
corresponding to different countries in @tbl-model4-data-preview. Remember due to the random nature of the
sampling, you will likely end up with a different subset of 10 rows.

```{r multiple-regression-alt, eval=FALSE}
UN_data_ch6 |> sample_n(size = 10)
```

```{r tbl-model4-data-preview, echo=FALSE, purl=FALSE}
set.seed(20)
UN_data_ch6 |>
  sample_n(size = 10) |>
  kbl(
...
analyses in @sec-model1EDA and @sec-model2EDA from @sec-regression, we find summary statistics:

```{r multiple-regression-select-vars, eval=FALSE}
UN_data_ch6 |> 
  select(life_exp, fert_rate, income) |> 
  tidy_summary()
```

```{r multiple-regression-select-vars-sized, echo=FALSE}
UN_data_ch6 |> 
  select(life_exp, fert_rate, income) |> 
  tidy_summary() |> 
  kbl() |>
...
factor with four levels: `Low Income`, `Lower middle income`,
...
dataset has 56 UN member states that are considered `High Income`
states.

Furthermore, we can compute the correlation coefficient between our two
numerical variables: `life_exp` and `fert_rate`. Recall from @sec-model1EDA that correlation coefficients only exist between
numerical variables. We observe that they are "strongly negatively"
correlated.

```{r multiple-regression-demo-code}
UN_data_ch6 |> 
...
scatterplot to display their relationship, as we did in @fig-numxplot1. But this time, we incorporate the categorical
variable `income` by `mapping` this variable to the `color` aesthetic,
thereby creating a *colored* scatterplot.

```{r multiple-regression-scatter-lifeexp, eval=FALSE}
...
```{r fig-numxcatxplot1, fig.alt="Scatterplot of fertility rate (x) vs life expectancy (y), with points colored by income group (low, lower-middle, upper-middle, high). Different income groups occupy different regions of the plot.", echo=FALSE, fig.cap="Colored scatterplot of life expectancy and fertility rate.", fig.height=ifelse(knitr::is_latex_output(), 3.2, 4), purl=FALSE, message=FALSE}
if (is_html_output()) {
  ggplot(UN_data_ch6, aes(x = life_exp, y = fert_rate, color = income)) +
    geom_point() +
    labs(x = "Life Expectancy", y = "Fertility Rate", color = "Income group") +
...
In the resulting @fig-numxcatxplot1, observe that
...
### Model with interactions {#sec-model4interactiontable}

We can represent the four regression lines in @fig-numxcatxplot1 as a multiple regression model with
*interactions*. \index{interaction model}

Before we do this, however, we review a linear regression with only one
categorical explanatory variable. Recall in @sec-model2table we fit a regression model for each country life
...
```{r multiple-regression-lm-fertility, eval=FALSE}
...
```{r multiple-regression-create-lifeExp_model, echo=FALSE, purl=FALSE}
...
interactions shown in @fig-numxcatxplot1. In this figure we
...
We can match the coefficients with the values computed in @tbl-regtable-interaction: the fitted
...
which is the equation of the regression line in @fig-numxcatxplot1 for low income countries. The regression has an
...
high-income group. This is consistent with our observation from @fig-numxcatxplot1. When the associated effect of one variable
...
### Model without interactions {#sec-model4table}
...
@fig-numxcatxplot1, but now the
`geom_smooth(method = "lm", se = FALSE)` layer is replaced with
`geom_parallel_slopes(se = FALSE)`.

```{r multiple-regression-scatter-lifeexp-colored, eval=FALSE}
ggplot(UN_data_ch6, aes(x = life_exp, y = fert_rate, color = income)) +
  geom_point() +
  labs(x = "Life expectancy", y = "Fertility rate", color = "Income group") +
  geom_parallel_slopes(se = FALSE)
...
```{r fig-numxcatx-parallel, fig.alt="Same colored scatterplot, with parallel-slope regression lines overlaid: one line per income group, all with the same slope but different intercepts.", echo=FALSE, fig.cap="Parallel slopes model of fertility rate with life expectancy and income.", fig.height=ifelse(knitr::is_latex_output(), 2.7, 4), purl=FALSE}
...
The regression lines for each income group are shown in @fig-numxcatx-parallel. Observe that the lines are now parallel:
...
On the other hand, any two regression lines in @fig-numxcatx-parallel have different intercepts representing the
...
```{r multiple-regression-lm-fertility-alt, eval=FALSE}
# Fit regression model:
...
```{r tbl-regtable-parallel1, echo=FALSE, purl=FALSE}
# Fit regression model:
...
In this model without interactions presented in @tbl-regtable-parallel1, the slope is the same for all the
...
compare the visualizations for both models side-by-side in @fig-numxcatx-comparison.

```{r fig-numxcatx-comparison, fig.alt="Two-panel comparison: left panel shows the parallel-slopes model (one slope, four intercepts), right panel shows the interaction model (four slopes, four intercepts) on the same data.", fig.width=8, echo=FALSE, fig.cap="Comparison of interaction and parallel slopes models.", purl=FALSE, message=FALSE}
...
clusters of points in @fig-numxcatx-comparison, it does
...
### Observed responses, fitted values, and residuals {#sec-model4points}
...
`model_int`, and are displayed in @tbl-regtable-interaction1:

```{r tbl-regtable-interaction1, echo=FALSE, purl=FALSE}
# Fit regression model:
model_int <- lm(fert_rate ~ income + life_exp + income:life_exp , data = UN_data_ch6)
b_int <- round(coef(model_int),2)
# Get the coefficients of the model
...
We illustrate this in @fig-fitted-values. The vertical line
...
```{r fig-fitted-values, fig.alt="Scatterplot with two new points highlighted, illustrating how the fitted regression model assigns predicted values to each new observation.", echo=FALSE, fig.cap="Fitted values for two new countries.", fig.height=ifelse(knitr::is_latex_output(), 4.7, 5), purl=FALSE, message=FALSE}
newpoints <- 
  get_regression_points(model_int, newdata = slice(UN_data_ch6, c(41, 102)))
...
countries in the `UN_data_ch6` dataset, as shown in @tbl-model4-points-table.

```{r multiple-regression-reg-points, eval=FALSE}
regression_points <- get_regression_points(model_int)
regression_points
```

```{r tbl-model4-points-table, echo=FALSE, purl=FALSE}
regression_points <- get_regression_points(model_int)
regression_points |>
  slice(1:10) |>
  kbl(
...
We do this by using the `select()` verb as we did in @sec-select 
...
```{r multiple-regression-load-ISLR2, message=FALSE}
library(ISLR2)
...
```{r multiple-regression-glimpse-credit_ch6}
glimpse(credit_ch6)
```

```{r multiple-regression-create-n_credit_ch6, echo=FALSE, purl=FALSE}
n_credit_ch6 <- credit_ch6 |> nrow()
```

Furthermore, we present a random sample of five out of the
`r n_credit_ch6` credit card holders in @tbl-model3-data-preview. As observed before, each time you run
this code, a different subset of five rows is given.

```{r multiple-regression-alt2, eval=FALSE}
credit_ch6 |> sample_n(size = 5)
```

```{r tbl-model3-data-preview, echo=FALSE, purl=FALSE}
credit_ch6 |>
  sample_n(5) |>
  kbl(
    digits = 3,
...
for our model as shown in @tbl-credittable:

```{r multiple-regression-select-vars-alt, eval=FALSE}
credit_ch6 |> select(debt, credit_limit, income) |> tidy_summary()
```

```{r tbl-credittable, echo=FALSE}
credit_ch6 |> 
...
```{r multiple-regression-scatter, eval=FALSE}
...
```{r fig-2numxplot1, fig.alt="Two scatterplots side-by-side: credit-card debt vs credit limit (left) and credit-card debt vs income (right). Both show positive linear relationships of similar strength.", echo=FALSE, fig.cap="Relationship between credit card debt and credit limit/income.", fig.height=ifelse(knitr::is_latex_output(), 3.2, 4), purl=FALSE, message=FALSE}
...
The left plot in @fig-2numxplot1 shows a positive and
...
the credit card debt value of zero. The right plot in @fig-2numxplot1 shows also positive and somewhat linear
...
`get_correlation()` command as seen in @sec-model1EDA, once
for each explanatory variable with the response `debt`:

```{r multiple-regression-alt2-dup1, eval=FALSE}
...
matrix* as shown in @tbl-model3-correlation.
...
```{r multiple-regression-select-vars-alt2, eval=FALSE}
credit_ch6 |> select(debt, credit_limit, income) |> cor()
```

```{r tbl-model3-correlation, echo=FALSE, purl=FALSE}
credit_ch6 |>
  select(debt, credit_limit, income) |>
  cor() |>
  kbl(
...
3.  The correlation between `debt` and `income` is 0.464. The linear
    ...
    higher income is only weakly associated with higher debt.
...
```{r multiple-regression-alt2-dup2}
...
The plots in @fig-2numxplot1 correspond to the
response and each of the explanatory variables *separately*. In
@fig-3D-scatterplot we show a 3-dimensional (3D)
...
```{r fig-3D-scatterplot, fig.alt="Three-dimensional scatterplot of credit-card debt (z-axis) versus credit limit and income (x and y axes), with a fitted regression plane cutting through the cloud of points.", echo=FALSE, fig.cap="3D scatterplot and regression plane.", purl=FALSE, out.width="60%", purl=FALSE}
include_graphics("images/ISLR2-credit_card_balance_regression_plane.png")
```

In addition, @fig-3D-scatterplot includes a *regression
plane*\index{regression!regression plane}. Recall from @sec-leastsquares that the linear regression with one numerical
...
```{r multiple-regression-create-model_lm, eval=FALSE, echo=FALSE, purl=FALSE}
...
```{r multiple-regression-alt2-dup3, echo=FALSE, eval=FALSE, purl=FALSE}
...
### Multiple regression with two numerical regressors {#sec-model3table}

As shown in @fig-3D-scatterplot, the linear regression with
...
```{r multiple-regression-fit-lm, eval=FALSE}
...
plot in @fig-2numxplot1 shows the relationship between
...
```{r multiple-regression-assign-simple_model, eval=FALSE}
# Fit regression model and get the coefficients of the model
simple_model <- lm(debt ~ income, data = credit_ch6)
coef(simple_model)
```

```{r multiple-regression-create-debt_model1, echo=FALSE, purl=FALSE}
# Fit regression model:
...
As shown in @sec-model4points for the UN member states
...
`residual`, using the `get_regression_points()` function. In @tbl-model3-points-table we present the first 10 rows of output.
Remember that the coordinates of each of the points in our 3D
scatterplot in @fig-3D-scatterplot can be found in the
`income`, `credit_limit`, and `debt` columns.

```{r multiple-regression-reg-points-alt, eval=FALSE}
get_regression_points(debt_model)
```

```{r tbl-model3-points-table, echo=FALSE, purl=FALSE}
set.seed(76)
...
a. Each category gets its own slope and intercept
b. Each category gets its own intercept but shares a common slope
c. The numerical and categorical predictors are independent
d. Only one regression line is fit

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Parallel slopes = same slope for all groups, different intercepts per category. Distinct from interaction models, where each group also gets its own slope.
:::

**Q2.** In an *interaction* model `lm(y ~ x * group)`, what does the interaction term capture?

a. Whether `group` predicts `y`
b. Whether the slope of `y` on `x` differs across levels of `group`
c. The intercept for the reference category
d. The variance of `y`

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Interaction terms allow the slope of `x` to vary across groups. If the interaction is large, the relationship between `x` and `y` depends on which group you're in.
:::
...
b. R has a bug
c. The original (single-predictor) model omitted a confounder; adding it can reveal the *partial* effect
d. The coefficient can never flip sign

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Simpson's paradox / omitted-variable bias. The bivariate slope and the multivariable partial slope can have opposite signs when a confounder is involved — a key reason multiple regression matters.
:::
...
a. Ignoring `income`
b. **Holding `income` constant** (the partial effect of `fert_rate`, controlling for income)
c. Adding `income`
d. Multiplied by `income`

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** This is the "controlling for" interpretation that makes multiple regression powerful. The fert_rate slope here is the effect of fert_rate *among countries with the same income*. It typically differs from the simple-regression slope.
:::
...
b. `low` is the **reference level** — its effect is absorbed into the intercept; the other coefficients are *differences from* `low`
c. R errored
d. Drop the categorical predictor

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Categorical predictors are encoded as a set of dummy variables, and one level is the baseline (alphabetical by default). The reported coefficients are how much each level *differs from* the baseline, not absolute effects.
:::

**Q6.** Adding more predictors to a regression model **never decreases** $R^2$. Why?

a. R² always increases with more data
b. Adding any predictor — even pure noise — can only fit MORE variance, never less
c. R² is a fixed property of the response
d. The compiler optimizes it

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Even random noise can fit a tiny bit of variance by chance. That's why adjusted $R^2$ (which penalizes extra predictors) and out-of-sample evaluation are more honest measures of model usefulness.
:::

**Q7.** Two predictors are highly correlated with each other (multicollinearity). Including both in a regression most commonly causes:

a. The model to fail entirely
b. **Inflated standard errors and unstable coefficient estimates** — small data changes can swing coefficients dramatically
c. Coefficients to become exactly zero
d. The slopes to become equal

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Multicollinearity doesn't break the model, but the regression can't tell which predictor is "doing the work". Standard errors balloon and individual coefficient interpretation becomes unreliable.
:::

**Q8.** What's the syntactic difference between `lm(y ~ x1 + x2, data)` and `lm(y ~ x1 * x2, data)`?

a. Identical
b. The first fits two main effects only; the second fits two main effects **plus their interaction** (`x1 * x2` is shorthand for `x1 + x2 + x1:x2`)
c. The second multiplies y by something
d. The second is deprecated

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** `*` in a model formula is shorthand for "main effects + interaction". To fit just the interaction term: `x1:x2`.
:::

**Q9.** When SHOULD you prefer the parallel-slopes model over the interaction model?

a. Always — simpler is better
b. **When the slopes don't visibly differ across groups, or a formal test shows no significant interaction** — the simpler model generalizes better
c. When you have lots of data
d. Never; interaction is always more accurate

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** This is the principle of parsimony / Occam's razor. If the interaction isn't supported by the data, the interaction model fits noise and predicts worse on new data.
:::
...
a. **In the NORTH neighborhood, price grows twice as fast with size** as it does in the SOUTH
b. NORTH houses are larger
c. NORTH has more data
d. NORTH and SOUTH have equal slopes
...
|---|---|---|
| `lm(y ~ x1 + x2, data = df)` | Multiple regression — *parallel slopes* when one predictor is categorical | `lm(life_exp ~ fert_rate + income, data = UN_data_ch6)` |
| `lm(y ~ x1 * x2, data = df)` | Multiple regression with *interaction* — slope of `x1` differs by `x2` group | `lm(life_exp ~ fert_rate * income, data = UN_data_ch6)` |
| `get_regression_table(model)` | Tidy coefficient table | `get_regression_table(model)` |
...
## Exercises {.unnumbered #sec-mult-reg-exercises}

The end-of-chapter exercises introduce a new dataset: `planets` from the [`exoplanets`](https://github.com/moderndive/exoplanets) package — **6,278 confirmed exoplanets** with merged host-star parameters. \index{R packages!exoplanets!planets} Key numerical variables: `radius_earth`, `mass_earth`, `orbital_period_days`, `eq_temp_k`, `insolation_earth`, `star_mass_solar`, `star_temp_k`, `distance_pc`. Categorical: `discovery_method`. Install with `remotes::install_github("moderndive/exoplanets")`. Solutions are available to instructors separately.
...
**EX6.4 (★★)** Scatter `mass_earth` (y, log) vs `radius_earth` (x, log). What do clusters tell you?
...
### One numerical + one categorical (interaction) {.unnumbered}

**EX6.5 (★★)** Filter to the **2 most common** `discovery_method`s and call it `pl2`.
...
**EX6.6 (★★)** Plot `mass_earth` vs `radius_earth` (both log) colored by `discovery_method`, with one regression line per group.

```{webr-r}
# your code here
```

**EX6.7 (★★★)** Fit the **interaction model**: `lm(log10(mass_earth) ~ log10(radius_earth) * discovery_method, data = pl2)`.

```{webr-r}
m_int <- lm(log10(mass_earth) ~ log10(radius_earth) * discovery_method,
            data = pl2)
...
**EX6.9 (★★★)** Fit the **parallel-slopes** model. Compare its R² to the interaction model's. Did adding the interaction substantially improve fit?

```{webr-r}
m_par <- lm(log10(mass_earth) ~ log10(radius_earth) + discovery_method,
            data = pl2)
get_regression_summaries(m_par)$r_squared
get_regression_summaries(m_int)$r_squared
```

**EX6.10 (★★★)** When R² *barely* changes after adding an interaction, what's the right modeling choice? Why?

### Two numerical predictors {.unnumbered}

**EX6.11 (★★)** Build `planets_temp` with `eq_temp_k`, `star_temp_k`, `insolation_earth` non-missing. Plot `eq_temp_k` vs `insolation_earth` (log-x).
...
**EX6.12 (★★★)** Fit `lm(eq_temp_k ~ log10(insolation_earth) + star_temp_k, data = planets_temp)`.

```{webr-r}
m_two <- lm(eq_temp_k ~ log10(insolation_earth) + star_temp_k,
            data = planets_temp)
get_regression_table(m_two)
```

**EX6.13 (★★★)** Interpret the **insolation slope** *holding star temperature constant*.

**EX6.14 (★★★)** Interpret the **star temperature slope** *holding insolation constant*.

**EX6.15 (★★★)** Why is the phrase "holding X constant" essential in *multiple* regression but not in *simple* regression?

**EX6.16 (★★★)** Predict `eq_temp_k` for an Earth-like exoplanet (`insolation_earth = 1`, `star_temp_k = 5800`). How close is the prediction to Earth's actual ~255 K?

```{webr-r}
# your code here
```

**EX6.17 (★★★)** Plot residuals vs fitted values for `m_two`. Comment on any structure.

```{webr-r}
# your code here
```

### Comparing models / model selection {.unnumbered}

**EX6.18 (★★★)** Compare three models for `eq_temp_k`: (a) intercept only; (b) `~ log10(insolation_earth)`; (c) `~ log10(insolation_earth) + star_temp_k`. Report each R².

```{webr-r}
# your code here
```

**EX6.19 (★★★)** A peer says "more predictors are always better." Push back in 2–3 sentences using the chapter's framework.

**EX6.20 (★★★)** What R² value would you consider "high" for an exoplanet relationship? Why is "high" context-dependent?

### Visualizing multiple regression {.unnumbered}

**EX6.21 (★★)** Plot the **parallel-slopes** fitted lines over the data on `pl2`. Describe what "parallel" means visually.

```{webr-r}
# your code here
```

**EX6.22 (★★★)** Same plot, **interaction model** — lines diverge. Describe the divergence.
...
**EX6.23 (★★★)** A colleague: "Radial Velocity for *massive* planets and Transit for *small* ones — that's why their populations differ." How does this affect interpretation of the discovery-method coefficient?

**EX6.24 (★★★)** This is a **selection-biased** sample. Name two ways selection could distort your `mass_earth ~ radius_earth` conclusions.
...
**EX6.26 (★★★)** Open exploration: pick a numerical outcome + two predictors. Fit parallel-slopes and interaction versions, compare R², and pick a model. Justify in 2–3 sentences.

```{webr-r}
# your exploration here
```

**EX6.27 (★★)** Why log-transform `mass_earth` and `radius_earth` but not `eq_temp_k`?

**EX6.28 (★★★)** What's the biggest *limitation* of regression for this dataset given how exoplanets are detected?

**EX6.29 (★★★)** Build a chart that visualizes the *interaction* from EX6.7 — colored points + diverging fitted lines. Add labels and a one-line headline.

```{webr-r}
# your code here
```

**EX6.30 (★★★)** Pick a discovery method with < 50 planets in `planets_lite` and fit `lm(log10(mass_earth) ~ log10(radius_earth))`. Why is the slope *less trustworthy*?

```{webr-r}
# your code here
```

**EX6.31 (★★★)** What single chart would convince you the `mass_earth` vs `radius_earth` relationship is **not** a single straight line?

**EX6.32 (★★★)** Show that fitting `lm(log10(mass_earth) ~ log10(radius_earth))` estimates the exponent `p` in `M = k R^p`. What does your slope suggest for typical exoplanets?

```{webr-r}
# your code here
```

**EX6.33 (★★★)** Compute the residual standard error of `m_two`. Translate it into temperature units and explain in plain English.

```{webr-r}
# your code here
```

**EX6.34 (★★★)** Why is `eq_temp_k ~ log10(insolation_earth) + star_temp_k` a *physics-informed* model? What rough astrophysics does each predictor capture?

**EX6.35 (★★★)** Open exploration #2 — pick any pair of numerical variables, build a multiple regression with a third numerical predictor, write a 2-sentence finding.

```{webr-r}
# your exploration here
```

## Conclusion {#sec-mult-reg-conclusion}

### Additional resources

```{r multiple-regression-conditional-text, echo=FALSE, results="asis", purl=FALSE}
if (is_latex_output()) {
...
```{r multiple-regression-demo-code-v2, echo=FALSE, purl=FALSE, results="asis"}
...
Once we have covered @sec-sampling on sampling,
@sec-confidence-intervals on confidence intervals, and
@sec-hypothesis-testing on hypothesis testing, we revisit the
regression models in @sec-inference-for-regression on
inference for regression. This will complete the topics in this book, as shown 
in @fig-part3!

Also in @sec-inference-for-regression, we revisit the
...
`get_regression_points()` outputs. Residual analyses enable us to verify
what are known as the *conditions for inference for regression*.


```{r fig-part3, fig.alt="Flowchart graphic transitioning from the Statistical Modeling part of the book to the Statistical Inference with infer part next.", echo=FALSE, fig.cap="*ModernDive* flowchart – on to Part III!", purl=FALSE, out.width="65%"}
