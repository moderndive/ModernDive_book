...
::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Least-squares fits coefficients to *minimize* the sum of squared residuals on the data at hand. A new predictor's coefficient can always be near zero (fitting nothing new) or fit a sliver of noise, but it can never *force* the residual sum of squares back up. So $R^2$ — the share of variance the model explains — can only stay flat or rise as predictors are added. Watch out: that means a higher $R^2$ on the same data doesn't automatically mean a *better* model.
:::
...
b. **When the slopes don't visibly differ across groups in the EDA plot, and fitting both models gives nearly the same $R^2$** — the simpler model is doing about as well with fewer moving parts
c. When you have lots of data
d. Never; interaction is always more accurate

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** This is the principle of parsimony: pick the simpler model when both fit about equally well. If the interaction-term coefficients are small and the two models' $R^2$ values are close, the interaction term is mostly fitting noise — the parallel-slopes model captures the same relationship with less risk.
:::
