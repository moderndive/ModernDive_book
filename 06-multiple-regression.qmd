...
a. Only one regression line is fit
b. The numerical and categorical predictors are independent
c. Each category gets its own intercept but shares a common slope
...
a. Whether the slope of `y` on `x` differs across levels of `group`
b. The intercept for the reference category
c. The variance of `y`
d. Whether `group` predicts `y`
...
b. The coefficient can never flip sign
c. R has a bug
d. The original (single-predictor) model omitted a confounder; adding it can reveal the *partial* effect
...
b. R errored
c. `low` is the **reference level** — its effect is absorbed into the intercept; the other coefficients are *differences from* `low`
...
c. The compiler optimizes it
d. The data is invariant
...
b. $R^2$ automatically drops to 0
c. The two coefficients become exactly equal
d. **The two slopes answer different questions: $+6.048$ is the simple-regression slope (ignoring `credit_limit`), while $-7.663$ is the partial slope *holding `credit_limit` constant* — with correlated explanatory variables these can differ, even in sign**
...
a. The second is deprecated
b. The second multiplies y by something
c. The first fits two main effects only; the second fits two main effects **plus their interaction** (`x1 * x2` is shorthand for `x1 + x2 + x1:x2`)
d. Identical
...
a. Never; interaction is always more accurate
...
c. Always — simpler is better
d. When you have lots of data
...
b. NORTH has more data
c. NORTH and SOUTH have equal slopes
d. NORTH houses are larger
