...
### One numerical + one categorical: EDA {.unnumbered}
...
### Model with interactions {.unnumbered}
...
### Model without interactions (parallel slopes) {.unnumbered}

**EX6.11 (★★★)** Fit the **parallel-slopes** model. Compare its R² to the interaction model's. Did adding the interaction substantially improve fit?
...
### Observed/fitted values and residuals (interaction model) {.unnumbered}

**EX6.14 (★★★)** Diagnostics for an interaction model: from `m_int` (EX6.7), pull residuals with `get_regression_points()`. (a) Plot residuals vs fitted values *colored by* `discovery_method` — does the residual cloud look the same for both methods? (b) For each `discovery_method`, compute the mean residual; both should be ≈ 0 by construction. Verify, and explain in one sentence *why* the means are mathematically forced to be ≈ 0 for an interaction model.

```{webr-r}
# your code here
```

### Two numerical predictors: EDA {.unnumbered}

**EX6.15 (★★)** Build `planets_temp` with `eq_temp_k`, `star_temp_k`, `insolation_earth` non-missing. Plot `eq_temp_k` vs `insolation_earth` (log-x).
...
### Multiple regression with two numerical regressors {.unnumbered}

**EX6.16 (★★★)** Fit `lm(eq_temp_k ~ log10(insolation_earth) + star_temp_k, data = planets_temp)`.
...
**EX6.20 (★★★)** Predict `eq_temp_k` for an Earth-like exoplanet (`insolation_earth = 1`, `star_temp_k = 5800`). How close is the prediction to Earth's actual ~255 K?

```{webr-r}
# your code here
```

### Observed/fitted values and residuals (two-numerical model) {.unnumbered}

**EX6.21 (★★★)** Plot residuals vs fitted values for `m_two`. Comment on any structure.
...
**EX6.22 (★★★)** Compare three models for `eq_temp_k`: (a) intercept only; (b) `~ log10(insolation_earth)`; (c) `~ log10(insolation_earth) + star_temp_k`. Report each R².
...
**EX6.29 (★★)** Why log-transform `mass_earth` and `radius_earth` but not `eq_temp_k`?

**EX6.30 (★★★)** What's the biggest *limitation* of regression for this dataset given how exoplanets are detected?

**EX6.31 (★★★)** Pick a discovery method with < 50 planets in `planets_lite` and fit `lm(log10(mass_earth) ~ log10(radius_earth))`. Why is the slope *less trustworthy*?
...
**EX6.33 (★★★)** Show that fitting `lm(log10(mass_earth) ~ log10(radius_earth))` estimates the exponent `p` in `M = k R^p`. What does your slope suggest for typical exoplanets?

```{webr-r}
# your code here
```

**EX6.34 (★★★)** Compute the residual standard error of `m_two`. Translate it into temperature units and explain in plain English.

```{webr-r}
# your code here
```

**EX6.35 (★★★)** Why is `eq_temp_k ~ log10(insolation_earth) + star_temp_k` a *physics-informed* model? What rough astrophysics does each predictor capture?
