...
First, let's download a *Comma Separated Values* (CSV) file of ratings of the level of democracy in different countries spanning 1952 to 1992: <https://moderndive.com/files/dem_score.csv>. We use the `read_csv()` function from the `readr` package to read it off the web and then take a look.

```{r message=FALSE, eval=FALSE}
library(readr)
dem_score <- read_csv("https://moderndive.com/files/dem_score.csv")
dem_score
```
...
Let's read in the same data saved in Excel format this time at <https://moderndive.com/files/dem_score. xlsx>, but using RStudio's graphical interface instead of via the R console. First download the Excel file, then go to the Files pane of RStudio -> Navigate to the directory where your downloaded `dem_score.xlsx` is saved -> Click on `dem_score.xlsx` -> Click "Import Dataset..." -> Click "Import Dataset..." At this point you should see an image like in 
...
life_expectancy <- read_csv('https://moderndive.com/files/le_mess.csv')
