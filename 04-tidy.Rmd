...
Second, an Excel `.xlsx` spreadsheet file. This format is based on Microsoft's proprietary Excel software. As opposed to bare-bones `.csv` filse, `.xlsx` Excel file contain a lot of meta-data\index{meta-data} (data about data). Recall we saw a previous example of meta-data in Section \@ref(groupby) when adding "group structure" meta-data to a data frame by using the `group_by()` verb. Some examples of Excel spreadsheet meta-data include the use of bold and italic fonts, colored cells, different column widths, and formula macros.
...
What does it mean for your data to be "tidy"? While "tidy" has a clear English meaning of "organized," the word "tidy" in the context of data science using R means that your data follows a standardized format. We will follow Hadley Wickham's \index{Wickham, Hadley} definition of *tidy data* \index{tidy data} [@tidy] and the subsequent diagram in Figure \@ref(fig:tidyfig):
...
```{r drinks-smaller-tidy-barplot, echo=FALSE, fig.cap='(ref:drinks-col)', fig.height=3.5}
if(knitr::is_html_output()){
  drinks_smaller_tidy_plot
} else {
  drinks_smaller_tidy_plot + scale_fill_grey()
...
We can now create the time-series plot in Figure \@ref(fig:guat-dem-tidy) to visualize how democracy scores in Guatemala have changed from 1952 to 1992 using a \index{ggplot2!geom\_line()} `geom_line()`.
...
```{r part2, echo=FALSE, fig.cap="ModernDive flowchart - On to Part II!", fig.align='center', purl = FALSE}
