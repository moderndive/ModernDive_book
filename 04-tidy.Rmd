...
In Chapter \@ref(programming-concepts) we introduced the concept of a data frame: a rectangular spreadsheet-like representation of data in R where the rows correspond to observations and the columns correspond to variables describing each observation. In Chapter \@ref(nycflights13) we introduced the `flights` data frame (included in the `nycflights13` package) and presented ways of exploring this and any other data frame using the `View()` and `glimpse()` functions. Then in Chapter \ref@(viz) we proceeded to visualize the data contained in data frames.

In this chapter, we'll discuss the importance of a type of data formatting called "tidy data" and you will see that having data stored in "tidy format" is about more than just having your data in a spreadsheet. 

Data comes to us in a variety of formats, from pictures to text to numbers.  Throughout this book, we'll focus on datasets that can be stored in a spreadsheet as that is among the most common way data is collected in the many fields.
