...
We can now create the plot to show how the democracy score of Guatemala changed from 1952 to 1992 using a line-graph and `ggplot2`.
...
**`r paste0("(LC", chap, ".", (lc), ")")`** When datasets are in normal form, we can easily `_join` them with other data sets! For example, can we join the `flights` data with the `planes` data?  We'll see this more in Chapter 5!
