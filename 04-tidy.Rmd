...
```{r tidy-2, message=FALSE, echo=FALSE, purl=FALSE}
...
```{r tidy-3, message=FALSE, eval=FALSE}
library(readr)
dem_score <- read_csv("https://moderndive.com/data/dem_score.csv")
dem_score
```
```{r tidy-4, message=FALSE, echo=FALSE, purl=FALSE}
...
```{r read-excel, echo=FALSE, fig.cap="Importing an Excel file to R.", purl=FALSE, out.width="100%"}
...
```{r tidy-5, echo=FALSE, purl=FALSE}
drinks |>
  head(5)
```
...
```{r drinks-smaller, fig.cap="Comparing alcohol consumption in 4 countries.", fig.height=ifelse(knitr::is_latex_output(), 3.9, 4), echo=FALSE, purl=FALSE}
...
```{r tidy-8, echo=FALSE, purl=FALSE}
...
```{r tidyfig, echo=FALSE, fig.cap="(ref:tidy-r4ds)", out.width="80%", purl=FALSE}
...
```{r drinks-smaller-tidy-barplot, echo=FALSE, fig.cap="(ref:drinks-col)", fig.height=ifelse(knitr::is_latex_output(), 2.5, 4), purl=FALSE}
if (is_html_output()) {
  drinks_smaller_tidy_plot
} else {
  drinks_smaller_tidy_plot + scale_fill_grey()
...
```{r tidy-14, eval=FALSE, purl=FALSE}
airline_safety
```
...
```{r guat-dem-tidy, fig.cap="Democracy scores in Guatemala 1952-1992.", fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
...
```{r tidy-18, eval=FALSE, purl=FALSE}
library(ggplot2)
library(dplyr)
library(readr)
library(tidyr)
...
```{r tidy-19, eval=FALSE, purl=FALSE}
library(tidyverse)
```

would be the same as running:

```{r tidy-20, eval=FALSE, purl=FALSE}
library(ggplot2)
library(dplyr)
library(readr)
library(tidyr)
...
```{r tidy-21, echo=FALSE, results="asis", purl=FALSE}
if (is_latex_output()) {
...
```{r tidy-22, echo=FALSE, purl=FALSE, results="asis"}
...
```{r tidyr-cheatsheet, echo=FALSE, fig.cap="Data Tidying cheatsheet (first page): tidyr package.", out.width="66%", purl=FALSE}
if (is_html_output()) {
...
```{r part2, echo=FALSE, fig.cap="(ref:flowchart-partii)", out.width="50%", purl=FALSE}
