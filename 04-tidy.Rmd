...
Let's now ask ourselves a question: "Using the `drinks_smaller` data frame, how would we create the side-by-side (i.e. dodged) barplot in Figure \@ref(fig:drinks-smaller)?" Recall we saw barplots displaying two categorical variables in Subsection \@ref(two-categ-barplot).
...
What does it mean for your data to be "tidy"? While "tidy" has a clear English meaning of "organized," the word "tidy" in the context of data science using R means that your data follows a standardized format. We will follow Hadley Wickham's \index{Wickham, Hadley} definition of *tidy data* \index{tidy data} [@tidy].
...
In this book so far, you've only seen data frames that were already in "tidy" format. Furthermore, for the rest of this book, you'll mostly only see data frames that are already in "tidy" format as well. This is not always the case however with all datasets in the world. If your original data frame is in wide i.e. non-"tidy" format and you would like to use the `ggplot2` or `dplyr` packages, you will first have to convert it to "tidy" format using the \index{tidyr!gather()} `gather()` function in the `tidyr` \index{R packages!tidyr} package [@R-tidyr]. 
...
1. The third argument is the columns you either want to or don't want to tidy. Observe how we set this to `-country` indicating that we don't want to tidy the `country` variable in `drinks_smaller` and rather only `beer`, `spirit`, and `wine`.
...
Note that the third argument now specifies which columns we want to tidy `c(beer, spirit, wine)`, instead of the columns we don't want to tidy using `-country`. We use the `c()` function to create a vector of the columns in `drinks_smaller` that we'd like to tidy. 
...
In this section, we'll show you another example of how to convert a data frame that isn't in "tidy" format ("wide" format) to a data frame that is in "tidy" format ("long/narrow" format). We'll do this using the `gather()` function from the `tidyr` package again. 
...
1. The third argument is the columns you either want to or don't want to tidy. Observe how we set this to `-country` indicating that we don't want to tidy the `country` variable in `guat_dem` and rather only variables `1952` through `1992`. 
...
We can now create the time-series plot to visualize how democracy scores in Guatemala have changed from 1952 to 1992 using a \index{ggplot2!geom\_line()} `geom_line()`.
...
After installing the `tidyverse` package as you would a normal package via `install.packages("tidyverse")`, running:
...
You've seen the first four of these packages: `ggplot2` for data visualization, `dplyr` for data wrangling, `tidyr` for converting data to "tidy" format, and `readr` for importing spreadsheet data into R. The remaining packages (`purrr`, `tibble`, `stringr`, and `forcats`) are left for a more advanced book; check out [R for Data Science](http://r4ds.had.co.nz/) to learn about these packages.
...
If you want to learn more about using the `readr` \index{R packages!readr!cheatsheet} and `tidyr` \index{R packages!tidyr!cheatsheet} package, we suggest that you check out RStudio's "Data Import Cheat Sheet." In the current version of RStudio in late 2019, you can access this cheatsheet by going to the RStudio Menu Bar -> Help -> Cheatsheets -> "Browse Cheatsheets" -> Scroll down the page to the "Data Import Cheat Sheet." The first page of this cheatsheet has information on using the `readr` package to import data while the second page has information on using the `tidyr` package to "tidy" data. `r if(knitr::is_html_output()) "You can see a preview of both cheatsheets in the figures below."`
