...
We do this directly using the regression model `simple_model` and the [`get_regression_points()`](https://moderndive.github.io/moderndive/reference/get_regression_points.html) function. 
...
```{r fig.cap="Boxplot of popularity by genre.", fig.height=ifelse(knitr::is_latex_output(), 3.2, 4)}
...
We can use the `lm()` function to fit a linear model with dummy variables for the genres. We'll then use the [`get_regression_table()`](https://moderndive.github.io/moderndive/reference/get_regression_table.html) function to get the regression table.
...
```{r pvalue1, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 4), fig.cap="Illustration of a two-sided p-value for a t-test."}
n <- n_old_faithful
...
We can calculate all the $n = `r n_old_faithful`$ residuals by applying the [`get_regression_points()`](https://moderndive.github.io/moderndive/reference/get_regression_points.html) function to the regression model `model_1`. 
...
-   Use the [`get_regression_points()`](https://moderndive.github.io/moderndive/reference/get_regression_points.html) function to get the observed values, fitted values, and residuals for all UN member countries.
...
```{r null-distribution-slope, echo=FALSE, fig.cap="Null distribution of slopes.", fig.height=ifelse(knitr::is_latex_output(), 2.5, 4), purl=FALSE}
...
The standard errors are shown when applying the [`get_regression_table()`](https://moderndive.github.io/moderndive/reference/get_regression_table.html) function on a regression model. This is the output for model `mod_mult`:
...
```{r, fig.cap="Residuals vs. fitted values plot and QQ-plot for the multiple regression model.", fig.height=ifelse(knitr::is_latex_output(), 4, 4)}
g1 <- fit_and_res_mult |>
...
We will now construct the bootstrap distribution for the partial slopes using the `infer` workflow. Just like in Section \@ref(infer-ci-slr), we are now resampling entire rows of values. We construct the bootstrap distribution for the fitted partial slopes using our full sample of `r n_coffee` coffees:
...
```{r boot-distn-slopes, fig.cap="Bootstrap distributions of partial slopes.", fig.height=8, fig.width=6}
