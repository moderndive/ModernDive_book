...
```{r regline-ch10, fig.cap="Relationship with regression line.", fig.height=ifelse(knitr::is_latex_output(), 3, 7), message=FALSE}
...
```{r geyserplot1, echo=F, fig.cap="Scatterplot of relationship of eruption duration and waiting time", fig.height=ifelse(knitr::is_latex_output(), 4.5, 7)}
...
```{r fig.cap="Boxplot of popularity by genre.", fig.height=ifelse(knitr::is_latex_output(), 3.2, 7)}
...
- C. The difference between the observed value and the predicted value of the response variable.
...
First, observe that $n = `r n_old_faithful`$, so the degrees of freedom are $n-2 = 112$. The critical value for a 95% confidence interval on a $t$-distribution with 112 degrees of freedom is $q = 1.981$. Second, the estimates $b_0$, $b_1$, and $s$ were found earlier and are shown here again:
...
```{r pvalue1, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 7), fig.cap="Illustration of a two-sided p-value for a t-test"}
n <- n_old_faithful
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** In the context of a linear regression model, what does the error term $\epsilon_i$ represent?

- A. The expected value of the response variable.
- B. The difference between the observed and predicted values of the response variable.
- C. The variation in the explanatory variable.
- D. The standard deviation of the response variable.

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Which of the following is an assumption of the linear regression model?

- A. The error terms $\epsilon_i$ are dependent on each other.
- B. The error terms $\epsilon_i$ have a non-zero mean.
- C. The error terms $\epsilon_i$ are normally distributed with constant variance.
...
```{r residual-example, echo=FALSE, fig.cap="Example of observed value, fitted value, and residual.", purl=FALSE, message=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2.2, 7)}
...
```{r scatt-and-residual, echo=FALSE, fig.cap="The scatterplot and residual plot for the Old Faithful data.", purl=FALSE, message=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2.3, 7)}
...
```{r non-linear, echo=FALSE, fig.cap="Example of a non-linear relationship.", fig.height=ifelse(knitr::is_latex_output(), 3.3, 7), message=FALSE, purl=FALSE}
set.seed(76)
...
```{r time-plot, echo=FALSE, fig.cap="Scatterplot of date (time) vs residuals for the Old Faithful example.", fig.height=ifelse(knitr::is_latex_output(), 3, 7), message=FALSE, purl=FALSE}
old_faithful_2024 |>
...
```{r not-normal-residuals, echo=FALSE, fig.cap="Non-normal residuals.", message=FALSE, purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 1.8, 7)}
set.seed(3)
...
```{r residual-plot, fig.cap="Plot of residuals against the regressor.", message=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2.2, 7)}
...
```{r equal-variance-residuals, echo=FALSE, fig.cap="Example of clearly non-equal variance.", purl=FALSE, fig.height=ifelse(is_latex_output(), 2.5, 7)}
old_faithful_2024 |>
...
- B. There is strong evidence against the null hypothesis that the slope coefficient is zero, suggesting a statistically significant linear relationship between the explanatory and response variables.
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`**  Which assumption of the linear regression model does the residual plot primarily help to assess?

- A. Linearity of the relationship between variables
- B. Independence of the error terms
- C. Normality of the error terms
- D. Equality or constancy of variance
...
```{r bootstrap-distribution-slope, fig.show="hold", fig.cap="Bootstrap distribution of slope.", fig.height=ifelse(knitr::is_latex_output(), 2.2, 7)}
...
```{r null-distribution-slope, echo=FALSE, fig.show="hold", fig.cap="Null distribution of slopes.", fig.height=ifelse(knitr::is_latex_output(), 2.5, 7), purl=FALSE}
...
```{r p-value-slope, echo=FALSE, fig.show="hold", fig.cap="Null distribution and $p$-value.", fig.height=ifelse(knitr::is_latex_output(), 3, 7), purl=FALSE}
visualize(null_distn_slope) +
...
To get the result of this test in R, we use the R function `anova()` and enter the reduced model followed by the full model:

```{r eval=FALSE}
anova(mod_mult_2, mod_mult_1) 
```

Table \@ref(tab:mult-model-4) provides information for this test.
...
from the original sample of `r n_coffee` coffee data. We generate 
...
```{r boot-distn-slopes, fig.cap="Bootstrap distributions of partial slopes.", fig.height=8}
...
```{r ci-slopes-multiple, fig.cap="95% confidence intervals for the partial slopes.", fig.height=8}
visualize(boot_distribution_mlr) +
