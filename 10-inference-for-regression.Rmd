...
We also include the regression line using the least-squares criterion explained above:

```{r regline-ch10, fig.cap="Relationship with regression line.", fig.height=ifelse(knitr::is_latex_output(), 3.2, 7), message=FALSE}
ggplot(UN_data_ch10, 
...
Note that the `p_value` for the `genre: Romance` row is the $p$-value for the hypothesis test of 

$$H_0: \text{action and romance have the same mean rating} \\ H_A: \text{action and romance have different mean ratings}$$ 
...
```{r time-plot, echo=FALSE, fig.cap="Scatterplot of date (time) vs residuals for the Old Faithful example.", fig.height=ifelse(knitr::is_latex_output(), 3.3, 7), message=FALSE, purl=FALSE}
old_faithful_2024 |>
...
mod_mult_table |>
  kbl(
#    caption = "The regression table for $\text{mod\_mult}$",
    digits = 3,
    booktabs = TRUE,
    linesep = ""
  ) |>
...
mod_mult_table |>
  kbl(
#    caption = "The regression table for 'mod_mult' with 98\% level",
    digits = 3,
    booktabs = TRUE,
    linesep = ""
  ) |>
...
mod_mult_table_1 |>
  kbl(
    caption = "The regression table for 'total cup points' by aroma + flavor + 'moisture percentage'",
    digits = 3,
    booktabs = TRUE,
    linesep = ""
  ) |>
