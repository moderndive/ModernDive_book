...
Recall from Section \@ref(histograms) that histograms allow us to visualize the *distribution* \index{distribution} of a numerical variable: where the values center and in particular how they vary. The resulting hand-drawn histogram can be seen in Figure \@ref(fig:sampling-exercise-5).
...
Let's display only the first 10 out of 33 rows of `tactile_prop_red`'s contents in Table \@ref(tab:tactile-red1).

```{r tactile-red1, echo=FALSE}
tactile_prop_red %>% 
  slice(1:10) %>% 
  kable(
    digits = 3,
...
<!-- Albert will remove `boundary` here on updates to chalkboard image if needed. -->
...
1. **Representative sampling**: A sample is said to be a *representative sample* if it is representative of the population. In other words, are the sample's characteristics a good representation of the population's characteristics? In our simulations, are the samples of $n$ balls extracted using our shovels representative of the bowl's $N$=2400 balls?
...
Let's tie elements of the real-life poll in this new article with our "tactile" and "virtual" simulations from Sections \@ref(sampling-activity) and \@ref(sampling-simulation) using the terminology, notations, and definitions we learned in Section \@ref(sampling-framework). You see that our sampling activity with the bowl is an idealized version of what pollsters are trying to do in real-life. 

1. **(Study) Population**: Who is the population of $N$ individuals or observations of interest? \index{sampling!population}
    + Simulation: $N$ = 2400 identically-sized red and white balls
    + Obama poll: $N$ = ? young Americans aged 18-29
1. **Population parameter**: What is the population parameter? \index{sampling!population parameter}
...
1. **Census**: What would a census look like? \index{sampling!census}
...
1. **Sampling**: How do you collect the sample of size $n$ individuals or observations? \index{sampling}
...
1. **Representative sampling**: Is the sampling procedure *representative*? \index{sampling!representative}
...
1. **Generalizability**: Are the samples *generalizable* to the greater population? \index{generalizability}
    + Simulation: Is the sample proportion $\widehat{p}$ of the shovel's balls that are red a "good guess" of the population proportion $p$ of the bowl's balls that are red? Given that the sample was representative, the answer is yes.
...
1. **Bias**: Is the sampling procedure unbiased? In other words, do all observations have an equal chance of being included in the sample? \index{bias}
...
1. **Random sampling**: Was the sampling random? \index{sampling!random}
...
In other words, their sampling distribution increasingly follows a *normal distribution* \index{distribution!normal} and the spread/variation of these sampling distributions, as quantified by their standard errors,  gets smaller. 
...
In the previous Subsection on the Central Limit Theorem, we introduced the notion of a normal distribution. Such distributions are defined by two values: 1) the mean $\mu$, which locates the center of the distribution, and 2) the standard deviation $\sigma$, which determines the spread of the distribution. In Figure \@ref(fig:normal-curves), we plot three normal distributions where:
...
1. The dotted line normal curve has mean $\mu$ = 14 and standard deviation $\sigma$ = 2.
...
ggplot(data = data.frame(x = c(-10, 25)), aes(x)) +
  ...
  stat_function(fun = dnorm, args = list(mean = 14, sd = 2), n = 500, linetype = "dotted") +
...
1. 68% of values will lie within $\pm$ 1 standard deviations of the mean.
...
Let's illustrate this on a standard normal curve in Figure \@ref(fig:normal-rule-of-thumb). The dashed lines are at -3, -1.96, -1, 0, 1, 1.96, and 3 and cut up the x-axis into 8 segments. The areas under the normal curve for each of the 8 segments are marked and add up to 100%.
