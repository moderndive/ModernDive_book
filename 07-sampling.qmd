---
description: &desc "Simulate repeated samples with rep_slice_sample(), build a sampling distribution, and connect it to the Central Limit Theorem."
open-graph:
  description: *desc
twitter-card:
  description: *desc
...
```{r lc-inline-setup-07, include=FALSE, purl=FALSE}
source("scripts/lc_solutions.R")
lc_setup(7)
```

The third portion of this book introduces statistical inference. This chapter is about *sampling*. Sampling involves drawing repeated random samples from a population. In @sec-sampling-activity, we illustrate sampling by working with samples of white and red balls and the proportion of red balls in these samples. In @sec-sampling-framework, we present a theoretical framework and define what is the sampling distribution. We introduce one of the fundamental theoretical results in Statistics: the *Central Limit Theorem* in @sec-central-limit-theorem. In @sec-sampling-activity-mean, we present a second sampling activity, this time working with samples of chocolate-covered almonds and the average weight of these samples.  In @sec-sampling-other-scenarios, we present the sampling distribution in other scenarios. The concepts behind *sampling* form the basis of inferential methods, in particular confidence intervals and hypothesis tests; methods that are studied in @sec-confidence-intervals and @sec-hypothesis-testing.

## Needed packages {.unnumbered #sec-sampling-packages}
...
The `bowl` has `r num_balls` rows representing the `r num_balls` balls in the bowl shown in @fig-sampling-exercise-1. You can view and scroll through the entire contents of the `bowl` in RStudio's data viewer by running `View(bowl)`. The first variable `ball_ID` is used as an *identification variable* as discussed in @sec-identification-vs-measurement-variables; none of the balls in the actual bowl are marked with numbers. 

The second variable `color` indicates whether a particular virtual ball is red or white. We compute the proportion of red balls in the bowl using the `dplyr` data-wrangling verbs presented in @sec-wrangling. A few steps are needed in order to determine this proportion. We present these steps separately to remind you how they work but later introduce all the steps together and simplify some of the code. First, for each of the balls, we identify if it is red or not using a test for equality with the logical operator `==`. We do this by using the `mutate()` function from @sec-mutate that allows us to create a new Boolean variable called `is_red`.
...
We compute this using the `sum()` function inside the `summarize()` function. Recall from @sec-summarize that `summarize()` takes a data frame with many rows and returns a data frame with a single row containing summary statistics such as the `sum()`:
...
### Manual sampling {#sec-sampling-manual}
...
#### One sample {.unnumbered}

We start by inserting a shovel into the bowl as seen in @fig-sampling-exercise-2 and collect $5 \cdot 10 = 50$ balls as shown in @fig-sampling-exercise-3. The set of balls retrieved is called a _sample_. 

```{r fig-sampling-exercise-2, fig.alt="Photograph of a slotted shovel being inserted into the bowl of balls.", echo=FALSE, fig.cap="Inserting a shovel into the bowl.", purl=FALSE, out.width="100%", purl=FALSE}
include_graphics("images/sampling/balls/sampling_bowl_2.jpg")
```


```{r fig-sampling-exercise-3, fig.alt="Photograph of a shovel holding 50 balls scooped from the bowl, with a mix of red and white visible.", echo=FALSE, fig.cap="Taking a sample of 50 balls from the bowl.", purl=FALSE, out.width="80%", purl=FALSE}
include_graphics("images/sampling/balls/sampling_bowl_3_cropped.jpg")
```

Observe that 17 of the balls are red, and thus the proportion of red balls in the sample is 17/50 = 0.34 or 34%. Compare this to the proportion of red balls in the entire bowl, `r prop_red`, that we found in @sec-population-proportion. The proportion from the sample seems actually pretty good, and it did not take much time or energy to get. But, was this approximate proportion just a lucky outcome? Could we be this lucky the next time we take a sample from the bowl? Next we take more samples from the bowl and calculate the proportions of red balls.

#### Thirty-three samples {.unnumbered}

We now take many more random samples as shown in @fig-sampling-exercise-3b. Each time we do the following:
...
```{r fig-sampling-exercise-3b, fig.alt="Three side-by-side photographs of repeated samples drawn from the bowl, illustrating that each sample contains a different mix of red and white balls.", echo=FALSE, fig.show='hold', fig.cap="Repeating sampling activity.", purl=FALSE, out.width="25%"}
...
When we perform this activity many times, we observe that different samples may produce different proportions of red balls. A proportion of red balls from a sample is called a _sample proportion_.  A group of 33 students performed this activity previously and drew a histogram using blocks to represent sample proportions of red balls. @fig-sampling-exercise-4 shows students working on the histogram with two blocks drawn already representing the first two sample proportions found and the third about to be added.

```{r fig-sampling-exercise-4, fig.alt="Photograph of students at a chalkboard drawing a histogram of sample proportions, with bars representing the count of red balls in each sample.", echo=FALSE, fig.cap="Students drawing a histogram of sample proportions.", purl=FALSE, out.width="60%"}
include_graphics("images/sampling/balls/tactile_3_a.jpg")
```

Recall from @sec-histograms that histograms help us visualize the *distribution* \index{distribution} of a numerical variable. In particular, where the center of the values falls and how the values vary. A histogram of the first 10 sample proportions can be seen in @fig-sampling-exercise-5.

```{r fig-sampling-exercise-5, fig.alt="Hand-drawn histogram on a chalkboard showing 10 sample proportions clustered between roughly 0.3 and 0.5.", echo=FALSE, fig.cap="Hand-drawn histogram of 10 sample proportions.", purl=FALSE, out.height="30%"}
...
```
```{r fig-samplingdistribution-tactile, fig.alt="Histogram of 33 sample proportions of red balls (n=50 each) collected from students. Distribution is roughly symmetric and centered near 0.4.", echo=FALSE, fig.cap="The distribution of sample proportions based on 33 random samples of size 50.", fig.height=ifelse(knitr::is_latex_output(), 3.3, 4), purl=FALSE}
...
In the previous @sec-sampling-manual, we performed a *tactile* sampling activity: students took physical samples using a real shovel from a bowl with white and red balls by hand. We now extend the entire process using simulations on a computer, a sort of *virtual* sampling activity.
...
We compute the proportion of red balls in our virtual sample. The code we use is similar to the one used for finding the proportion of red balls in the entire bowl in @sec-population-proportion:
...
As was the case in the tactile activity, there is sampling variation in the resulting 33 proportions from the virtual samples. As we did manually in @sec-sampling-manual, we construct a histogram with these sample proportions as shown in @fig-samplingdistribution-virtual. The histogram helps us visualize the sampling distribution of the sample proportion. Observe again the histogram was constructed using `ggplot()`, `geom_histogram()`, and including the arguments `binwidth = 0.05` and `boundary = 0.4`.
...
```
```{r fig-samplingdistribution-virtual, fig.alt="Same kind of histogram, but with 33 virtual (computer-generated) sample proportions instead of student-collected ones. Same general shape and center.", echo=FALSE, fig.cap="The distribution of 33 proportions based on 33 virtual samples of size 50.", fig.height=ifelse(knitr::is_latex_output(), 3.5, 4), purl=FALSE}
...
When observing the histogram we can see that some proportions are lower than 25% and others are greater than 45%. Also, the sample proportions observed more frequently are between 35% and 40% (for 11 out of 33 samples). We found similar results when sampling was done by hand in @sec-sampling-manual, and the histogram was presented in @fig-samplingdistribution-tactile. We present both histograms side by side in @fig-tactile-vs-virtual for an easy comparison. Note that they are somewhat similar in their center and variation, although not identical. The differences are also due to *sampling variation*.

```{r fig-tactile-vs-virtual, fig.alt="Two histograms side by side comparing virtual sample proportions (left) and tactile student-generated sample proportions (right). The shapes are very similar, validating the simulation approach.", echo=FALSE, fig.cap="The sampling distribution of the sample proportion and sampling variation:  showing a histogram for virtual sample proportions (left) and another histogram for tactile sample proportions (right).", fig.height=ifelse(knitr::is_latex_output(), 2.9, 4), purl=FALSE}
...
```{r lc-sol-07-03, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 3))
```

:::

#### One thousand virtual samples {.unnumbered}
...
As done previously, a histogram for these 1000 sample proportions is given in @fig-samplingdistribution-virtual-1000.
...
```
```{r fig-samplingdistribution-virtual-1000, fig.alt="Histogram of 1000 virtual sample proportions (n=50 each). The shape is now clearly bell-shaped and centered at the true population proportion.", echo=FALSE, fig.cap="The distribution of 1000 proportions based on 1000 random samples of size 50.", fig.height=ifelse(knitr::is_latex_output(), 2.3, 4), purl=FALSE}
virtual_histogram <- ggplot(virtual_prop_red, aes(x = prop_red)) +
  geom_histogram(binwidth = 0.04, boundary = 0.4, color = "white")
virtual_histogram +
  labs(
...
```{r lc-sol-07-04, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 4))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Looking at @fig-samplingdistribution-virtual-1000, would you say that sampling 50 balls where 30% of them were red is likely or not? What about sampling 50 balls where 10% of them were red?

```{r lc-sol-07-05, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 5))
```
...
We must first decide the sample size we want to use, and then take samples using that size. As an illustration, we can perform the sampling activity three times, for each activity using a different sample size, think of having three shovels of sizes 25, 50, and 100 as shown in @fig-three-shovels. Of course, we do this virtually: with each shovel size we gather many random samples, calculate the corresponding sample proportions, and plot those proportions in a histogram. Therefore we create three histograms, each one describing the sampling distribution for sample proportions from samples of size 25, 50, and 100, respectively. As we show later in this subsection, the size of the sample has a direct effect on the sampling distribution and the magnitude of its sampling variation.
...
```{r fig-three-shovels, fig.alt="Photograph of three different-sized shovels for extracting samples of 25, 50, and 100 balls respectively.", echo=FALSE, fig.cap="Three shovels to extract three different sample sizes.", out.width='100%', purl=FALSE}
...
```{r fig-comparing-sampling-distributions, fig.alt="Three histograms (n=25, 50, 100) on the same x-axis, showing how the sampling distribution of the proportion narrows as sample size increases.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 4), fig.cap="Histograms of sample proportions for different sample sizes.", purl=FALSE}
# n = 25
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** As shown in @fig-comparing-sampling-distributions the histograms of sample proportions are somewhat bell-shaped. What can you say about the center of the histograms?
...
```{r lc-sol-07-06, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 6))
```



**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** As shown in @fig-comparing-sampling-distributions as the sample size increases, the histogram gets narrower. What happens with the sample proportions?

- A. They vary less.
- B. They vary by the same amount.
- C. They vary more.

```{r lc-sol-07-07, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 7))
```
...
```{r lc-sol-07-08, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 8))
```
...
```{r lc-sol-07-09, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 9))
```
...
In @sec-sampling-activity, we gained some intuition about sampling and its characteristics. In this section, we introduce some statistical definitions and terminology related to sampling. We conclude by introducing key characteristics that will be formally studied in the rest of the chapter.


### Population, sample, and the sampling distribution {#sec-terminology-and-notation}
...
**Three different distributions, easily confused.**
- The *data distribution* — the spread of values in **one** sample.
- The *population distribution* — the spread of values in the **entire population** (usually unknown).
- The *sampling distribution* — the spread of a **statistic** (like $\bar{x}$ or $\widehat{p}$) across **many possible samples** of the same size.

Almost every confusion in inference comes from mixing these up. When you see a histogram in this part of the book, ask yourself: *what is one dot/observation on this plot?* If it's a single data value → data or population distribution. If it's a *sample statistic* → sampling distribution.
:::

The **distribution** of a list of numbers is the set of the possible values in the list and how often they occur. The **sampling distribution of the sample proportion** is the **distribution**  of sample proportions from **each possible** random samples of a given size. \index{sampling distributions} To illustrate this concept recall that in @sec-sampling-simulation we drew three histograms shown in @fig-comparing-sampling-distributions. The histogram on the left, for example, was constructed from taking 1000 random samples of size $n=25$, then finding the sample proportion for each sample and using these proportions to draw the histogram. This histogram is a good visual approximation of the **sampling distribution** of the sample proportion.
...
- A histogram from a simulation of sample proportions is only a visual approximation of the sampling distribution. It is not the exact distribution. Still, when the simulations produce a large number of sample proportions, the resulting histogram provides a good approximation of the sampling distribution. This was the case in @sec-sampling-simulation and the three histograms shown in @fig-comparing-sampling-distributions. 

The lessons we learned by performing the activities in @sec-sampling-activity contribute to gaining insights about key characteristics of the *sampling distribution* of the *sample proportion*, namely:
...
```{r lc-sol-07-10, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 10))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** How did we ensure that the samples collected with the shovel were random?

```{r lc-sol-07-11, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 11))
```
...
$$\{1,0,0,1,0,0,0,1,0,0,1,0,1,0,1,1,0,0,0,1,0,1,0,0,0\}$$ 
...
Indeed, the results are about the same as the population proportion. Note that the average of 1000 sample proportions for samples of size 50 was actually `r e_xbar_50_pull`, close to but not exactly 0.375. This happens because the simulations only approximate the sampling distribution and the expected value. When using simulations we do not expect to achieve the exact theoretical results, rather values that are close enough to support our understanding of the theoretical results.
...
```{r lc-sol-07-12, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 12))
```

:::

### Sampling variation: standard deviation and standard error {#sec-sampling-variation}

Another relevant characteristic observed in @fig-comparing-sampling-distributions-3 is how the amount of dispersion or *sampling variation* changes when the sample size changes. While all the histograms have a similar bell-shaped configuration and are centered at the same value, observe that when...
...
As the sample size $n$ increases from 25 to 50 to 100, \index{sampling distributions!relationship to sample size} the variation of the sampling distribution decreases. Thus, the values are clustered more and more tightly around the center of the distribution. In other words, the histogram on the left of @fig-comparing-sampling-distributions-3 is more spread out than the one in the middle, which in turn is more spread out than the one on the right.
...
#### The standard deviation {.unnumbered}
...
$$\bar y = \frac14\sum_{i=1}^4y_i =\frac14 (y_1 + y_2 + y_3 + y_4)=  \frac{3-1+5+9}{4}= 4.$$ 

The capital Greek letter $\Sigma$ represents the summation of values, and it is useful when a large number of values need to be added. The letter $i$ underneath $\Sigma$ is the index of summation. It starts at $i=1$, so the first value we are adding is $y_{\bf 1} = 3$. Afterwards $i=2$, so we add $y_{\bf 2}=-1$ to our previous result, and so on, as shown in the equation above. The summation symbol can be very useful when adding many numbers or making more complicated operations, such as defining the standard deviation.
...
$$\begin{aligned}
SD &= \sqrt{\frac14\sum_{i=1}^4(y_i - \bar y)^2} = \sqrt{\frac{(3-4)^2+(-1-4)^2+(5-4)^2+(9-4)^2}{4}} \\ 
   &= \sqrt{\frac{1+25+1+25}{4}}=\sqrt{13} = 3.61
\end{aligned}$$ 
...
So, the proportion of red balls is 0.375 and the standard deviation of the individual red/not-red values is 0.484. The intuition behind this standard deviation can be expressed as follows: if you were to select many balls, with replacement, from the bowl, and score each one as 1 for red and 0 for not red, those individual 0/1 values would center at 0.375 and typically fall about 0.484 away from it. Note that this describes how much one ball at a time varies, not how much the *proportion* of red balls in a sample varies; a proportion of "0.375 give or take 0.484" would run outside the range from 0 to 1. The variability of the proportion itself is much smaller, and it is what we study in the rest of this chapter.
...
#### The sampling distribution of the sample proportion {.unnumbered}
...
As mentioned before, histograms (such as those seen earlier) provide a good approximation of the sampling distribution of the sample proportion, the distribution of $\overline X$. Since we are interested in the shape of the distribution, we redraw again the histograms using sample proportions from random samples of size $n=25$, $n=50$, and $n=100$, but this time we add a smooth curve that appears to connect the top parts of each bar in the histogram. These histograms are presented in @fig-sample-proportion-25-with-normal-pdf, @fig-sample-proportion-50-with-normal-pdf, and @fig-sample-proportion-100-with-normal-pdf. The figures represent density histograms where the area of each bar represents the percentage or proportion of observations for the corresponding bin and the total area of each histogram is 1 (or 100%). The ranges for the $x-$ and $y-$axis on all these plots have been kept constant for appropriate comparisons among them. 

```{r fig-sample-proportion-25-with-normal-pdf, fig.alt="Histogram of the sampling distribution of the sample proportion (n=25) with a fitted normal density curve overlaid. The bell shape is visible but rough.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 4), fig.cap="Histogram of the distribution of the sample proportion and the normal curve (n=25).", purl=FALSE}
n = 25
p=9/24
sd.p = sqrt(p*(1-p)/n)
...
```{r fig-sample-proportion-50-with-normal-pdf, fig.alt="Same histogram-with-normal-overlay, n=50. The fit is tighter than at n=25.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 4), fig.cap="Histogram of the distribution of the sample proportion and the normal curve (n=50).", purl=FALSE}
n = 50
p=9/24
sd.p = sqrt(p*(1-p)/n)
...
```{r fig-sample-proportion-100-with-normal-pdf, fig.alt="Same histogram-with-normal-overlay, n=100. Closely matches the smooth normal curve.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 4), fig.cap="Histogram of the sampling distribution of the sample proportion and the normal curve (n=100).", purl=FALSE}
n = 100
p=9/24
sd.p = sqrt(p*(1-p)/n)
...
```{r lc-sol-07-13, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 13))
```
...
```{r lc-sol-07-14, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 14))
```
...
## Second activity: chocolate-covered almonds {#sec-sampling-activity-mean}

We want to extend the results achieved for the sample proportion to a more general case: the **sample mean**. In this section we show how 
...
As we did with sample proportions, we start by illustrating these results with another activity: sampling from a bowl of chocolate-covered almonds, as seen in @fig-bowl-almond. 

```{r fig-bowl-almond, fig.alt="Photograph of a bowl filled with chocolate-covered almonds.", echo=FALSE, fig.cap="A bowl of chocolate-covered almonds.", purl=FALSE, out.width="40%", purl=FALSE}
knitr::include_graphics("images/sampling/almonds/v2_side_almonds.jpg")
```
...
### The population mean weight of almonds in the bowl {#sec-population-mean}
...
```{r fig-almonds-bowl-histogram, fig.alt="Histogram of weights for the entire bowl of almonds, ranging from about 2.6 to 4.6 grams. The distribution is irregular rather than bell-shaped, with the most common weights between 3.6 and 4.0 grams.", fig.cap="Distribution of weights for the entire bowl of almonds."}
...
### Manual sampling and sample means {#sec-resampling-tactile-bowl}

If we randomly select one almond from the bowl, we could determine its weight using a scale, as shown in @fig-one-almond:

```{r fig-one-almond, fig.alt="Photograph of a single chocolate-covered almond on a kitchen scale showing its weight.", echo=FALSE, fig.show="hold", fig.cap="One almond on a scale.", purl=FALSE, out.width = "40%"}
include_graphics("images/sampling/almonds/one-almond.png")
```

Let's now take a random sample of 25 almonds, as shown in @fig-twenty-five-almonds, and determine the sample average weight, or *sample mean* weight, in grams.

```{r fig-twenty-five-almonds, fig.alt="Photograph of 25 almonds arranged on a scale platform for weighing.", echo=FALSE, fig.cap="A random sample of 25 almonds on a scale.",  out.width = "40%", purl=FALSE}
include_graphics("images/sampling/almonds/twenty-five-almonds.png")
```

Since the total weight is 88.6 grams, as shown in @fig-twenty-five-almonds, the sample mean weight will be $88.6/25 = 3.544$. The `moderndive` \index{R packages!moderndive!almonds\_sample} package contains the information of this sample in the `almonds_sample` data frame. Here, we present the weight of the first 10 almonds in the sample:
 
```{r sampling-v32}
almonds_sample
```

```{r sampling-create-num_almonds, echo=FALSE}
num_almonds <- length(almonds_sample$weight)
```

The `almonds_sample` data frame in the `moderndive` package has $n=$ `r num_almonds` rows corresponding to each almond in the sample shown in @fig-twenty-five-almonds.
The first variable `replicate` indicates this is the first and only replicate since it is a single sample. The second variable `ID` gives an identification to the particular almond. The third column `weight` gives the corresponding weight for each almond in grams as a numeric variable, also known as a double (`dbl`). The distribution of the weights of these `r num_almonds` are shown in the histogram in @fig-almonds-sample-histogram.

```{r fig-almonds-sample-histogram, fig.alt="Histogram of weights for one sample of 25 almonds. Roughly bell-shaped but with visible irregularity due to small sample size.", fig.cap="Distribution of weight for a sample of 25 almonds.", fig.height=ifelse(knitr::is_latex_output(), 1.5, 4)}
ggplot(almonds_sample, aes(x = weight)) +
  geom_histogram(binwidth = 0.1, color = "white")
```

The weights of almonds in this sample range from `r min(almonds_sample$weight)` to `r max(almonds_sample$weight)` grams. There is not an obvious pattern in the distribution of this sample. We now compute the sample mean using our data-wrangling tools from @sec-wrangling.

```{r sampling-compute-mean-v9}
...
### Virtual sampling {#sec-virtual-samples-mean-bowl}

We now perform sampling virtually. The data frame `almonds_bowl` has `r num_pop_almonds` rows, each representing an almond in the bowl. As we did in @sec-sampling-simulation we use again the `rep_slice_sample()` function to retrieve 1000 random samples with a sample `size` set to be 25, and with the number of replicates `reps` set to `1000`. Be sure to scroll through the contents of `virtual_samples` in RStudio's viewer. 

```{r sampling-assign-virtual_samples_al}
...
```{r sampling-assign-virtual_mean_weigh}
virtual_mean_weight <- virtual_samples_almonds |> 
  summarize(mean_weight = mean(weight))
virtual_mean_weight
```

@fig-sampling-mean-virtual-1000 presents the histogram for these sample means:

```{r sampling-hist-white-border-v2-dup3, echo=TRUE, fig.show='hide'}
...
```{r fig-sampling-mean-virtual-1000, fig.alt="Histogram of 1000 sample means (each from a sample of size 25). Bell-shaped, centered at the true population mean weight.", echo=FALSE, fig.cap="The distribution of 1000 means based on 1000 random samples of size 25.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 1.5, 4)}
virtual_mean_weight <- virtual_samples_almonds |> 
  summarize(mean_weight = mean(weight))
...
The sample mean weights observed in the histogram appear to go below 3.4 grams and above 3.85 grams, but those extreme sample means are rare. The most frequent sample means found seem to be those between 3.5 and 3.8 grams. 
Furthermore, the histogram is almost symmetric and showing that bell-shaped form, although the left tail of the histogram appears to be slightly longer than the right tail. While we are dealing with sample means now, the conclusions are strikingly similar to those presented in @sec-sampling-simulation when discussing the sampling distribution for the sample proportion.
...
### Random variables {#sec-random-variable-sample-mean}
...
Using random variables, we now let uppercase $X_1$ be the random variable that represents the weight of the first almond before it has been selected, $X_2$ the weight of the second almond, and so on. These are random variables because they can take any possible almond weight value from the bowl.
After the first trial is completed, the value of $X_1$ is realized as the weight in grams of the first almond selected. We can represent this value by the lowercase $x_1$ as it is no longer a random variable but a number and we can write $X_1 = x_1$. After the second trial is completed, $X_2 = x_2$, where lowercase $x_2$ is the observed almond weight, and so on.
...
```{r sampling-hist-white-border-v2-dup5, eval=FALSE}
# Segment 1: sample size = 25 ------------------------------
...
We present the three resulting histograms in a single row with matching x and y axes in @fig-comparing-sampling-distributions-means so the comparison among them is clear.

```{r fig-comparing-sampling-distributions-means, fig.alt="Three histograms (n=25, 50, 100) of sample means, showing how the distribution narrows as sample size increases.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 4), fig.cap="Comparing histograms of sample means when using different sample sizes.", purl=FALSE}
# n = 25
...
comparing_sampling_distributions <- ggplot(virtual_mean, aes(x = mean_weight)) +
  ...
  facet_wrap(~n) +
  geom_vline(xintercept = mean(almonds_bowl$weight), col = "red", size = 1)
...
```{r tbl-comparing-n1, echo=FALSE, purl=FALSE}
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** How does increasing the sample size affect the standard error of the sample mean?
...
```{r lc-sol-07-15, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 15))
```

:::

### The Central Limit Theorem revisited {#sec-CLT-mean}

Finally, observe that the shapes of the histograms in @fig-comparing-sampling-distributions-means are bell-shaped and seem to approximate the normal distribution. As we did with proportions, in @fig-sample-mean-25-with-normal and @fig-sample-mean-100-with-normal we compare our histograms with the theoretical curve for the normal distribution. 

```{r fig-sample-mean-25-with-normal, fig.alt="Histogram of the sampling distribution of the sample mean (n=25) with the corresponding normal density curve overlaid.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.1, 4), fig.cap="The distribution of the sample mean (n=25).", purl=FALSE}
n = 25
...
```{r fig-sample-mean-100-with-normal, fig.alt="Same histogram-with-normal-overlay, n=100. Closely matches the smooth normal curve.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.1, 4), fig.cap="The distribution of the sample mean (n=100).", purl=FALSE}
n = 100
x = almonds_bowl$weight
mu = mean(x)
sigma = sqrt(mean(x^2)-mean(x)^2)
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** For each of the following cases, explain whether the sampling distribution of the sample mean approximates a normal distribution.
...
#### Difference in sample means {.unnumbered}
...
respectively. If the distributions of $\overline X_1$ and $\overline X_2$ are approximately normal due to the CLT, so is the distribution of $\overline X_1 - \overline X_2$. We can write all these properties at once:

$$\overline X_1 - \overline X_2 \sim Normal \left(\mu_1 - \mu_2, \sqrt{\frac{\sigma_1^2}{n_1} + \frac{\sigma^2_2}{n_2}} \right)$$

Observe how the standard error of the difference is the square root of the sum of the variances (squared standard errors) of each sample mean. The reason we add variances instead of subtracting them is because whether you add or subtract statistics, you are effectively adding more uncertainty into your results and the dispersion increases because of it.

::: {.learncheck}
**Learning Check**

 **`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** In the context of comparing two samples, why do we add variances (squared standard deviations) instead of subtracting them when finding the standard error of the difference?
...
```{r lc-sol-07-17, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 17))
```

:::

#### Difference in sample proportions {.unnumbered}
...
```{r sampling-join-tables}
prop_joined
```

As we did before, we build a histogram for these 1000 differences in @fig-samplingdistribution-virtual-diff-1000.

```{r sampling-hist-white-border-v2-dup6, echo=TRUE, fig.show='hide'}
...
```
```{r fig-samplingdistribution-virtual-diff-1000, fig.alt="Histogram of 1000 differences in sample proportions (proportion of red balls in the first bowl minus proportion of almonds heavier than 3.8 grams in the second bowl), bell-shaped and centered at the true difference.", echo=FALSE, fig.cap="The distribution of 1000 differences in sample proportions based on 1000 random samples of size 50 from the first bowl and 1000 random samples of size 60 from the second bowl.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 5.5, 7),}
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What is the sampling distribution of the difference in sample proportions expected to look like if both samples are large enough?
...
```{r lc-sol-07-18, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(7, 18))
```
...
**Q7-1.** The *sampling distribution* of a statistic describes:

a. The distribution of values in the population
b. How the statistic varies across many samples of the same size
c. The distribution of values in a single sample
d. How the population parameter varies across many possible populations

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Sampling distributions are distributions of *statistics*, not of data. One dot on a sampling distribution corresponds to one summary statistic (e.g., $\bar{x}$) computed from one sample.
:::

**Q7-2.** As sample size $n$ increases, the standard error of the sample mean:

a. Stays the same
b. Is unrelated to sample size
c. Decreases
d. Increases

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Standard error of the mean = $\sigma/\sqrt{n}$. Larger samples → less variability in $\bar{x}$ → tighter sampling distribution.
:::

**Q7-3.** The Central Limit Theorem says:

a. The sampling distribution of the mean is approximately normal for large $n$ and any population
b. Larger samples produce smaller sample means than smaller samples do
c. Any sample of data will itself look approximately normal so long as the sample size is large enough
d. The sample mean is always exactly equal to the population mean

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** That's why we can use normal-based methods for inference about means even when the underlying data isn't normal — *if* $n$ is large enough.
:::

**Q7-4.** A population has a heavily right-skewed distribution. According to the Central Limit Theorem, the sampling distribution of the sample mean (for large $n$) is approximately:
...
::: {.callout-tip collapse="true" title="Show answer"}
**(d)** That's the surprising power of the CLT: regardless of the population shape, the *sampling distribution* of the mean approaches normal as $n$ grows. Skewness in the data does not imply skewness in the sampling distribution of $\bar{x}$.
:::

**Q7-5.** You take 1000 samples of size 30 each, computing the mean of each. The histogram of those 1000 means is centered approximately at:

a. The population mean $\mu$
b. Zero, by construction
c. The sample size, 30
d. The mean of the first of the 1000 samples

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** The expected value of the sample mean is the population mean. So the *center* of the sampling distribution = the parameter being estimated: the histogram of the 1000 sample means piles up around $\mu$.
:::

**Q7-6.** To **halve** the standard error of the sample mean, you need to:

a. Halve the sample size
b. Double the sample size
c. Quadruple the sample size
d. Use a different standard error formula

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** $\text{SE}_{\bar{x}} = \sigma/\sqrt{n}$. To halve SE, you need to *quadruple* $n$ (since $\sqrt{4n} = 2\sqrt{n}$). This is why bigger samples have rapidly diminishing returns on precision.
:::

**Q7-7.** A "sample statistic" is:

a. A type of plot
b. Always equal to the parameter
c. A number computed from a sample
d. A property of the population (often unknown)

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Statistics come from samples; parameters describe populations. Statistics are random; parameters are fixed (though usually unknown).
:::

**Q7-8.** The notation $\widehat{p}$ refers to:

a. The population proportion
b. The hat function in R
c. The population variance
d. The sample proportion

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** The "hat" in $\widehat{p}$ signals an estimate computed from a sample. Parameters like $p$, $\mu$, and $\sigma$ describe the population; their sample-based estimates wear hats.
:::

**Q7-9.** You take a *single* shovel of 50 balls from the bowl and make a histogram of the ball colors in that one shovel. This histogram displays:

a. The population distribution of the bowl
b. The standard error of $\widehat{p}$
c. The sampling distribution of the sample proportion $\widehat{p}$
d. The distribution of a single sample

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** A histogram of one sample's *data* is just a picture of that sample. A *sampling* distribution only appears when you compute a statistic (like $\widehat{p}$) from *many* samples of the same size and plot how that statistic varies, like the histograms of 1000 sample proportions built with `rep_slice_sample()`. A histogram of *all* the balls in the bowl would show the population distribution, and the standard error is a single number (the standard deviation of the sampling distribution), not a histogram.
:::

**Q7-10.** A poll of 200 voters reports 52% support for candidate A. The TRUE population proportion is 50%. What's most surprising about getting 52%?

a. 52% support always means the candidate is winning
b. Nothing; sample proportions vary around $p$
c. The sample is too small to mean anything
d. The poll must be biased

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Sampling variability is *expected*, not surprising. With $n = 200$ and $p = 0.5$, the standard error of $\widehat{p}$ is about $\sqrt{0.5 \times 0.5 / 200} \approx 0.035$, so values within roughly $0.5 \pm 0.07$ are well within normal sampling fluctuation.
:::
...
|---|---|---|
| `rep_slice_sample(df, n = ..., reps = ...)` | Take `reps` random samples of size `n` from `df`; the result comes back already grouped by `replicate` | `bowl |> rep_slice_sample(n = 50, reps = 1000)` |
| `set.seed(N)` | Fix the random seed so resamples are reproducible | `set.seed(42)` |
| `summarize(prop = mean(color == "red"))` | Compute a sample statistic per replicate (no `group_by()` needed — `rep_slice_sample()` already groups by `replicate`) | `samples |> summarize(prop = mean(color == "red"))` |
| `rep_sample_n(df, size, reps)` | Older interface for the same repeated sampling — you'll see it again in Chapter 8 | `bowl |> rep_sample_n(size = 50, reps = 1000)` |

:::

## Exercises {.unnumbered #sec-sampling-exercises}

The end-of-chapter exercises introduce `volcanoes` from the [`volcanoes`](https://github.com/moderndive/volcanoes) package — **~1,200 Holocene volcanoes** with `volcano_number`, `volcano_name`, `primary_volcano_type`, `country`, `region`, `latitude`, `longitude`, `elevation` (m), and more. \index{R packages!volcanoes!volcanoes} For sampling exercises, treat the full `volcanoes` table as a **population** and study what happens when you sample from it. A few exercises use `eruptions` (10k rows, with `vei` — Volcanic Explosivity Index) and reuse `planets` from the @sec-regression exercises.

Install with `remotes::install_github("moderndive/volcanoes")`. Solutions are available to instructors separately.
...
## Summary and final remarks {#sec-sampling-final-remarks}

### Summary of scenarios

These are not the only cases where the sampling distribution can be determined. For example, when performing linear regression, we can find the sampling distribution for the slope of the regression line and the behavior will be similar to what we have described here. We will discuss inference in the context of linear regression in @sec-inference-for-regression. For now, we present a summary of the different scenarios presented in this chapter.

```{r tbl-table-ch7, echo=FALSE, message=FALSE, purl=FALSE}
...
# Google Sheet at https://docs.google.com/spreadsheets/d/1j7JWF_o2NlA5FFhufSaEiHV6-3b_LtGaSYS-4qrXORM/edit?usp=sharing
  ...
  sampling_scenarios <- readRDS("rds/sampling_scenarios-v2.rds")
}
...
```{r sampling-conditional-text-v2, echo=FALSE, results='asis', purl=FALSE}
if (is_latex_output())
  cat("Check the online version of the book for a table that also includes the sampling distribution of each of these statistics as shown in @tbl-table-ch7 using the Central Limit Theorem.")
```


### Random sampling vs. random assignment {#sec-sampling-vs-assignment}

This chapter has been entirely about **random sampling** — *how we pick observations from a population to put in our sample*. There is a related but distinct idea you'll meet repeatedly in statistics: **random assignment** — *how we allocate already-collected subjects to treatment groups in an experiment*. The two answer different questions:

* **Random sampling** addresses *generalizability* — "Do my findings apply to the broader population?" If volcanoes were sampled randomly from all Holocene volcanoes, then a sample mean elevation can be used to draw conclusions about the population mean elevation.
* **Random assignment** addresses *causality* — "Did the treatment *cause* the observed difference?" If patients are randomly assigned to "drug" vs. "placebo," then any group difference at the end is attributable to the drug rather than to pre-existing differences between the patients.

The two combine to form a 2×2 of possibilities, summarized in @tbl-sampling-vs-assignment.

```{r tbl-sampling-vs-assignment, echo=FALSE, purl=FALSE}
tibble::tribble(
  ~`Design`, ~`Random sampling? (generalizable?)`, ~`Random assignment? (causal?)`,
  "Randomized experiment on a random sample", "Yes",  "Yes — gold standard: causal claims about the population",
  "Observational study on a random sample",   "Yes",  "No — generalizable, but only *associations*, not causation",
  "Randomized experiment on a convenience sample", "No",  "Yes — causal *for these subjects*, but not generalizable beyond them",
  "Observational study on a convenience sample",  "No",  "No — exploratory only; neither causal nor generalizable"
) |>
  knitr::kable(
    caption = "Random sampling and random assignment serve different roles.",
    booktabs = TRUE
  )
```

Most of the datasets you've worked with in this book sit in the bottom-right cell. The `bob_ross` dataset is *every* episode of *The Joy of Painting* — a full census, not a sample, and there is no treatment to assign. The `olympic_athletes` data is similarly observational. We can describe patterns and quantify uncertainty about them, but we cannot use this data alone to claim that, say, Sport *causes* Height.

In @sec-confidence-intervals and @sec-hypothesis-testing you'll build inferential machinery that *assumes random sampling* — the standard error formulas, bootstrap intervals, and permutation tests all rely on it. When you read a study, the first questions to ask are: "How were the subjects sampled?" and "Were the groups assigned, or just observed?" The answers determine what kind of conclusion the data can support.


### Additional resources

```{r sampling-conditional-text-v2-dup1, echo=FALSE, results="asis", purl=FALSE}
if (is_latex_output()) {
...
```{r sampling-v48, echo=FALSE, purl=FALSE, results="asis"}
