...
- Use `rep_sample_n()` from `moderndive` to simulate repeated samples from a population
...
We now take many more random samples as shown in @fig-sampling-exercise-3b-1. Each time we do the following:
...
```{r fig-almonds-bowl-histogram, fig.alt="Histogram of weights for the entire bowl of almonds. Roughly bell-shaped, centered near 3.7 grams, with most weights between 2 and 5 grams.", fig.cap="Distribution of weights for the entire bowl of almonds."}
...
a. The values in a single sample
b. The values in the population
c. How the statistic varies across many possible samples of the same size
d. The probability that the null hypothesis is true

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Sampling distributions are distributions of *statistics*, not of data. One dot on a sampling distribution corresponds to one summary statistic (e.g., $\bar{x}$) computed from one sample.
:::
...
c. The sampling distribution of the mean is approximately normal for large enough $n$, regardless of the population distribution's shape
d. Larger samples have smaller means

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** That's why we can use normal-based methods for inference about means even when the underlying data isn't normal — *if* $n$ is large enough.
:::
...
::: {.callout-tip collapse="true" title="Show answer"}
**(b)** That's the surprising power of the CLT: regardless of the population shape, the *sampling distribution* of the mean approaches normal as $n$ grows. Skewness in the data does not imply skewness in the sampling distribution of $\bar{x}$.
:::
...
a. The first sample's mean
b. **The population mean $\mu$**
c. Zero
d. The sample size 30

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** The expected value of the sample mean is the population mean. So the *center* of the sampling distribution = the parameter being estimated. (This is why $\bar{x}$ is called an *unbiased* estimator of $\mu$.)
:::
...
d. Use a different formula

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** $\text{SE}_{\bar{x}} = \sigma/\sqrt{n}$. To halve SE, you need to *quadruple* $n$ (since $\sqrt{4n} = 2\sqrt{n}$). This is why bigger samples have rapidly diminishing returns on precision.
:::

**Q7.** A "sample statistic" is:

a. A property of the population (often unknown)
b. **A number computed from a sample (varies sample to sample)**
c. A type of plot
d. Always equal to the parameter

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Statistics come from samples; parameters describe populations. Statistics are random; parameters are fixed (though usually unknown).
:::

**Q8.** The notation $\widehat{p}$ refers to:

a. The population proportion
b. **The sample proportion (an estimate of $p$)**
c. The population variance
d. The hat function in R

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** The "hat" in $\widehat{p}$ signals an estimate computed from a sample. Greek letters (e.g., $p$, $\mu$, $\sigma$) typically represent population parameters; their estimates wear hats.
:::

**Q9.** Why is sampling **without replacement** usually preferred for collecting real-world data?

a. It's faster
b. **Each subject contributes once — independent observations and broader population coverage**
c. With-replacement gives wrong answers
d. R doesn't support with-replacement

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Without-replacement avoids interviewing the same person twice. (Bootstrapping uses *with*-replacement intentionally — different goal: simulating the sampling process from one observed sample.)
:::
...
a. **Nothing — sample proportions naturally vary around $p$**
b. The poll must be biased
c. The sample is too small to mean anything
d. 52% always means the candidate is winning

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** Sampling variability is *expected*, not surprising. With $n = 200$ and $p = 0.5$, the standard error of $\widehat{p}$ is about $\sqrt{0.5 \times 0.5 / 200} \approx 0.035$, so values within roughly $0.5 \pm 0.07$ are well within normal sampling fluctuation.
:::
...
|---|---|---|
| `rep_sample_n(df, size, reps, replace = FALSE)` | Take `reps` random samples of size `size` from `df` | `bowl |> rep_sample_n(size = 50, reps = 1000)` |
| `rep_slice_sample(df, n = ..., reps = ...)` | Newer `dplyr`-style interface for the same thing | `bowl |> rep_slice_sample(n = 50, reps = 1000)` |
| `set.seed(N)` | Fix the random seed so resamples are reproducible | `set.seed(42)` |
| `summarize(prop = mean(color == "red"))` | Compute a sample statistic per resample (group_by `replicate` first) | `samples |> group_by(replicate) |> summarize(prop = mean(color == "red"))` |

:::

## Exercises {.unnumbered #sec-sampling-exercises}

The end-of-chapter exercises introduce `volcanoes` from the [`volcanoes`](https://github.com/moderndive/volcanoes) package — **~1,200 Holocene volcanoes** with `volcano_number`, `volcano_name`, `primary_volcano_type`, `country`, `region`, `latitude`, `longitude`, `elevation` (m), and more. \index{R packages!volcanoes!volcanoes} For sampling exercises, treat the full `volcanoes` table as a **population** and study what happens when you sample from it. A few exercises use `eruptions` (10k rows, with `vei` — Volcanic Explosivity Index) and reuse `planets` from @sec-mult-reg.
...
### Setup and population statistics {.unnumbered}
...
```{webr-r}
volcanoes |>
  summarize(mu = mean(elevation, na.rm = TRUE),
            sigma = sd(elevation, na.rm = TRUE))
```
...
### Building the sampling distribution {.unnumbered}
...
ggplot(samp_dist_30, aes(x = xbar)) + geom_histogram(bins = 30)
```
...
**EX7.12 (★★★)** Run the same experiment for $n \in \{10, 30, 100, 300\}$. Plot `SE` vs $n$ on a log–log scale. The slope should be roughly $-\frac{1}{2}$ (the CLT). Verify.

```{webr-r}
# your code here
```

### The Central Limit Theorem {.unnumbered}

**EX7.13 (★★★)** Even though `elevation` is heavily right-skewed (population), `samp_dist_30` looks roughly normal. Explain in 2 sentences why — invoking the CLT.

**EX7.14 (★★★)** Build the population histogram and `samp_dist_30` histogram on the same panel for visual comparison. Use `patchwork` if helpful.

```{webr-r}
# your code here
```

**EX7.15 (★★★)** Compute the **theoretical** SE under the CLT: $\sigma / \sqrt{n}$ for $n = 30$. Compare to the simulated SE from EX7.9. They should match closely.

```{webr-r}
# your code here
```

**EX7.16 (★★★)** What's the smallest $n$ for which the sampling distribution of $\bar{x}$ already looks roughly bell-shaped? (Try $n = 5$, $10$, $30$ — eyeball the histograms.)

```{webr-r}
# your code here
```

### Sampling distribution for a proportion {.unnumbered}

**EX7.17 (★★)** Take 1,000 samples of size $n = 50$ and compute $\hat{p}$ (sample proportion `Stratovolcano`) for each. Plot the histogram.
...
samp_p <- volcanoes |>
  ...
  summarize(phat = mean(primary_volcano_type == "Stratovolcano"),
            .groups = "drop")
ggplot(samp_p, aes(x = phat)) + geom_histogram(bins = 25)
```

**EX7.18 (★★)** Compute the simulated mean and SE of $\hat{p}$. Compare the simulated SE to the theoretical $\sqrt{p(1-p)/n}$ using `p_strato` from EX7.3.

```{webr-r}
# your code here
```

**EX7.19 (★★★)** What happens to the shape of the sampling distribution if you set $n = 5$ instead? Build it and comment.

```{webr-r}
# your code here
```

**EX7.20 (★★★)** When the population proportion is very small (e.g., a rare volcano type — try `"Caldera"`), the sampling distribution can look notably skewed even at $n = 50$. Demonstrate with one chart and one sentence.

```{webr-r}
# your code here
```

### Sampling on `eruptions` and `planets` {.unnumbered}

**EX7.21 (★★★)** Treat `volcanoes::eruptions` as a population (one row per recorded eruption, with `vei`). What's the population mean `vei`?
...
**EX7.22 (★★★)** Take 1,000 samples of size $n = 50$ from `eruptions` and build the sampling distribution of mean `vei`. SE?

```{webr-r}
# your code here
```

**EX7.23 (★★★)** Treat `exoplanets::planets$radius_earth` as a population. The distribution is heavily right-skewed. Take 1,000 samples of $n = 30$ and 1,000 of $n = 200$ — compare the sampling distributions visually.

```{webr-r}
# your code here
```

### Critical thinking and synthesis {.unnumbered}

**EX7.24 (★★★)** Suppose your sampling design *systematically* over-represents large volcanoes (e.g., only sampling those tall enough to be famous). Will the sampling distribution still center on $\mu$? Use the chapter's terminology to explain.

**EX7.25 (★★★)** Distinguish the **standard deviation** (a property of the population) from the **standard error** (a property of an estimator). Use one Olympic-volcano-themed example each.

**EX7.26 (★★★)** A friend reports "I only need 30 observations because of the CLT — that always works, right?" Critique this in 2–3 sentences. When does $n = 30$ fail?

**EX7.27 (★★★)** The CLT says SE shrinks like $\sigma/\sqrt{n}$. To **halve** SE, by what factor must $n$ grow? What does this imply for cost-benefit of bigger samples?

**EX7.28 (★★★)** Why is the *number of replicates* (`reps = 1000`) different in nature from the *sample size* ($n$)? Which one would you increase to make the *histogram smoother* vs. *the SE smaller*?

```{webr-r}
# your code here
```

**EX7.29 (★★★)** Sample 30 volcanoes **without replacement** (the `rep_sample_n()` default) vs **with replacement**. For a population of ~1,200 and $n = 30$, do the sampling distributions differ much? Why?

```{webr-r}
# your code here
```

**EX7.30 (★★★)** Build a chart that summarizes the *whole story of the chapter* in your own words — population, single sample, sampling distribution, CLT — using volcano elevations.

```{webr-r}
# your code here
```

**EX7.31 (★★★)** Build a sampling distribution for the *median* elevation (not the mean) at $n = 30$. Is the median's sampling distribution also approximately normal?

```{webr-r}
# your code here
```

**EX7.32 (★★★)** Open exploration: pick a numerical variable in `volcanoes`, `eruptions`, or `planets`, build its sampling distribution, and write a 2-sentence finding (population skewness, simulated SE, comparison to theoretical SE).
