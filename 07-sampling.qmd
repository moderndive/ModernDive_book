...
```{r sampling-conditional-text, echo=FALSE, results="asis"}
if (is_latex_output()) {
...
```{r sampling-load-internal, message=FALSE, echo=FALSE}
...
```{r sampling-exercise-1, echo=FALSE, fig.cap="A bowl with red and white balls.", out.width = "95%"}
...
```{r sampling-exercise-2, echo=FALSE, fig.cap="Inserting a shovel into the bowl.", out.width="100%"}
knitr::include_graphics("images/sampling/balls/sampling_bowl_2.jpg")
```


```{r sampling-exercise-3, echo=FALSE, fig.cap="Taking a sample of 50 balls from the bowl.", out.width="80%"}
...
```{r sampling-exercise-3b, echo=FALSE, fig.show='hold', fig.cap="Repeating sampling activity.", out.width="25%"}
...
```{r sampling-exercise-4, echo=FALSE, fig.cap="Students drawing a histogram of sample proportions.", out.width="60%"}
...
```{r sampling-exercise-5, echo=FALSE, fig.cap="Hand-drawn histogram of 10 sample proportions.", out.height="30%"}
knitr::include_graphics("images/sampling/balls/tactile_3_c-2e.png")
```
...
```
```{r samplingdistribution-tactile, echo=FALSE, fig.cap="The distribution of sample proportions based on 33 random samples of size 50.", fig.height=ifelse(knitr::is_latex_output(), 3.3, 4)}
...
```
```{r samplingdistribution-virtual, echo=FALSE, fig.cap="The distribution of 33 proportions based on 33 virtual samples of size 50.", fig.height=ifelse(knitr::is_latex_output(), 3.5, 4)}
...
```{r tactile-vs-virtual, echo=FALSE, fig.cap="The sampling distribution of the sample proportion and sampling variation:  showing a histogram for virtual sample proportions (left) and another histogram for tactile sample proportions (right).", fig.height=ifelse(knitr::is_latex_output(), 2.9, 4)}
...
```
```{r samplingdistribution-virtual-1000, echo=FALSE, fig.cap="The distribution of 1000 proportions based on 1000 random samples of size 50.", fig.height=ifelse(knitr::is_latex_output(), 2.3, 4)}
virtual_histogram <- ggplot(virtual_prop_red, aes(x = prop_red)) +
  geom_histogram(binwidth = 0.04, boundary = 0.4, color = "white")
virtual_histogram +
  labs(
...
```{r sampling-conditional-text-dup1, echo=FALSE, results="asis"}
if(!is_latex_output()) 
...
```{r three-shovels, echo=FALSE, fig.cap="Three shovels to extract three different sample sizes.", out.width='100%'}
...
```{r comparing-sampling-distributions, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 4), fig.cap="Histograms of sample proportions for different sample sizes."}
# n = 25
...
```{r comparing-sampling-distributions-3, echo=FALSE, fig.cap="Three sampling distributions with population proportion $p$ marked by vertical line.", fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
p <- bowl |>
...
```{r sampling-compute-mean-v8, echo=FALSE}
...
```{r sample-proportion-25-with-normal-pdf, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 4), fig.cap="Histogram of the distribution of the sample proportion and the normal curve (n=25)."}
n = 25
p=9/24
sd.p = sqrt(p*(1-p)/n)
...
```{r sample-proportion-50-with-normal-pdf, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 4), fig.cap="Histogram of the distribution of the sample proportion and the normal curve (n=50)."}
n = 50
p=9/24
sd.p = sqrt(p*(1-p)/n)
...
```{r sample-proportion-100-with-normal-pdf, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 4), fig.cap="Histogram of the sampling distribution of the sample proportion and the normal curve (n=100)."}
n = 100
p=9/24
sd.p = sqrt(p*(1-p)/n)
...
```{r CLT-video-preview, echo=FALSE, fig.cap="Preview of Central Limit Theorem video.", out.width = "75%"}
...
```{r bowl-almond, echo=FALSE, fig.cap="A bowl of chocolate-covered almonds.", out.width="40%"}
knitr::knitr::include_graphics("images/sampling/almonds/v2_side_almonds.jpg")
```
...
```{r one-almond, echo=FALSE, fig.show="hold", fig.cap="One almond on a scale.", out.width = "40%"}
...
```{r twenty-five-almonds, echo=FALSE, fig.cap="A random sample of 25 almonds on a scale.",  out.width = "40%"}
knitr::include_graphics("images/sampling/almonds/twenty-five-almonds.png")
```
...
The weights of almonds in this sample range from `r min(almonds_sample$weight)` to `r max(almonds_sample$weight)` grams. There is not an obvious pattern in the distribution of this sample. We now compute the sample mean using our data-wrangling tools from Chapter \@ref(wrangling).
...
We now perform sampling virtually. The data frame `almonds_bowl` has `r num_pop_almonds` rows, each representing an almond in the bowl. As we did in Section \@ref(sampling-simulation) we use again the `rep_slice_sample()` function to retrieve 1000 random samples with a sample `size` set to be 25, and with the number of replicates `reps` set to `1000`. Be sure to scroll through the contents of `virtual_samples` in RStudio's viewer. 
...
```{r sampling-mean-virtual-1000, echo=FALSE, fig.cap="The distribution of 1000 means based on 1000 random samples of size 25.", fig.height=ifelse(knitr::is_latex_output(), 1.5, 4)}
virtual_mean_weight <- virtual_samples_almonds |> 
  summarize(mean_weight = mean(weight))
...
```{r comparing-sampling-distributions-means, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 4), fig.cap="Comparing histograms of sample means when using different sample sizes."}
# n = 25
...
```{r sample-mean-25-with-normal, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.1, 4), fig.cap="The distribution of the sample mean (n=25)."}
n = 25
...
```{r sample-mean-100-with-normal, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.1, 4), fig.cap="The distribution of the sample mean (n=100)."}
n = 100
x = almonds_bowl$weight
mu = mean(x)
sigma = sqrt(mean(x^2)-mean(x)^2)
...
```
```{r samplingdistribution-virtual-diff-1000, echo=FALSE, fig.cap="The distribution of 1000 differences in sample proportions based on 1000 random samples of size 50 from the first bowl and 1000 random samples of size 60 from the second bowl.", fig.height=ifelse(knitr::is_latex_output(), 5.5, 7),}
...
```{r table-ch7, echo=FALSE, message=FALSE}
...
```{r sampling-conditional-text-v2, echo=FALSE, results='asis'}
if (is_latex_output())
  cat("Check the online version of the book for a table that also includes the sampling distribution of each of these statistics as shown in Table \\@ref(tab:table-ch7) using the Central Limit Theorem.")
```


### Additional resources

```{r sampling-conditional-text-v2-dup1, echo=FALSE, results="asis"}
if (is_latex_output()) {
...
```{r sampling-v48, echo=FALSE, results="asis"}
