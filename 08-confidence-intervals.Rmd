...
Let's mark these percentiles on the bootstrap distribution with vertical lines in Figure \@ref(fig:percentile-method). About 95% of the `mean_year` variable values in `virtual_resampled_means` fall between `r percentile_ci[["2.5%"]]` and `r percentile_ci[["97.5%"]]`, with 2.5% to the left of the left-most line and 2.5% to the right of the right-most line. 
...
```{r infer-generate, fig.align='center', out.width='70%', echo=FALSE, fig.cap="Diagram of generate() replicates.", purl=FALSE}
...
```{r infer-calculate, fig.align='center', out.width='60%', echo=FALSE, fig.cap="Diagram of calculate() summary statistics.", purl=FALSE}
...
```{r se-ci-viz, echo=FALSE, fig.show='hold', fig.cap='(ref:se-viz)', purl=FALSE, fig.height=2.8}
# Will need to make a tweak to the {infer} package so that it doesn't always display "Null" here
if(knitr::is_html_output()){
  visualize(bootstrap_distribution) + 
    shade_confidence_interval(endpoints = standard_error_ci) +
...
```
```{r shovel-bootstrap-1-infer, echo=FALSE, fig.show='hold', fig.cap="Bootstrap distribution.", fig.height=5, purl=FALSE}
# Will need to make a tweak to the {infer} package so that it doesn't always display "Null" here
if(knitr::is_html_output()){
  sample_1_bootstrap %>% 
    visualize(bins = 15) + 
...
```{r reliable-percentile, fig.cap='(ref:reliable-perc)', echo=FALSE, fig.height=4.5}
...
Observe that as the confidence level increases from 80% to 95% to 99%, the confidence intervals tend to get wider as shown in Table \@ref(tab:perc-cis-average-width).
