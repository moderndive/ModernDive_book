...
```{r percentile-method, echo=FALSE, message=FALSE, fig.cap='(ref:perc-method)'}
ggplot(virtual_resampled_means, aes(x = mean_year)) +
...
```{r percentile-and-se-method, echo=FALSE, message=FALSE, fig.cap='(ref:both-methods)'}
both_CI <- bind_rows(
...
```{r percentile-ci-viz, echo=FALSE, fig.show='hold', fig.cap='(ref:perc-ci-viz)', purl=FALSE}
# Will need to make a tweak to the {infer} package so that it doesn't always display "Null" here (added to `develop` branch on 2019-10-26)
...
```{r se-ci-viz, echo=FALSE, fig.show='hold', fig.cap='(ref:se-viz)', purl=FALSE}
# Will need to make a tweak to the {infer} package so that it doesn't always display "Null" here (added to `develop` branch on 2019-10-26)
if(knitr::is_html_output()){
  visualize(bootstrap_distribution) + 
    shade_confidence_interval(endpoints = standard_error_ci) #+
...
```{r reliable-percentile, fig.cap='(ref:reliable-perc)', echo=FALSE}
...
```{r reliable-se, fig.cap='(ref:rel-se)',echo=FALSE}
...
```{r reliable-percentile-80-95-99, fig.cap='(ref:many-percs)', echo=FALSE}
...
(ref:rel-perc-n) Ten 95% confidence intervals for $p$ based on $n = 25, 50,$ and $100$.

```{r reliable-percentile-n-25-50-100, fig.cap='(ref:rel-perc-n)', echo=FALSE}
...
Let's first focus on the `"control"` group participants who were not exposed to yawning where 12 such participants did not yawn, while four such participants did. So out of the 16 people who were not exposed to yawning, 4/16 = 0.25 = 25% did yawn. 

Let's now focus on the `"seed"` group participants who were exposed to yawning where 24 such participants did not yawn, while 10 such participants did yawn. So out of the 34 people who were exposed to yawning, 10/34 = 0.294 = 29.4% did yawn. 
...
```{r bootstrap-distribution-mythbusters-CI, echo=FALSE, fig.show='hold', fig.cap="Two 95\\% confidence intervals: percentile method (solid) and standard error method (dashed).", purl=FALSE}
# Will need to make a tweak to the {infer} package so that it doesn't always display "Null" here (added to `develop` branch on 2019-10-26)
visualize(bootstrap_distribution_yawning) + 
  ggtitle("") +
...
Here is the code you previously saw in Subsection \@ref(shovel-1000-times) to construct the sampling distribution of $\widehat{p}$ shown again in Figure \@ref(fig:sampling-distribution-part-deux), with some small changes to incorporate the statistical terminology relating to sampling you learned in Subsection \@ref(terminology-and-notation).

```{r,echo=FALSE}
set.seed(76)
```

```{r sampling-distribution-part-deux, fig.show='hold', fig.cap="Previously seen sampling distribution of sample proportion red for $n = 1000$.", echo=TRUE}
