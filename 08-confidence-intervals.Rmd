...
```{r sample-mean-100-with-normal-redraw, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 7), fig.cap="The distribution of the sample mean.", purl=FALSE}
...
Please review [Appendix A online](https://moderndive.com/A-appendixA.html) where we provide R code to work with different areas, probabilities, and values under a normal density curve. Here, we place focus on the insights of specific values and areas without dedicating time to those calculations.

We assume that a random variable $Z$ follows a standard normal distribution. We would like to know how likely it is for this random variable to take a value that is within one standard deviation from the mean or, equivalently, what is the probability that the observed value $z$ (in the X-axis) is between -1 and 1 as shown in Figure \@ref(fig:normal-curve-shaded-1a)

```{r normal-curve-shaded-1a, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 7), fig.width=3, fig.cap="Normal area within one standard deviation"}
...
```{r normal-curve-shaded-2a, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 7), fig.width=3, fig.cap="Normal area within two standard deviations"}
ggplot(NULL, aes(c(-4,4))) +
...
We are now ready to return to the our main goal: how to find an interval estimate of the population mean based on a single sample.
...
$$SE_{\bar x} = \frac{\sigma}{\sqrt{100}} = \frac{`r sigma`}{\sqrt{100}} = `r se_xbar`$$ 
grams. In Figure \@ref(fig:normal-curve-1) we plot the density curve for this distribution using these values.

```{r normal-curve-1, echo=FALSE, fig.cap="normal density curve for the sample mean weightweight of almonds.", purl=FALSE, out.width="90%", warning=FALSE}
...
The horizontal axis (X-axis) represents the sample means that we can determine from all the possible random samples of 100 almonds. The red dot represents the expected value of the sampling distribution, $\mu = 3.64$, located on the X-axis at the center of the distribution. The density curve's height can be thought of as how likely those sample means are to be observed. For example, it is more likely to get a random sample with a sample mean around $`r mu`$ grams (which corresponds to the highest point of the curve) than it is to get a sample with a sample mean at around $3.5$ grams, since the curve's height is almost zero at that value. The blue dot is the sample mean from our sample of 100 almonds, $\overline{x} = `r sample_mean`$ grams. It is located `r deviance` grams above the population mean weight. How far is `r deviance` grams? It is helpful to express this distance in standardized values: 
...
Recall that due to the Central Limit Theorem, the sampling distribution of the sample mean was approximately normal with mean equal to the population mean $\mu$ and standard deviation given by the standard error $SE_{\overline X} = \sigma/\sqrt{n}$. We can standardize this for any sample mean $\overline{x}$ such that 
...
```{r t-curve-1, echo=FALSE, fig.cap="The standard normal and two t-distributions.", purl=FALSE, out.width="90%"}
...
Please see [Appendix A online](https://moderndive.com/A-appendixA.html) for calculations of probabilities for $t$ density curves with different degrees of freedom. Using that knowledge, the calculation for our specific example shows that 95% of the sample means are within 1.98 standard errors from the population mean weight. The number of standard errors needed is not that different from before, 1.98 versus 1.96, because the degrees of freedom are fairly large.
...
\end{aligned}
$$ 
Here is the R code that can be used to find the lower and upper bounds on this interval
...
In the literature, this explanation has been encapsulated in a short-hand version: we are 95% confident that the interval contains the population parameter. For example, in Subsection \@ref(t-distribution-CI) the 95% confidence interval for the population mean weight of almonds was (`r round(lower_bound_t, 3)`, `r round(upper_bound_t, 3)`), and we would say: "We are 95% confident that the population mean weight of almonds is between `r round(lower_bound_t, 3)` and `r round(upper_bound_t, 3)` grams". It is perfectly acceptable to use the short-hand statement, but always remember that the 95% confidence refers to the process, or method, and can be thought of as a chance or probability only before the random sample has been acquired. To further ensure that the probability-type of language is not misused, quotation marks are sometimes put around "95% confident" to emphasize that it is a short-hand version of the more accurate explanation.
...
Observe that the confidence interval is centered at the sample mean and it extends to each side 1.96 standard errors 
...
If we want to determine a 90% confidence interval, we need to use `r qnorm(0.95)` standard errors in our calculations. We can update the R code to calculate the lower and upper bounds of a 98% confidence interval:
...
As we studied changes to the confidence level, the researcher can determine how big is the random sample to be used. The margin of error for a 95% confidence interval is 
...
In 1979, Brad Efron published an article introducing a method called the bootstrap\index{bootstrap!statistical reference}. The idea is the following: a random sample of size $n$ is taken from the population. 
...
Even with modern powerful computers, it would be an onerous task to calculate every possible bootstrap sample. Instead, a few thousand bootstrap samples are retrieved, similar to the simulations performed in Chapter \@ref(sampling), and this number is often large enough to provide useful results.

<!-- References need to be in bibtex -->
...
In the next few subsections, we explore how to incorporate this and similar methods to construct confidence intervals.
...
```{r tactile-resampling-2, echo=FALSE, fig.cap="Step 2: Weighing one almond at random.", fig.show="hold", purl=FALSE, out.width="60%"}
...
By using `summarize()` and `mean()` on the bootstrap sample `boot_sample`, we determine that the mean weight is `r resample_mean |> pull(mean_weight) |> round(2)` grams. Recall that the sample mean of the original sample was found in the previous subsection as `r x_bar |> round(2)`. So, the sample mean of the bootstrap sample is different than the sample mean of the original sample. This variation is induced by resampling with replacement, the method used to find the bootstrap sample.
...
The data frame `boot_means` contains `r n_virtual_resample` sample mean weights, each calculated from a different bootstrap sample. In Figure \@ref(fig:one-thousand-sample-means) the histogram of these `r n_virtual_resample` sample mean weights is presented.
...
Recall that we took bootstrap samples, then calculated the sample means from these samples. Let us revisit the original workflow using `dplyr` verbs and the `|>` pipe operator.
...
After we `generate()` `r n_virtual_resample` bootstrap samples, we want to summarize each of them, for example, by calculating the sample mean of each one of them. As the diagram shows, the `calculate()` \index{infer!calculate()} function does this.
...
```{r bootstrap-distribution-infer, echo=FALSE, fig.show="hold", fig.cap="Bootstrap distribution.", purl=FALSE}
visualize(bootstrap_means)
```
...
```{r percentile-ci-viz, echo=FALSE, fig.cap="(ref:perc-ci-viz)", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 7)}
...
where $\overline{x}$ is the sample mean of the original sample, 1.96 is the number of standard errors around the mean needed to account for 95% of the area under the density curve (when the distribution is normal), and $SE_{\bar x}$ is the standard error of the sample mean that can be computed as $\sigma /\sqrt{n}$ if the population standard deviation is known, or estimated as $s/\sqrt{n}$ if we have to use the sample standard deviation, $s$, and the sample size, $n$.
...
```{r se-ci-viz, echo=FALSE, fig.show="hold", fig.cap="(ref:se-viz)", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.4, 7)}
if (knitr::is_html_output()) {
  visualize(bootstrap_means) +
    shade_confidence_interval(endpoints = standard_error_ci)
} else {
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Construct a 95% confidence interval for the *median* weight of *all* almonds. Use the percentile method and, if appropriate, then use the standard-error method.
...
This section expands explanations about the bootstrap methods, provides some 
historical context, delves a little deeper into the theory of the bootstrap and
comparison between the theory-based approach and the simulation-based approach 
when working with confidence intervals.

The presentation is a little more theoretical than other sections in this 
chapter and you are welcome to skip to Section \@ref(case-study-two-prop-ci) if 
you want to go over directly another application of the bootstrap methods in R.

### The bootstrap and other resampling methods



As discussed in Chernick (2011)\cite{Chernick2011} the bootstrap is one of many 
resampling methods.
...
Since then, it has been shown in multiple context that the bootstrap performs at 
least as well as traditional methods in estimating standard errors, constructing 
confidence intervals, performing hypothesis testing, and many other statistical 
problems.

Furthermore, since the 1980s, soon after the bootstrap's inception, but even 
more in the last two decades, the use of simulations to compare advanced 
...
small sample sizes. 
The use of the bootstrap and bootstrap-related method have become a cornerstone 
in modern statistical practice.

In this section we introduce some of the theory of the bootstrap and then 
explain what are the advantages and limitations of using the bootstrap.
...
histogram can be thought as the visual representation of this distribution. 
...
parameter or characteristic of this population. We then take a sample of size
...
The bootstrap consist in using the empirical distribution only and nothing else 
...
Let us recap. We want to study the population distribution $F$ and we take a 
sample of size $n$ from this distribution, call it $F_n$, and use it as an 
empirical distribution that estimates $F$. We then let $F_n$ to play 
...
Let us translate these ideas to parameters and estimators. We are interested in
...
Hall(1992)\cite{Hall1992} has provided the theoretical justification to use 
...
### Bootstrap methods in the context of confidence intervals
...
certain instances, it is even a stronger method.

We start with an illustration using the theory-based confidence interval. 
...
when $\sigma$ is unknown. We use 95% and $n = 100$ as an illustration, but 
...
contains $\mu$. 
This is true if the distribution of $\overline X$ is exactly normal, 
and we say that 95% is the *true coverage probability*\index{true coverage 
probability}. 
...
In this case, the 95% is an *approximate coverage probability*\index{true 
...
normal. 
We do have some control over this. The Central Limit Theorem states that if $n$ 
...
In real-life applications, $n$ cannot be infinite, but we can try to make $n$ 
...
size of our sample. 
If you can imagine a sequence of sample sizes $n_1, n_2, \dots$ getting bigger 
...
*rate of convergence*\index{rate of convergence} of the difference between 
approximate and true coverage probabilities. We want this rate to converge as 
quickly as possible.

Now, if you have two different methods that produce 95% confidence intervals and 
...
higher (quicker) rate of convergence.
...
percentile method has also a rate of about $1/\sqrt{n}$. Therefore, in the case 
of a 95% confidence interval for $\mu$, using either method is comparable. 
The theory-based approach and the percentile method are both considered 
*first-order accurate*\cite{Hall1992}\cite{Chernick2011}.


### Why the bootstrap-based approach


There are a few reasons why it is suitable to learn about bootstrap methods.

First, in this Chapter we discuss main confidence intervals for $\mu$ because 
...
can construct confidence intervals for any of these or many other parameters.

Furthermore, as stated earlier, the theory-based method as well as the 
simulation-based method using the bootstrap, can be used to obtain a 95% 
confidence interval for $\mu$. However, the results depend directly of the 
...
present a problem as $\overline X$ for these sample would not be close to normal 
and the difference between the approximate and true coverage probability could 
be quite different; that is, the confidence intervals would be inaccurate. 

This could happen for many reasons. 
First, the sample mean $\overline X$ is biased when $n$ is not too large. 
...
When bias is present, the confidence intervals created using the theory-based 
approach in Subsection \@ref(CI-general) or the bootstrap methods introduced in 
Subsections \@ref(percentile-method-infer) and \@ref(se-infer), could be 
suspect, even completely useless.



Since the bootstrap was proposed, many improvements have been made to the 
...
or incorporating these methods. Two of these methods are the percentile-$t$ 
and the Bias Correction and Acceleration bootstrap method (BCa). In terms of the 
rate of convergence, these methods are
*second-order accurate*\index{second-order accurate}; that is, they have a rate 
of convergence of about $1/n$. The double bootstrap, or more generally, 
iterated bootstrap, can be a *third-order accurate*.

We have not included these methods directly here because the theory that 
justify them goes beyond the scope of this book and, when dealing with 
confidence interval for $\mu$ from populations with distributions that are not
extreme, there are not real gains in using them over the theory-based approach 
or the percentile method.

The benefits are clearer, on the other hand, when working with skewed 
distributions, small sample sizes, estimators parameters other than $\mu$ 
(such as the median), or the estimation of standard error when there are no
formulas to obtain them. We explore some of these methods in Appendix E and also
present simulations that show when these methods are preferred over the 
percentile method or the theory-based approach.

Let us turn now to a case study and apply the percentile method in this context.
...
```
```{r bootstrap-distribution-mythbusters, echo=FALSE, fig.show="hold", fig.cap="Bootstrap distribution.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.5, 7)}
visualize(bootstrap_distribution_yawning) +
  geom_vline(xintercept = 0)
```
