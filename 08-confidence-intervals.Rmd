...
```{r confidence-intervals-load-packages-2, message=FALSE}
library(tidyverse)
library(moderndive)
library(infer)
```
...
```{r confidence-intervals-compute-mean-2, echo=FALSE, purl=FALSE}
# This code is used for dynamic non-static in-line text output purposes
...
```{r confidence-intervals-compute-mean-3}
almonds_sample_100 |>
...
```{r confidence-intervals-mean-and-sd-2}
almonds_sample_100 |> 
  summarize(mean_weight = mean(weight), 
            sd_weight = sd(weight), 
            sample_size = n())
...
```{r confidence-intervals-conditional-text-2, echo=FALSE, results="asis", purl=FALSE}
if(!is_latex_output()) 
...
```{r confidence-intervals-compute-mean-4}
almonds_sample_100 |>
  summarize(
...
```{r confidence-intervals-conditional-text-3, echo=FALSE, results="asis", purl=FALSE}
if(!is_latex_output()) 
...
```{r confidence-intervals-mean-and-sd-3}
almonds_sample_100 |>
...
```{r confidence-intervals-mean-and-sd-4}
almonds_sample_100 |>
  summarize(sample_mean = mean(weight), sample_sd = sd(weight),
...
```{r confidence-intervals-compute-mean-5}
almonds_sample_100 |>
  summarize(sample_mean = mean(weight),
...
```{r confidence-intervals-compute-mean-6}
boot_sample |> 
  summarize(mean_weight = mean(weight))
```
```{r confidence-intervals-compute-mean-7, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-compute-mean-8}
...
```{r confidence-intervals-compute-mean-9}
...
```{r confidence-intervals-compute-mean-10, echo=-1}
set.seed(20)
...
```{r confidence-intervals-mean-and-sd-5}
boot_means |> 
  summarize(mean_of_means = mean(mean_weight),
            sd_of_means = sd(mean_weight))
```
```{r confidence-intervals-compute-mean-11, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-virtual-sample-2, results='hide'}
almonds_sample_100 |> 
  rep_sample_n(size = 100, replace = TRUE, reps = 1000)
```
...
```{r confidence-intervals-compute-mean-12, results='hide'}
almonds_sample_100 |> 
  rep_sample_n(size = 100, replace = TRUE, reps = 1000) |> 
  summarize(mean_weight = mean(weight))
```
...
```{r confidence-intervals-compute-mean-13, results='hide'}
almonds_sample_100 |> 
...
```{r confidence-intervals-specify-2}
almonds_sample_100 |> 
  specify(response = weight)
```
...
```{r confidence-intervals-specify-3, results='hide'}
almonds_sample_100 |> 
...
```{r confidence-intervals-bootstrap-2, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-bootstrap-3, eval=FALSE, purl=FALSE}
...
```{r confidence-intervals-bootstrap-4, results='hide'}
...
```{r confidence-intervals-bootstrap-5, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-compute-mean-14, eval=FALSE, purl=FALSE}
# infer workflow:                   # Original workflow:
almonds_sample_100 |>                  almonds_sample_100 |> 
  specify(response = weight) |>        rep_sample_n(size = 100, replace = TRUE, 
  generate(reps = 1000) |>                          reps = 1000) |>              
...
```{r confidence-intervals-hist-2, eval=FALSE, purl=FALSE}
# infer workflow:                    # Original workflow:
...
```{r confidence-intervals-mean-and-sd-6}
...
```{r confidence-intervals-compute-mean-15}
almonds_sample_100 |>
...
```{r confidence-intervals-specify-4}
x_bar <- almonds_sample_100 |> 
  specify(response = weight) |> 
  calculate(stat = "mean")
x_bar
...
```{r confidence-intervals-conf-interval-2}
...
```{r confidence-intervals-viz-dist-2, echo=TRUE, fig.show='hide'}
visualize(bootstrap_means) + 
...
```{r confidence-intervals-specify-5}
mythbusters_yawn |> 
...
```{r confidence-intervals-bootstrap-6, results='hide'}
mythbusters_yawn |> 
  specify(formula = yawn ~ group, success = "yes") |> 
  generate(reps = 1000, type = "bootstrap")
```

```{r confidence-intervals-bootstrap-7, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-bootstrap-8, results='hide'}
mythbusters_yawn |> 
  specify(formula = yawn ~ group, success = "yes") |> 
  generate(reps = 1000, type = "bootstrap") |> 
...
```{r confidence-intervals-bootstrap-9, results='hide'}
...
```{r confidence-intervals-bootstrap-10, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-conf-interval-3}
bootstrap_distribution_yawning |> 
  get_confidence_interval(type = "percentile", level = 0.95)
```
```{r confidence-intervals-conf-interval-4, include=FALSE, purl=FALSE}
...
```{r confidence-intervals-bootstrap-11}
...
```{r confidence-intervals-conf-interval-5}
...
```{r confidence-intervals-conditional-text-4, echo=FALSE, results="asis", purl=FALSE}
