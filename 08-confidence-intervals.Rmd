...
- A. It measures the average distance between each almond's weight and the sample mean.
- B. It measures the average distance between each almond's weight and the population mean.
...
- A. The distribution becomes wider with thicker tails.
- B. The distribution becomes narrower with thinner tails.
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Which of the following is true about the confidence level in constructing a confidence interval?
...
Chernick and LaBudde showed that the bootstrap is one of many resampling methods [@Chernick2011].
...
In this section we introduce some more details about the bootstrap, and then 
...
The theory-based approach and the percentile method are both considered *first-order accurate* [@Hall1992] [@Chernick2011]. 
...
First, in this Chapter we discussed mainly confidence intervals for $\mu$ because 
...
present a problem as $\overline X$ for these samples would not be close to normal 
