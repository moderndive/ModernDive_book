...
```{r confidence-intervals-1, echo=FALSE, message=FALSE, purl=FALSE}
library(dplyr)
library(moderndive)
library(tidyverse)
library(knitr)
...
```{r confidence-intervals-2, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-3, message=FALSE}
library(tidyverse)
library(moderndive)
library(infer)
```
...
```{r confidence-intervals-4, message=FALSE, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-5, echo=FALSE}
...
```{r confidence-intervals-7, echo=FALSE, purl=FALSE}
# This code is used for dynamic non-static in-line text output purposes
...
```{r confidence-intervals-8}
almonds_sample_100 |>
  summarize(sample_mean = mean(weight))
```

```{r confidence-intervals-9, echo=FALSE}
...
```{r confidence-intervals-10, echo=FALSE}
...
```{r confidence-intervals-13, echo=FALSE, results="asis", purl=FALSE}
if(!is_latex_output()) 
...
```{r confidence-intervals-14, echo=FALSE, results="asis", purl=FALSE}
if(!is_latex_output()) 
...
```{r confidence-intervals-15, echo=FALSE}
...
```{r normal-curve-1, echo=FALSE, fig.cap="Normal density curve for the sample mean weight of almonds.", purl=FALSE, out.width="90%", warning=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
...
```{r confidence-intervals-16, echo=FALSE}
...
```{r confidence-intervals-17, echo=FALSE}
...
```{r t-curve-1, echo=FALSE, fig.cap="The standard normal and two t-distributions.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2.5, 4)}
...
```{r confidence-intervals-19, echo=FALSE, results="asis", purl=FALSE}
if(!is_latex_output()) 
...
```{r confidence-intervals-20}
almonds_sample_100 |>
  summarize(sample_mean = mean(weight), sample_sd = sd(weight))
```

```{r confidence-intervals-21, echo=FALSE}
...
```{r confidence-intervals-25, results='hide'}
qnorm(0.95)
```

```{r confidence-intervals-26, echo=FALSE, purl=FALSE}
round(qnorm(0.95), 3)
```
...
```{r confidence-intervals-28, results='hide'}
qnorm(0.9)
```

```{r confidence-intervals-29, echo=FALSE, purl=FALSE}
round(qnorm(0.9), 3)
```
...
```{r tactile-resampling-2, echo=FALSE, fig.cap="Step 2: Weighing one almond at random.", fig.show="hold", purl=FALSE, out.width="30%"}
...
```{r confidence-intervals-32, echo=-1}
set.seed(202)
...
```{r confidence-intervals-33}
boot_sample
```

We can also study some of the characteristics of this bootstrap sample, such as its sample mean:

```{r confidence-intervals-34}
boot_sample |> 
  summarize(mean_weight = mean(weight))
```
```{r confidence-intervals-35, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-36, echo=TRUE, fig.show='hide'}
...
```{r confidence-intervals-37, echo= -1}
set.seed(20)
...
```{r confidence-intervals-40, echo=-1}
set.seed(20)
...
```{r confidence-intervals-41}
boot_means |> 
  summarize(mean_of_means = mean(mean_weight),
            sd_of_means = sd(mean_weight))
```
```{r confidence-intervals-42, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-43, results='hide'}
almonds_sample_100 |> 
  rep_sample_n(size = 100, replace = TRUE, reps = 1000)
```
...
```{r confidence-intervals-44, results='hide'}
almonds_sample_100 |> 
  rep_sample_n(size = 100, replace = TRUE, reps = 1000) |> 
  summarize(mean_weight = mean(weight))
```
...
```{r confidence-intervals-45, results='hide'}
almonds_sample_100 |> 
...
```{r confidence-intervals-46, results='hide'}
almonds_sample_100 |> 
...
```{r confidence-intervals-48, results='hide'}
almonds_sample_100 |> 
...
```{r confidence-intervals-49, results='hide'}
almonds_sample_100 |> 
  specify(response = weight) |> 
  generate(reps = 1000, type = "bootstrap")
```

```{r confidence-intervals-50, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-51, eval=FALSE, purl=FALSE}
...
```{r confidence-intervals-52, results='hide'}
...
```{r confidence-intervals-53, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-54, eval=FALSE, purl=FALSE}
# infer workflow:                   # Original workflow:
almonds_sample_100 |>                  almonds_sample_100 |> 
  specify(response = weight) |>        rep_sample_n(size = 100, replace = TRUE, 
  generate(reps = 1000) |>                          reps = 1000) |>              
...
```{r bootstrap-distribution-infer, fig.height=ifelse(is_latex_output(), 1.7, 4) , fig.cap="Bootstrap distribution.", purl=FALSE}
...
```{r confidence-intervals-55, eval=FALSE, purl=FALSE}
# infer workflow:                    # Original workflow:
...
```{r confidence-intervals-58, echo=TRUE, fig.show='hide'}
visualize(bootstrap_means) + 
...
```{r confidence-intervals-59, echo=TRUE, fig.show='hide'}
visualize(bootstrap_means) + 
...
```{r confidence-intervals-64, echo=TRUE, fig.show='hide'}
visualize(bootstrap_means) + 
...
```{r confidence-intervals-65}
mythbusters_yawn
```

```{r confidence-intervals-66, echo=FALSE, purl=FALSE}
# This code is used for dynamic non-static in-line text output purposes
n_participants <- 50L
```
...
```{r confidence-intervals-70, echo=FALSE}
set.seed(22)
```

```{r confidence-intervals-71}
first_six_rows |> 
...
```{r confidence-intervals-72, results='hide'}
mythbusters_yawn |> 
  specify(formula = yawn ~ group, success = "yes") |> 
  generate(reps = 1000, type = "bootstrap")
```

```{r confidence-intervals-73, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-74, results='hide'}
mythbusters_yawn |> 
  specify(formula = yawn ~ group, success = "yes") |> 
  generate(reps = 1000, type = "bootstrap") |> 
...
```{r confidence-intervals-75, results='hide'}
...
```{r confidence-intervals-76, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-77}
bootstrap_distribution_yawning |> 
  get_confidence_interval(type = "percentile", level = 0.95)
```
```{r confidence-intervals-78, include=FALSE, purl=FALSE}
...
```{r confidence-intervals-81, echo=FALSE, results="asis", purl=FALSE}
