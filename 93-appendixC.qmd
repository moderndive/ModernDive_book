...
# Tips and Tricks {#sec-appendixC}

::: {.learncheck}
**Note: This appendix is still under construction as we update some materials from the First Edition to the Second Edition. If you would like to contribute, please check us out on GitHub at <https://github.com/moderndive/moderndive_book>.**
:::

## Needed packages {.unnumbered}

Let's load all the packages needed for this chapter (this assumes you've already installed them). Recall from our discussion in @sec-tidyverse-package that loading the `tidyverse` package by running `library(tidyverse)` loads the following commonly used data science packages all at once:
...
```{r appendixC-1, message=FALSE, warning=FALSE}
library(tidyverse)
library(scales)
library(janitor)
library(dygraphs)
...
* @sec-appendix-missing-values: Dealing with missing values
* @sec-appendix-reordering-bars: Reordering bars in a barplot
* @sec-appendix-money-on-axis: Showing money on an axis
* @sec-appendix-changing-values: Changing values inside cells
* @sec-appendix-convert-numerical-categorical: Converting a numerical variable to a categorical one
* @sec-appendix-prop: Computing proportions
* @sec-appendix-commas: Dealing with %, commas, and \$

![Cartoon ninja mascot representing the "data wrangling" appendix.](images/data_ninja1.png){width=30%}

Let's load an example movies dataset, pare down the rows and columns a bit, and then show the first 10 rows using `slice()`.

```{r appendixC-alt, eval=FALSE}
...
movies_ex %>%
  slice(1:10)
```
```{r appendixC-alt2, echo=FALSE, purl=FALSE}
...
### Dealing with missing values {#sec-appendix-missing-values}
...
```{r appendixC-v4}
movies_ex %>%
  summarize(median_revenue = median(millions))
```
...
### Reordering bars in a barplot {#sec-appendix-reordering-bars}

Let's compute the total revenue for each movie type and plot a barplot.

```{r appendixC-bar-simple, fig.alt="Barplot of total box-office revenue by movie genre. Bars appear in default alphabetical order along the x-axis, with action and drama as the tallest bars."}
...
```{r appendixC-bar-simple-v2, fig.alt="Same revenue-by-genre barplot as the previous figure, but with the x-axis manually reordered to: rom comedy, action, drama, animated, comedy, fantasy."}
...
```{r appendixC-bar-simple-v2-dup1, fig.alt="Same revenue-by-genre barplot but with bars sorted in ascending order of total revenue via `reorder(type, total_revenue)` — shortest bars on the left, tallest on the right."}
revenue_by_type <- revenue_by_type %>%
...
```{r appendixC-bar-simple-v2-dup2, fig.alt="Same revenue-by-genre barplot but with bars sorted in descending order of total revenue via `reorder(type, -total_revenue)` — tallest bars on the left, shortest on the right."}
revenue_by_type <- revenue_by_type %>%
  mutate(type = reorder(type, -total_revenue))
...
![Hex logo for the `forcats` package — a stylized fox above the package name.](https://github.com/tidyverse/forcats/blob/main/man/figures/logo.png?raw=true){width=20%}


### Showing money on an axis {#sec-appendix-money-on-axis}

```{r appendixC-v11, fig.alt="Side-by-side boxplots of movie revenue (in raw dollars) by MPAA rating. The y-axis labels show large dollar values in scientific notation, motivating the dollar-formatting fix shown in the next code chunk.", fig.height=3, fig.width=5}
movies_ex <- movies_ex %>%
...
```{r appendixC-v12, fig.alt="Same side-by-side boxplots of movie revenue by rating, but with y-axis labels formatted as dollar amounts (e.g., $50,000,000) via `scale_y_continuous(labels = dollar)` from the scales package.", fig.height=3, fig.width=5}
...
### Changing values inside cells {#sec-appendix-changing-values}
...
### Converting a numerical variable to a categorical one {#sec-appendix-convert-numerical-categorical}
...
included in the lesser category (e.g. 60.0 is 'so-so' not 'good'), to 
...
### Computing proportions {#sec-appendix-prop}
...
### Dealing with %, commas, and \$ {#sec-appendix-commas}
...
![Cartoon R ninja mascot celebrating completion of the data-wrangling appendix.](images/Rninja.png){width=40%}
...
```{r appendixC-create-flights_day, warning=FALSE, out.width="100%"}
library(dygraphs)
library(nycflights23)
