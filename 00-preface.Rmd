...
* *Are you an instructor hoping to use this book in your courses? We recommend you first read the "Introduction for students" section first and then read the "Introduction for instructors" section for more information on how to teach with this book.*
...
We'll end with a discussion on what it means to "tell your story with data" in Chapter \@ref(thinking-with-data) by presenting example case studies.
...
We will begin by digging into the grey **Understand** portion of the cycle with data visualization, then with a discussion on what is meant by tidy data and data wrangling, and then conclude by talking about interpreting and discussing the results of our models via **Communication**.  These steps are vital to any statistical analysis.  But why should you care about statistics?  "Why did they make me take this class?"
...
When looking over the large number of introductory statistics textbooks that currently exist, we found there wasn't one that incorporated many newly developed R packages directly into the text, in particular the many packages included in the [`tidyverse`](http://tidyverse.org/) collection of packages, such as `ggplot2`, `dplyr`, `tidyr`, and `readr`. Additionally, there wasn't an open-source and easily reproducible textbook available that exposed new learners to all four of the learning goals we listed in "Introduction for students."

### Who is this book for? {-}

This book is intended for instructors of traditional introductory statistics classes using RStudio, who would like to inject more data science topics into their syllabus. RStudio can be used in either the server version or the desktop version. (This is discussed further in Section \@ref(installing).) We assume that students taking the class will have no prior algebra, no calculus, nor programming/coding experience.
...
1. **Complete reproducibility and customizability**
    + We are frustrated when textbooks give examples, but not the source code and the data itself. We give you the source code for all examples as well as the whole book! We have made choices in certain spots to not show the code directly in the book to produce more complicated figures, but reviewing the textbook's repository will provide you with all code needed.
...
<!--
* **Printed version** The release corresponding to this printed CRC Press book:
   + Version 1.0.0 released on <insert date>
   + Available at https://moderndive.com/previous_versions/v1.0.0/index.html
-->

* **Latest published online version** The most up-to-date release:
