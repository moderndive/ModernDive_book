...
We aren't so sure about this claim and as researchers believe that it is
...
There is also a left-sided test that corresponds to a researcher's claim 
such as "the average 
weight is __less__ than 3.6 grams" and associated hypotheses 
$H_0: \mu \ge 3.6$ versus $H_A: \mu < 3.6$. The null hypothesis is usually
truncated to only check for equality, so the competing hypotheses would be
$H_0: \mu = 3.6$ versus $H_A: \mu < 3.6$.
Similarly, a right-sided test corresponds to a researchers claim such as 
"the average weight is at 
most 3.6 grams" with associated hypotheses $H_0: \mu = 3.6$ versus 
$H_A: \mu > 3.6$. We work with the two-sided test in what follows, but we will 
comment about the changes needed in the process if we are working with the left- 
...
```{r t-curve-hypo, echo=FALSE, fig.height=ifelse(is_latex_output(), 2, 7), fig.width=3, purl=FALSE, fig.cap="The tails of a t curve for this hypothesis test"}
...
Using the simulation-based approach via bootstrap percentile method, the 95% confidence interval is
