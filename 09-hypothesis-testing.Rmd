...
When using the samples, as we did for one-sample problems, we standardize the difference in sample means,  $\overline{x}_a - \overline{x}_r$, by subtracting by its mean, the differences in population means, and dividing by its standard error. This construct a test statistics known as the *Welch's two-sample $t$-test statistic*\index{Welch's two-sample t-test statistic}:
...
Based on the $p$-value = `r round(t_table_2_sample[1,3][[1]],3)` we reject the null hypothesis, the average rating for the `Romance` movies is not the same as the average rating for the `Action` movies. This results is similar to the one calculated using the simulation-based approach.
...
2. The sample of one population should be independent from the sample of the other population.
3. The samples should be random samples.

Let us see if these conditions hold for our `movies_sample` data:

1. This is met since $n_a$ = `r n_action` and $n_r$ = `r n_romance` do not seem to be highly skewed and there are somewhat symmetric.
...
1. Unfortunately, we do not know how IMDb computes the ratings. For example, if the same person can rated multiple movies, then those observations may be related.
