...
ggplot(spotify_52_shuffled, aes(x = track_genre, fill = popular_or_not)) +
  ...
  labs(x = "Genre of track")
```
```{r spotify-genre-barplot-permuted, fig.cap="Barplots of relationship of genre with `popular or not' (left) and shuffled `popular or not' (right).", fig.height=ifelse(knitr::is_latex_output(), 2.6, 7), echo=FALSE, purl=FALSE, message=FALSE}
...
The resulting data frame has `r (n_songs*n_reps) |> comma()` rows. This is because we performed shuffles/permutations for each of the `r n_songs` rows `r n_reps` times and $`r (n_songs*n_reps) |> comma()` = `r n_reps` \cdot `r n_songs`$. If you explore the `spotify_generate` data frame with `View()`, you will notice that the variable `replicate` indicates which resample each row belongs to. So it has the value `1` `r n_songs` times, the value `2` `r n_songs` times, ... to the value `` `r n_reps` `` `r n_songs` times. 
