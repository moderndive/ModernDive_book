...
```{r hypothesis-testing-mean-and-sd-2, eval=FALSE}
almonds_sample_100 |>
...
```{r hypothesis-testing-mean-and-sd-3}
almonds_sample_100 |>
...
```{r hypothesis-testing-select-vars-2, eval=FALSE}
spotify_52_shuffled |> 
  select(-track_id) |> 
  head(10)
```
...
```{r hypothesis-testing-bar-2, eval=FALSE}
...
```{r hypothesis-testing-null-dist-2}
spotify_metal_deephouse |> 
  specify(formula = popular_or_not ~ track_genre, success = "popular") |> 
...
```{r hypothesis-testing-null-dist-3, eval=FALSE}
...
```{r hypothesis-testing-null-dist-4, echo=FALSE, purl=FALSE}
...
```{r hypothesis-testing-null-dist-5, eval=FALSE}
...
```{r hypothesis-testing-specify-2}
...
```{r hypothesis-testing-null-dist-6, eval=FALSE}
null_distribution <- spotify_metal_deephouse |> 
  specify(formula = popular_or_not ~ track_genre, success = "popular") |> 
  hypothesize(null = "independence") |> 
  generate(reps = 1000, type = "permute") |> 
...
```{r hypothesis-testing-null-dist-7, eval=FALSE}
...
```{r hypothesis-testing-null-dist-8, echo=FALSE, purl=FALSE}
...
```{r hypothesis-testing-conf-interval-2}
...
```{r hypothesis-testing-conf-interval-3, eval=FALSE}
...
```{r hypothesis-testing-viz-dist-2, eval=FALSE}
visualize(bootstrap_distribution) + 
...
```{r hypothesis-testing-load-packages-2, eval=FALSE}
library(moderndive)
library(infer)
...
```{r hypothesis-testing-dynamic-text-2, echo=FALSE, purl=FALSE}
# This code is used for dynamic non-static in-line text output purposes
...
```{r hypothesis-testing-grouped-summary-2}
movies_sample |> 
  group_by(genre) |> 
...
```{r hypothesis-testing-specify-3}
movies_sample |> 
...
```{r hypothesis-testing-null-dist-9}
movies_sample |> 
  specify(formula = rating ~ genre) |> 
  hypothesize(null = "independence")
```
...
```{r hypothesis-testing-null-dist-10, echo=FALSE, purl=FALSE}
set.seed(76)
...
```{r hypothesis-testing-null-dist-11, eval=FALSE}
...
```{r hypothesis-testing-specify-4}
...
```{r hypothesis-testing-grouped-summary-3}
movies_sample |> 
  group_by(genre) |> 
  summarize(n = n(), mean_rating = mean(rating), std_dev = sd(rating))
```
```{r hypothesis-testing-specify-5, echo=FALSE, purl=FALSE}
