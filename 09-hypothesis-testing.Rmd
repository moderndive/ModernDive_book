...
We have studied confidence intervals in Chapter \@ref(confidence-intervals). This chapter introduces hypothesis testing, another widely used method for statistical inference. Hypothesis tests allow us to take a sample from a population and infer about the plausibility of competing hypotheses. For example, in the upcoming music popularity activity by genre in Section \@ref(ht-activity), you will study data collected from Spotify to investigate whether metal music is more popular than deep-house music. 
...
We believe that this approach is better for long-term learning than focusing on specific details for specific confidence intervals. We prefer this approach also for hypothesis tests as well.

In Section \@ref(tying-CI-hypo) we review confidence intervals and introduce hypothesis tests for one-sample problems; in particular, for the mean $\mu$. We use both the theory-based and simulation-based approaches, and provide some justification why we consider a better idea to carefully unpack the simulation-based approach for hypothesis testing in the context of two-sample problems. We also show the direct link between confidence intervals and hypothesis tests. In Section \@ref(ht-activity) we introduce the activity that motivate the simulation-based approach for two-sample problems, data collected from Spotify to investigate whether metal music is more popular than deep-house music. In Sections \@ref(understanding-ht), \@ref(ht-infer), and \@ref(ht-interpretation) we explain, conduct, and interpret hypothesis test, respectively, using the simulation-based approach of permutation. We introduce a case study in Section \@ref(ht-case-study), and in Section \@ref(nhst-conclusion) we conclude the discusstion of the theory-based approach for two-sample problems and some additional remarks.



```{r appendixb, echo=FALSE, results="asis", purl=FALSE}
if (!is_latex_output()) {
  cat("If you'd like more practice or you are curious to see how this framework applies to different scenarios, you can find fully-worked out examples for many common hypothesis tests and their corresponding confidence intervals in Appendix B. ")
...
Hypothesis test takes advantages of similar tools but the nature and the goal of 
...
### The one-sample hypothesis test for the population mean
...
Let us continue working with the population mean, $mu$. 
...
website that the average weight of a chocolate covered almond is exactly 3.6 gr. 
This is the claim we want to test. To do this we have collected a random sample 
of $n = 100$ almonds. The data is stored in the object `almonds_sample_100` and 
...
extreme than the one we have observed? 

When the answer to the question is: "If the claim was true, it would be very 
...
hypothesis, $H_0$. It is a statement about $\mu$ and it is
initially assumed to be true.
A competing statement called the alternative hypothesis, $H_A$, is also a 
...
of $\mu$ under the null hypothesis. This test is called a two-sided test because 
evidence against the null hypothesis could come from either direction (greater 
or less). 

There is also a left-sided test that corresponds to a claim such as "the average 
weight is __greater__ than 3.6 grams" and associated hypotheses 
$H_0: \mu \ge 3.6$ versus $H_A: \mu < 3.6$ (even though the statement is 
strictly greater, we include the equal part __always__ under the null hypothesis 
as this is required for the procedure).
Similarly, a right-sided test corresponds to a claim such as "the average weight is at 
most 3.6 grams" with associated hypotheses $H_0: \mu \le 3.6$ versus 
$H_A: \mu > 3.6$. 

In the exposition that follows we work with the two-sided test, but we will 
comment what are the changes needed in the process if we have to work with the left- 
or right-sided alternatives.

#### The theory-based approach {#theory-based-one-sample}


We use the theory-based approach to illustrate how the hypothesis test is 
...
in the code below. Table \@ref(tab:hypo-test-1) presents this values.
...
The value of $t = `r round(hypo_test[1,5][[1]],2)`$ is the sample mean 
...
statistic (t = `r round(hypo_test[1,5][[1]],2)`). 

Assuming that the null hypothesis is true ($\mu = 3.6$ grams) how likely is it 
to observe a sample as extreme as of more extreme than `almonds_sample_100`? 
...
$t = `r round(hypo_test[1,5][[1]],2)`$ units or more away from the center of 
the $t$ distribution?

Because this is a two-sided test, we care about extreme values that are 
`r round(hypo_test[1,5][[1]],2)` away in either direction of the distribution. 
...
```{r t-curve-hypo, echo=FALSE, fig.height=2, fig.width=3, purl=FALSE}}
t_value <- hypo_test[1,5][[1]]
...
the value of $t$ on the left part of the plot (`r -abs(round(hypo_test[1,5][[1]],2))`) 
...
symmetric, we simply multiply that results by 2:
...
we reject the null hypothesis. While technically not incorrect, this type of statements 
do not provide enough insight for students to fully understand the conclusion.

Let us unpack some of these elements and provide additional context to them:
...
actually true. In statistics, making this mistake is call a Type I Error. So, we 
only reject the null hypothesis if the chances of committing a Type I Error are 
truly small. The significance level, $\alpha$, is precisely the probability of 
...
we should be able to work with this number, after all we may have to interact 
...
We provide an example. Assume that you 
...
a "one" you score zero in your test, otherwise you score 100. There is a 1 in 6 
chance that you get zero. Would you play this game?

If you would not play the game, let us change it. Now the instructor rolls the die 
...
Converted to probabilities, the three games shown above give you the probabilities to 
...
In the context of hypothesis test, when the random sample collected is extreme, the $p$ value is really small, and 
we reject the null hypothesis, there is always a chance that null hypothesis was true, the random sample collected was very atypical, and led us to commit a Type I Error.
...
Returning to the almond example, if we had set $\alpha = 0.04$ and we observe the $p$-value = `r round(p_val,3)`, we reject the null hypothesis and conclude that 
...
Let us summarize the steps for hypothesis testing:
...
2. Set a significance level, $\alpha$, based on your tolerance for committing a Type I Error. 
    - Set this value always before working with the sample.
3. Obtain the sample mean, sample standard deviation, $t$-test statistic, and $p$ value.
    ...
    - For a left-sided test, find the area under the $t$ distribution to the left of the $t$ test statistic.
    - For a right-sided test, find the area under the $t$ distribution to the right of the $t$ test statistic.
4. Determine whether the result of the test is statistically significant (if null is rejected) or non significant (the null is not rejected).


#### The simulation-based approach

When using a simulation-based approach such as the bootstrap percentile method, 
we repeat the first 2 steps of the theory-based approach:
...
Even though hypothesis test and confidence intervals are two different approaches that have different goals, they complement each other.
For example, in Subsection \@ref(t-distribution-CI) we calculated the 95% confidence interval for the almonds' mean weight, $\mu$, using the sample `almonds_sample_100`. The theory-based approach is given by:
...
If using the simulation-based approach via bootstrap percentile method, the 95% confidence interval is
...
$\alpha$ is used, calculate a confidence interval for a confidence level equal 
...
we can do it also for other quantities, such as population median, quartiles, etc.

We focus now on building hypothesis tests for two-sample problems.

## Music popularity activity {#ht-activity}

Let us start with an activity studying the effect of music genre on Spotify song popularity. 
...
Let us perform an exploratory data analysis of the relationship between the two categorical variables `track_genre` and `popular_or_not`. Recall that we saw in Subsection \@ref(two-categ-barplot) that one way we can visualize such a relationship is by using a stacked barplot. 
...
```{r spotify-genre-barplot, echo=FALSE, fig.cap="Barplot relating genre to popularity.", fig.height=ifelse(knitr::is_latex_output(), 1.6, 7), purl=FALSE}
...
Let us repeat the same exploratory data analysis we did for the original `spotify_metal_deephouse` data on our `spotify_52_original` and `spotify_52_shuffled` data frames. Let us create a barplot visualizing the relationship between `track_genre` and the new shuffled `popular_or_not` variable, and compare this to the original un-shuffled version in Figure \@ref(fig:spotify-genre-barplot-permuted).
...
ggplot(spotify_52_shuffled, 
  ...
  labs(x = "Genre of track")
```
```{r spotify-genre-barplot-permuted, fig.cap="Barplots of relationship of genre with `popular or not' (left) and shuffled `popular or not' (right).", fig.height=ifelse(knitr::is_latex_output(), 4.7, 7), echo=FALSE, purl=FALSE, message=FALSE}
...
Let us also compute the proportion of tracks that are now "popular" in the `popular_or_not` column for each genre:
...
Observe how the alternative hypothesis $H_A$ is one-sided with $p_{m} - p_{d} > 0$. Had we opted for a two-sided alternative, we would have set $p_{m} - p_{d} \neq 0$. To keep things simple for now, we will stick with the simpler one-sided alternative. We will present an example of a two-sided alternative in Section \@ref(ht-case-study).
...
Alternatively, if the $p$-value does not fall below $\alpha$, we would "fail to reject $H_0$." Note the latter statement is not quite the same as saying we "accept $H_0$." This distinction is rather subtle and not immediately obvious. So we will revisit it later in Section \@ref(ht-interpretation).

While different fields tend to use different values of $\alpha$, some commonly used values for $\alpha$ are 0.1, 0.01, and 0.05; with 0.05 being the choice people often make without putting much thought into it. We will talk more about $\alpha$ significance levels in Section \@ref(ht-interpretation), but first let us fully conduct the hypothesis test corresponding to our genre popularity activity using the `infer` package.
...
Furthermore, we will use a pre-specified significance level $\alpha$ = `r alpha` for this hypothesis test. Let us leave discussion on the choice of this $\alpha$ value until later on in Section \@ref(ht-interpretation). 
...
Let us save the result in a data frame called `null_distribution`:
...
```{r null-distribution-infer, fig.show="hold", fig.cap="Null distribution.", fig.height=ifelse(knitr::is_latex_output(), 1.8, 7)}
visualize(null_distribution, bins = 25)
```

Let us now add what happened in real life to Figure \@ref(fig:null-distribution-infer), the observed difference in popularity rates of `r p_metal_popular` - `r p_deephouse_popular` = `r observed_test_statistic` = `r observed_test_statistic*100`%. However, instead of merely adding a vertical line using `geom_vline()`, let us use the \index{infer!shade\_p\_value()} `shade_p_value()` function with `obs_stat` set to the observed test statistic value we saved in `obs_diff_prop`. 
...
Using this `bootstrap_distribution`, let us first compute the percentile-based confidence intervals, as we did in Section \@ref(bootstrap-process):
...
Using our shorthand interpretation for 90% confidence intervals, we are 90% "confident" that the true difference in population proportions $p_{m} - p_{d}$ is between (`r percentile_ci[["lower_ci"]] |> round(4)`, `r percentile_ci[["upper_ci"]] |> round(4)`). Let us visualize `bootstrap_distribution` and this percentile-based 90% confidence interval for $p_{m} - p_{d}$ in Figure \@ref(fig:bootstrap-distribution-two-prop-percentile).

```{r eval=FALSE}
visualize(bootstrap_distribution) + 
  shade_confidence_interval(endpoints = percentile_ci)
```
```{r bootstrap-distribution-two-prop-percentile, echo=FALSE, fig.show="hold", fig.cap="Percentile-based 95\\% confidence interval.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.2, 7)}
if (is_html_output()) {
  visualize(bootstrap_distribution) +
    shade_confidence_interval(endpoints = percentile_ci)
} else {
...
Let us visualize `bootstrap_distribution` again, but now the standard error based 95% confidence interval for $p_{m} - p_{d}$ in Figure \@ref(fig:bootstrap-distribution-two-prop-se). Again, notice how the value 0 is not included in our confidence interval, again suggesting that $p_{m}$ and $p_{d}$ are truly different!
...
Let us recap the steps necessary to conduct a hypothesis test using the terminology, notation, and definitions related to sampling you saw in Section \@ref(understanding-ht) and the `infer` workflow from Subsection \@ref(infer-workflow-ht):
...
In other words, _not guilty_ verdicts are not suggesting the defendant is _innocent_, but instead that "while the defendant may still actually be guilty, there was not enough evidence to prove this fact." Now let us make the connection with hypothesis tests:
...
```{r eval=!file.exists("images/gt_error_table_v2.png"), echo=FALSE, purl=FALSE}
trial_table <- tibble(
...
Thus, a Type I error corresponds to incorrectly putting a truly innocent person in jail, whereas a Type II error corresponds to letting a truly guilty person go free. Let us show the corresponding table in Figure \@ref(fig:hypo-test-errors) for hypothesis tests.

```{r eval=!file.exists("images/gt_error_table_ht_v2.png"), echo=FALSE, purl=FALSE}
hypo_table <- tibble(
...
```{r hypo-test-errors, echo=FALSE, fig.cap="Type I and Type II errors in hypothesis tests.", purl=FALSE}
...
Let us perform an exploratory data analysis of this data. Recall from Subsection \@ref(geomboxplot) that a boxplot is a visualization we can use to show the relationship between a numerical and a categorical variable. Another option you saw in Section \@ref(facets) would be to use a faceted histogram. However, in the interest of brevity, let us only present the boxplot in Figure \@ref(fig:action-romance-boxplot).

```{r action-romance-boxplot, fig.cap="Boxplot of IMDb rating vs. genre.", fig.height=ifelse(knitr::is_latex_output(), 2.7, 7)}
...
We will repeat this resampling without replacement of `type = "permute"` a total of ``reps = `r n_reps` `` times. Feel free to run the code below to check out what the `generate()` step produces.
...
Now that we have `r n_reps` replicated "shuffles" assuming the null hypothesis $H_0$ that both `Action` and `Romance` movies on average have the same ratings on IMDb, let us `calculate()` the appropriate summary statistic for these `r n_reps` replicated shuffles. From Section \@ref(understanding-ht), summary statistics relating to hypothesis testing have a specific name: *test statistics*. Since the unknown population parameter of interest is the difference in population means $\mu_{a} - \mu_{r}$, the test statistic of interest here is the difference in sample means $\overline{x}_{a} - \overline{x}_{r}$. 

For each of our `r n_reps` shuffles, we can calculate this test statistic by setting `stat = "diff in means"`. Furthermore, since we are interested in $\overline{x}_{a} - \overline{x}_{r}$, we set `order = c("Action", "Romance")`. Let us save the results in a data frame called `null_distribution_movies`:
...
```{r null-distribution-movies-2, echo=FALSE, fig.cap="Null distribution, observed test statistic, and $p$-value.", fig.height=ifelse(knitr::is_latex_output(), 1.8, 7), purl=FALSE}
if (is_html_output()) {
  visualize(null_distribution_movies, bins = 10) +
    shade_p_value(obs_stat = obs_diff_means, direction = "both")
} else {
...
As we did in the case of one-sample problems, we standardize  $\overline{x}_a - \overline{x}_r$ of action versus romance movies, by subtracting its mean and dividing by its standard deviation. First, if the sampling was done in a representative fashion, then the sampling distribution of $\overline{x}_a - \overline{x}_r$ will be centered at the true population parameter $\mu_a - \mu_r$. Second, the standard deviation of point estimates like $\overline{x}_a - \overline{x}_r$ has a special name: the standard error.

Applying these ideas, we present the *two-sample Welch's $t$-statistic*\index{two-sample Welch's t-statistic}:
...
In the numerator, $\bar{x}_a-\bar{x}_r$ is the difference in sample means, while $\mu_a - \mu_r$ is the difference in population means. In the denominator, $s_a$ and $s_r$ are the *sample standard deviations* of the action and romance movies in our sample `movies_sample`. Lastly, $n_a$ and $n_r$ are the sample sizes of the action and romance movies. Putting this together under the square root gives us the standard error $\text{SE}(\bar{x}_a - \bar{x}_r)$.
    
Observe that the formula for $\text{SE}(\bar{x}_a - \bar{x}_r)$ has the sample sizes $n_a$ and $n_r$ in them. So as the sample sizes increase, the standard error goes down. We have seen this concept numerous times now, in particular (1) in our simulations using the three virtual shovels with $n$ = 25, 50, and 100 slots in Figure \@ref(fig:comparing-sampling-distributions-3), (2) where we studied the effect of using larger sample sizes on the widths of confidence intervals, and (3) where we studied the formula-based approximation to the standard error of the sample proportion $\widehat{p}$. 

So how can we use the two-sample Welch's $t$-statistic as a test statistic in our hypothesis test? First, assuming the null hypothesis $H_0: \mu_a - \mu_r = 0$ is true, the right-hand side of the numerator (to the right of the $-$ sign), $\mu_a - \mu_r$, becomes 0. We do this to test whether $\mu_a = \mu_r$ which is often the question of interest.

Second, similarly to how the Central Limit Theorem states that sample means follow a normal distribution, it can be mathematically proven that the two-sample Welch's $t$-statistic follows a *$t$ distribution*.
Recall that in Section \@ref(t-distribution-CI) we have discussed the properties of the $t$ distribution and in Figure \@ref(fig:t-curve-1) we have shown different $t$ distributions with different degrees of freedom. 
Recall that the $t$ distribution is similar to the standard normal; its density curve is also bell-shaped, and it is symmetric around zero, but the tails of the $t$ distribution are a little thicker than those of the standard normal. 
This is important for hypothesis testing, since the $p$ value is calculated as areas on those tails. 
...
$$\widehat{df} = \frac{\left(\dfrac{{s_a}^2}{n_a} + \dfrac{{s_r}^2}{n_r}\right)^2}{\frac{\left(\frac{s_a^2}{n_a}\right)^2}{n_a - 1} + \frac{\left(\frac{s_r^2}{n_r}\right)^2}{n_r - 1}}$$
This formula is just too complicated to enter manually every time that is needed. Fortunately, R and other statistical software have already done that for us by introducing relevant functions. While it is important to get good approximations to the degrees of freedom in order to get the appropriate $p$ values, learning this formula goes beyond the reach of those new to statistical inference and it does little to build the intuition of the $t$-test. Therefore, we will trust the results that R or other statistical packages provide for us.

Assuming that the null hypothesis $H_0$ is true, the $t$ test statistic becomes:
...
Let us compute the values necessary for this two-sample $t$-statistic. Recall the summary statistics we computed during our exploratory data analysis in Section \@ref(imdb-data).
...
Based on the $p$-value = `r round(t_table_2_sample[1,3][[1]],3)` we reject the null hypothesis, the average rating for the `Romance` movies is not the same as the average rating for the `Action` movies. do not  is similar to the one obta


Let us come back to that earlier warning message: `Check to make sure the conditions have been met for the theoretical method. {infer} currently does not check these for you.` To be able to use the $t$-test and other such theoretical methods, there are always a few conditions to check. The `infer` package does not automatically check these conditions, hence the warning message we received. These conditions are necessary so that the underlying mathematical theory holds. In order for the results of our two-sample $t$-test to be valid, three conditions must be met:
...
Let us see if these conditions hold for our `movies_sample` data:
...
1. Unfortunately, we do not know how IMDb computes the ratings. For example, if the same person rated multiple movies, then those observations would be related and hence not independent.
...
Let us consider an example. Say we are interested in the following question: Of *all* flights leaving a New York City airport, are Hawaiian Airlines flights in the air for longer than Alaska Airlines flights? Furthermore, let us assume that 2023 flights are a representative sample of all such flights. Then we can use the `flights` data frame in the `nycflights23` \index{R packages!nycflights23} package we introduced in Section \@ref(nycflights) to answer our question. Let us filter this data frame to only include Hawaiian and Alaska Airlines using their `carrier` codes `HA` and `AS`:
...
However, let us first construct an exploratory visualization as we suggested earlier. Since `air_time` is numerical and `carrier` is categorical, a boxplot can display the relationship between these two variables, which we display in Figure \@ref(fig:ha-as-flights-boxplot).

```{r ha-as-flights-boxplot, fig.cap="Air time for Hawaiian and Alaska Airlines flights departing NYC in 2023.", fig.height=ifelse(knitr::is_latex_output(), 2.8, 7)}
ggplot(data = flights_sample, mapping = aes(x = carrier, y = air_time)) +
  geom_boxplot() +
  labs(x = "Carrier", y = "Air Time")
```

This is what we like to call "no PhD in Statistics needed" moments. You do not have to be an expert in statistics to know that Alaska Airlines and Hawaiian Airlines have *significantly* different air times. The two boxplots do not even overlap! Constructing a confidence interval or conducting a hypothesis test would frankly not provide much more insight than Figure \@ref(fig:ha-as-flights-boxplot). 

Let us investigate why we observe such a clear cut difference between these two airlines using data wrangling. Let us first group the rows of `flights_sample` not only by `carrier` but also by destination `dest`. Subsequently, we will compute two summary statistics: the number of observations using `n()` and the mean airtime:
