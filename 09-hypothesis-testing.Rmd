...
```{r shuffling, echo=FALSE, fig.cap="Shuffling a deck of cards.", purl=FALSE, out.width="70%"}
...
We've saved one such shuffling in the `promotions_shuffled` data frame of the `moderndive` package. If you compare the original `promotions` and the shuffled `promotions_shuffled` data frames, you'll see that while the `decision` variable is identical, the `gender` variable has changed. Let's look at the six rows that we selected at random before when viewing the `promotions` data frame using the `slice()` function in the `dplyr` package.
...
```
```{r promotions-barplot-permuted, fig.cap="Barplots of relationship of promotion with gender (left) and shuffled gender (right).", fig.height=2.4, echo=FALSE}
height1 <- promotions %>% 
  group_by(gender, decision) %>% 
  summarize(n = n()) %>% 
  pull(n) %>% 
...
```{r null-distribution-infer, fig.show='hold', fig.cap="Null distribution.", fig.height=3.5}
...
```{r null-distribution-movies-2, echo=FALSE, fig.cap="Null distribution, observed test statistic, and $p$-value.", fig.height=2.8}
if(knitr::is_html_output()){
  visualize(null_distribution_movies, bins = 10) + 
    shade_p_value(obs_stat = obs_diff_means, direction = "both")
} else {
...
```{r zcurve, echo=FALSE, out.width="100%", fig.cap="Standard normal z curve."}
