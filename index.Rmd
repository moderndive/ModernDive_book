...
needed_CRAN_pkgs <- c(
  # Data packages:
  "nycflights13", "ggplot2movies", "fivethirtyeight", "okcupiddata", 
  "gapminder", "ISLR",
  # Used packages:
  #"ggplot2", 
  "tibble", "tidyr", "dplyr", "readr", "dygraphs", "rmarkdown", 
  "knitr", "mosaic", "broom", "remotes", "forcats", "plotly", "janitor",
  #"moderndive", 
  #"infer", 
  # "patchwork",
  # Internally used packages:
  "webshot", "tufte", "mvtnorm", "stringr", "gridExtra", "remotes"
  )
...
if(!"infer" %in% installed.packages()){
  devtools::install_github("andrewpbray/infer", ref = "p_value")
}
...
knitr::write_bib(
  c(.packages(), "bookdown", "knitr", "rmarkdown", "nycflights13", "remotes",
  "ggplot2", "webshot", "dygraphs", "okcupiddata", "dplyr",
...
* We then proceed to cover topics related to **statistical inference**, the bread and butter of statistics. To this end, we'll leverage a new package for tidyverse-friendly inference called [`infer`](https://infer.netlify.com/index.html). In particular:
    + Chapter \@ref(sampling) on sampling theory
    + Chapter \@ref(ci) on confidence intervals (Under construction) 
    + Chapter \@ref(hypo) on hypothesis testing (Under construction) 
* (Under construction) After studying simple instances of statistical inference, we revisit the data modeling topics from Chapters \@ref(regression) and \@ref(multiple-regression) and boost your abilities to interpret the results of regression in Chapter \@ref(inference-for-regression) on inference for regression.
* (Under construction) We'll end with a discussion on what it means to "think with data" in Chapter \@ref(thinking-with-data).
...
* Albert Y. Kim - Lecturer of Statistics, Amherst College. Amherst, MA, USA.
