...
* *Are you an instructor hoping to use this book in your courses? Then head to Section \@ref(sec:intro-instructors) for more information on how to teach with this book.*
* *Are you looking to connect with and contribute to ModernDive? Then head to Section \@ref(sec:connect-contribute) for information on how.*
* *Are you curious about the publishing of this book? Then head to Section \@ref(sec:about-book) for more information on the open-source technology, in particular R Markdown and the bookdown package.*


```{r echo=FALSE, purl=FALSE, results="asis"}
if(knitr::is_html_output())
  cat(glue::glue('This is version {version} of ModernDive published on {date}. For previous versions of ModernDive, see Section \\@ref(sec:about-book).'))
```
...
1. **Statistical inference**: Once again using your newly acquired data science tools, we'll unpack statistical inference using the `infer` package. In particular:
    ...
    + Ch.\@ref(confidence-intervals): Building confidence intervals through the use of bootstrapping.
    + Ch.\@ref(hypothesis-testing): Conducting hypothesis tests through the use of permutation testing.
