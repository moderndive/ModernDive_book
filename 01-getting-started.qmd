...
```{r getting-started-load-internal, message=FALSE, echo=FALSE, purl=FALSE}
...
We'll introduce these concepts in the upcoming @sec-r-rstudio--@sec-packages. If you are already somewhat familiar with these concepts, feel free to skip to @sec-nycflights where we'll introduce our first dataset: all domestic flights departing one of the three main New York City (NYC) airports in 2023. We will explore this dataset in depth for much of the rest of this book.



## What are R and RStudio? {#sec-r-rstudio}

Throughout this book, we will assume that you are using R via RStudio. First time users often confuse the two. At its simplest, R is like a car's engine\index{R} while RStudio is like a car's dashboard\index{RStudio} as illustrated in @fig-R-vs-RStudio-1.
...
```{r fig-R-vs-RStudio-1, echo=FALSE, fig.cap="Analogy of difference between R and RStudio.", out.width="95%", purl=FALSE}
include_graphics("images/shutterstock/R_vs_RStudio_1.png")
```

More precisely, R is a programming language that runs computations, whereas 
```{r getting-started-conditional-text, results="asis", echo=FALSE, purl=FALSE}
...
```                                                           
is an *integrated development environment (IDE)* that provides an interface by adding many convenient features and tools. So just as the way of having access to a speedometer, rear-view mirrors, and a navigation system makes driving much easier, using RStudio's interface makes using R much easier as well. 
...
1. **You must do this second:** Download and install RStudio at <https://posit.co/download/rstudio-desktop/>.
...
Recall our car analogy from earlier. Much as we don't drive a car by interacting directly with the engine but rather by interacting with elements on the car's dashboard, we won't be using R directly but rather we will use RStudio's interface. After you install R and RStudio on your computer, you'll have two new *programs* (also called *applications*) you can open. We'll always work in RStudio and not in the R application. @fig-R-vs-RStudio-2 shows what icon you should be clicking on your computer. 
...
```{r fig-R-vs-RStudio-2, echo=FALSE, fig.cap="Icons of R versus RStudio on your computer.", out.width="90%", purl=FALSE}
include_graphics("images/logos/R_vs_RStudio.png")
```

After you open RStudio, you should see something similar to @fig-RStudio-interface. (Note that slight differences might exist if the RStudio interface is updated to not be this by default.)

```{r fig-RStudio-interface, echo=FALSE, fig.cap="RStudio interface to R.", out.width="93%", purl=FALSE}
...
* *Data frames*: rectangular spreadsheets. They are representations of datasets in R where the rows correspond to *observations* and the columns correspond to *variables* that describe the observations. \index{data frame} We'll cover data frames later in @sec-nycflights.
...
### Errors, warnings, and messages {#sec-messages}
...
* **Errors**: \index{R!errors} when the red text is a legitimate error, it will be prefaced with "Error in…" and will try to explain what went wrong. Generally when there's an error, the code will not run. For example, we'll see in @sec-package-use if you see `Error in ggplot(...) : could not find function "ggplot"`, it means that the `ggplot()` function is not accessible because the package that contains the function (`ggplot2`) was not loaded with `library(ggplot2)`. Thus you cannot use the `ggplot()` function without the `ggplot2` package being loaded first.
...
* **Messages**: \index{R!messages} when the red text doesn't start with either "Error" or "Warning," it's *just a friendly message*. You'll see these messages when you load *R packages* in the upcoming @sec-package-loading or when you read data saved in spreadsheet files with the `read_csv()` function as you'll see in @sec-tidy. These are helpful diagnostic messages and they don't stop your code from working. Additionally, you'll see these messages when you install packages too using `install.packages()` as discussed in @sec-package-installation.
...
### Tips on learning to code {#sec-tips-code}
...
For example, among the many packages we will use in this book are the `ggplot2` package [@R-ggplot2] for data visualization in @sec-viz\index{R packages!ggplot2}, the `dplyr` package [@R-dplyr] for data wrangling in @sec-wrangling\index{R packages!dplyr}, the `moderndive` package [@R-moderndive] that accompanies this book\index{R packages!moderndive}, and the `infer` package [@R-infer] for "tidy" and transparent statistical inference in @sec-confidence-intervals, @sec-hypothesis-testing, and @sec-inference-for-regression\index{R packages!infer}.

A good analogy for R packages \index{R packages} is they are like apps you can download onto a mobile phone like in @fig-R-vs-R-packages:
...
```{r fig-R-vs-R-packages, echo=FALSE, fig.cap="Analogy of R versus R packages.", out.width="70%", purl=FALSE}
...
There are two ways to install an R package: an easy way and a more advanced way. \index{R packages!installation} Let's install the `ggplot2` package the easy way first as shown in @fig-easy-way-install. In the Files pane of RStudio:
...
```{r fig-easy-way-install, echo=FALSE, fig.cap="Installing packages in R the easy way.", out.width="49%", purl=FALSE}
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Repeat the earlier installation steps, but for the `dplyr`, `nycflights23`, and `knitr` packages. This will install the earlier mentioned `dplyr` package for data wrangling, the `nycflights23` package containing data on all domestic flights leaving a New York City airport in 2023, and the `knitr` package for generating easy-to-read tables in R. We'll use these packages in the next section.
...
```{r getting-started-load-ggplot2, eval=FALSE}
library(ggplot2)
```
...
`...` it means that you didn't successfully install it. This is an example of an "error message" we discussed in @sec-messages. If you get this error message, go back to @sec-package-installation on R package installation and make sure to install the `ggplot2` package before proceeding. 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** "Load" the `dplyr`, `nycflights23`, and `knitr` packages as well by repeating the earlier steps.
...
```{r getting-started-load-packages, message=FALSE}
...
```{r getting-started-dynamic-text, echo=FALSE, purl=FALSE}
...
```{r getting-started-kable-airlines, eval=FALSE}
airlines
kable(airlines)
```
...
```{r getting-started-demo-code, eval=FALSE}
airlines$name
```
...
### Identification and measurement variables {#sec-identification-vs-measurement-variables}
...
```{r getting-started-glimpse-airports}
glimpse(airports)
```
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Provide the names of variables in a data frame with at least three variables where one of them is an identification variable and the other two are not.
...
### Looking ahead to end-of-chapter exercises {#sec-exercise-packages}

Starting with @sec-viz, each chapter ends with a set of *end-of-chapter exercises* that go beyond the `nycflights23` data and ask you to apply what you've learned to other real-world datasets. The first new dataset arrives in @sec-viz: a tidied collection of every Olympic athlete from 1896 to 2026, in the [`olympicAthletes`](https://github.com/moderndive/olympicAthletes) package. \index{R packages!olympicAthletes}

Because `olympicAthletes` is not yet on CRAN, you'll install it from GitHub using the `remotes` package:

```{r getting-started-install-olympic, eval=FALSE}
install.packages("remotes")
remotes::install_github("moderndive/olympicAthletes")
```

This is a one-time install. Once `olympicAthletes` is on CRAN (it will be eventually), you'll be able to install it the usual way with `install.packages("olympicAthletes")`. Either way, you load it the same way:

```{r getting-started-load-olympic, eval=FALSE}
library(olympicAthletes)
```

We'll also introduce four other datasets later in the book — episodes of *Rick Steves' Europe* (`steves`), Bob Ross paintings (`bob_ross` from `fivethirtyeight`), confirmed exoplanets (`exoplanets`), and Holocene volcanoes (`volcanoes`). Each will be introduced when it first appears.
...
a. R is a web browser; RStudio is a desktop application
b. R is the programming language; RStudio is an integrated development environment (IDE) for using R
c. R is a graphical interface; RStudio is a command-line tool
d. They are two different names for the same software

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** R is the programming language and engine that runs your code. RStudio is an IDE — a friendlier interface for writing R code, viewing data, and managing projects. The chapter's analogy: R is the engine, RStudio is the dashboard.
:::

**Q2.** A column of phone numbers stored as text like `"(555) 123-4567"`. This variable is best classified as:

a. Numerical, because it contains digits
b. Categorical, because phone numbers identify entities rather than measure a quantity
c. Numerical, because you could perform calculations on the digits
d. Both numerical and categorical

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** The test for "numerical" is whether *arithmetic on the values is meaningful*, not whether digits appear. Adding two phone numbers makes no sense — they're identifiers, so they're categorical. Same logic applies to ZIP codes, customer IDs, and area codes.
:::

**Q3.** After running `install.packages("dplyr")` once, you close R and reopen it the next day. To use `filter()` again, you need to:

a. Re-install the package
b. Run `library(dplyr)` again — the install persists, but the loading does not
c. Nothing — packages stay loaded forever once installed
d. Type `filter()` and R will auto-load the package

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Installation is a one-time act per machine. *Loading* with `library(pkg)` happens at the start of every new R session — that's why scripts begin with `library()` calls.
:::
...
::: {.callout-tip collapse="true" title="Show answer"}
**(c)** R object names cannot start with a digit. `2021_flights` errors; `flights2021` (digit not first) is fine. They also cannot start with `_` or contain spaces.
:::

**Q5.** Both `x <- 5` and `x = 5` assign 5 to `x`. Why does the book recommend `<-`?

a. `=` is slower
b. `<-` is conventional for assignment; `=` is reserved for naming function arguments (like `mean(x, na.rm = TRUE)`), so distinguishing the two visually keeps code readable
c. `=` doesn't actually work for assignment
d. `<-` is required by RStudio

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Both work, but using `<-` for assignment and `=` for argument-naming keeps the two roles visually distinct. Mixing them up muddies code.
:::

**Q6.** What's the difference between `flights` and `"flights"` in R?

a. None
b. `flights` (no quotes) refers to the OBJECT named flights; `"flights"` (with quotes) is just an 8-character string of text
c. `flights` works; `"flights"` errors
d. `"flights"` is the data frame; `flights` is the column

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Quotes turn something into a literal string. Without quotes, R looks for an object by that name. This distinction matters in `library(dplyr)` (no quotes — refer to the package by name) vs `install.packages("dplyr")` (with quotes — pass the name as a string).
:::
...
::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Temperature has meaningful arithmetic — you can average two temperatures, take their difference, etc. The others are identifiers stored as digits.
:::

**Q8.** A user runs `View(flights)` to inspect the data. Then they edit values in the spreadsheet view. What happens to `flights`?

a. The data frame is updated with the edits
b. Nothing — `View()` is read-only; the spreadsheet display is for inspection only
c. R errors
d. A new data frame called `flights_edited` is created

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** `View()` opens a non-editable display. To modify a data frame, use `dplyr` verbs (`mutate()`, `filter()`, etc.) and reassign the result with `<-`.
:::

**Q9.** A `tibble` differs from a base R `data.frame` mainly because:

a. A tibble can hold mixed-type columns; a data frame cannot
b. A tibble prints only the first 10 rows by default and shows column types — friendlier behaviors
c. A tibble can be passed to dplyr verbs; a data frame cannot
...
**Q10.** When loading multiple packages at the top of a script, two are: `dplyr` and `stats` (loaded automatically with R). Both export `filter()`. Which version does R use when you call `filter()`?

a. The first one loaded — `stats::filter()`
b. The most recently loaded — `dplyr::filter()` overrides `stats::filter()` in your session
c. R errors due to ambiguity
d. Whichever is alphabetically first

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Later `library()` calls "mask" earlier ones. R prints a message at load time noting masked functions. To force a specific version, qualify the call: `dplyr::filter()` or `stats::filter()`.
:::
...
| `head(df, n)` | Show the first `n` rows | `head(flights, 5)` |
| `kable(df)` | Render a data frame as a formatted table | `kable(head(flights))` |

:::

## Exercises {.unnumbered #sec-getting-started-exercises}

A short set of warm-up exercises using the `olympic_athletes` dataset you installed in @sec-exercise-packages. These prompts focus on inspecting a real-world data frame — they're shorter than later chapters' sets to match Chapter 1's introductory scope. Solutions are available to instructors separately.
...
**EX1.1 (★)** Run `glimpse(olympic_athletes)`. How many rows and how many columns does the data frame have?
...
**EX1.2 (★)** What does each *row* of `olympic_athletes` represent? (Hint: it's not "one athlete" — it's something more specific.)

**EX1.3 (★)** Run `View(olympic_athletes)` (in RStudio) or `head(olympic_athletes, 10)`. What does each *column* represent? Pick three columns and write a one-line description for each.

```{webr-r}
head(olympic_athletes, 10)
```

**EX1.4 (★)** Among the columns, identify two **identification** variables and three **measurement** variables. Justify each.

**EX1.5 (★)** What does running `?olympic_athletes` do? Try it in RStudio. What does the help file tell you about the `Medal` column?

**EX1.6 (★)** What do you think the `NOC` column abbreviates? Look at a few rows to confirm. (Hint: 3-letter codes — Olympic-related.)

```{webr-r}
olympic_athletes |> select(Name, Team, NOC) |> head(10)
```

**EX1.7 (★)** Classify each of these columns as **numerical** or **categorical**: `Sex`, `Age`, `Height`, `Year`, `Sport`, `Medal`. Justify any that are tricky.

**EX1.8 (★)** What's the difference between `library(olympicAthletes)` and `install.packages("olympicAthletes")`? Why do you only need to do one of them once per machine?

**EX1.9 (★)** Run the following two pieces of code. Why do you get different output?
...
```{webr-r}
# olympic_athletes  |>  glimpse()
```

(One uses parentheses-style; the other uses the pipe `|>`. We'll learn the pipe in @sec-wrangling.)

**EX1.10 (★)** Look at the `medal_table` data frame (also from `olympicAthletes`). Describe one row of `medal_table` in plain English. How does its observational unit differ from `olympic_athletes`?
...
```{r getting-started-conditional-text-dup1, echo=FALSE, results="asis", purl=FALSE}
if (is_latex_output()) {
...
If you are new to the world of coding, R, and RStudio and feel you could benefit from a more detailed introduction, we suggest the short book, [*Getting Used to R, RStudio, and R Markdown*](https://rbasics.netlify.app/) [@usedtor2016], previewed in @fig-ismay-book. It includes screencasts that you can follow along and pause as you learn. This book also contains an introduction to R Markdown, a tool used for reproducible research.


```{r fig-ismay-book, echo=FALSE, fig.cap="Preview of *Getting Used to R, RStudio, and R Markdown*.", purl=FALSE, out.width=if(is_latex_output()) "82%"}
# out.height="40%", out.width="100%"
include_graphics("images/copyright/getting_used_to_r.png")
```   


### What's to come?

We're next heading into the "Data Science with `tidyverse`" portion in @sec-viz as shown in @fig-moderndive-flowchart with what we feel is the most important tool in a data scientist's toolbox: data visualization. We'll continue to explore the data included in the `moderndive` and `nycflights23` packages using the `ggplot2` package for data visualization. Data visualization is a powerful tool to add to your toolbox for data exploration that provides additional insight to what the `View()` and `glimpse()` functions can provide. 


```{r fig-moderndive-flowchart, echo=FALSE, fig.cap="*ModernDive* flowchart – on to Part I!", out.width="70%", purl=FALSE}
