...
b. R is the programming language; RStudio is an integrated development environment for using R
...
**Q2.** Which of the following is a *categorical* variable in the `flights` data frame?

a. `dep_delay` (departure delay in minutes)
b. `distance` (flight distance in miles)
c. `carrier` (two-letter airline code)
d. `air_time` (flight duration in minutes)

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** `carrier` is categorical — its values fall into discrete groups (airline codes). The other three are quantitative — meaningful numerical measurements you can do arithmetic on.
:::

**Q3.** What is the purpose of `library(pkg)`?

a. Install a package from CRAN
b. Build a new R package
c. Load an already-installed package into the current R session
d. List all packages on your system

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** `library(pkg)` (no quotes) loads an already-installed package. To install, you'd use `install.packages("pkg")` (with quotes).
:::
