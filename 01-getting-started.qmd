...
# Set random number generator for replicable pseudo-randomness.
set.seed(76)
```

```{r getting-started-load-internal, message=FALSE, echo=FALSE}
...
We'll introduce these concepts in the upcoming Sections \@ref(r-rstudio)--\@ref(packages). If you are already somewhat familiar with these concepts, feel free to skip to Section \@ref(nycflights) where we'll introduce our first dataset: all domestic flights departing one of the three main New York City (NYC) airports in 2023. We will explore this dataset in depth for much of the rest of this book.
...
```{r R-vs-RStudio-1, echo=FALSE, fig.cap="Analogy of difference between R and RStudio.", out.width="95%"}
...
```{r getting-started-conditional-text, results="asis", echo=FALSE}
...
```{r R-vs-RStudio-2, echo=FALSE, fig.cap="Icons of R versus RStudio on your computer.", out.width="90%"}
...
```{r RStudio-interface, echo=FALSE, fig.cap="RStudio interface to R.", out.width="93%"}
...
A good analogy for R packages \index{R packages} is they are like apps you can download onto a mobile phone like in Figure \@ref(fig:R-vs-R-packages):
...
```{r R-vs-R-packages, echo=FALSE, fig.cap="Analogy of R versus R packages.", out.width="70%"}
...
```{r easy-way-install, echo=FALSE, fig.cap="Installing packages in R the easy way.", out.width="49%"}
...
```{r getting-started-dynamic-text, echo=FALSE}
...
```{r getting-started-conditional-text-dup1, echo=FALSE, results="asis"}
if (is_latex_output()) {
...
If you are new to the world of coding, R, and RStudio and feel you could benefit from a more detailed introduction, we suggest the short book, [*Getting Used to R, RStudio, and R Markdown*](https://rbasics.netlify.app/) [@usedtor2016], previewed in Figure \@ref(fig:ismay-book). It includes screencasts that you can follow along and pause as you learn. This book also contains an introduction to R Markdown, a tool used for reproducible research.

(ref:ismay-book) Preview of *Getting Used to R, RStudio, and R Markdown*.

```{r ismay-book, echo=FALSE, fig.cap="(ref:ismay-book)", out.width=if(is_latex_output()) "82%"}
# out.height="40%", out.width="100%"
knitr::include_graphics("images/copyright/getting_used_to_r.png")
```   
...
```{r moderndive-flowchart, echo=FALSE, fig.cap="(ref:flowchart2)", out.width="70%"}
