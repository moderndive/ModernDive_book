...
```{r lc-inline-setup-01, include=FALSE, purl=FALSE}
...
```{r lc-sol-01-01, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(1, 1))
```
...
```{r lc-sol-01-02, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(1, 2))
```
...
```{r lc-sol-01-03, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(1, 3))
```
...
```{r lc-sol-01-04, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(1, 4))
```
...
```{r lc-sol-01-05, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(1, 5))
```
...
```{r lc-sol-01-06, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(1, 6))
```
...
```{r lc-sol-01-07, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(1, 7))
```
...
a. R is the programming language; RStudio is an interface for using R
b. R is a web browser; RStudio is a desktop application
c. R is a graphical point-and-click interface; RStudio is a command-line tool
...
a. Numerical, because you could perform calculations on the digits
b. Both numerical and categorical
c. Categorical, because the values label rather than measure
d. Numerical, because it contains digits
...
a. Nothing; packages stay loaded forever once installed
b. Re-install the package
c. Run `library(dplyr)` again
...
a. R is scolding you and you must restart RStudio before continuing
b. It's just a friendly diagnostic message with no effect on the output
c. The code failed to run and no output was produced
d. The scatterplot was drawn without the two rows with missing values
...
a. `flights` names an object; `"flights"` is a text string
b. `"flights"` names an object; `flights` is a text string
c. `flights` names a column; `"flights"` names the data frame
d. There is no difference; R treats them identically
...
a. A new data frame called `flights_edited` is created
b. Nothing happens; the display is read-only
c. R errors
d. The data frame is updated with the edits
...
b. A tibble prints fewer rows and shows column types
c. They are identical
d. A tibble can be passed to dplyr verbs; a data frame cannot
