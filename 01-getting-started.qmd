...
a. R is the programming language; RStudio is an integrated development environment (IDE) for using R
...
c. Categorical, because phone numbers identify entities rather than measure a quantity
...
c. Run `library(dplyr)` again — the install persists, but the loading does not
...
d. The code still ran and produced the scatterplot, but with a caveat: two points with missing values were left out
...
a. `flights` (no quotes) refers to the OBJECT named flights; `"flights"` (with quotes) is just an 8-character string of text
b. `"flights"` is the data frame; `flights` is the column
...
a. **One flight** (a specific departure of one plane from one airport on one date)
b. An airline
c. A passenger
d. An airport
