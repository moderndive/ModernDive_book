---
description: &desc "Get oriented with R and RStudio, install and load packages from CRAN, and explore data frames using glimpse() and View()."
open-graph:
  description: *desc
twitter-card:
  description: *desc
...
```{r fig-R-vs-RStudio-1, fig.alt="Side-by-side photographs illustrating the R-is-the-engine, RStudio-is-the-dashboard analogy — a car engine on the left, a car dashboard on the right.", echo=FALSE, fig.cap="Analogy of difference between R and RStudio.", out.width="95%", purl=FALSE}
...
```{r fig-R-vs-RStudio-2, fig.alt="Side-by-side comparison of the two application icons you may see on your computer: the blue R logo (labeled \"Do not open this\") next to the cyan RStudio sphere icon (labeled \"Open this\").", echo=FALSE, fig.cap="Icons of R versus RStudio on your computer.", out.width="90%", purl=FALSE}
...
```{r fig-RStudio-interface, fig.alt="Screenshot of the default RStudio interface showing four panes: the source editor (top-left), the console (bottom-left), the environment/history pane (top-right), and the files/plots/packages/help pane (bottom-right).", echo=FALSE, fig.cap="RStudio interface to R.", out.width="93%", purl=FALSE}
...
```{r fig-R-vs-R-packages, fig.alt="Side-by-side photographs illustrating the R-is-a-new-phone, R-packages-are-the-apps-you-download analogy — a smartphone showing only built-in apps on the left, a smartphone with many downloaded app icons on the right.", echo=FALSE, fig.cap="Analogy of R versus R packages.", out.width="70%", purl=FALSE}
...
```{r fig-easy-way-install, fig.alt="Screenshot of RStudio's Packages pane with the Install button highlighted, and the resulting Install Packages dialog with a text field for entering package names.", echo=FALSE, fig.cap="Installing packages in R the easy way.", out.width="49%", purl=FALSE}
...
We used the `$` operator to extract only the `name` variable and return it as a vector of length 14\index{vectors}. We'll only be occasionally exploring data frames using the `$` operator, instead favoring the `View()` and `glimpse()` functions.
...
d. Type `glimpse()` and R will auto-load the package
...
**Q4.** You run code to create a scatterplot and red text appears in the console, beginning with `Warning: Removed 2 rows containing missing values (geom_point)`. What happened?

a. The code failed to run and no output was produced
b. The code still ran and produced the scatterplot, but with a caveat: two points with missing values were left out
c. It's just a friendly diagnostic message with no effect on the output
d. R is scolding you and you must restart RStudio before continuing

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Red text starting with "Warning:" is a yellow traffic light: your code still works, but with some caveats to pay attention to. Here the scatterplot is still drawn, just without the two rows that had missing values. Red text starting with "Error" is a red light (the code did not run), and any other red text is just a friendly message — a green light.
:::

**Q5.** You want to check whether `2 + 1` equals `3` in R. Which code correctly tests for equality?
...
::: {.callout-tip collapse="true" title="Show answer"}
**(b)** `View()` opens a non-editable display — it's for inspection only. To actually *modify* a data frame, you'd assign a new version back to the name with `<-`, using the data-wrangling tools introduced in @sec-wrangling.
:::
...
```{webr-r}
#| context: setup
# Shadow library() that handles the GitHub-only ModernDive companion packages
# (olympicAthletes/steves/exoplanets/volcanoes) by reading CSV mirrors. Other
# library() calls fall through to base::library().
...
library(moderndive)        # for View(); the shim (sourced above) redirects its
                           # install to the r-universe dev build that exports View()
```
...
```{r fig-ismay-book, fig.alt="Cover preview of the companion guide *Getting Used to R, RStudio, and R Markdown*, recommended for readers needing additional R-language onboarding.", echo=FALSE, fig.cap="Preview of *Getting Used to R, RStudio, and R Markdown*.", purl=FALSE, out.width=if(is_latex_output()) "82%"}
...
```{r fig-moderndive-flowchart, fig.alt="ModernDive book flowchart with Part I (Data Science with tidyverse) highlighted, signaling the transition out of the introductory chapter and into visualization.", echo=FALSE, fig.cap="*ModernDive* flowchart – on to Part I!", out.width="70%", purl=FALSE}
