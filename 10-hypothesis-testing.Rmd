...
The good news is we've already covered many of the necessary concepts to understand hypothesis testing in Chapters \@ref(sampling) and \@ref(confidence-intervals). We will expand further on these ideas here and also provide a general framework for understanding hypothesis tests. By understanding this general framework, you'll be able to adapt it to different scenarios, including all the scenarios of sampling for inference you saw in Table \@ref(tab:sampling-conclusion-table).
...
Much like with our case study on whether yawning is contagious from Section \@ref(case-study-two-prop-ci), the above example involves inference about an unknown difference of population proportions: $p_{m} - p_{f}$, where $p_{m}$ is the population proportion of "male" resumes being recommended for promotion and  $p_{f}$ is the population proportion of "male" resumes being recommended for promotion. Recall this is the third sampling scenario from Table \@ref(table:summarytable-ch8-c).
...
```
```{r boostrap-distribution-two-prop-percentile, echo=FALSE, fig.show='hold', fig.cap="Percentile-based 95 percent confidence interval.", purl=FALSE}
...
To help understand the concepts of Type I error and Type II errors, we use apply these terms in our criminal justice analogy in Table \@ref(tab:trial-errors-table). 
