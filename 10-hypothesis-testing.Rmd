...
Let's start with an activity gender differences in promotions from a 1970's study.

* Openintro
* Link to Jo Hardin's blogpost


### Is a man more likely to get promoted than a woman?

* Recall, that these are not actually 24 men and 24 women. Rather, these are 48 identical resumes, of which 24 were assigned gendered male names and 24 were assigned genered female names.

The `moderndive` package contains the data on the 48 applicants in the `promotions` frame. While in the text below we show you a preview of the first ten rows, we highly recommend you explore all 48 rows in RStudio's spreadsheet viewer as well.
...
The variable `id` acts as an identification variable for all 48 rows, the `decision` variable indicates whether the applicant was selected for promotion or not, while the `gender` variable indicates the (binary) gender of the name used on the resume. 

Let's do an exploratory data analysis in Figure \@ref(fig:promotions-barplot). Recall that we saw in Section \@ref(two-categ-barplot) that we can visualize the relationship between two categorical variables using a dodged barplot. 

```{r promotions-barplot, fig.cap="Barplot of relationship between (binary) gender and promotion decision."}
ggplot(promotions, aes(x = gender, fill = decision)) +
  geom_bar(position = "dodge") +
  labs(x = "gender of resume name")
```

It appears that resumes with male names got promoted at a higher rate. Let's compute the proportions using the `dplyr` package for data wrangling:
...
So of the 24 resumes with male names 21 were selected for promotion, for a proportion of 21/24 = 0.875 = 87.5%. On the other hand, of the 24 resumes with female names 14 were selected for promotion, for a proportion of 14/24 = 0.583 = 58.3%. Comparing these two rates of promotion, it appears that resumes with male names were selected for promotion at a 0.875 - 0.583 = `r observed_test_statistic` = `r observed_test_statistic*100`% higher rate than resumes with female names. A clear edge for the "male" applicants.



<!--
Say you are working at a bank in the 1970's and you are applying for a promotion. 
...
* Imagine a hypothetical universe where there was no gender discrimination
...
```{r promotions-barplot-permuted, fig.cap="Barplot of relationship between shuffled gender and promotion decision."}
...
# diff
diff_prop <- round(observed_test_statistic, 3)
...
So in this hypothetical universe of no discrimination, of the 24 resumes with male names `r n_men_promoted` were selected for promotion, for a proportion of `r n_men_promoted`/24 = `r prop_men_promoted` = `r prop_men_promoted*100`%. On the other hand, of the 24 resumes with female names `r n_women_promoted` were selected for promotion, for a proportion of `r n_women_promoted`/24 = `r prop_women_promoted` = `r prop_women_promoted*100`%. Comparing these two rates of promotion, it appears that resumes with male names were selected for promotion at a `r prop_men_promoted ` - `r prop_women_promoted ` = `r diff_prop` = `r diff_prop*100`% differen rate than resumes with female names. 
...
* We had groups do this in 33 hypothetical universes
* We'll show you how to this later.
...
* Performed a hypothesis test
* What we'll do for

Much like with our case study on whether yawning is contagious from Section \@ref(#case-study-two-prop-ci), we are interested in inferring about the difference in population proportions $p_{m} - p_{f}$. Recall our summary table of sampling scenarios in Table \@ref(table:summarytable-ch8-c).
...
First, a **hypothesis** is a statement about the value of an unknown population parameter. In our resume activity, our population parameter is the difference in population proportions $p_{m} - p_{f}$. Second, a **hypothesis test**: A hypothesis test consists of two competing hypotheses: 

* A **null hypothesis** $H_0$ (pronounced "H-naught", where "naught" means "nothing"), generally a hypothesis reflecting some status quo or theory that "nothing is going on" versus
* An **alternative hypothesis** $H_A$ (also denoted $H_1$), generally a new "challenger" hypothesis to the null hypothesis $H_0$. 

In our resume activity, an appropriate hypothesis test would be:
...
```{r null-distribution-2, fig.cap = "Null distribution", purl=FALSE, echo = FALSE}
ggplot(data = tactile_permutes, aes(x = stat)) +
  geom_histogram(binwidth = 0.1, color = "white") +
  geom_vline(xintercept = observed_test_statistic, color = "red", size = 1) +
...
We'll see later on however, the p-value isn't quite 1/33, but rather (`r num` + 1)/(`r denom` + 1) = `r num + 1`/`r denom + 1` = `r p_val` as we need to compute the proportion of test statistics greater than the one observed while including the observed test statistic. 
...
<!--
Moved observed test stat computation here from Ch 9.


#### 4. Observed statistic / point estimate calculations {-}
...
## Case study: Are action or romance movies rated higher on IMDb {#ht-case-study}
