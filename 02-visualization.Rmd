...
```{r visualization-conditional-text, echo=FALSE, results="asis", purl=FALSE}
if (is_latex_output()) {
...
```{r gapminder, echo=FALSE, fig.cap="Life expectancy over GDP per capita in 2007.", fig.height=ifelse(knitr::is_latex_output(), 2.95, 4), purl=FALSE}
...
```{r noalpha, fig.cap="Arrival delays versus departure delays for Envoy Air flights from NYC in 2023.", fig.height=ifelse(knitr::is_latex_output(), 1.8, 4), warning=FALSE, echo=FALSE, purl=FALSE}
ggplot(data = envoy_flights, mapping = aes(x = dep_delay, y = arr_delay)) +
  geom_point()
```
...
```{r visualization-create-jitter_example, fig.cap="Regular and jittered scatterplot.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 5, 5), purl=FALSE}
...
```{r histogramexample, echo=FALSE, fig.cap="Example histogram.", fig.height=ifelse(knitr::is_latex_output(), 2, 4), purl=FALSE}
...
```{r visualization-hist-white-border, message=FALSE, fig.cap="Histogram of hourly wind speeds at three NYC airports with white borders.", fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
ggplot(data = weather, mapping = aes(x = wind_speed)) +
...
```{r facethistogram, fig.cap="Faceted histogram of hourly wind speeds by month.", echo=FALSE, purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.3, 4)}
...
```{r facethistogram2, fig.cap="Faceted histogram with 4 instead of 3 rows.", echo=FALSE, purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.3, 4)}
...
```{r apr1, echo=FALSE, fig.cap="April wind speeds represented as jittered points.", fig.height=ifelse(knitr::is_latex_output(), 1.3, 4), purl=FALSE}
...
```{r apr2, echo=FALSE, fig.cap="Building up a boxplot of April wind speeds.", out.width="80%", purl=FALSE}
...
```{r visualization-bar, geomcol, fig.cap="Barplot when counts are pre-counted.", fig.height=ifelse(knitr::is_latex_output(), 1.3, 4)}
...
```{r carrierpie, echo=FALSE, fig.cap="The dreaded pie chart.", fig.height=ifelse(knitr::is_latex_output(), 6, 5), purl=FALSE}
if (is_html_output()) {
...
```{r flights-stacked-bar, echo=FALSE, fig.cap="Stacked barplot of flight amount by carrier and origin.", fig.height=ifelse(knitr::is_latex_output(), 2.2, 4), purl=FALSE}
if (is_html_output()) {
  ggplot(data = flights, mapping = aes(x = carrier, fill = origin)) +
    geom_bar()
} else {
...
```{r flights-stacked-bar-color, fig.cap="Stacked barplot with color aesthetic used instead of fill.", fig.height=ifelse(knitr::is_latex_output(), 2.2, 4), echo=FALSE, purl=FALSE}
if (is_html_output()) {
  ggplot(data = flights, mapping = aes(x = carrier, color = origin)) +
    geom_bar()
} else {
...
```{r flights-dodged-bar-color, fig.cap="Side-by-side barplot comparing number of flights by carrier and origin.", fig.height=ifelse(knitr::is_latex_output(), 3.5, 4), echo=FALSE, purl=FALSE}
if (is_html_output()) {
  ggplot(data = flights, mapping = aes(x = carrier, fill = origin)) +
    geom_bar(position = "dodge")
} else {
...
```{r ggplot-cheatsheet, echo=FALSE, fig.cap="Data Visualization with ggplot2 cheatsheet (first page).", purl=FALSE, out.width=if(!knitr::is_latex_output()) "100%"}
if (is_html_output()) {
