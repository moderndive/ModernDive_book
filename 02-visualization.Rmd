...
In February 2006, a Swedish physician and data advocate named Hans Rosling gave a TED talk titled ["The best stats you've ever seen"](https://www.ted.com/talks/hans_rosling_shows_the_best_stats_you_ve_ever_seen) where he presented global economic, health, and development data from the website [gapminder.org](http://www.gapminder.org/tools/#_locale_id=en;&chart-type=bubbles). For example, for data on `r nrow(gapminder_2007)` countries in 2007, let's consider only a few countries in Table \@ref(tab:gapminder-2007) as a peek into the data.
...
Each row in this table corresponds to a country in 2007. For each row, we have `r ncol(gapminder_2007)` columns:
...
for Alaska Airlines flights leaving NYC in 2013. This requires paring down the data from all `r comma(nrow(nycflights13::flights))` flights that left NYC in 2013, to only the `r nrow(nycflights13::flights %>% filter(carrier == "AS"))` *Alaska Airlines* flights that left NYC in 2013. We do this so our scatterplot will involve a manageable `r nrow(nycflights13::flights %>% filter(carrier == "AS"))` points, and not an overwhelmingly large number like `r comma(nrow(nycflights13::flights))`. To achieve this, we'll take the `flights` data frame, filter the rows so that only the `r nrow(nycflights13::flights %>% filter(carrier == "AS"))` rows corresponding to Alaska Airlines flights are kept, and save this in a new data frame called `alaska_flights` using the `<-` *assignment* operator: \index{operators!assignment (<-)} 
...
```{r viz-map, echo=FALSE, fig.cap="Mind map for Data Visualization.", out.width="200%", purl=FALSE}
library(knitr)
if (knitr:::is_html_output()) {
