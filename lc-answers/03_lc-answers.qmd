...
```{r lc-03-mean-sd-temp, echo=FALSE}
...
```{r lc-03-assign-by_monthly_origin}
by_monthly_origin <- flights |>
  group_by(month, origin) |>
  summarize(count = n())
```

```{r lc-03-v7, eval=FALSE}
by_monthly_origin
```

```{r lc-03-v8, echo=FALSE}
...
```{r lc-03-v10, eval=FALSE}
count_flights_by_airport
```

```{r lc-03-v11, echo=FALSE}
