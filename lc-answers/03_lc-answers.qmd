...
**(LC3.5)** Recall from @sec-viz when we looked at wind speeds by months in NYC. What does the standard deviation column in the `summary_monthly_windspeed` data frame tell us about wind speeds in NYC throughout the year?

```{r lc-03-mean-sd-temp, echo=FALSE, purl=FALSE}
summary_windspeed_by_month <- weather |>
  group_by(month) |>
  summarize(
    mean = mean(wind_speed, na.rm = TRUE),
    std_dev = sd(wind_speed, na.rm = TRUE)
  )

knitr::kable(summary_windspeed_by_month, digits = 2) |>
...
**Solution**: The SD shows **within-month variability** of wind speeds. The largest spread for a given month was in February with the second highest in March. This would lead us to expect different wind speeds on different days in those months compared to summer months like June, July, and August that have the smallest standard deviations.
...
**(LC3.13)** Looking at @fig-reldiagram, when joining `flights` and `weather` (or, in other words, matching the hourly weather values with each flight), why do we need to join by all of `year`, `month`, `day`, `hour`, and `origin`, and not just `hour`?
...
1. @fig-reldiagram above showing how the various datasets can be joined will also be useful. 
...
However, because for certain carriers certain flights have missing `NA` values, the resulting table also returns `NA`'s. We can eliminate these by adding a `na.rm = TRUE` argument to `sum()`, telling R that we want to remove the `NA`'s in the sum. We saw this in @sec-summarize:
