...
The population least squares line is defined by the formula $y = \beta_0 + \beta_1 x + \epsilon$.  Here $\epsilon$ corresponds to the error term.  It corresponds to the part of the response variable $y$ that remains unexplained after considering the predictor variable $x$.  Often it is standard practice to assume that this error term follows a normal distribution.  We will focus on checking whether that assumption is valid in Section \@ref(resid).
