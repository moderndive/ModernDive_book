...
**EX2.8 (★★)** Filter the data to `Sport == "Gymnastics"` and rebuild the Height vs Weight scatterplot. How does the cloud differ from the all-sport view?
...
**EX2.10 (★★)** Add `labs()` to your favorite of the above plots — title, axis labels in human language, and a caption naming the data source. Why are good labels especially important when sharing plots?
...
**EX2.12 (★★)** Compute the **number of distinct athletes per Games** in `olympic_athletes` and plot it as a linegraph against `Year`. Filter to a single season (Summer **or** Winter) — combining them would mix two cycles.

```{webr-r}
# your code here
```

**EX2.13 (★★)** For one sport of your choice, plot mean `Height` per `Year` over time. Has the typical height of athletes in that sport grown, shrunk, or stayed flat?
...
**EX2.15 (★★)** Try `bins = 10`, then `bins = 80`. Which gives a more honest view of the age distribution? Why?
...
**EX2.19 (★★)** Identify the **5 sports with the most athletes** (use `count(Sport, sort = TRUE)`), then build boxplots of `Height` for those 5 sports only. Which sport has the highest median height? Which has the largest IQR?
...
**EX2.21 (★★★)** A boxplot of `Weight` for one sport shows many "outlier" dots above the upper whisker. Are these necessarily data errors? Justify in 1–2 sentences.
...
**EX2.23 (★★)** Modify your top-10 plot to *stack* gold/silver/bronze in each bar. Hint: `pivot_longer()` first, then map `fill = medal_type`.

```{webr-r}
# your code here
```

**EX2.24 (★★)** Build a barplot of medal counts for **the 2024 Summer Games only** (one bar per top-10 NOC). What's the top NOC?
...
**EX2.28 (★★)** Build a faceted histogram of `Age`, one panel per `Season`. How do the two distributions differ?
...
**EX2.30 (★★★)** Imagine a news outlet posts a chart of "USA's medal dominance" using a y-axis that starts at 50 (cropping the bar baseline). Explain in 2–3 sentences how the chart misleads viewers, and what a more honest chart would look like.

**EX2.31 (★★★)** Build a chart that visualizes the *rise of female participation* in the Summer Olympics over time. (Hint: count distinct athletes per `Year` × `Sex`, filter to Summer.) State your finding in one sentence above the chart.

```{webr-r}
# your code here
```

**EX2.32 (★★★)** Pick a sport whose athletes' average height has clearly *changed* over time (e.g., basketball, swimming, gymnastics). Build a chart that supports the change. Acknowledge any caveats — e.g., missing height data for older Games.

```{webr-r}
# your code here
```

**EX2.33 (★★★)** The chapter doesn't show NA visualization. Build a barplot showing the *count of `NA`s per column* in `olympic_athletes`. Which columns have the most missingness? Why might that be?
...
**EX2.35 (★★★)** Open exploration: using any of the three datasets, find one pattern that surprises you. Build the **single chart** that communicates it best, and write a one-sentence headline above it.
