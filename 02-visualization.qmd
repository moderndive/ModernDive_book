...
```{r fig-apr1, fig.alt="Jittered strip plot of all April hourly wind speeds along a vertical line, showing the spread from near zero up to roughly 35 mph with most points clustered between 5 and 15 mph.", echo=FALSE, fig.cap="April wind speeds represented as jittered points.", fig.height=ifelse(knitr::is_latex_output(), 1.3, 4), purl=FALSE}
...
```{r fig-flights-stacked-bar, fig.alt="Stacked barplot with one bar per airline carrier; each bar is divided into three colored segments representing the number of flights originating at EWR, JFK, and LGA. Tall bars for UA, B6, EV; shorter ones for boutique carriers.", echo=FALSE, fig.cap="Stacked barplot of flight amount by carrier and origin.", fig.height=ifelse(knitr::is_latex_output(), 2.2, 4), purl=FALSE}
if (is_html_output()) {
  ggplot(data = flights, mapping = aes(x = carrier, fill = origin)) +
    geom_bar()
} else {
...
# Interim single-Games subset for the over-plotting exercises (Paris 2024).
# Defined inline so we can iterate on the subset; once finalized this will move
# into the olympicAthletes package as `olympic_athletes_2024`.
olympic_athletes_2024 <- subset(olympic_athletes, year == 2024 & season == "Summer")
```
