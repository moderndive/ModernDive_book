...
```{r visualization-conditional-text, echo=FALSE, results="asis"}
if (is_latex_output()) {
...
```{r visualization-load-internal, message=FALSE, echo=FALSE}
...
```{r visualization-create-gapminder_2007, echo=FALSE}
...
```{r gapminder, echo=FALSE, fig.cap="Life expectancy over GDP per capita in 2007.", fig.height=ifelse(knitr::is_latex_output(), 2.95, 4)}
...
```{r visualization-conditional-note, echo=FALSE, results="asis"}
if (is_latex_output()) {
...
```{r noalpha, fig.cap="Arrival delays versus departure delays for Envoy Air flights from NYC in 2023.", fig.height=ifelse(knitr::is_latex_output(), 1.8, 4), warning=FALSE, echo=FALSE}
ggplot(data = envoy_flights, mapping = aes(x = dep_delay, y = arr_delay)) +
  geom_point()
```
...
```{r nolayers, fig.cap="A plot with no layers.", fig.height=ifelse(knitr::is_latex_output(), 2, 4)}
ggplot(data = envoy_flights, mapping = aes(x = dep_delay, y = arr_delay))
```
...
```{r alpha, fig.cap="Arrival vs. departure delays scatterplot with alpha = 0.2.", fig.height=ifelse(knitr::is_latex_output(), 3.8, 4)}
ggplot(data = envoy_flights, mapping = aes(x = dep_delay, y = arr_delay)) + 
...
```{r jitter-example-plot-1, fig.cap="Regular and jittered scatterplot.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 5, 5)}
...
```{r jitter, fig.cap="Arrival versus departure delays jittered scatterplot.", fig.height=ifelse(knitr::is_latex_output(), 4.7, 5)}
ggplot(data = envoy_flights, mapping = aes(x = dep_delay, y = arr_delay)) + 
...
Let's create a time series plot (as seen in Figure \@ref(fig:hourlytemp)) of the hourly wind speeds saved in the `early_january_2023_weather` data frame by using `geom_line()` to create a linegraph\index{R packages!ggplot2!geom\_line()}, instead of using `geom_point()` like we used previously to create scatterplots:
...
```{r windspeed-on-line, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 0.8, 4), fig.cap="Plot of hourly wind speed recordings from NYC in 2023."}
...
```{r histogramexample, echo=FALSE, fig.cap="Example histogram.", fig.height=ifelse(knitr::is_latex_output(), 2, 4)}
...
```{r weather-histogram, warning=TRUE, fig.cap="Histogram of hourly wind speeds at three NYC airports.", fig.height=ifelse(knitr::is_latex_output(), 2.3, 4), warning=FALSE, message=FALSE}
ggplot(data = weather, mapping = aes(x = wind_speed)) +
  geom_histogram()
```
...
```{r weather-histogram-2, message=FALSE, fig.cap="Histogram of hourly wind speeds at three NYC airports with white borders.", fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
ggplot(data = weather, mapping = aes(x = wind_speed)) +
...
```{r hist-bins, message=FALSE, fig.cap= "Setting histogram bins in two ways.", echo=FALSE}
...
```{r facethistogram, fig.cap="Faceted histogram of hourly wind speeds by month.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.3, 4)}
...
```{r facethistogram2, fig.cap="Faceted histogram with 4 instead of 3 rows.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.3, 4)}
...
```{r visualization-filter-month, echo=FALSE}
...
```{r apr1, echo=FALSE, fig.cap="April wind speeds represented as jittered points.", fig.height=ifelse(knitr::is_latex_output(), 1.3, 4)}
...
```{r visualization-create-boxplot-components, echo=FALSE, message=FALSE}
...
```{r apr2, echo=FALSE, fig.cap="Building up a boxplot of April wind speeds.", out.width="80%"}
# Display the three plots side-by-side
knitr::knitr::include_graphics("images/patchwork_boxplot.png")
```
...
```{r visualization-dynamic-text, echo=FALSE}
...
```{r badbox, fig.cap="Invalid boxplot specification.", fig.height=ifelse(knitr::is_latex_output(), 1.9, 4)}
...
```{r monthtempbox, fig.cap="Side-by-side boxplot of wind speed split by month.", fig.height=ifelse(knitr::is_latex_output(), 4, 4)}
...
```{r geombar, fig.cap="Barplot when counts are not pre-counted.", fig.height=ifelse(knitr::is_latex_output(), 1.3, 4)}
...
```{r geomcol, fig.cap="Barplot when counts are pre-counted.", fig.height=ifelse(knitr::is_latex_output(), 1.3, 4)}
...
```{r flightsbar, fig.cap="(ref:geombar)", fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
...
```{r carrierpie, echo=FALSE, fig.cap="The dreaded pie chart.", fig.height=ifelse(knitr::is_latex_output(), 6, 5)}
if (is_html_output()) {
...
```{r flights-stacked-bar, echo=FALSE, fig.cap="Stacked barplot of flight amount by carrier and origin.", fig.height=ifelse(knitr::is_latex_output(), 2.2, 4)}
if (is_html_output()) {
  ggplot(data = flights, mapping = aes(x = carrier, fill = origin)) +
    geom_bar()
} else {
...
```{r flights-stacked-bar-color, fig.cap="Stacked barplot with color aesthetic used instead of fill.", fig.height=ifelse(knitr::is_latex_output(), 2.2, 4), echo=FALSE}
if (is_html_output()) {
  ggplot(data = flights, mapping = aes(x = carrier, color = origin)) +
    geom_bar()
} else {
...
```{r flights-dodged-bar-color, fig.cap="Side-by-side barplot comparing number of flights by carrier and origin.", fig.height=ifelse(knitr::is_latex_output(), 3.5, 4), echo=FALSE}
if (is_html_output()) {
  ggplot(data = flights, mapping = aes(x = carrier, fill = origin)) +
    geom_bar(position = "dodge")
} else {
...
Lastly, another type of barplot is a \index{barplot!faceted} *faceted barplot*. Recall in Section \@ref(facets) we visualized the distribution of hourly wind speeds at the 3 NYC airports *split* by month using facets. We apply the same principle to our barplot visualizing the frequency of `carrier` split by `origin`. Instead of mapping `origin` to `fill` we include it as the variable to create small multiples of the plot across the levels of `origin` in Figure \@ref(fig:facet-bar-vert).
...
```{r facet-bar-vert, fig.cap="Faceted barplot comparing the number of flights by carrier and origin.", fig.height=ifelse(knitr::is_latex_output(), 6, 7), echo=FALSE}
...
```{r visualization-conditional-text-dup1, echo=FALSE, results="asis"}
if (is_latex_output()) {
...
```{r visualization-v26, echo=FALSE, results="asis"}
...
```{r ggplot-cheatsheet, echo=FALSE, fig.cap="Data Visualization with ggplot2 cheatsheet (first page).", out.width=if(!knitr::is_latex_output()) "100%"}
if (is_html_output()) {
...
```{r visualization-filter-envoy-alt, echo=FALSE}
# This redundant code is used for dynamic non-static in-line text output purposes
