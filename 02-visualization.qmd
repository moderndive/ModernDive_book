...
a. `geom_point()` doesn't support alpha
b. Alpha must be between 0 and 1
c. Inside `aes()`, the constant `0.2` is *mapped* like a data variable — ggplot builds a legend for it and its alpha scale picks the transparency, which isn't the 0.2 the student asked for
...
a. `geom_bar()` counts rows for you (no `y` needed); `geom_col()` expects pre-computed bar heights mapped to `y`
b. They're identical
...
d. Humans are bad at comparing angles; small differences in angle are hard to perceive, while length differences in a bar chart are obvious
...
c. R's parser treats `+` at start-of-line as continuing an arithmetic expression that may not be in scope; ending lines with `+` signals "I'm not done yet"
d. RStudio adds it automatically
...
c. Plots all data colored by `origin`
