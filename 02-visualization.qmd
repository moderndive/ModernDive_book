...
The cell below runs **R in your browser** — no installation required. Hit the **Run Code** button to draw the scatterplot, then edit the code (try changing `Sepal.Length` to `Petal.Length`, or swapping `geom_point()` for `geom_jitter()`) and run it again to see the result update live.
