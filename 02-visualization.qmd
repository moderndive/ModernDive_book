---
description: &desc "Build the five named ggplot2 graphs (scatterplots, line graphs, histograms, boxplots, and bar plots) using the grammar of graphics."
open-graph:
  description: *desc
twitter-card:
  description: *desc
...
```{r lc-inline-setup-02, include=FALSE, purl=FALSE}
...
for Envoy Air flights leaving NYC in 2023. In other words, `envoy_flights` does not consist of *all* flights that left NYC in 2023, but rather only those flights where `carrier` is `MQ` (which is Envoy Air's carrier code).
...
```{r lc-sol-02-01, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 1))
```
...
```{r lc-sol-02-02, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 2))
```
...
```{r lc-sol-02-03, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 3))
```
...
```{r lc-sol-02-04, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 4))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What are some other features of the plot that stand out to you?

```{r lc-sol-02-05, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 5))
```
...
```{r lc-sol-02-06, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 6))
```
...
```{r fig-jitter-example-plot-1, fig.alt="Two scatterplots side by side, each showing four identical (0, 0) points. The left panel (\"Regular\") shows a single visible point because all four overlap exactly; the right panel (\"Jittered\") shows four distinct points scattered slightly around the origin.", fig.cap="Regular and jittered scatterplot.", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 5, 5), purl=FALSE}
...
```{r lc-sol-02-07, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 7))
```
...
```{r lc-sol-02-08, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 8))
```
...
```{r lc-sol-02-09, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 9))
```
...
```{r lc-sol-02-10, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 10))
```
...
1. The `aes`thetic `mapping` by setting `mapping = aes(x = time_hour, y = wind_speed)`. Specifically, the variable `time_hour` maps to the `x` position aesthetic, while the variable `wind_speed` maps to the `y` position aesthetic.
...
```{r lc-sol-02-11, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 11))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Why are linegraphs frequently used when time is the explanatory variable on the x-axis?

```{r lc-sol-02-12, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 12))
```
...
```{r lc-sol-02-13, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 13))
```
...
```{r fig-histogramexample, fig.alt="Histogram of hourly wind speeds in 5 mph bins, right-skewed with a mode near 5–10 mph and a long tail extending past 30 mph.", echo=FALSE, fig.cap="Example histogram.", fig.height=ifelse(knitr::is_latex_output(), 2, 4), purl=FALSE}
...
```{r fig-weather-histogram-2, fig.alt="Right-skewed histogram of hourly wind speeds in mph from the weather data, default 30 bins with white borders between bars making each bin clearly visible.", message=FALSE, fig.cap="Histogram of hourly wind speeds at three NYC airports with white borders.", fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
ggplot(data = weather, mapping = aes(x = wind_speed)) +
...
If you're curious, run \index{colors()} `colors()` to see all `r colors() |> length()` possible choices of colors in R!


### Adjusting the bins {#sec-adjustbins}

Observe in @fig-weather-histogram-2 that in the 10-20 mph range there appear to be roughly 8 bins. Thus each bin has width 10 divided by 8, or 1.25 mph, which is not a very easily interpretable range to work with. Let's improve this by adjusting the number of bins in our histogram in one of two ways:
...
```{r fig-hist-bins, fig.alt="Two side-by-side histograms of hourly wind speeds. The left panel uses 20 bins; the right panel uses a binwidth of 5 mph. Both show the same right-skewed shape but with different bin granularities.", message=FALSE, fig.cap= "Setting histogram bins in two ways.", echo=FALSE, purl=FALSE}
...
```{r lc-sol-02-14, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 14))
```
...
```{r lc-sol-02-15, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 15))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What would you guess is the "center" value in this distribution? Why did you make that choice?

```{r lc-sol-02-16, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 16))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Is this data spread out greatly from the center or is it close? Why?

```{r lc-sol-02-17, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 17))
```
...
```{r fig-facethistogram, fig.alt="Twelve small-multiple histograms of hourly wind speeds, one per month, arranged in a 3-row by 4-column grid. Each shows a right-skewed distribution with seasonal differences; winter months tend to have slightly higher modal wind speeds than summer.", fig.cap="Faceted histogram of hourly wind speeds by month.", echo=FALSE, purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.3, 4)}
...
```{r fig-facethistogram2, fig.alt="Same 12 monthly wind-speed histograms as the previous figure, but reorganized into a 4-row by 3-column grid via nrow = 4.", fig.cap="Faceted histogram with 4 instead of 3 rows.", echo=FALSE, purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3.3, 4)}
...
```{r lc-sol-02-18, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 18))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What do the numbers 1-12 correspond to in the plot? What about 10, 20, and 30?

```{r lc-sol-02-19, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 19))
```
...
```{r lc-sol-02-20, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 20))
```
...
```{r lc-sol-02-21, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 21))
```
...
```{r fig-apr1, fig.alt="Jittered strip plot of all April hourly wind speeds along a vertical line, showing the spread from near zero up to roughly 30 mph with most points clustered between 5 and 15 mph.", echo=FALSE, fig.cap="April wind speeds represented as jittered points.", fig.height=ifelse(knitr::is_latex_output(), 1.3, 4), purl=FALSE}
...
```{r visualization-create-boxplot-components, echo=FALSE, purl=FALSE, message=FALSE, fig.show='hide'}
...
```{r fig-apr2, fig.alt="Three-panel build-up of a boxplot for April wind speeds: leftmost shows raw jittered points, middle adds horizontal lines at the quartiles, rightmost wraps a box around the IQR with whiskers and the median line, the final standard boxplot.", echo=FALSE, fig.cap="Building up a boxplot of April wind speeds.", out.width="80%", purl=FALSE}
...
```{r lc-sol-02-22, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 22))
```
...
```{r lc-sol-02-23, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 23))
```
...
```{r lc-sol-02-24, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 24))
```
...
```{r lc-sol-02-25, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 25))
```
...
... `fruits_counted` has a variable `number` which represents the "pre-counted" values of each fruit. 
...
```{r fig-geombar, fig.alt="Barplot built from the `fruits` data frame (one row per piece of fruit) using `geom_bar()`. Two bars: \"apple\" at height 3 and \"orange\" at height 2.", fig.cap="Barplot when counts are not pre-counted.", fig.height=ifelse(knitr::is_latex_output(), 1.3, 4)}
ggplot(data = fruits, mapping = aes(x = fruit)) +
  geom_bar()
```

However, using the `fruits_counted` data frame where the fruits have been "pre-counted," we once again map the `fruit` variable to the x-position aesthetic. Here, we also map the `number` variable to the y-position aesthetic, and add a \index{R packages!ggplot2!geom\_col()} `geom_col()` layer instead.

```{r fig-geomcol, fig.alt="Visually identical barplot to the previous figure (apples 3, oranges 2), but built from the pre-counted `fruits_counted` data frame using `geom_col()` with `y = number`.", fig.cap="Barplot when counts are pre-counted.", fig.height=ifelse(knitr::is_latex_output(), 1.3, 4)}
...
```{r lc-sol-02-26, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 26))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What is the difference between histograms and barplots?

```{r lc-sol-02-27, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 27))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** How many Alaska Air flights departed NYC in 2023?

```{r lc-sol-02-28, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 28))
```
...
```{r lc-sol-02-29, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 29))
```
...
```{r lc-sol-02-30, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 30))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Why do you think people continue to use pie charts?

```{r lc-sol-02-31, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 31))
```
...
```{r fig-flights-stacked-bar, fig.alt="Stacked barplot with one bar per airline carrier; each bar is divided into three colored segments representing the number of flights originating at EWR, JFK, and LGA. Tall bars for YX, UA, and B6; shorter ones for boutique carriers.", echo=FALSE, fig.cap="Stacked barplot of number of flights by carrier and origin.", fig.height=ifelse(knitr::is_latex_output(), 2.2, 4), purl=FALSE}
if (is_html_output()) {
  ggplot(data = flights, mapping = aes(x = carrier, fill = origin)) +
    geom_bar()
} else {
...
```{r fig-flights-stacked-bar-color, fig.alt="Same stacked barplot as the previous figure but with `color = origin` instead of `fill = origin`: bars are grey-filled with colored outlines per origin segment, demonstrating the difference between `color` (outline) and `fill` (interior).", fig.cap="Stacked barplot with color aesthetic used instead of fill.", fig.height=ifelse(knitr::is_latex_output(), 2.2, 4), echo=FALSE, purl=FALSE}
if (is_html_output()) {
  ggplot(data = flights, mapping = aes(x = carrier, color = origin)) +
    geom_bar()
} else {
...
```{r fig-flights-dodged-bar-color, fig.alt="Side-by-side (\"dodged\") barplot of flight counts by carrier, with three bars per carrier (one per origin airport, EWR, JFK, LGA) placed next to one another for easy direct comparison.", fig.cap="Side-by-side barplot comparing number of flights by carrier and origin.", fig.height=ifelse(knitr::is_latex_output(), 3.5, 4), echo=FALSE, purl=FALSE}
if (is_html_output()) {
  ggplot(data = flights, mapping = aes(x = carrier, fill = origin)) +
    geom_bar(position = "dodge")
} else {
...
```{r lc-sol-02-32, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 32))
```
...
```{r lc-sol-02-37, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(2, 37))
```
...
a. the data
b. the faceting specification
c. the geometric object
d. the aesthetic mapping
...
a. The `+` operator is missing
b. `geom_point()` must come before `ggplot()`
c. `dep_delay` and `arr_delay` aren't variables in `flights`
d. No `geom_*()` layer was added
...
a. `geom_point()` doesn't support constant alpha values
b. Alpha must be between 0 and 1
c. `0.2` inside `aes()` is mapped like a data variable
d. `aes()` always creates a legend
...
a. `geom_bar()` counts rows; `geom_col()` uses `y` values
b. They're identical
...
b. Pie charts use too much ink
c. R's pie chart function distorts the slice proportions
d. Humans judge angles less accurately than lengths
...
a. Convention only
b. Both placements are valid
c. A trailing `+` tells R the statement isn't finished
d. RStudio moves the `+` to the correct place automatically
...
a. Adding more bins to the plot
b. Using `geom_jitter()` to perturb each point slightly
c. Setting `alpha = 0.1` for transparency
d. Subsetting the data to fewer observations
...
a. Filters the data to one origin
b. Draws one panel per `origin` value
c. Plots all the data colored by `origin`
d. Sorts the data by `origin` level
...
| `position = "dodge"` | Side-by-side (dodged) bars instead of stacked | `... + geom_bar(position = "dodge")` |
| `+ labs(...)` | Add title, axis labels, and caption (see EX 2.11) | `... + labs(title = "...", x = "...", y = "...")` |
...
```{r fig-ggplot-cheatsheet, fig.alt="First page of RStudio's \"Data Visualization with ggplot2\" cheatsheet, listing the main geometric objects (geom_*) and their aesthetic options in compact visual reference form.", echo=FALSE, fig.cap="Data Visualization with ggplot2 cheatsheet (first page).", purl=FALSE, out.width=if(!knitr::is_latex_output()) "100%"}
if (is_html_output()) {
