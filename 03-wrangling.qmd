...
a. A data frame with one row per origin and one column for the average delay
...
c. To exclude missing (`NA`) values from the calculation
...
a. The second uses `=` and triggers an error inside `filter()` — `==` is the *comparison* operator; `=` is for naming function arguments
...
a. Is unchanged — the result was computed but not stored anywhere
...
d. The flight with the most negative `dep_delay` (i.e., it left earliest before its scheduled time)

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** `arrange()` sorts ascending by default. The smallest value (most negative) comes first. Use `arrange(desc(dep_delay))` for largest first.
:::
...
d. EITHER `dep_delay` or `arr_delay` (or both) exceeds 60
...
c. Nothing visible — `group_by()` sets metadata; the effect appears in subsequent verbs like `summarize()` or grouped `mutate()`
...
c. **They produce exactly the same result — `|>` rewrites to nested function calls**
