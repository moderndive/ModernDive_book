...
**Q10.** Which of these two expressions are *equivalent*?

a. `flights |> filter(carrier == "DL") |> summarize(mean_delay = mean(dep_delay, na.rm = TRUE))`
b. `summarize(filter(flights, carrier == "DL"), mean_delay = mean(dep_delay, na.rm = TRUE))`
c. They give different answers
d. (a) errors

::: {.callout-tip collapse="true" title="Show answer"}
**Both (a) and (b) are exactly the same.** The pipe `|>` takes the value on its left and passes it as the *first argument* of the function on its right, so a chain of `|>`s is shorthand for the same nested function call you'd write without the pipe. Prefer the piped form when you have three or more verbs in a row — it reads top-to-bottom like a recipe instead of inside-out.
:::
