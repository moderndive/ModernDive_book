...
```{r fig-filter, fig.cap="Diagram of filter() rows operation.", echo=FALSE, purl=FALSE, out.width="80%"}
...
```{r fig-summary-function, fig.cap="Diagram illustrating summary() function in R.", echo=FALSE, purl=FALSE, out.width="80%"}
...
```{r fig-sum1, fig.cap="Diagram of summarize() rows.", echo=FALSE, purl=FALSE, out.width="80%"}
...
```{r fig-groupsummarize, fig.cap="Diagram of group_by() and summarize().", echo=FALSE, purl=FALSE, out.width="90%"}
...
```{r fig-mutatevars, fig.cap="Diagram of mutate() columns.", echo=FALSE, purl=FALSE, out.width="40%"}
...
```{r fig-gain-hist, fig.cap="Histogram of gain variable.", message=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
...
```{r fig-reldiagram, fig.cap="Data relationships in nycflights from *R for Data Science*.", echo=FALSE, purl=FALSE, out.width="65%"}
...
```{r fig-ijdiagram, fig.cap="Diagram of inner join from *R for Data Science*.", echo=FALSE, purl=FALSE, out.width="80%"}
...
```{r fig-selectfig, fig.cap="Diagram of select() columns.", echo=FALSE, purl=FALSE, out.width="70%"}
...
**Q1.** Which `dplyr` verb would you use to keep only the rows where `carrier == "DL"`?
...
::: {.callout-tip collapse="true" title="Show answer"}
**(c)** `group_by()` + `summarize()` collapses the data to one row per group, with the requested summary statistic per group.
:::
...
::: {.callout-tip collapse="true" title="Show answer"}
**(b)** `na.rm = TRUE` tells the summary function to skip `NA` values. Without it, *any* missing value will cause the entire summary to return `NA`.
:::
...
```{r fig-available-seat-miles, fig.cap="Example of available seat miles for one flight.", echo=FALSE, purl=FALSE, out.width="60%"}
...
```{r fig-dplyr-cheatsheet, fig.cap="Data Transformation with dplyr cheatsheet (first page).", echo=FALSE, purl=FALSE, out.width="100%"}
if (is_html_output()) {
