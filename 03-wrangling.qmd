...
Note that even though colloquially speaking one might say "all flights heading to Burlington, Vermont *and* Seattle, Washington," in terms of computer operations, we really mean "all flights heading to Burlington, Vermont *or* heading to Seattle, Washington." For a given row in the data, `dest` can be `"BTV"`, or `"SEA"`, or something else, but not both `"BTV"` and `"SEA"` at the same time. Furthermore, note the careful use of parentheses around `dest == "BTV" | dest == "SEA"`.
...
Let's calculate two summary statistics of the `wind_speed` variable in the `weather` data frame: the mean and standard deviation (recall from @sec-nycflights that the `weather` data frame is included in the `nycflights23` package). To compute these summary statistics, we need the `mean()` and `sd()` *summary functions* in R. Summary functions in R take in many values and return a single value, as shown in @fig-summary-function. 
...
The code that follows computes the mean and standard deviation of all non-missing values of `wind_speed`:
...
Recall our code from earlier that computed the average wind speed and its spread, keeping the `na.rm = TRUE` set just in case any missing values are stored in the `wind_speed` column:
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Recall from @sec-viz when we looked at wind speeds by months in NYC. What does the standard deviation column in the `summary_monthly_windspeed` data frame tell us about wind speeds in NYC throughout the year?
...
**Q3-5.** After running `flights |> mutate(gain = dep_delay - arr_delay)` *without* `<-`, the `flights` data frame:
