...
```{r fig-summary-function, fig.alt="Schematic showing how summary() collapses a column of values into one summary statistic.", fig.cap="Diagram illustrating summary() function in R.", echo=FALSE, purl=FALSE, out.width="80%"}
...
a. A vector of departure delays
b. The original `flights` data frame
c. A data frame with one row per origin and one column for the average delay
d. An error

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** `group_by()` + `summarize()` collapses the data to one row per group.
:::

**Q3.** Why do many `summarize()` calls include `na.rm = TRUE`?

a. To increase the precision of the result
b. To exclude missing (`NA`) values from the calculation
c. To remove negative values
d. To round to whole numbers

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** `na.rm = TRUE` tells the summary function to skip `NA` values. Without it, *any* missing value will cause the entire summary to return `NA`. (But always think about *why* the values are missing before reaching for it.)
:::

**Q4.** What's the difference between `filter(flights, carrier == "DL")` and `filter(flights, carrier = "DL")`?

a. They're equivalent
b. The second uses `=` and triggers an error inside `filter()` — `==` is the *comparison* operator; `=` is for naming function arguments
c. The first returns Delta flights; the second returns all flights
d. Both work but the second is slower

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** `==` compares; `=` assigns argument values. Mixing them up is one of the most common dplyr errors.
:::

**Q5.** After running `flights |> mutate(gain = arr_delay - dep_delay)` *without* `<-`, the `flights` data frame:

a. Has the new `gain` column added
b. Is unchanged — the result was computed but not stored anywhere
c. Has the new column AND its previous columns are removed
d. Errors

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Verbs like `mutate()` *return* a modified copy. To keep it, you must reassign with `<-`: `flights <- flights |> mutate(...)`. Without that, the modified version is printed and discarded.
:::

**Q6.** `flights |> arrange(dep_delay)` returns rows sorted by departure delay. The first row is most likely:

a. The flight with the largest delay
b. The flight with the most negative `dep_delay` (i.e., it left earliest before its scheduled time)
c. A flight with no delay
d. A random flight

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** `arrange()` sorts ascending by default. The smallest value (most negative) comes first. Use `arrange(desc(dep_delay))` for largest first.
:::

**Q7.** `flights |> inner_join(airlines, by = "carrier")` returns:

a. Only rows from `flights` whose `carrier` value also appears in `airlines`
b. All rows from `flights`, with airline info filled in where matched (`NA` otherwise)
c. All rows from `airlines`, with flight info added
d. The union of both data frames

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** **Inner** join keeps only rows matched in BOTH tables. (b) describes a `left_join`; (c) is a `right_join`. With `nycflights23`, every carrier in `flights` is in `airlines`, so the visible row count is the same — but the conceptual difference matters when keys don't all match.
:::

**Q8.** `flights |> filter(dep_delay > 60 | arr_delay > 60)` returns rows where:

a. BOTH `dep_delay` and `arr_delay` exceed 60
b. EITHER `dep_delay` or `arr_delay` (or both) exceeds 60
c. Neither delay exceeds 60
d. An error

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** `|` is the *OR* operator in R. `&` is *AND*. Confusing the two is a common student error. (Note: `||` and `&&` exist too — they're for single TRUE/FALSE values, not vectors.)
:::

**Q9.** After `flights |> group_by(origin)`, what visible change happens to the data frame?

a. Rows are reordered by origin
b. Nothing visible — `group_by()` sets metadata; the effect appears in subsequent verbs like `summarize()` or grouped `mutate()`
c. A new column appears
d. Rows with `NA` are dropped

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** `group_by()` is a labeling step. The data frame looks the same; downstream verbs see the grouping and act per-group.
:::

**Q10.** `flights |> mutate(speed = distance / air_time)` is run, but `air_time` has some `0` values for cancelled flights. What happens to those rows?

a. R errors and stops
b. The `speed` column gets `Inf` (R's representation of division by zero) for those rows
c. The whole result becomes `NA`
d. R automatically removes those rows

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Division by zero in R returns `Inf` (not `NA` or an error). Subsequent calls like `mean(speed, na.rm = TRUE)` would still be derailed by `Inf`. Always inspect new columns for sentinel values like `Inf`, `-Inf`, and `NaN`.
:::
...
```r
remotes::install_github("ismayc/steves")
```
...
**EX3.3 (★)** `original_air_date` has type `Date`. Why does that matter for arithmetic (e.g., computing days between episodes) compared to storing the same as `character`?

### `filter()` {.unnumbered}

**EX3.4 (★)** Filter `olympic_athletes` to athletes from `Sport == "Swimming"` who won a `Gold` medal. How many rows result?
...
**EX3.5 (★)** Filter `episodes` to those where `primary_country == "Italy"`. How many Italy episodes does the show have?

```{webr-r}
# your code here
```

**EX3.6 (★)** Filter `episodes` to **highly-rated AND well-watched** episodes: `imdb_rating >= 8` AND `imdb_votes >= 50`.

```{webr-r}
# your code here
```

**EX3.7 (★)** Filter `episodes` to the union: episodes about `"Italy"` OR `"France"`. Use `%in%`.

```{webr-r}
# your code here
```

**EX3.8 (★★)** Filter `olympic_athletes` to the most recent Summer Games (`Year == 2024 & Season == "Summer"`). How many distinct athletes participated?
...
**EX3.9 (★)** From `olympic_athletes`, select only the identifier columns (`ID`, `Name`) plus the medal-related columns (`Medal`, `Event`, `Sport`, `Year`).

```{webr-r}
# your code here
```

**EX3.10 (★)** Use a `tidyselect` helper (`starts_with`, `contains`, `ends_with`) to keep only the IMDB-related columns of `episodes`.

```{webr-r}
# your code here
```

**EX3.11 (★)** Drop the `image_url` and `tvmaze_url` columns from `episodes` using `select(-...)`.
...
**EX3.12 (★★)** Add a `height_in` column to `olympic_athletes`: convert `Height` (cm) to inches (1 inch ≈ 2.54 cm).

```{webr-r}
# your code here
```

**EX3.13 (★★)** Add an `air_year` column to `episodes` extracted from `original_air_date` (already exists for verification — recompute it yourself first, then check). Hint: `lubridate::year()` or `format(date, "%Y")`.

```{webr-r}
# your code here
```

**EX3.14 (★★)** Add a logical column `won_medal` to `olympic_athletes` that is `TRUE` for any non-missing `Medal`. What proportion of athlete-rows won a medal?

```{webr-r}
# your code here
```

**EX3.15 (★★)** Add an ordinal `age_bucket` column to `olympic_athletes` using `cut()` with breakpoints at 18, 22, 28, 35. Assign each row to a bucket.
...
**EX3.16 (★★)** Show the **10 oldest athletes** (by `Age`, descending) in `olympic_athletes`. Use `arrange(desc(Age))`.

```{webr-r}
# your code here
```

**EX3.17 (★★)** Show the **5 highest-rated** episodes of *Rick Steves' Europe*. Print only `season`, `episode_in_season`, `title`, `imdb_rating`.

```{webr-r}
# your code here
```

**EX3.18 (★★)** Sort `episodes` first by `season` ascending, then `imdb_rating` descending. Inspect rows 1–10.

```{webr-r}
# your code here
```

### `group_by()` + `summarize()` {.unnumbered}

**EX3.19 (★★)** Compute the mean `Height` per `Sport` (drop `NA`s with `na.rm = TRUE`). Show the 10 sports with the *tallest* mean.

```{webr-r}
# your code here
```

**EX3.20 (★★)** Using `medal_table`, compute the **total medals per NOC** across all five recent Games. Show the top 10.
...
**EX3.21 (★★)** From `episodes`, compute the median `imdb_rating` per `region`. Which region rates highest?

```{webr-r}
# your code here
```

**EX3.22 (★★)** Count the number of episodes per `season` of *Rick Steves' Europe*. Plot it as a barplot.

```{webr-r}
# your code here
```

**EX3.23 (★★)** From `olympic_athletes`, count distinct *athletes* (not athlete-rows) per `NOC`. Show the top 10.

```{webr-r}
# your code here
```

**EX3.24 (★★★)** Compute mean `Age` per `Sport`, but **only for sports with at least 1,000 athlete-rows**. Why is the second filter important?

```{webr-r}
# your code here
```

**EX3.25 (★★)** For each `Games`, count the number of distinct `Event`s. Has the number of events grown over time? Plot it.
...
**EX3.26 (★★★)** `editions` (5 rows) has metadata for the 2018–2026 Games. Use `inner_join()` to attach `editions` to `olympic_athletes`. What is the resulting row count? Why didn't it equal the original 314,749?

```{webr-r}
# your code here
```

**EX3.27 (★★★)** Use `left_join()` instead. What's the row count, and why is it different from the inner-join result?

```{webr-r}
# your code here
```

**EX3.28 (★★★)** Use `anti_join()` between `olympic_athletes` and `medal_table` (joining on `Year`/`year` and `NOC`/`noc`) to find athletes whose NOC did **not** appear in `medal_table` for that Games. What does this tell you about coverage?

```{webr-r}
# your code here
```

**EX3.29 (★★★)** Use `semi_join()` to keep only the athletes from countries that appear in `medal_table` (i.e., medal-winning countries from 2018–2026). How many rows remain?
...
**EX3.30 (★★)** Run `mean(olympic_athletes$Height)` (without `na.rm`). What do you get, and why?

```{webr-r}
# your code here
```

**EX3.31 (★★★)** Compare two coding styles for the same task: (a) save each intermediate result to a variable, (b) chain everything with `|>`. Pick a 3-step `dplyr` chain and write it both ways. Which is easier to *modify*? Which is easier to *debug*?

```{webr-r}
# your code here (write both versions in comments + code)
```

**EX3.32 (★★★)** Pick *two countries* (e.g., `"USA"` and `"GBR"`) and produce a single linegraph of their total medals across the five recent Games. What story does the chart tell?

```{webr-r}
# your code here
```

**EX3.33 (★★★)** From `episodes`, compute *episodes per primary_country* and identify the most-featured country. Then compute the same with `geo_match == "full"` (only fully geo-resolved episodes) — does the leader change?

```{webr-r}
# your code here
```

**EX3.34 (★★★)** Construct a "country diversity" metric per Games: for each `Year`/`Season`, count the number of distinct `NOC`s in `olympic_athletes`. Plot the trend over time. Has the Olympics become more globally inclusive?

```{webr-r}
# your code here
```

**EX3.35 (★★★)** Open exploration: combining at least *two* `dplyr` verbs and one of the two datasets, find one finding that surprises you. State your finding in one sentence above the supporting code.
