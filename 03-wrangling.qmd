...
::: {.callout-tip collapse="true" title="Show answer"}
**(b)** `|` is the *OR* operator in R. `&` is *AND*. Confusing the two is a common student error.
:::
...
a. **They produce exactly the same result — `|>` rewrites to nested function calls**
b. They give different answers because piping changes what each function "sees"
c. (i) errors because `|>` can't be used with multiple verbs
d. (ii) errors because `summarize()` needs the grouped data first

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** The pipe `|>` takes the value on its left and passes it as the *first argument* of the function on its right, so a chain of `|>`s is just shorthand for the same nested call. Prefer the piped form when you have three or more verbs in a row — it reads top-to-bottom like a recipe instead of inside-out.
:::
...
library(ggplot2)
library(readr)
# olympicAthletes and steves are GitHub-only; read package datasets from v2 raw mirror.
