...
```{r setup_wrangling, include=FALSE}
...
```{r wrangling-load-internal, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r wrangling-nested-functions, eval=FALSE}
h(g(f(x)))
```
...
```{r wrangling-pipe-example, eval=FALSE}
x |> 
  f() |> 
  g() |> 
  h()
...
```{r filter, fig.cap="Diagram of filter() rows operation.", echo=FALSE, out.width="80%"}
...
```{r summary-function, fig.cap="Diagram illustrating summary() function in R.", echo=FALSE, out.width="80%"}
...
```{r sum1, fig.cap="Diagram of summarize() rows.", echo=FALSE, out.width="80%"}
...
```{r groupsummarize, fig.cap="(ref:groupby)", echo=FALSE, out.width="90%"}
...
```{r wrangling-create-cut_levels, echo=FALSE}
...
```{r mutatevars, fig.cap="Diagram of mutate() columns.", echo=FALSE, out.width="40%"}
...
```{r five-flights-save, include=FALSE}
# Load common setup (image functions, knitr, etc.)
source("R/common_setup.R")
first_five <- flights |> 
...
```{r reldiagram, fig.cap="(ref:relationships-nycflights)", echo=FALSE, out.width="65%"}
...
```{r ijdiagram, fig.cap="(ref:inner-join-r4ds)", echo=FALSE, out.width="80%"}
...
```{r selectfig, fig.cap="Diagram of select() columns.", echo=FALSE, out.width="70%"}
knitr::include_graphics("images/cheatsheets/select.png")
```

```{r wrangling-dynamic-text, echo=FALSE}
...
```{r wrangle-summary-table, message=FALSE, echo=FALSE}
...
```{r available-seat-miles, fig.cap="Example of available seat miles for one flight.", echo=FALSE, out.width="60%"}
...
```{r wrangling-conditional-text-dup1, results="asis", echo=FALSE}
if(is_latex_output()){
...
```{r wrangling-v55, results="asis", echo=FALSE}
...
```{r dplyr-cheatsheet, fig.cap="Data Transformation with dplyr cheatsheet (first page).", echo=FALSE, out.width="100%"}
if (is_html_output()) {
  knitr::include_graphics("images/cheatsheets/dplyr_cheatsheet-1.png")
}
```
