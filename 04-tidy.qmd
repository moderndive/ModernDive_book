...
```{r fig-read-excel, echo=FALSE, fig.cap="Importing an Excel file to R.", purl=FALSE, out.width="100%"}
...
```{r fig-drinks-smaller, fig.cap="Comparing alcohol consumption in 4 countries.", fig.height=ifelse(knitr::is_latex_output(), 3.9, 4), echo=FALSE, purl=FALSE}
...
```{r fig-tidyfig, echo=FALSE, fig.cap="Tidy data graphic from *R for Data Science*.", out.width="80%", purl=FALSE}
...
```{r fig-drinks-smaller-tidy-barplot, echo=FALSE, fig.cap="Comparing alcohol consumption in `r n_countries` countries using geom_col().", fig.height=ifelse(knitr::is_latex_output(), 2.5, 4), purl=FALSE}
if (is_html_output()) {
  drinks_smaller_tidy_plot
} else {
  drinks_smaller_tidy_plot + scale_fill_grey()
...
```{r fig-guat-dem-tidy, fig.cap="Democracy scores in Guatemala 1952-1992.", fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
...
**(b)** Tidy data is about *structure*, not contents. Wickham's three rules are: variables in columns, observations in rows, observational units in their own tables.
:::
...
```{r fig-import-cheatsheet, echo=FALSE, fig.cap="Data Import cheatsheet (first page): readr package.", out.width="66%", purl=FALSE}
if (is_html_output()) {
  include_graphics("images/cheatsheets/data-import-1.png")
}
```

```{r fig-tidyr-cheatsheet, echo=FALSE, fig.cap="Data Tidying cheatsheet (first page): tidyr package.", out.width="66%", purl=FALSE}
if (is_html_output()) {
...
```{r fig-part2, echo=FALSE, fig.cap="*ModernDive* flowchart – on to Part II!", out.width="50%", purl=FALSE}
