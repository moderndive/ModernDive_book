...
```{r tidy-load-internal, message=FALSE, echo=FALSE}
...
dem_score
```
```{r tidy-create-dem_score, message=FALSE, echo=FALSE}
...
```{r read-excel, echo=FALSE, fig.cap="Importing an Excel file to R.", out.width="100%"}
...
```{r drinks-smaller, fig.cap="Comparing alcohol consumption in 4 countries.", fig.height=ifelse(knitr::is_latex_output(), 3.9, 4), echo=FALSE}
...
```{r tidy-show-drinks-smaller-tidy}
drinks_smaller_tidy
```

```{r tidy-create-n_row_drinks, echo=FALSE}
...
```{r tidyfig, echo=FALSE, fig.cap="(ref:tidy-r4ds)", out.width="80%"}
...
```{r drinks-smaller-tidy-barplot, echo=FALSE, fig.cap="(ref:drinks-col)", fig.height=ifelse(knitr::is_latex_output(), 2.5, 4)}
if (is_html_output()) {
  drinks_smaller_tidy_plot
} else {
  drinks_smaller_tidy_plot + scale_fill_grey()
...
```{r tidy-demo-code-v2, eval=FALSE}
airline_safety
```
...
```{r tidy-example-load-packages-2, eval=FALSE}
library(ggplot2)
library(dplyr)
library(readr)
library(tidyr)
...
```{r tidy-load-tidyverse, eval=FALSE}
library(tidyverse)
```

would be the same as running:

```{r tidy-example-load-packages-3, eval=FALSE}
library(ggplot2)
library(dplyr)
library(readr)
library(tidyr)
...
```{r tidy-conditional-text, echo=FALSE, results="asis"}
if (is_latex_output()) {
...
```{r tidy-v22, echo=FALSE, results="asis"}
...
```{r tidyr-cheatsheet, echo=FALSE, fig.cap="Data Tidying cheatsheet (first page): tidyr package.", out.width="66%"}
if (is_html_output()) {
  knitr::include_graphics("images/cheatsheets/data-tidying-1.png")
}
```
...
```{r part2, echo=FALSE, fig.cap="(ref:flowchart-partii)", out.width="50%"}
