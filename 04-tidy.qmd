...
```{r lc-inline-setup-04, include=FALSE, purl=FALSE}
...
```{r lc-sol-04-01, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(4, 1))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What makes "tidy" data frames useful for organizing data?

```{r lc-sol-04-02, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(4, 2))
```
...
```{r lc-sol-04-03, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(4, 3))
```
...
```{r lc-sol-04-04, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(4, 4))
```
...
```{r lc-sol-04-05, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(4, 5))
```
...
b. Variables in columns, observations in rows
c. The column names are lowercase, with no spaces
d. The data is sorted alphabetically
...
a. Reshapes data from wide to long format
b. Joins two data frames
c. Sorts a data frame by the longest column
d. Adds new columns to a data frame
...
a. When the file has fewer than 1000 rows
b. They are exactly equivalent
c. When you want a `tibble` and speed
d. Only when the file is read from a URL

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** As the chapter notes, `readr::read_csv()` can more easily read data off the web, generally imports data at a much faster speed, and saves data frames as `tibble`s by default, unlike base R's `read.csv()`.
:::

**Q4-4.** A data frame has columns `country`, `1990`, `1995`, `2000`, `2005`, population values per country per year. Why is this **NOT** tidy?

a. It's missing a primary key
b. `year` has no column of its own
c. Column names cannot start with numbers
d. The country values are repeated
...
a. The names of the columns to leave unchanged
b. The package to load
c. The new column that holds the old names
d. The destination file name

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** `names_to` names the *new* column that will receive the values currently sitting in the *names* of the columns being pivoted. (`values_to` names the new column that holds the cell values.) The naming is symmetric: `names_to` ↔ from old column NAMES; `values_to` ↔ from old column VALUES.
:::
...
c. Fewer rows and more columns, like `pivot_wider()`
d. More rows and fewer columns
...
b. The `readr` package isn't loaded
c. RStudio is broken
...
a. 0 variables and 3 observations
b. Depends on the data
c. 1 variable and 3 observations
d. 3 variables and 1 observation
...
a. No; wide is often easier for humans to read
b. Yes; only tidy is acceptable
c. Yes, wide formats cannot be analyzed correctly
...
a. The R Markdown packages `knitr` and `bookdown`
b. Only base R functions
c. Just `ggplot2` and its extensions
d. The eight core tidyverse packages
...
# Long form of bob_ross built in EX 4.10 (and used downstream) — pivots the 67
# indicator columns into (element, present) rows so nrow(bob_long) == 403 * 67 == 27001.
bob_long <- bob_ross |>
