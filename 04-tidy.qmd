...
c. The column names are in lowercase
...
a. Reshapes data from wide format to long/tidy format
...
c. **When you want a `tibble` and faster, easier importing**
d. Only when the file is online
...
a. The names of variables to drop
b. The package to load
c. The new column that will hold the OLD column names
...
a. No — wide formats are often easier for humans to read (think of a spreadsheet with one row per country and a column per year); tidy is required for `dplyr`/`ggplot2` workflows
...
**Q4-10.** `library(tidyverse)` loads which packages (among others)?

a. `bookdown`
b. Only base R
c. Just `ggplot2`
d. `ggplot2`, `dplyr`, `tidyr`, `readr`, `purrr`, `tibble`, `stringr`, `forcats`

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** `tidyverse` is an "umbrella" package that loads the eight core tidyverse packages in one call. It's a convenience, not a separate code library — the actual functions still live in the individual packages.
:::
