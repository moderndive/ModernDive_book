...
b. **When you want a `tibble` and faster, easier importing**
c. Only when the file is online
d. They are exactly equivalent

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** As the chapter notes, `readr::read_csv()` can more easily read data off the web, generally imports data at a much faster speed, and saves data frames as `tibble`s by default — unlike base R's `read.csv()`.
:::
