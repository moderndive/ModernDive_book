...
**EX4.7 (★)** Two variables are encoded in the *names* of the 67 indicator columns. What are they? *(Hint: think about what each column name represents and what the cell values represent.)*
...
**EX4.21 (★★★)** For each season, find the **single most-painted element** (with ties broken arbitrarily). Are there elements that dominate one season but not another?
...
### Case study: democracy in Guatemala {.unnumbered}

**EX4.27 (★★)** The case study in Section 4.3 tidies the `dem_score` data frame (from `moderndive`) — one column per year of democracy score. (a) Without running code, identify which *variable* is hiding inside the *column names*. (b) Write the skeleton `pivot_longer()` call that would tidy it: what would you put for `cols`, `names_to`, and `values_to`?

```{webr-r}
library(moderndive)
glimpse(dem_score)

# Sketch your pivot_longer() call here
```

### `tidyverse` package {.unnumbered}
...
**EX4.32 (★★★)** Open exploration: find one pattern the chapter examples did *not* uncover and create a chart that communicates it clearly. State your finding in a single sentence above the chart.
