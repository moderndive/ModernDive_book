...
b. No — wide formats are often easier for humans to read and are required by some statistical procedures (e.g., paired t-tests in some packages); tidy is required for `dplyr`/`ggplot2` workflows
...
### Setup and first look {.unnumbered}
...
### Recognizing tidy structure {.unnumbered}

**EX4.5 (★)** Is `bob_ross` in tidy format? Justify by referring to the three rules of tidy data.

**EX4.6 (★)** Two variables are encoded in the *names* of the 67 indicator columns. What are they? *(Hint: think about what each column name represents and what the cell values represent.)*
...
### Summarizing the long-form data {.unnumbered}
...
**EX4.19 (★★★)** For each season, find the **single most-painted element** (with ties broken arbitrarily). Are there elements that dominate one season but not another?

```{webr-r}
# your code here
```

### Visualizing the long-form data {.unnumbered}

**EX4.20 (★★)** Build a horizontal bar chart of the top 15 elements by total appearance count across the full series. Use `ggplot2`.
...
### Pivoting wider, round trips, and importing data {.unnumbered}
...
```{webr-r}
# your code here (no need to actually run it)
```
...
```{webr-r}
# Compute its frequency to support your interpretation
```
...
**EX4.30 (★★★)** Open exploration: find one pattern the chapter examples did *not* uncover and create a chart that communicates it clearly. State your finding in a single sentence above the chart.
