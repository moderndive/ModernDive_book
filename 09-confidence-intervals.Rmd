...
## Constructing confidence intervals {#ci-build-up}

Let's start this section with an analogy involving fishing. Say you are trying to catch a fish. On the one hand you could use a spear, while on the other you could use a net. Using the net will probably yield better results!

Bringing things back to the pennies: you are trying to estimate the true population mean year $\mu$ of all US pennies. Think of the value of $\mu$ as the fish.
...
```{r percentile-method, echo=FALSE, message=FALSE, fig.cap="Percentile method 95% confience interval."}
ggplot(virtual_resampled_means, aes(x = mean_year)) +
...
ggplot(virtual_resampled_means, aes(x = mean_year)) +
  ...
  labs(x = "sample mean") +
  scale_x_continuous(breaks = seq(1988, 2006, 2)) +
  geom_vline(xintercept = standard_error_ci[[1, 1]], color = "red", size = 1) +
  geom_vline(xintercept = standard_error_ci[[1, 2]], color = "red", size = 1)
```
