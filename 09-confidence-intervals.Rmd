...
### What is the average age of all US pennies?

What do we do when we have access to one sample?

### What is resampling?

### Resampling pennies once

### Resampling pennies 33 times
...
Simply put: generate an estimate that accounts for sampling variation using a single sample
...
### Constructing a CI using percentile method {#percentile-method}
...
Using the percentile method, our range of plausible values for the mean age of US pennies in circulation in 2011 is `percentile_ci[["2.5%"]]` years to `percentile_ci[["97.5%"]]` years. We can use the `visualize()` function to view this using the `endpoints` and `direction` arguments, setting `direction` to `"between"` (between the values) and `endpoints` to be those stored with name `percentile_ci`.
...
### Constructing a CI using SE method
...
We see that both methods produce nearly identical confidence intervals with the percentile method being $[`round(percentile_ci[["2.5%"]], 2)`, `round(percentile_ci[["97.5%"]], 2)`]$ and the standard error method being $[`round(standard_error_ci[["lower"]], 2)`, `round(standard_error_ci[["upper"]],2)`]$.
...
### The bootstrap method {#bootstrap-process}
...
As shown above in Subsection \@ref(percentile-method), one range of plausible values for the population mean age of pennies in 2011, denoted by $\mu$, is $[`round(percentile_ci[["2.5%"]], 2)`, `round(percentile_ci[["97.5%"]], 2)`]$. Recall that this confidence interval is based on bootstrapping using `pennies_sample`. Note that the mean of `pennies` (`pennies_mu`) does fall in this confidence interval. If we had a different sample of size 40 and constructed a confidence interval using the same method, would we be guaranteed that it contained the population parameter value as well? Let's try it out:
...
This new confidence interval also contains the value of $\mu$. Let's further investigate by repeating this process 100 times to get 100 different confidence intervals derived from 100 different samples of `pennies`. Each sample will have size of 40 just as the original sample. We will plot each of these confidence intervals as horizontal lines. We will also show a line corresponding to the known population value of `pennies_mu` years.
...
**Interpretation:** We are 95% confident that the true mean age of pennies in circulation in 2011 is between `percentile_ci[["2.5%"]]` and `percentile_ci[["97.5%"]]` years. This level of confidence is based on the percentile-based method including the true mean 95% of the time if many different samples (not just the one we used) were collected and confidence intervals were created.
...
### Table of inference scenarios

Let's update the inference table
