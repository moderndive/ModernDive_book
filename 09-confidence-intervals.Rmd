...
Now that we've shown you how to construct confidence intervals using a sample drawn from the population, let's now focus on how to interpret them. 

In order to interpret a confidence interval thoroughly however, we need to know the true value of the population parameter in question. 
...
if(!file.exists("rds/balls_percentile_cis.rds")){
  # Set random number generator seed value to number 9 in honour of Bobby Orr.
  set.seed(4)
...
We now present some of the determinants of confidence interval widths.

#### Impact of confidence level {-}

When comparing the widths of the 95% confidence intervals in Figure \@ref(fig:reliable-percentile) with the widths of the 80% confidence intervals in Figure \@ref(fig:reliable-se), recall that the 95% confidence intervals were wider. The quantification of the confidence level should match what you expect of the word "confident." In order to be more confidence of our best guess of a range of values, we need to widen the range of values.

To elaborate on this, imagine we want to guess the forecasted high temperature in Seoul, South Korea on August 15th. Given Seoul's temperate climate with 4 distinct seasons, we could say somewhat confidently that the high temperature wouldn't be below 50&deg;F (10&deg;C) and that it wouldn't be above 95&deg;F (35&deg;C). 

However, if we wanted a temperature range we were *absolutely* confident about, would we need to widen or narrow this range? We'd need to widen it! We need this wider range since it is possible to have a freak cold spell or heat wave. So a range of temperatures we could be near certain about would be 32&deg;F (0&deg;C) to 110&deg;F (43&deg;C). On the other hand, if we wanted a range we could tolerate being a little less confident about, we could narrow this range to something along the lines of 70&deg;F (21&deg;C) to 85&deg;F (30&deg;C). 



As it relates to sampling for inference: **Higher confidence levels tend to produce wider confidence intervals.**
...
<!-- 
Chester says: Should we load these into the moderndive package too so that readers can explore them a bit?
Albert says: Don't have time right now, maybe next edition?
-->
...
if(!file.exists("rds/balls_perc_cis_80.rds")){
  balls_perc_cis_80 <- nested_balls2 %>% 
    mutate(percentile_ci = map(data, infer_pipeline, ci_level = 0.8)) %>% 
    mutate(point_estimate = map_dbl(data, ~mean(.x$color == "red")))
...
if(!file.exists("rds/balls_perc_cis_95.rds")){
  balls_perc_cis_95 <- nested_balls2 %>% 
    mutate(percentile_ci = map(data, infer_pipeline, ci_level = 0.95)) %>% 
    mutate(point_estimate = map_dbl(data, ~mean(.x$color == "red")))
...
if(!file.exists("rds/balls_perc_cis_99.rds")){
  balls_perc_cis_99 <- nested_balls2 %>% 
    mutate(percentile_ci = map(data, infer_pipeline, ci_level = 0.99)) %>% 
    mutate(point_estimate = map_dbl(data, ~mean(.x$color == "red")))
...
if(!file.exists("rds/balls_perc_cis_n_25.rds")){
  balls_perc_cis_n_25 <- nested_balls_25 %>% 
    mutate(percentile_ci = map(data, infer_pipeline)) %>% 
    mutate(point_estimate = map_dbl(data, ~mean(.x$color == "red")))
