...
So in this case, based on two separate samples of size $n_{seed}$ = 34 and $n_{control}$ = 16, our estimate of $p_{seed} - p_{control}$, the difference in population proportions yawning, is $\widehat{p}_{seed} - \widehat{p}_{control}$ = 4.4%. This is known as a situtation of *two-sample* inference since we have two separate samples, in this case those who were exposed to yawning and those who were not. 
...
As we did in Section \@ref(infer-workflow), let's spell out the steps of the `infer` workflow to construct the 95% confidence interval for $p_{seed} - p_{control}$. However, since this is a new scenario for inference (difference in proportions), we'll need identify some new arguments to include in the `infer` functions:

* We need to define a `success`, or event of interest to count. In the case of the sampling bowl from Chapter \@ref(sampling), it was observing a red ball. In our case, it will be a participant having a `yawn` value of `"yes"`.
...
We see another error here. `infer` is telling us that it needs to know is the difference in proportions $\widehat{p}_{seed} - \widehat{p}_{control}$ or $\widehat{p}_{control} - \widehat{p}_{seed}$. This isn't clear. So we set the `order` argument as `c("seed", "control")` to indicate we are interested in $\widehat{p}_{seed} - \widehat{p}_{control}$. Let's save this in a data frame `bootstrap_distribution_yawning`:
...
Figure \@ref(fig:boostrap-distribution-mythbusters)
...
```
```{r include=FALSE}
myth_ci <- bootstrap_distribution_yawning %>% 
  get_confidence_interval(type = "percentile", level = 0.95)
```
...
Our point estimate is thus calculated:
