...
The value of the resampled mean is `r round(resample_mean[[1, 1]], 2)`. We might have guessed that our mean here is similar to what we saw for the mean of `pennies_sample` of `r x_bar %>% pull(mean_year) %>% round(2)` based on the distributions in the histogram in Figure \@ref(fig:orig-and-resample).
...
The mean of the 1000 means from 1000 resamples is `r mean_of_means`. Note that this is quite close to the mean of our original sample: `r x_bar %>% pull(mean_year) %>% round(2)`. This will always be the case when we have 1000 or so replicates since each of the resamples is based on the original sample.
