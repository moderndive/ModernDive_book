...
\ref{sec:about-book}. While a PDF version of this book can be found
\href{https://github.com/moderndive/moderndive_book/raw/master/docs/ismaykim.pdf}{here},
this is very much a work in progress with many things that still need to
be fixed. We appreciate your patience.

\begin{center}\rule{0.5\linewidth}{\linethickness}\end{center}

\hypertarget{sec:intro-for-students}{\subsection{Introduction for
...
\subsubsection{What you will learn from this
...
\subsubsection{Data/science pipeline}\label{subsec:pipeline}
...
\subsubsection{Reproducible research}\label{subsec:reproducible}
...
\subsubsection{Final note for students}\label{final-note-for-students}
...
\hypertarget{sec:intro-instructors}{\subsection{Introduction for
...
\tightlist
\item
  ``Mathematical Statistics with Resampling and R'' (Chihara and
  Hesterberg \protect\hyperlink{ref-hester2011}{2011}),
\item
  ``OpenIntro: Intro Stat with Randomization and Simulation'' (Diez,
  Barr, and Çetinkaya-Rundel \protect\hyperlink{ref-isrs2014}{2014}),
  and
\item
  ``R for Data Science'' (Grolemund and Wickham
  \protect\hyperlink{ref-rds2016}{2016}).
\end{itemize}
...
\subsubsection{Who is this book for?}\label{who-is-this-book-for}
...
\hypertarget{sec:connect-contribute}{\subsection{Connect and
...
\hypertarget{sec:about-book}{\subsection{About this
book}\label{sec:about-book}}

This book was written using RStudio's
\href{https://bookdown.org/}{bookdown} package by Yihui Xie (Xie
\protect\hyperlink{ref-R-bookdown}{2018}). This package simplifies the
publishing of books by having all content written in
...
\subsection{About the authors}\label{sec:about-authors}
...
\item
  ...
  RStudio, and R Markdown} (Ismay
  \protect\hyperlink{ref-usedtor2016}{2016}) short book, which includes
  video screen recordings that you can follow along and pause as you
  learn.
\item
...
\subsubsection{Tips on learning to code}\label{tips-on-learning-to-code}
...
\item
  ...
  \item
    Click ``Install''\\
    \includegraphics[height=4.00000in]{images/install_packages_easy_way.png}
  \end{enumerate}
\item
...
\subsection{Explore your first dataset}\label{nycflights13}
...
\subsubsection{nycflights13 package}\label{nycflights13-package}
...
package (Wickham \protect\hyperlink{ref-R-nycflights13}{2018}).
Specifically, this package contains five datasets saved as ``data
frames'' (see Section \ref{code}) with information about all domestic
flights departing from New York City in 2013, from either Newark Liberty
International (EWR), John F. Kennedy International (JFK), or LaGuardia
(LGA) airports:
...
\subsubsection{flights data frame}\label{flights-data-frame}
...
\subsubsection{Exploring data frames}\label{exploredataframes}
...
\subsubsection{What's to come?}\label{whats-to-come}
...
\section{Data Visualization via ggplot2}\label{viz}
...
\addcontentsline{toc}{subsubsection}{DataCamp}
...
\subsection{The Grammar of Graphics}\label{grammarofgraphics}
...
Wilkinson (Wilkinson \protect\hyperlink{ref-wilkinson2005}{2005}) allows
us to specify the components of a statistical graphic.

\subsubsection{Components of the
Grammar}\label{components-of-the-grammar}
...
\subsubsection{Other components of the
...
for Data Science} (Grolemund and Wickham
\protect\hyperlink{ref-rds2016}{2016})):
...
R (Wickham, Chang, et al. \protect\hyperlink{ref-R-ggplot2}{2018}). You
may have noticed that a lot of the previous text in this chapter is
written in computer font. This is because the various components of the
Grammar of Graphics are specified in the \texttt{ggplot} function, which
expects at a bare minimum as arguments:
...
\subsection{Five Named Graphs - The 5NG}\label{FiveNG}
...
\subsection{5NG\#1: Scatterplots}\label{scatterplots}
...
\subsubsection{Scatterplots via geom\_point}\label{geompoint}
...
\subsubsection{Over-plotting}\label{overplotting}
...
\subsection{5NG\#2: Linegraphs}\label{linegraphs}
...
\subsubsection{Linegraphs via geom\_line}\label{geomline}
...
\subsection{5NG\#3: Histograms}\label{histograms}
...
\subsubsection{Histograms via geom\_histogram}\label{geomhistogram}
...
\subsubsection{Adjusting the bins}\label{adjustbins}
...
\subsection{5NG\#4: Boxplots}\label{boxplots}
...
\subsubsection{Boxplots via geom\_boxplot}\label{geomboxplot}
...
\subsection{5NG\#5: Barplots}\label{geombar}
...
\subsubsection{Must avoid pie charts!}\label{must-avoid-pie-charts}
...
(Robbins \protect\hyperlink{ref-robbins2013}{2013}), we overestimate
angles greater than 90 degrees and we underestimate angles less than 90
degrees. In other words, it is difficult for us to determine relative
...
\subsubsection{Using barplots to compare two categorical
...
R package} (Kim, Ismay, and Chunn
\protect\hyperlink{ref-R-fivethirtyeight}{2018}) with links to the
corresponding FiveThirtyEight.com articles in our free DataCamp course
...
\subsubsection{What's to come?}\label{whats-to-come-1}
...
\subsubsection*{Needed packages}\label{needed-packages-1}
\addcontentsline{toc}{subsubsection}{Needed packages}
...
\subsubsection*{DataCamp}\label{datacamp-1}
\addcontentsline{toc}{subsubsection}{DataCamp}
...
\emph{tidy data} here (Wickham \protect\hyperlink{ref-tidy}{2014}):
...
\texttt{gather()} function in the \texttt{tidyr} package (Wickham and
Henry \protect\hyperlink{ref-R-tidyr}{2018}) in the \texttt{tidyverse};
we'll show an example of this in Section \ref{tidying}. For other
examples of converting a dataset into ``tidy'' format, check out the
different functions available for data tidying and a case study using
data from the World Health Organization in
\href{http://r4ds.had.co.nz/tidy-data.html}{R for Data Science}
(Grolemund and Wickham \protect\hyperlink{ref-rds2016}{2016}).
...
\subsection{Back to nycflights13}\label{back-to-nycflights13}
...
\subsubsection{Observational units}\label{observational-units}
...
different observational units (Wickham
\protect\hyperlink{ref-R-nycflights13}{2018}):
...
\subsubsection{Identification vs measurement
...
\subsection{Importing spreadsheets into R}\label{csv}
...
\subsubsection{Method 1: From the
console}\label{method-1-from-the-console}
...
\subsubsection{Method 2: Using RStudio's
...
\subsection{\texorpdfstring{Converting to ``tidy'' data
...
\subsection{Optional: Normal forms of
...
\subsubsection{Review questions}\label{review-questions-1}

Review questions have been designed using the \texttt{fivethirtyeight} R
package (Kim, Ismay, and Chunn
\protect\hyperlink{ref-R-fivethirtyeight}{2018}) with links to the
corresponding FiveThirtyEight.com articles in our free DataCamp course
\textbf{Effective Data Storytelling using the \texttt{tidyverse}}. The
...
\subsubsection{What's to come?}\label{whats-to-come-2}
...
\subsubsection{Script of R code}\label{script-of-r-code-1}

An R script file of all R code used in this chapter is available
\href{scripts/04-tidy.R}{here}.

\section{Data Wrangling via dplyr}\label{wrangling}
...
data wrangling tools from the \texttt{dplyr} package (Wickham, François,
et al. \protect\hyperlink{ref-R-dplyr}{2018}) for your data science
toolbox.

\subsubsection*{Needed packages}\label{needed-packages-2}
\addcontentsline{toc}{subsubsection}{Needed packages}
...
\subsubsection*{DataCamp}\label{datacamp-2}
\addcontentsline{toc}{subsubsection}{DataCamp}
...
\subsection{\texorpdfstring{The pipe
...
\subsection{Summarize variables using summarize}\label{summarize}
...
\subsection{Group rows using group\_by}\label{groupby}
...
\subsubsection{Grouping by more than one
...
Wickham \protect\hyperlink{ref-rds2016}{2016}):
...
\subsection{Reorder the data frame using arrange}\label{arrange}
...
\subsection{Joining data frames}\label{joins}
...
appropriate matched during the join. Hadley and Garrett (Grolemund and
Wickham \protect\hyperlink{ref-rds2016}{2016}) created the following
diagram to help us understand how the different datasets are linked by
...
\subsubsection{\texorpdfstring{Joining by ``key''
...
\subsubsection{\texorpdfstring{Joining by ``key'' variables with
different
...
\subsubsection{\texorpdfstring{Joining by multiple ``key''
...
\subsection{Other verbs}\label{other-verbs}
...
Hadley and Garrett's book (Grolemund and Wickham
\protect\hyperlink{ref-rds2016}{2016}).

\subsubsection{Select variables using select}\label{select}
...
\subsubsection{Rename variables using rename}\label{rename}
...
\subsubsection{Find the top number of values using
...
\subsubsection{Putting it all together: Available seat
...
\subsubsection{What's to come?}\label{whats-to-come-3}
...
\subsubsection{Script of R code}\label{script-of-r-code-2}
...
\section{Basic Regression}\label{regression}
...
\subsubsection*{Needed packages}\label{needed-packages-3}
\addcontentsline{toc}{subsubsection}{Needed packages}
...
\subsubsection*{DataCamp}\label{datacamp-3}
\addcontentsline{toc}{subsubsection}{DataCamp}
...
\subsection{One numerical explanatory variable}\label{model1}
...
\subsubsection{Exploratory data analysis}\label{model1EDA}
...
\subsubsection{Simple linear regression}\label{model1table}
...
\subsubsection{Observed/fitted values and residuals}\label{model1points}
...
\subsubsection{Residual analysis}\label{model1residuals}
...
\subsection{One categorical explanatory variable}\label{model2}
...
\subsubsection{Exploratory data analysis}\label{model2EDA}
...
\subsubsection{Linear regression}\label{model2table}
...
\subsubsection{Observed/fitted values and residuals}\label{model2points}
...
\subsubsection{Residual analysis}\label{model2residuals}
...
\subsubsection{Correlation coefficient}\label{correlationcoefficient}
...
\subsubsection{Correlation is not necessarily
...
\subsubsection{Best fitting line}\label{leastsquares}
...
\subsubsection{\texorpdfstring{\texttt{get\_regression\_x()}
...
\subsubsection{Script of R code}\label{script-of-r-code-3}
...
\subsubsection*{Needed packages}\label{needed-packages-4}
\addcontentsline{toc}{subsubsection}{Needed packages}
...
\subsubsection*{DataCamp}\label{datacamp-4}
\addcontentsline{toc}{subsubsection}{DataCamp}
...
\subsection{Two numerical explanatory variables}\label{model3}
...
\subsubsection{Exploratory data analysis}\label{model3EDA}
...
\subsubsection{Multiple regression}\label{model3table}
...
\subsubsection{Observed/fitted values and residuals}\label{model3points}
...
\subsubsection{Residual analysis}\label{model3residuals}
...
\subsection{One numerical \& one categorical explanatory
...
\subsubsection{Exploratory data analysis}\label{model4EDA}
...
\subsubsection{Multiple regression: Parallel slopes
...
\subsubsection{Multiple regression: Interaction
...
\subsubsection{Observed/fitted values and residuals}\label{model4points}
...
\subsubsection{Residual analysis}\label{model4residuals}
...
\subsubsection{More on the correlation
...
\subsubsection{Simpson's Paradox}\label{simpsonsparadox}
...
\subsubsection{What's to come?}\label{whats-to-come-4}
...
\subsubsection{Script of R code}\label{script-of-r-code-4}
...
\subsubsection*{Needed packages}\label{needed-packages-5}
\addcontentsline{toc}{subsubsection}{Needed packages}
...
\subsection{Introduction to sampling}\label{introduction-to-sampling}
...
\subsubsection*{Concepts related to
sampling}\label{concepts-related-to-sampling}
\addcontentsline{toc}{subsubsection}{Concepts related to sampling}
...
\subsubsection*{Inference via sampling}\label{inference-via-sampling}
\addcontentsline{toc}{subsubsection}{Inference via sampling}
...
\subsection{Tactile sampling simulation}\label{tactile}
...
\subsubsection{Using the shovel once}\label{using-the-shovel-once}
...
\subsubsection{Using the shovel 33 times}\label{student-shovels}
...
\subsection{Virtual sampling simulation}\label{virtual}
...
\subsubsection{Using the shovel once}\label{using-the-shovel-once-1}
...
\subsubsection{Using shovel 33 times}\label{using-shovel-33-times}
...
\subsubsection{Using shovel 1000 times}\label{using-shovel-1000-times}
...
\subsubsection{Using different shovels}\label{using-different-shovels}
...
\subsection{In real-life sampling: Polls}\label{polls}
...
\subsubsection{What's to come?}\label{whats-to-come-5}
...
\subsubsection{Script of R code}\label{script-of-r-code-5}
...
\subsubsection*{Needed packages}\label{needed-packages-6}
\addcontentsline{toc}{subsubsection}{Needed packages}
...
\subsubsection*{DataCamp}\label{datacamp-5}
\addcontentsline{toc}{subsubsection}{DataCamp}
...
\subsubsection{The Bootstrapping Process}\label{bootstrap-process}
...
package (Bray et al. \protect\hyperlink{ref-R-infer}{2018}) that was
created to help users perform statistical inference such as confidence
intervals and hypothesis tests using verbs similar to what you've seen
with \texttt{dplyr}. We'll walk through setting up each of the
\texttt{infer} verbs for confidence intervals using this
\texttt{pennies\_sample} example, while also explaining the purpose of
the verbs in a general framework.

\subsection{The infer package for statistical
...
\subsubsection{Calculate summary
...
\paragraph{Observed statistic / point estimate
calculations}\label{observed-statistic-point-estimate-calculations}
\addcontentsline{toc}{paragraph}{Observed statistic / point estimate
calculations}
...
\subsection{Now to confidence
intervals}\label{now-to-confidence-intervals}
...
\subsubsection{The percentile method}\label{percentile-method}
...
\subsubsection{The standard error
method}\label{the-standard-error-method}
...
\subsection{Comparing bootstrap and sampling
...
\paragraph{Sampling distribution}\label{sampling-distribution}
\addcontentsline{toc}{paragraph}{Sampling distribution}
...
\paragraph{Bootstrap distribution}\label{bootstrap-distribution}
\addcontentsline{toc}{paragraph}{Bootstrap distribution}
...
\subsection{Interpreting the confidence
...
\paragraph{Back to our pennies
example}\label{back-to-our-pennies-example}
\addcontentsline{toc}{paragraph}{Back to our pennies example}
...
\subsection{EXAMPLE: One proportion}\label{one-prop-ci}
...
\subsubsection{Observed Statistic}\label{observed-statistic}
...
\subsubsection{Theory-based confidence
...
\paragraph{\texorpdfstring{Procedure for building a theory-based CI for
\(p\)}{Procedure for building a theory-based CI for p}}\label{procedure-for-building-a-theory-based-ci-for-p}
\addcontentsline{toc}{paragraph}{Procedure for building a theory-based
CI for \(p\)}
...
\paragraph{Confidence intervals based on 33 tactile
samples}\label{confidence-intervals-based-on-33-tactile-samples}
\addcontentsline{toc}{paragraph}{Confidence intervals based on 33
tactile samples}
...
\paragraph{Confidence intervals based on 100 virtual
samples}\label{confidence-intervals-based-on-100-virtual-samples}
\addcontentsline{toc}{paragraph}{Confidence intervals based on 100
virtual samples}
...
\subsection{EXAMPLE: Comparing two
...
\subsubsection{Compute the point
...
\subsubsection{Bootstrap distribution}\label{bootstrap-distribution-2}
...
\subsubsection{What's to come?}\label{whats-to-come-6}
...
\subsubsection{Script of R code}\label{script-of-r-code-6}
...
\subsubsection*{Needed packages}\label{needed-packages-7}
\addcontentsline{toc}{subsubsection}{Needed packages}
...
\subsubsection*{DataCamp}\label{datacamp-6}
\addcontentsline{toc}{subsubsection}{DataCamp}
...
\subsection{When inference is not
...
\subsection{Basics of hypothesis testing}\label{ht-basics}
...
\subsubsection{Logic of hypothesis
...
\subsection{Hypothesis testing with
...
\subsubsection{Randomization/permutation}\label{randomizationpermutation}
...
\subsubsection{Comparing action and romance
...
users of IMDB.com (Wickham
\protect\hyperlink{ref-R-ggplot2movies}{2015}). We are interested in the
...
\subsubsection{\texorpdfstring{Sampling \(\rightarrow\)
...
\subsubsection{\texorpdfstring{Observed effect
...
\subsubsection{\texorpdfstring{Distribution of \(\delta\) under
...
\subsubsection{Corresponding confidence
...
\subsection{Building theory-based methods using
...
\subsubsection{\texorpdfstring{Example: \(t\)-test for two independent
...
\subsubsection{Script of R code}\label{script-of-r-code-7}
...
\subsubsection*{Needed packages}\label{needed-packages-8}
\addcontentsline{toc}{subsubsection}{Needed packages}
...
\subsubsection*{DataCamp}\label{datacamp-7}
\addcontentsline{toc}{subsubsection}{DataCamp}
...
\subsection{Simulation-based Inference for
...
\subsubsection{The p-value}\label{the-p-value-1}
...
\subsection{Bootstrapping for the regression
...
\subsubsection{Refresher: Professor evaluations
...
Visualizations}\label{refresher-visualizations}
...
\subsubsection{Refresher: Regression
...
\subsubsection{Script of R code}\label{script-of-r-code-8}
...
\section{Thinking with Data}\label{thinking-with-data}
...
\subsubsection*{Needed packages}\label{needed-packages-9}
\addcontentsline{toc}{subsubsection}{Needed packages}
...
\subsubsection*{DataCamp}\label{datacamp-8}
\addcontentsline{toc}{subsubsection}{DataCamp}
...
\subsection{Case study: Seattle house
prices}\label{seattle-house-prices}
...
\subsubsection{log10 transformations}\label{log10-transformations}
...
\subsubsection{EDA Part II}\label{eda-part-ii}
...
\subsubsection{Regression modeling}\label{house-prices-regression}
...
predictions}\label{house-prices-making-predictions}
...
\subsection{Case study: Effective data
storytelling}\label{data-journalism}
...
\subsubsection{Bechdel test for Hollywood gender
...
\subsubsection{US Births in 1999}\label{us-births-in-1999}
...
\subsubsection{Script of R code}\label{script-of-r-code-9}
...
\section{Statistical Background}\label{appendixA}

\subsection{Basic statistical terms}\label{basic-statistical-terms}
...
\section{Inference Examples}\label{appendixB}
...
\subsection{Inference mind map}\label{inference-mind-map}
...
women from 2006 to 2010 is greater than 23 years? (Tweaked a bit from
Diez, Barr, and Çetinkaya-Rundel \protect\hyperlink{ref-isrs2014}{2014}
{[}Chapter 4{]})

\subsubsection{Competing hypotheses}\label{competing-hypotheses}

\paragraph{In words}\label{in-words}
\addcontentsline{toc}{paragraph}{In words}
...
\paragraph{In symbols (with
annotations)}\label{in-symbols-with-annotations}
\addcontentsline{toc}{paragraph}{In symbols (with annotations)}
...
\paragraph{\texorpdfstring{Set
\(\alpha\)}{Set \textbackslash{}alpha}}\label{set-alpha}
\addcontentsline{toc}{paragraph}{Set \(\alpha\)}
...
\subsubsection{Exploring the sample
data}\label{exploring-the-sample-data}
...
\paragraph{Guess about statistical
significance}\label{guess-about-statistical-significance}
\addcontentsline{toc}{paragraph}{Guess about statistical significance}
...
\subsubsection{Non-traditional methods}\label{non-traditional-methods}

\paragraph{Bootstrapping for hypothesis
test}\label{bootstrapping-for-hypothesis-test}
\addcontentsline{toc}{paragraph}{Bootstrapping for hypothesis test}
...
\addcontentsline{toc}{subparagraph}{Calculate \(p\)-value}
...
\paragraph{Bootstrapping for confidence
interval}\label{bootstrapping-for-confidence-interval}
\addcontentsline{toc}{paragraph}{Bootstrapping for confidence interval}
...
\subsubsection{Traditional methods}\label{traditional-methods}

\paragraph{Check conditions}\label{check-conditions}
...
\subparagraph{Observed test statistic}\label{observed-test-statistic}
\addcontentsline{toc}{subparagraph}{Observed test statistic}
...
\addcontentsline{toc}{paragraph}{Compute \(p\)-value}
...
\paragraph{State conclusion}\label{state-conclusion}
\addcontentsline{toc}{paragraph}{State conclusion}
...
\subsubsection{Competing hypotheses}\label{competing-hypotheses-1}

\paragraph{In words}\label{in-words-1}
\addcontentsline{toc}{paragraph}{In words}
...
\subsubsection{Non-traditional methods}\label{non-traditional-methods-1}

\paragraph{Simulation for hypothesis
test}\label{simulation-for-hypothesis-test}
\addcontentsline{toc}{paragraph}{Simulation for hypothesis test}
...
\subsubsection{Traditional methods}\label{traditional-methods-1}

\paragraph{Check conditions}\label{check-conditions-1}
\addcontentsline{toc}{paragraph}{Check conditions}
...
\paragraph{Test statistic}\label{test-statistic-1}
\addcontentsline{toc}{paragraph}{Test statistic}
...
\subparagraph{Observed test statistic}\label{observed-test-statistic-1}
\addcontentsline{toc}{subparagraph}{Observed test statistic}
...
\paragraph{\texorpdfstring{Visualize and compute
\(p\)-value}{Visualize and compute p-value}}\label{visualize-and-compute-p-value}
\addcontentsline{toc}{paragraph}{Visualize and compute \(p\)-value}
...
\paragraph{State conclusion}\label{state-conclusion-1}
\addcontentsline{toc}{paragraph}{State conclusion}
...
\subsubsection{Comparing results}\label{comparing-results-1}
...
issue is different than that of non-college graduates. (Tweaked a bit
from Diez, Barr, and Çetinkaya-Rundel
\protect\hyperlink{ref-isrs2014}{2014} {[}Chapter 6{]})

\subsubsection{Competing hypotheses}\label{competing-hypotheses-2}

\paragraph{In words}\label{in-words-2}
\addcontentsline{toc}{paragraph}{In words}
...
\paragraph{Another way in words}\label{another-way-in-words}
\addcontentsline{toc}{paragraph}{Another way in words}
...
\subsubsection{Non-traditional methods}\label{non-traditional-methods-2}

\paragraph{Collecting summary info}\label{collecting-summary-info}
\addcontentsline{toc}{paragraph}{Collecting summary info}
...
\paragraph{Randomization for hypothesis
test}\label{randomization-for-hypothesis-test}
\addcontentsline{toc}{paragraph}{Randomization for hypothesis test}
...
\subsubsection{Traditional methods}\label{traditional-methods-2}

\subsubsection{Check conditions}\label{check-conditions-2}
...
\subsubsection{Test statistic}\label{test-statistic-2}
...
\paragraph{Observed test statistic}\label{observed-test-statistic-2}
...
\subsubsection{State conclusion}\label{state-conclusion-2}
...
\subsubsection{Comparing results}\label{comparing-results-2}
...
\subsection{Two means (independent
...
Census. (Tweaked a bit from Diez, Barr, and Çetinkaya-Rundel
\protect\hyperlink{ref-isrs2014}{2014} {[}Chapter 5{]})

\subsubsection{Competing hypotheses}\label{competing-hypotheses-3}

\paragraph{In words}\label{in-words-3}
\addcontentsline{toc}{paragraph}{In words}
...
\paragraph{Another way in words}\label{another-way-in-words-1}
\addcontentsline{toc}{paragraph}{Another way in words}
...
\subsubsection{Non-traditional methods}\label{non-traditional-methods-3}

\paragraph{Collecting summary info}\label{collecting-summary-info-1}
\addcontentsline{toc}{paragraph}{Collecting summary info}
...
\subsubsection{Traditional methods}\label{traditional-methods-3}

\subparagraph{Check conditions}\label{check-conditions-3}
\addcontentsline{toc}{subparagraph}{Check conditions}
...
\subsubsection{Test statistic}\label{test-statistic-3}
...
\paragraph{Observed test statistic}\label{observed-test-statistic-3}
\addcontentsline{toc}{paragraph}{Observed test statistic}
...
\subsubsection{State conclusion}\label{state-conclusion-3}
...
\subsubsection{Comparing results}\label{comparing-results-3}
...
\subsection{Two means (paired samples)}\label{two-means-paired-samples}

\paragraph{Problem statement}\label{problem-statement-4}
\addcontentsline{toc}{paragraph}{Problem statement}
...
\subsubsection{Competing hypotheses}\label{competing-hypotheses-4}

\paragraph{In words}\label{in-words-4}
\addcontentsline{toc}{paragraph}{In words}
...
\subsubsection{Non-traditional methods}\label{non-traditional-methods-4}
...
\subsubsection{Traditional methods}\label{traditional-methods-4}

\paragraph{Check conditions}\label{check-conditions-4}
\addcontentsline{toc}{paragraph}{Check conditions}
...
\paragraph{Test statistic}\label{test-statistic-4}
\addcontentsline{toc}{paragraph}{Test statistic}
...
\subparagraph{Observed test statistic}\label{observed-test-statistic-4}
\addcontentsline{toc}{subparagraph}{Observed test statistic}
...
\paragraph{State conclusion}\label{state-conclusion-4}
\addcontentsline{toc}{paragraph}{State conclusion}
...
\subsubsection{Comparing results}\label{comparing-results-4}
...
\section{Reach for the Stars}\label{appendixC}

\subsection*{Needed packages}\label{needed-packages-11}
\addcontentsline{toc}{subsection}{Needed packages}
...
\subsection{Sorted barplots}\label{sorted-barplots}
...
ordering of different airlines in terms of departed flights (Robbins
\protect\hyperlink{ref-robbins2013}{2013}). We can also much more easily
answer questions like ``How many airlines have more departing flights
...
\subsubsection{Interactive linegraphs}\label{interactive-linegraphs}
...
\section{Learning Check Solutions}\label{appendixD}

\subsection{Needed packages}\label{needed-packages-12}
...
\subsection{Chapter 2 Solutions}\label{chapter-2-solutions}
...
\subsection{Chapter 3 Solutions}\label{chapter-3-solutions}
...
\hypertarget{refs}{}
\hypertarget{ref-R-infer}{}
Bray, Andrew, Chester Ismay, Ben Baumer, and Mine Cetinkaya-Rundel.
2018. \emph{Infer: Tidy Statistical Inference}.
\url{https://github.com/tidymodels/infer}.

\hypertarget{ref-hester2011}{}
Chihara, Laura M., and Tim C. Hesterberg. 2011. \emph{Mathematical
Statistics with Resampling and R}. Hoboken, NJ: John Wiley; Sons.
\url{https://sites.google.com/site/chiharahesterberg/home}.

\hypertarget{ref-isrs2014}{}
Diez, David M, Christopher D Barr, and Mine Çetinkaya-Rundel. 2014.
\emph{Introductory Statistics with Randomization and Simulation}. First
Edition.
\url{https://www.openintro.org/stat/textbook.php?stat_book=isrs}.

\hypertarget{ref-rds2016}{}
Grolemund, Garrett, and Hadley Wickham. 2016. \emph{R for Data Science}.
\url{http://r4ds.had.co.nz/}.

\hypertarget{ref-usedtor2016}{}
Ismay, Chester. 2016. \emph{Getting Used to R, RStudio, and R Markdown}.
\url{http://ismayc.github.io/rbasics-book}.

\hypertarget{ref-R-fivethirtyeight}{}
Kim, Albert Y., Chester Ismay, and Jennifer Chunn. 2018.
\emph{Fivethirtyeight: Data and Code Behind the Stories and Interactives
at 'Fivethirtyeight'}.
\url{https://CRAN.R-project.org/package=fivethirtyeight}.

\hypertarget{ref-robbins2013}{}
...
\hypertarget{ref-R-ggplot2movies}{}
---------. 2015. \emph{Ggplot2movies: Movies Data}.
\url{https://CRAN.R-project.org/package=ggplot2movies}.

\hypertarget{ref-R-nycflights13}{}
---------. 2018. \emph{Nycflights13: Flights That Departed Nyc in 2013}.
