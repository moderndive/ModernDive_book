...
**Interpretation**:  We are 95% confident the true mean age of first marriage for all US women from 2006 to 2010 is between `r ci$lower` and `r ci$upper`.
...
```{r t.test1a, eval=FALSE}
t_test_results <- age_at_marriage %>% 
  t_test(formula = age ~ NULL,
       alternative = "greater",
       mu = 23)
t_test_results
```
...
**Interpretation**:  We are 95% confident the true proportion of customers who are satisfied with the service they receive is between `r ci$lower` and `r ci$upper`.
...
```{r}
boot_distn_two_props %>% visualize()
```
...
```{r}
boot_distn_two_means %>% visualize()
```
...
```{r read_data5, message=FALSE, warning=FALSE}
...
```{r}
boot_distn_paired_means %>% visualize()
```

```{r}
ci_boot <- boot_distn_paired_means %>% 
  summarize(lower = quantile(stat, probs = 0.025),
    upper = quantile(stat, probs = 0.975))
ci_boot
```
...
```{r t.test3a, echo=FALSE}
t_test_results <- as.data.frame(zinc_diff) %>% 
  t_test(formula = pair_diff ~ NULL, 
         alternative = "less",
         mu = 0)
t_test_results
```

```{r t.test3b, eval=FALSE}
...
The $p$-value---the probability of observing a $t_{obs}$ value of `r  t_test_results$statistic` or less in our null distribution of a $t$ with 9 degrees of freedom---is ` t_test_results$p_value`.  This can also be calculated in R directly:
