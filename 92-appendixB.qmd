...
# Inference Examples {#sec-appendixB}

This appendix is designed to provide you with examples of basic hypothesis tests and their corresponding confidence intervals.  Traditional theory-based methods as well as computational-based methods are presented. <!-- You can also use this appendix as a way to check for understanding of which statistical graphic is most appropriate given the problem set-up. -->

```{r appendixB-1, include=FALSE}
...
```{r appendixB-alt, message=FALSE}
library(tidyverse)
library(infer)
library(janitor)
```

```{r appendixB-alt2, message=FALSE, echo=FALSE, purl=FALSE}
...
```{r fig-infer-map, fig.alt="Decision flowchart for choosing the right inference procedure: branches by number of variables (1 vs 2), then by variable type (categorical vs numerical), leading to the appropriate test.", echo=FALSE, fig.cap="Mind map for Inference.", out.width="100%"}
if (is_html_output()) {
...
```{r appendixB-assign-age_at_marriage, eval=FALSE}
...
```{r hist1b, fig.alt="Histogram of the `age` variable in the `age_at_marriage` sample, binwidth 3 years. Right-skewed shape with most women's age-at-first-marriage between 20 and 30 and a long tail toward older ages."}
...
#### Guess about statistical significance {.unnumbered}
...
#### Bootstrapping for hypothesis test {.unnumbered}
...
```{r appendixB-assign-mu0, echo=FALSE}
mu0 <- 23
```
...
```{webr-r}
library(infer)
age_at_marriage <- read.csv("https://raw.githubusercontent.com/moderndive/ModernDive_book/v2/data/ageAtMar.csv")
set.seed(2018)
# reps reduced to 1000 for in-browser speed; raise it if you want
null_distn_one_mean <- age_at_marriage |>
  specify(response = age) |>
  hypothesize(null = "point", mu = 23) |>
  generate(reps = 1000) |>
  calculate(stat = "mean")
visualize(null_distn_one_mean)
```
:::
```{r appendixB-v8, fig.alt="Histogram of the simulated null distribution of sample means under H_0: mu = 23. Roughly bell-shaped and centered at 23, with spread reflecting the standard error of the sample mean."}
...
```{r appendixB-v9, fig.alt="Same null distribution of sample means with the right tail beyond the observed sample mean shaded to represent the one-sided p-value for the test of H_0: mu = 23 vs H_A: mu > 23."}
null_distn_one_mean |>
  visualize() +
  shade_p_value(obs_stat = x_bar, direction = "greater")
```

##### Calculate $p$-value {.unnumbered}

```{r appendixB-assign-pvalue}
...
#### Bootstrapping for confidence interval {.unnumbered}
...
library(infer)
age_at_marriage <- read.csv("https://raw.githubusercontent.com/moderndive/ModernDive_book/v2/data/ageAtMar.csv")
# reps reduced to 1000 for in-browser speed
boot_distn_one_mean <- age_at_marriage |>
  specify(response = age) |>
  generate(reps = 1000) |>
  calculate(stat = "mean")
visualize(boot_distn_one_mean)
```
:::

```{r appendixB-assign-ci}
ci <- boot_distn_one_mean |>
  get_ci()
ci
```

```{r appendixB-v13, fig.alt="Histogram of the bootstrap distribution of sample means with the 95% percentile confidence interval shaded between the two endpoint vertical lines."}
boot_distn_one_mean |>
  visualize() +
...
##### Observed test statistic {.unnumbered}
...
```{r bar, fig.alt="Bar chart of the categorical `satisfy` variable in the `elec` sample, with two bars: `satisfied` and `not satisfied`. The `satisfied` bar is taller."}
...
#### Simulation for hypothesis test {.unnumbered}
...
library(infer)
elec <- data.frame(
  satisfy = c(rep("satisfied", 73), rep("unsatisfied", 27))
)
set.seed(2018)
# reps reduced to 1000 for in-browser speed; raise it if you want
null_distn_one_prop <- elec |>
...
```{r appendixB-v16, fig.alt="Histogram of the simulated null distribution of sample proportions under H_0: p = 0.8. Roughly bell-shaped and centered at 0.8."}
...
```{r appendixB-v17, fig.alt="Same null distribution of sample proportions with both tails (values at least 0.07 away from 0.8 in either direction) shaded to represent the two-sided p-value."}
null_distn_one_prop |>
  visualize() +
...
```{r appendixB-assign-ci-v2}
ci <- boot_distn_one_prop |>
  get_ci()
ci
```

```{r appendixB-v21, fig.alt="Histogram of the bootstrap distribution of sample proportions with the 95% percentile confidence interval shaded between its endpoint vertical lines."}
boot_distn_one_prop |>
  visualize() +
  shade_ci(endpoints = ci)
```
...
The test statistic is a random variable based on the sample data.  Here, we want to look at a way to estimate the population proportion $\pi$.  A good guess is the sample proportion $\hat{P}$.  Recall that this sample proportion is actually a random variable that will vary as different samples are (theoretically, would be) collected.  We are looking to see how likely is it for us to have observed a sample proportion of $\hat{p}_{obs} = `r p_hat`$ or more extreme assuming that the population proportion is 0.80 (assuming the null hypothesis is true).  If the conditions are met and assuming $H_0$ is true, we can standardize this original test statistic of $\hat{P}$ into a $Z$ statistic that follows a $N(0, 1)$ distribution.
...
#### Visualize and compute $p$-value {.unnumbered}

```{r pvaloneprop, fig.alt="Standard normal density curve with both tails beyond +/- |z_obs| shaded to represent the theory-based two-sided p-value for the one-proportion z-test of H_0: p = 0.8."}
elec |>
  specify(response = satisfy, success = "satisfied") |>
  hypothesize(null = "point", p = 0.8) |>
...
#### Another way in words {.unnumbered}
...
```{r appendixB-assign-offshore, eval=FALSE}
...
```{r stacked_bar, fig.alt="Horizontal stacked bar chart comparing the proportion of college graduates vs non-graduates who have \"no opinion\" on offshore drilling. The two bars are similar in length, suggesting the two groups respond similarly."}
offshore <- offshore |>
...
#### Collecting summary info {.unnumbered}
...
#### Randomization for hypothesis test {.unnumbered}
...
```{webr-r}
library(infer)
offshore <- read.csv("https://raw.githubusercontent.com/moderndive/ModernDive_book/v2/data/offshore.csv")
set.seed(2018)
# reps reduced to 1000 for in-browser speed; raise it if you want
null_distn_two_props <- offshore |>
  specify(response ~ college_grad, success = "no opinion") |>
  ...
  generate(reps = 1000) |>
  calculate(stat = "diff in props", order = c("yes", "no"))
visualize(null_distn_two_props)
```
:::

```{r appendixB-v26, fig.alt="Histogram of the simulated null distribution of differences in sample proportions under H_0: independence between college graduation and offshore-drilling opinion. Roughly bell-shaped and centered at 0."}
null_distn_two_props |> visualize()
```

We can next use this distribution to observe our $p$-value.  Recall this is a two-tailed test so we will be looking for values that are greater than or equal to `r abs(obs_diff)` or less than or equal to `r -abs(obs_diff)` for our $p$-value.

```{r appendixB-v27, fig.alt="Same null distribution of differences in sample proportions with both tails (values at least |obs_diff| away from 0 in either direction) shaded to represent the two-sided p-value."}
...
```{r appendixB-assign-pvalue-v2-dup1}
...
boot_distn_two_props <- offshore |>
  ...
  generate(reps = 1000) |>
  calculate(stat = "diff in props", order = c("yes", "no"))
visualize(boot_distn_two_props)
```
:::

```{r appendixB-assign-ci-v2-dup1}
ci <- boot_distn_two_props |>
  get_ci()
ci
```

```{r appendixB-v31, fig.alt="Histogram of the bootstrap distribution of differences in sample proportions with the 95% percentile confidence interval shaded between its endpoint vertical lines."}
...
```{r boxplot, fig.alt="Side-by-side boxplots of income split by metro area (Cleveland vs Sacramento) with a red dot marking the mean of each group. The two boxes overlap heavily, suggesting little difference between the metro areas."}
...
```{webr-r}
library(infer)
cle_sac <- read.delim("https://raw.githubusercontent.com/moderndive/ModernDive_book/v2/data/cleSac.txt") |>
  rename(
    metro_area = Metropolitan_area_Detailed,
    income = Total_personal_income
  ) |>
...
visualize(null_distn_two_means)
```
:::

```{r appendixB-v35, fig.alt="Histogram of the simulated null distribution of differences in sample means (Sacramento minus Cleveland) under H_0: independence. Roughly bell-shaped and centered at 0."}
...
```{r appendixB-v36, fig.alt="Same null distribution of differences in sample means with both tails (values at least |d_hat| from 0 in either direction) shaded to represent the two-sided p-value."}
...
```{r appendixB-assign-pvalue-v2-dup2}
...
```{r appendixB-assign-ci-v2-dup2}
ci <- boot_distn_two_means |>
  get_ci()
ci
```

```{r appendixB-v40, fig.alt="Histogram of the bootstrap distribution of differences in sample means with the 95% percentile confidence interval shaded between its endpoint vertical lines."}
...
```{r hist, fig.alt="Two faceted histograms of income — one for Cleveland, one for Sacramento — bin width $20,000. Both are right-skewed, deviating from a normal shape but with comparable spread."}
...
```{r appendixB-assign-zinc_tidy, eval=FALSE}
...
```{r hist1a, fig.alt="Histogram of paired differences (`pair_diff`) in zinc concentrations across the matched-pairs sample. Most differences are slightly negative, suggesting surface concentrations tend to be lower than bottom concentrations."}
...
```{webr-r}
library(infer)
zinc_tidy <- read.csv("https://raw.githubusercontent.com/moderndive/ModernDive_book/v2/data/zinc_tidy.csv")
zinc_diff <- zinc_tidy |>
  group_by(loc_id) |>
  summarize(pair_diff = diff(concentration)) |>
  ungroup()
...
null_distn_paired_means <- zinc_diff |>
  ...
  generate(reps = 1000) |>
  calculate(stat = "mean")
visualize(null_distn_paired_means)
```
:::

```{r appendixB-v44, fig.alt="Histogram of the simulated null distribution of paired mean differences under H_0: mu_diff = 0. Roughly bell-shaped and centered at 0."}
null_distn_paired_means |> visualize()
```

We can next use this distribution to observe our $p$-value.  Recall this is a left-tailed test so we will be looking for values that are less than or equal to `r d_hat` for our $p$-value.

```{r appendixB-v45, fig.alt="Same null distribution of paired mean differences with the left tail beyond the observed d_hat shaded to represent the one-sided p-value for H_A: mu_diff < 0."}
...
```{r appendixB-assign-pvalue-v2-dup3}
...
boot_distn_paired_means <- zinc_diff |>
  ...
  generate(reps = 1000) |>
  calculate(stat = "mean")
visualize(boot_distn_paired_means)
```
:::

```{r appendixB-assign-ci-v2-dup3}
ci <- boot_distn_paired_means |>
  get_ci()
ci
```

```{r appendixB-v49, fig.alt="Histogram of the bootstrap distribution of paired mean differences with the 95% percentile confidence interval shaded between its endpoint vertical lines."}
...
So far every example has compared means or proportions. Inference also applies to the *relationship between two numerical variables*: a **correlation coefficient** and the **slope** of a simple linear regression line are both sample statistics, so each has a sampling distribution we can simulate.

### Problem statement

We use the `episodes` data frame from the [`steves`](https://github.com/ismayc/steves) package, which records one row per episode of a long-running travel show along with its IMDb rating. We ask a simple question: **as the show went on, did episodes tend to be rated more highly?** Concretely, is the IMDb rating (`imdb_rating`) associated with how far into the series an episode aired (`overall_episode`, the running episode number)?

### Competing hypotheses

#### In words {.unnumbered}

- **Null hypothesis**: there is no linear relationship between an episode's running number and its IMDb rating in the population of all episodes.
- **Alternative hypothesis**: there is a linear relationship (the rating tends to change as the series progresses).

#### In symbols (with annotations) {.unnumbered}

- $H_0: \rho = 0$ (equivalently $\beta_1 = 0$), where $\rho$ is the population correlation and $\beta_1$ is the population regression slope.
- $H_A: \rho \neq 0$ (equivalently $\beta_1 \neq 0$); this is a two-sided test.

#### Set $\alpha$ {.unnumbered}

As before we set the significance level to $\alpha = 0.05$.

### Exploring the sample data

```{r read_data6, message=FALSE, warning=FALSE}
library(steves)

episodes_sub <- episodes |>
  select(overall_episode, season, air_year, imdb_rating) |>
  filter(!is.na(imdb_rating), !is.na(overall_episode))
```

There are `r nrow(episodes_sub)` episodes with a recorded IMDb rating. The scatterplot below adds a regression line:

```{r reg-scatter, message=FALSE, fig.alt="Scatterplot of IMDb rating (y) versus running episode number (x) for the steves episodes data, with an upward-sloping fitted regression line, indicating later episodes tend to be rated more highly."}
ggplot(episodes_sub, aes(x = overall_episode, y = imdb_rating)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Running episode number", y = "IMDb rating")
```

The observed **correlation** and the observed **regression slope** are:

```{r reg-obs}
obs_corr <- episodes_sub |>
  specify(imdb_rating ~ overall_episode) |>
  calculate(stat = "correlation")
obs_corr

obs_slope <- episodes_sub |>
  specify(imdb_rating ~ overall_episode) |>
  calculate(stat = "slope")
obs_slope
```

The correlation is a moderate positive `r round(obs_corr$stat, 3)`, and the slope says each later episode is associated with about `r round(obs_slope$stat, 4)` more IMDb points on average — small per episode, but it accumulates over `r max(episodes_sub$overall_episode)` episodes.

### Non-traditional methods

#### Randomization for the correlation {.unnumbered}

To test $H_0: \rho = 0$ we *permute* (shuffle) the ratings against the episode numbers, breaking any real association, and recompute the correlation many times to build the null distribution.

```{r reg-null-corr, eval=FALSE}
set.seed(2018)
null_distn_corr <- episodes_sub |>
  specify(imdb_rating ~ overall_episode) |>
  hypothesize(null = "independence") |>
...
library(infer)
library(steves)            # shadow library() loads `episodes`
episodes_sub <- subset(episodes, !is.na(imdb_rating) & !is.na(overall_episode))
obs_corr <- episodes_sub |>
  specify(imdb_rating ~ overall_episode) |>
  calculate(stat = "correlation")
set.seed(2018)
...
```{r reg-null-corr-run, echo=FALSE}
set.seed(2018)
null_distn_corr <- episodes_sub |>
  specify(imdb_rating ~ overall_episode) |>
  hypothesize(null = "independence") |>
...
```{r reg-null-corr-viz, fig.alt="Histogram of the simulated null distribution of the correlation under H_0: rho = 0. It is centered at 0; the observed correlation lies far in the right tail, with both two-sided rejection regions shaded."}
null_distn_corr |>
  visualize() +
  shade_p_value(obs_stat = obs_corr, direction = "two-sided")
```
...
```{r reg-corr-pval}
null_distn_corr |>
  get_pvalue(obs_stat = obs_corr, direction = "two-sided")
```

The observed correlation falls far outside everything the null produced, so the $p$-value is essentially 0 and we reject $H_0$.

#### Bootstrapping a confidence interval for the slope {.unnumbered}

We can also put a confidence interval on the regression slope by bootstrapping (resampling episodes with replacement) and recomputing the slope each time.

```{r reg-boot-slope, eval=FALSE}
set.seed(2018)
boot_distn_slope <- episodes_sub |>
  specify(imdb_rating ~ overall_episode) |>
  generate(reps = 1000, type = "bootstrap") |>
  calculate(stat = "slope")
```
...
boot_distn_slope <- episodes_sub |>
  ...
  generate(reps = 1000, type = "bootstrap") |>
  calculate(stat = "slope")
visualize(boot_distn_slope)
```
:::

```{r reg-boot-slope-run, echo=FALSE}
set.seed(2018)
boot_distn_slope <- episodes_sub |>
  specify(imdb_rating ~ overall_episode) |>
  generate(reps = 1000, type = "bootstrap") |>
  calculate(stat = "slope")
ci_slope <- boot_distn_slope |>
  get_confidence_interval(level = 0.95, type = "percentile")
```

```{r reg-boot-slope-viz, fig.alt="Histogram of the bootstrap distribution of the regression slope with the 95% percentile confidence interval shaded between its two endpoint vertical lines; the entire interval lies above zero."}
boot_distn_slope |>
  visualize() +
  shade_ci(endpoints = ci_slope)
```
...
The entire interval lies above 0, agreeing with the hypothesis test: there is a positive relationship.

### Traditional methods

The formula-based counterpart is the usual regression table. Its slope estimate, standard error, $t$-statistic, $p$-value, and confidence interval should line up closely with the simulation results above.

```{r reg-traditional}
library(moderndive)
get_regression_table(lm(imdb_rating ~ overall_episode, data = episodes_sub))
```

### Comparing results

The simulation-based correlation test, the bootstrap confidence interval for the slope, and the theory-based regression table all tell the same story: a small but clearly non-zero positive association between how late an episode aired and its IMDb rating. As elsewhere in this appendix, the computational and formula-based approaches agree.


## Multiple regression with `infer::fit()`

`infer` is not limited to a single slope. The `fit()` function fits a regression model inside the same `specify() |> generate() |> fit()` pipeline and returns *one row per coefficient per replicate*, so we can build a null distribution or a bootstrap confidence interval for **every coefficient in a multiple regression** at once.

### Problem statement

We use `saratoga_houses` from the `moderndive` package — house sale prices in Saratoga County, NY. We model `price` as a function of three predictors at once: living area (`living_area`, square feet), number of `bathrooms`, and number of `bedrooms`.

### Competing hypotheses

For each predictor $j$ we test, holding the other predictors in the model fixed:

- $H_0: \beta_j = 0$ (that predictor has no partial linear association with price), versus
- $H_A: \beta_j \neq 0$ (it does), at $\alpha = 0.05$.

### Exploring the sample data

```{r read_data7}
saratoga_sub <- saratoga_houses |>
  select(price, living_area, bathrooms, bedrooms)
```

```{r mreg-eda, fig.alt="Scatterplot of house price (y) versus living area in square feet (x) for Saratoga houses, with a clear positive trend."}
ggplot(saratoga_sub, aes(x = living_area, y = price)) +
...
#### Observed fitted coefficients {.unnumbered}

`specify()` followed by `fit()` returns the observed least-squares coefficients — the same numbers `lm()` would give:

```{r mreg-obs}
obs_fit <- saratoga_sub |>
  specify(price ~ living_area + bathrooms + bedrooms) |>
  fit()
obs_fit
```

#### Randomization for the hypothesis tests {.unnumbered}

Permuting the response breaks its relationship with *all* predictors simultaneously. Refitting the model on each permuted dataset gives a null distribution for every coefficient.

```{r mreg-null, eval=FALSE}
set.seed(2018)
null_fits <- saratoga_sub |>
  specify(price ~ living_area + bathrooms + bedrooms) |>
  hypothesize(null = "independence") |>
  generate(reps = 1000, type = "permute") |>
  fit()
...
null_fits <- saratoga_sub |>
  ...
  generate(reps = 1000, type = "permute") |>
  fit()
get_pvalue(null_fits, obs_stat = obs_fit, direction = "two-sided")
```
:::

```{r mreg-null-run, echo=FALSE}
set.seed(2018)
null_fits <- saratoga_sub |>
  specify(price ~ living_area + bathrooms + bedrooms) |>
  hypothesize(null = "independence") |>
...
```{r mreg-null-pval, message=FALSE}
null_fits |>
  get_pvalue(obs_stat = obs_fit, direction = "two-sided")
```

Permuting the response breaks its link with *all* predictors at once, so this tests the *joint* null. `living_area` and `bathrooms` land essentially at $p = 0$; `bedrooms` is borderline under this particular null (its permutation $p$-value hovers near 0.05). Keep in mind this response-shuffling null answers a slightly different question than the *partial* $t$-test for one coefficient with the others held fixed — the bootstrap intervals and the theory-based table below are the sharper tools for individual coefficients, and as we will see they show `bedrooms` is in fact clearly non-zero.

#### Bootstrapping confidence intervals for every coefficient {.unnumbered}

```{r mreg-boot, eval=FALSE}
set.seed(2018)
boot_fits <- saratoga_sub |>
  specify(price ~ living_area + bathrooms + bedrooms) |>
  generate(reps = 1000, type = "bootstrap") |>
  fit()
```
...
get_confidence_interval(
  boot_fits, level = 0.95, type = "percentile", point_estimate = obs_fit
)
```
:::

```{r mreg-boot-run, echo=FALSE}
set.seed(2018)
boot_fits <- saratoga_sub |>
  specify(price ~ living_area + bathrooms + bedrooms) |>
  generate(reps = 1000, type = "bootstrap") |>
...
boot_fits |>
  get_confidence_interval(
    level = 0.95, type = "percentile", point_estimate = obs_fit
  )
```

All three predictor intervals exclude 0. `living_area` and `bathrooms` are positive, and `bedrooms` is *negative*: holding living area and number of bathrooms fixed, houses split into more (therefore smaller) bedrooms tend to sell for less. This is the partial-effect story the response-permutation test could only hint at, and it matches the theory-based table next.

### Traditional methods

The theory-based multiple regression table gives the same coefficients, standard errors, and confidence intervals:

```{r mreg-traditional}
get_regression_table(lm(price ~ living_area + bathrooms + bedrooms,
                         data = saratoga_sub))
```

### Comparing results

`infer::fit()` reproduces the least-squares estimates exactly and, through permutation and bootstrapping, delivers $p$-values and confidence intervals that match the theory-based `get_regression_table()` output — while making explicit the simulation logic underlying multiple regression inference.
