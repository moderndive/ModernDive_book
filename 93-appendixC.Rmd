...
> Shout out to [Dr. Jenny Smetzer](https://www.scsparkscience.org/fellow/jennifer-smetzer/) for her work setting this up!
...
```{r}
movies <- read_csv("https://moderndive.com/data/movies.csv") %>% 
...
`dplyr::rename()` renames column/variable names. To "rename" values inside cells of a particular column, you need to `mutate()` the column using one of the three functions below. There might be other ones too, but these are the three I've seen the most. In these examples, we'll change values in the variable `type`.
...
Sometimes we want to turn a numerical, continuous variable into a categorical variable. For instance, what if we wanted to have a variable that tells us if a movie made one hundred million dollars or more. In other words a binary variable in other words a categorical variable with 2 levels. We can again use the `mutate` function:
...
### Dealing with %, commas, and \$ {#numerical}
