...
* ``A tibble: `r comma(nrow(nycflights13::flights))` x `r ncol(nycflights13::flights)` ``: A `tibble` is a specific kind of data frame in R.\index{tibble}  This particular data frame has
    + `` `r comma(nrow(nycflights13::flights))` `` rows corresponding to different *observations*. Here, each observation is a flight.
    + `` `r ncol(nycflights13::flights)` `` columns corresponding to 19 *variables* describing each observation.
...
* We then have a preview of the first 10 rows of observations corresponding to the first 10 flights. R is only showing the first 10 rows, because if it showed all `` `r comma(nrow(nycflights13::flights))` `` rows, it would overwhelm your screen.
