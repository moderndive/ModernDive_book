...
* ``A tibble: `r flights_rows` x `r flights %>% ncol() %>% comma()` ``: A `tibble` is a specific kind of data frame in R.\index{tibble}  This particular data frame has
    + `` `r flights_rows` `` rows corresponding to different *observations*. Here, each observation is a flight.
    + `` `r flights %>% ncol() %>% comma()` `` columns corresponding to 19 *variables* describing each observation.
