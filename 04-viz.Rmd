...
Before we proceed with this chapter, let's load all the necessary packages, in particular the `nycflights13` package introduced in Chapter \@ref(tidy) containing various data sets.
...
1. `geom`: the geometric object in question. This refers to our type of objects we can observe in our plot. Ex: points, lines, bars, etc
1. `aes`: aesthetic attributes of the geometric object that we can perceive on a graphic. Ex: x/y position, color, shape, size.  Each assigned aesthetic attribute can be mapped to a variable in our data set.  If not assigned, they are set to defaults.
...
For example, the data variable `longitude` gets mapped to `x` `aes`thetic of the points `geom`etric objects on the map while the annotated line-graph displays `date` and `temperature` variable information via its mapping to the `x` and `y` aesthetic of the line `geom`etric object. 
...
- `stat`istical transformations: this includes smoothing, binning values into a histogram, or just itself untransformed `"identity"`.
- `scales` both
...
- `coord`inate system for x/y values: typically `cartesian`, but can also be `polar`, `map`
- `position` adjustments

In this text, we will only focus on the first two: `facet`ing (introduced in Chapter \@ref(facets)) and `stat`istical transformations (in a limited sense when consider Barplots in Chapter \@ref(geombar)) ; the other components are left to a more advanced text. This is not a problem when producing a plot as each of these components have default setings.
...
We introduce Hadley Wickham's `ggplot2` package, which is an implementation of the Grammar of Graphics for R [@R-ggplot2]. You may have noticed that a lot of previous text in this chapter is written in computer font. This is because the various components of the Grammar of Graphics are specified using the `ggplot` function, which expects at a bare minimal as arguments
...
* By clicking [here](https://www.rstudio.com/wp-content/uploads/2015/12/ggplot2-cheatsheet-2.0.pdf)
* or by clicking the RStudio Menu Bar -> Help -> Cheatsheets -> "Data Visualization with ggplot2"
...
1. `dep_delay`: departure delay on the horizontal "x" axis
1. `arr_delay`: arrival delay on the vertical "y" axis

for Alaska Airlines flights leaving NYC in 2013. This requires paring down the `flights` data frame to a smaller data frame `alaska_flights` consisting of only Alaska Airlines (carrier code "AS") flights. 
...
```{block lc-alaska_flights, type='learncheck'}
**_Learning check_**
```
...
Let's break down this keeping in mind our discussion in Chapter \@ref(grammarofgraphics):
...
In Figure \@ref(fig:noalpha) we see that a positive relationship exists between `dep_delay` and `arr_delay`: as departure delays increase, arrival delays tend to also increase.  We also note that the majority of points fall near the point (0, 0).  There is a large mass of points clustered there.        
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Create a new scatter-plot using different variables in the `alaska_flights` data frame by modifying the example above.
...
The first way of relieving over-plotting is by changing the `alpha` argument to `geom_point()` which controls the transparency of the points.  By default, this value is set to `1`.  We can change this value to a smaller fraction to change the transparency of the points in the plot:
...
Note how this function call is identical to the one in Chapter \@ref(scatterplots), but with `geom_point()` replaced with `alpha=0.2` added. 
...
Note how this function call is identical to the one in Chapter \@ref(geompoint), but with `geom_point()` replaced with `geom_jitter()`. The plot in \@ref{fig:jitter} helps us a little bit in getting a sense for the over-plotting, but with a relatively large dataset like this one (`r nrow(alaska_flights)` flights), it cn be argued that changing the transparency of the points by setting `alpha` proved more effective.
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** After viewing the \@ref(fig:alpha) above, give a range of arrival times and departure times that occur most frequently?  How has that region changed compared to when you observed the same plot without the `alpha = 0.2` set in \@ref(fig:noalpha)?
...
The next of the 5NG is a line-graph.  They are most frequently used when the x-axis represents time and the y-axis represents some other numerical variable; such plots are known as **time series**.  Time represents a variable that is connected together by each day following the previous day.  In other words, time has a natural ordering.  Line-graphs should be avoided when there is not a clear sequential ordering to the explanatory variable i.e. the x-variable.
...
We can see that the `temp` variable corresponds to hourly temperature (in Fahrenheit) recordings at weather stations near airports in New York City. Instead of considering all hours in 2013 for all three airports in NYC, let's focus in the hourly temperature at Newark airport (`origin` code "EWR") for the first 15 days in January 2013. The `weather` data frame in the `nycflights13` package contains this data, but we first need to filter it to only include those rows that correspond to Newark in the first 15 days of January.
...
This is very similar to the previous use of the `filter` command in Chapter \@ref(scatterplots), however we now use the `&` operator. The above selects only those rows in `weather` where `origin=="EWR" **and** `month=1` **and** `day <= 15`.
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`**  The weather data is recorded hourly. Why does the `time_hour` variable correctly identify the hour of the measurement and not the just the `hour` variable?
...
Much as with the `ggplot()` call in Chapter \@ref(geompoint), we specify the components of the Grammar of Graphics:
...
* There is only one varible being mapped in `aes()`: the single continuous variable `temp`. You don't need to compute the y-aesthetic: it gets computed automatically.
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What other things do you notice about the faceted plot above?  How does a faceted plot help us see how relationships between two variables?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What do the numbers 1-12 correspond to in the plot above?  What about 25, 50, 75, 100?

<!--
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What could be done to make the faceted plot above more readable?  (Focus on tweaking the histograms and not on making a different type of plot here.)
-->
...
While using facetted histograms can provide a way to compare distributions of a continuous variable split by groups of a categorical variable as in Chapter \@ref(facets), an alternative plot called a **boxplot** (also called a **side-by-side boxplot**) achieves the same task.  The **boxplot** uses the information provided in the **five-number summary** referred to in Appendix {#appendixA}.  It gives a way to compare this summary information across the different levels of a categorical variable.  
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What was the seventh highest airline in terms of departed flights from NYC in 2013? How can we better present the table to get this answer quickly.
...
Note how the `facet_grid` function arguments are written here.  We are wanting the names of the airports vertically and the `carrier` listed horizontally.  As you may have guessed, this argument and other _formulas_ of this sort in R are in `y ~ x` order.  

<!--
We will see more examples of this in Chapter \@ref(regress).
-->
