...
This is very similar to the previous use of the `filter` command in Section \@ref(scatterplots), however we now use the `&` operator. The above selects only those rows in `weather` where `origin=="EWR" **and** `month=1` **and** `day <= 15`.
...
<!--
**`paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Why did we use the `flights_summarized` dataframe to produce the line-graph in Figure @ref(fig:lineflights) instead of `flights` or `flights_day`?
-->
