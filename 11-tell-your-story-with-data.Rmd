...
```
```{r tell-your-story-with-data-glimpse-house_prices-2, echo=FALSE, purl=FALSE}
glimpse(house_prices)
```
...
```{r tell-your-story-with-data-select-vars-2, echo=FALSE}
house_prices |> 
  select(price, sqft_living, condition) |> 
  tidy_summary() |> 
...
```{r tell-your-story-with-data-select-vars-3}
house_prices |> 
...
```{r tell-your-story-with-data-hist-price-2, eval=FALSE}
...
```{r tell-your-story-with-data-hist-price-3, eval=FALSE}
# Before log10 transformation:
ggplot(house_prices, aes(x = sqft_living)) +
  geom_histogram(color = "white") +
...
```{r tell-your-story-with-data-16}
house_prices |> 
  count(condition)
```
...
```{r tell-your-story-with-data-20}
2.45 + 1 * log10(1900)
```
...
```{r tell-your-story-with-data-conditional-text-2, echo=FALSE, results="asis"}
if(!is_latex_output()) 
  cat("This described in [Appendix A online](https://moderndive.com/v2/appendixa).")
```

```{r tell-your-story-with-data-22}
...
```{r tell-your-story-with-data-lm-price-2, eval=FALSE}
price_interaction <- lm(log10_price ~ log10_size * condition, data = house_prices)
get_regression_table(price_interaction)
```
...
```{r tell-your-story-with-data-null-dist-2, echo=FALSE}
...
```{r tell-your-story-with-data-viz-pvalue-2, echo=FALSE, message=FALSE}
...
```{r tell-your-story-with-data-31}
...
```{r tell-your-story-with-data-35, echo=FALSE, purl=FALSE, results="asis"}
...
```{r tell-your-story-with-data-conditional-text-3, echo=FALSE, results='asis'}
if(!is_latex_output()){
