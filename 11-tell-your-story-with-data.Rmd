...
```{r house-prices-viz, echo=FALSE, message=FALSE, fig.cap="Exploratory visualizations of Seattle house prices data.", fig.height=ifelse(knitr::is_latex_output(), 4.8, 7), purl=FALSE}
...
```{r log10-price-viz, echo=FALSE, message=FALSE, fig.cap="House price before and after log10 transformation.", fig.height=ifelse(knitr::is_latex_output(), 2.3, 7), purl=FALSE}
p1 <- ggplot(house_prices, aes(x = price)) +
  geom_histogram(color = "white") +
  labs(x = "price (USD)", title = "House price: Before")
...
```
```{r log10-size-viz, echo=FALSE, message=FALSE, fig.cap="House size before and after log10 transformation.", fig.height=ifelse(knitr::is_latex_output(), 2.3, 7), purl=FALSE}
...
```{r house-price-interaction-3, echo=FALSE, message=FALSE, fig.cap="Interaction model with prediction.", fig.height=ifelse(knitr::is_latex_output(), 4.5, 7), purl=FALSE}
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Create, visual, and interpret confidence intervals using both theory-based and simulation-based approaches to mirror the hypothesis testing done in Subsection \@ref(house-prices-inference-for-regression).
