...
Unfortunately, the `geom_smooth()` function in the `ggplot2` package does not have a convenient way to plot parallel slopes models. We therefore created our own special purpose `geom_parallel_slopes()`\index{moderndive!geom\_parallel\_slopes()} and included it in the `moderndive` package. In other words, you won't find `geom_parallel_slopes()` in the `ggplot2` package, but rather the `moderndive` package. Thus if you want to be able to use it, you will have to ensure both the `ggplot2` and `moderndive` packages are loaded. 
