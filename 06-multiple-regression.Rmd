...
```{r numxcatxplot1, echo=FALSE, fig.cap="Colored scatterplot of life expectancy and fertility rate.", fig.height=ifelse(knitr::is_latex_output(), 3.2, 7), purl=FALSE, message=FALSE}
if (is_html_output()) {
  ggplot(UN_data_ch6, aes(x = life_exp, y = fert_rate, color = income)) +
    geom_point() +
    labs(x = "Life Expectancy", y = "Fertility Rate", color = "Income group") +
...
```{r numxcatx-parallel, echo=FALSE, fig.cap="Parallel slopes model of fertility rate with life expectancy and income.", fig.height=ifelse(knitr::is_latex_output(), 2.7, 7), purl=FALSE}
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What does a "parallel slopes model" imply about the relationship between the explanatory variables and the response variable?

  -   A. All explanatory variables have different effects on the response
  -   B. All explanatory variables affect the response in the same way, but with different intercepts
  -   C. There is no relationship between the explanatory variables and the response
  -   D. The model includes interaction terms
...
```{r fitted-values, echo=FALSE, fig.cap="Fitted values for two new countries.", fig.height=ifelse(knitr::is_latex_output(), 4.7, 7), purl=FALSE, message=FALSE}
newpoints <- 
  get_regression_points(model_int, newdata = slice(UN_data_ch6, c(41, 102)))
...
```{r 2numxplot1, echo=FALSE, fig.cap="Relationship between credit card debt and credit limit/income.", fig.height=ifelse(knitr::is_latex_output(), 3.2, 7), purl=FALSE, message=FALSE}
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Which of the following statements best describes the interpretation of a regression coefficient in a multiple regression model?
  ...
  -   A. It represents the average change in the response variable per unit increase in the corresponding explanatory variable, keeping all other explanatory variables constant
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What is the significance of the "best-fitting plane" in a multiple regression model with two numerical explanatory variables?
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Why is it important to check for multicollinearity in a multiple regression model?

  -   A. Multicollinearity can inflate the variance of coefficient estimates
  -   B. Multicollinearity always improves the model's predictive power
  -   C. Multicollinearity has no effect on the model's coefficients
  -   D. Multicollinearity is only important in simple linear regression

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What does the term "partial slope" refer to in a multiple regression model?

-   A. The slope of an explanatory variable's effect on the response variable, holding other variables constant
