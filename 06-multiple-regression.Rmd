...
── Variable type: numeric
 ...
 score                 0             1  4.17 0.544  2.3  3.8  4.3  4.6    5
 age                   0             1 48.4  9.80  29   42   48   57     73
```

Observe that we have no missing data, that there are `r evals_ch6 %>% filter(gender == "male") %>% nrow()` courses taught by male instructors and `r evals_ch6 %>% filter(gender == "female") %>% nrow()` courses taught by female instructors, and that the average instructor age is `r evals_ch6$age %>% mean() %>% round(digits = 1)`. Recall that each row represents a particular course and that the same instructor often teaches more than one course. Therefore, the average age of the unique instructors may differ. 
...
2 credit_limit          0             1 4736.  2308.  855   3088   4622.  5873.  13913 
3 income                0             1   45.2   35.2  10.4   21.0   33.1   57.5   187.
```
