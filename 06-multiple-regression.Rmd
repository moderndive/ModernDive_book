...
```{r multiple-regression-1, include=FALSE, purl=FALSE}
...
```{r multiple-regression-2, eval=FALSE}
library(tidyverse)
library(moderndive)
library(ISLR2)
```

```{r multiple-regression-3, echo=FALSE, message=FALSE, purl=TRUE}
library(tidyverse)
library(moderndive)
library(ISLR2)
```

```{r multiple-regression-4, message=FALSE, echo=FALSE, purl=FALSE}
...
```{r multiple-regression-7, echo=FALSE, purl=FALSE}
...
```{r multiple-regression-9, eval=FALSE}
UN_data_ch6 |> 
  select(life_exp, fert_rate, income) |> 
  tidy_summary()
```

```{r multiple-regression-10, echo=FALSE}
UN_data_ch6 |> 
  select(life_exp, fert_rate, income) |> 
  tidy_summary() |> 
  kbl() |>
...
```{r multiple-regression-12, eval=FALSE}
...
```{r multiple-regression-13, eval=FALSE}
...
```{r multiple-regression-14, echo=FALSE, purl=FALSE}
...
```{r multiple-regression-15, eval=FALSE}
...
```{r multiple-regression-16, eval=FALSE}
ggplot(UN_data_ch6, aes(x = life_exp, y = fert_rate, color = income)) +
  geom_point() +
  labs(x = "Life expectancy", y = "Fertility rate", color = "Income group") +
  geom_parallel_slopes(se = FALSE)
...
```{r multiple-regression-17, eval=FALSE}
# Fit regression model:
...
```{r multiple-regression-18, eval=FALSE}
...
```{r multiple-regression-19, message=FALSE}
library(ISLR2)
...
```{r multiple-regression-20}
glimpse(credit_ch6)
```

```{r multiple-regression-21, echo=FALSE, purl=FALSE}
...
```{r multiple-regression-23, eval=FALSE}
...
```{r multiple-regression-24, eval=FALSE}
...
```{r multiple-regression-26, eval=FALSE}
...
```{r multiple-regression-28, eval=FALSE, echo=FALSE, purl=FALSE}
...
```{r multiple-regression-30, eval=FALSE}
...
```{r multiple-regression-31, eval=FALSE}
# Fit regression model and get the coefficients of the model
simple_model <- lm(debt ~ income, data = credit_ch6)
coef(simple_model)
```

```{r multiple-regression-32, echo=FALSE, purl=FALSE}
# Fit regression model:
...
```{r multiple-regression-33, eval=FALSE}
...
```{r multiple-regression-34, echo=FALSE, results="asis", purl=FALSE}
if (is_latex_output()) {
