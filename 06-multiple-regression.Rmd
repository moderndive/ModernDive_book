...
variable is taking into account to explain changes in the outcome
...
We continue using the UN member states data set introduced in Section
...
We also display a random sample of 5 rows of the `r n_UN_data_ch6` rows
corresponding to different countries in Table
\@ref(tab:model4-data-preview). Remember due to the random nature of the
sampling, you will likely end up with a different subset of 5 rows.
...
analyses in Sections \@ref(model1EDA) and \@ref(model2EDA) from the
previous chapter, we obtain summary statistics:
...
summary, the mean, and the standard deviation. For example, the first
...
When a variable in the data set is categorical, also called a `factor`,
...
data set has 56 UN members states that are considered `High Income`
states.
...
data set, we can include the $i$th subscript:
...
We obtain the regression coefficients using the `lm()` function and the
command `coef()` to obtain the coefficients of the linear regression:
...
variables in the model and each is equal to one or zero depending the
...
modify the slope for different levels of income. For example, for a
...
regression for the UN member state data set in R. We first "fit" the
model using the `lm()` "linear model" function and then obtain the
coefficients using the function `coef()`. In R, the formula used is
`y ~ x1 + x2 + x1:x2` where `x1` and `x2` are the variable names in the
data set and represent the main effects while `x1:x2` is the interaction
...
make sure the data set format is appropriate for the regression we want
...
We can match the coefficients with the values obtained: the fitted
...
appropriate line. The interpretation of the slope is the following: for
...
groups are obtained by taking into account the appropriate offsets. As
an illustration, for `High income` countries $D_4 = 1$ and all other
...
$b_0 + b_{04}$ =
$`(Intercept) + incomeHigh income` = `r b_int[[1]]` +(`r b_int[[4]]`) = `r b_int[[1]]+b_int[[4]]`$,
and the slope for these `High income` countries is
`life_exp + life_exp:incomeHigh income`
...
units. The intercepts and slopes for other income levels are obtained
similarly.
...
the same slope: they are parallel\index{parallel slopes model} as shown
in Figure \@ref(fig:numxcatx-parallel).

To plot parallel slopes we use the function
`geom_parallel_slopes()`\index{moderndive!geom\_parallel\_slopes()} that
is included in the `moderndive` package. To use this function you need
...
To obtain the coefficients for this regression in R, the formula used is
`y ~ x1 + x2` where `x1` and `x2` are the variable names in the data set
...
The intercepts for other income levels can be obtained similarly. We
...
The coefficients for this model were obtained earlier, saved in
`model_int`, and shown below:
...
We can use these coefficient to obtain the fitted values and residuals
for any given observation. As an illustration, we chose two observations
from the UN member states data set, provided the values for the
...
response determined by the regression line. This value is obtained by
...
countries in the `UN_data_ch6` data set, as shown in Table
...
variables. To illustrate this situation we use the `Credit` data set
from the `ISLR2` R package. This data set contains simulated information
for 400 customers. For the regression model we use the credit card
balance (`Balance`) as the response variable; and the credit limit
...
frame\index{R packages!ISLR2!Credit data frame} and construct a new data
frame `credit_ch6` with only the variables needed. We do this by using
the `select()` verb as we did in Subsection \@ref(select) and, in
addition, we save the selecting variables with different names:
`Balance` becomes `debt`, `Limit` becomes `credit_limit`, and `Income`
becomes `income`. Here is the code:
...
You can observe the effect of our use of `select()` but looking at the
...
this code a different subset of five rows is obtained.
...
two explanatory variables using the R code below. There plots are shown
...
association between income an credit card debt, but this association
...
variables appear to be linear or close to linear, we can also obtain the
...
obtain all correlations simultaneously by returning a *correlation
...
The plots in Figure \@ref(fig:2numxplot1) and the corresponding
the response and each of the explanatory variables *separately*. In
...
website](https://moderndive.com/regression-plane-ISLR2) to open an
...
a formula of the form `y ~ x1 + x2`. We obtain the regression
coefficients using the `lm()` function and the command `coef()` to
obtain the coefficients of the linear regression. The regression
...
slope of income is positive. We can obtain the value of this slope by
...
plane from the regression table by applying the `get_regression_table()`
function. How do the regression results match up with the results from
...
regression plane obtained in the previous section:
...
future chapters we learn how we can use information obtained from a
