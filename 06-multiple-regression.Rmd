...
First, the variance of $y$ (denote this $var(y)$) tells us how much do Massachusetts high schools differ in average math SAT scores. The goal of regression modeling is to fit a model that hopefully *explains* this variation. In other words, what factors explain why certain schools have high math SAT scores, while others have low scores. This is independent of the model; this is just data. In other words, whether we fit an interaction or parallel slopes model, $var(y)$ remains the same. 

Second, the variance of $\widehat{y}$ (denote this $var(\widehat{y})$) tells us how much the fitted values from our interaction model vary. In other words, after account for the percentage of the study body that is socieconomically disadvanted and school size in an interaction model, how much do our model's explanations of average math SAT scores vary?
