...
```{r numxcatxplot1, echo=FALSE, fig.cap="Colored scatterplot of relationship of life expectancy and fertility rate.", fig.height=3.2, purl=FALSE, message=FALSE}
if (is_html_output()) {
  ggplot(UN_data_ch6, aes(x = life_exp, y = fert_rate, color = income)) +
    geom_point() +
    labs(x = "Life Expectancy", y = "Fertility Rate", color = "Income group") +
...
```{r numxcatx-parallel, echo=FALSE, fig.cap="Parallel slopes model of fertility rate with life expectancy and income group.", fig.height=3.5, purl=FALSE}
...
We can use these coefficients to obtain the fitted values and residuals
...
```{r fitted-values, echo=FALSE, fig.cap="Fitted values for two new countries.", fig.height=4.7, purl=FALSE, message=FALSE}
newpoints <- 
  get_regression_points(model_int, newdata = slice(UN_data_ch6, c(41, 102)))
...
variables. To illustrate this situation we explore the `ISLR2` R package for the first time in this book using its `Credit` data set. This data set contains 
