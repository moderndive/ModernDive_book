...
```{r multiple-regression-load-packages-2, echo=FALSE, message=FALSE, purl=TRUE}
library(tidyverse)
library(moderndive)
library(ISLR2)
```
...
```{r multiple-regression-8, eval=FALSE}
...
```{r multiple-regression-select-vars-2, echo=FALSE}
UN_data_ch6 |> 
  select(life_exp, fert_rate, income) |> 
  tidy_summary() |> 
  kbl() |>
...
```{r multiple-regression-lm-fertility-2, echo=FALSE, purl=FALSE}
...
```{r multiple-regression-lm-fertility-3, eval=FALSE}
...
```{r multiple-regression-scatter-lifeexp-2, eval=FALSE}
ggplot(UN_data_ch6, aes(x = life_exp, y = fert_rate, color = income)) +
  geom_point() +
  labs(x = "Life expectancy", y = "Fertility rate", color = "Income group") +
  geom_parallel_slopes(se = FALSE)
...
```{r multiple-regression-lm-fertility-4, eval=FALSE}
# Fit regression model:
...
```{r multiple-regression-22, eval=FALSE}
...
```{r multiple-regression-select-vars-3, eval=FALSE}
...
```{r multiple-regression-25, eval=FALSE}
...
```{r multiple-regression-select-vars-4, eval=FALSE}
...
```{r multiple-regression-load-packages-3, eval=FALSE, echo=FALSE, purl=FALSE}
...
```{r multiple-regression-29, echo=FALSE, eval=FALSE, purl=FALSE}
...
```{r multiple-regression-fit-lm-2, eval=FALSE}
# Fit regression model and get the coefficients of the model
simple_model <- lm(debt ~ income, data = credit_ch6)
coef(simple_model)
```

```{r multiple-regression-fit-lm-3, echo=FALSE, purl=FALSE}
# Fit regression model:
...
```{r multiple-regression-reg-points-2, eval=FALSE}
...
```{r multiple-regression-35, echo=FALSE, purl=FALSE, results="asis"}
