...
### Model fit and assumptions (LINE) {.unnumbered}

**EX10.5 (★★)** What four assumptions does `lm()` inference require? (acronym L.I.N.E.)

**EX10.6 (★★★)** Build the four diagnostic plots for `m_b` and note any concerns.
...
### Theory-based inference for multiple regression {.unnumbered}
...
### Simulation-based inference for multiple regression {.unnumbered}
...
**EX10.19 (★★★)** Plot regression line with 95% CI band.

```{webr-r}
# your code here
```

**EX10.20 (★★★)** Confidence band vs prediction band — which is wider, why?

**EX10.21 (★★★)** Parallel-slopes lines with CI bands on `pl2`.
...
**EX10.30 (★★★)** Extract R², adjusted R², F-statistic. What does F test?

```{webr-r}
# your code here
```

**EX10.31 (★★★)** Why does adjusted R² *decrease* when adding a useless predictor?

**EX10.32 (★★★)** Try to demonstrate Simpson's paradox in `planets` (slope flips sign when grouping is collapsed).
