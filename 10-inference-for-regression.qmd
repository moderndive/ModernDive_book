...
```{r inference-for-regression-viz-dist-alt, echo=FALSE}
...
```{r inference-for-regression-viz-pvalue-alt, echo=FALSE}
...
a. Linearity
b. Independence
c. **Equal variance (homoscedasticity)** — the "E" in LINE
d. Normality

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Funnel-shaped residual spread = unequal variance. Predictions become less reliable for higher-fitted-value observations. Common remedy: a log or square-root transformation of the response.
:::
...
b. **Residuals are not normal — heavy tails**
c. Equal variance is violated
d. Independence is violated

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Tails diverging from the diagonal = the empirical distribution has thicker tails than a normal distribution. Mild deviations are usually fine; extreme ones can affect confidence intervals.
:::
...
b. QQ plot
c. **A plot of residuals over time/order if observations are sequential** (looking for patterns or autocorrelation)
d. Histogram of $y$

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Independence is largely about study design (random sampling, no clustered observations). When data has natural ordering (time, location), plotting residuals in that order can reveal autocorrelation — a signal of non-independence.
:::
...
### Theory-based inference for simple regression {.unnumbered}
...
### Simulation-based inference for simple regression {.unnumbered}

**EX10.5 (★★★)** Bootstrap distribution of the slope.
...
**EX10.6 (★★★)** Bootstrap percentile 95% CI; compare to theory-based.

```{webr-r}
# your code here
```

**EX10.7 (★★★)** Permutation test of $H_0$: slope = 0.
...
**EX10.8 (★★★)** Visualize the null + p-value shading.

```{webr-r}
# your code here
```

**EX10.9 (★★★)** Why do simulation-based and theory-based p-values agree here?

### Model assumptions {.unnumbered}

**EX10.10 (★★)** What four assumptions does `lm()` inference require? (acronym L.I.N.E.)

**EX10.11 (★★★)** Build the four diagnostic plots for `m_b` and note any concerns.

```{webr-r}
par(mfrow = c(2, 2)); plot(m_b); par(mfrow = c(1, 1))
```

**EX10.12 (★★★)** "I trust the t-test more than the bootstrap because it has a closed form." Push back.

### Inference on multiple regression {.unnumbered}
...
**EX10.14 (★★★)** Interpret the 95% CI for `log_radius`.
...
**EX10.17 (★★★)** Bootstrap CI for `log_radius` coefficient; compare to theory.

```{webr-r}
# your code here
```

### Practical questions {.unnumbered}

**EX10.18 (★★★)** Test $H_0$: slope = 0 in `lm(Height ~ Year, data = bball)`.

```{webr-r}
# your code here
```

**EX10.19 (★★★)** Reader concludes "Olympic basketball trains taller players." Refute.

**EX10.20 (★★★)** Repeat EX10.18 for gymnastics. Compare.

```{webr-r}
# your code here
```

**EX10.21 (★★★)** What's "the population" in regression here?

### Visualizing uncertainty {.unnumbered}

**EX10.22 (★★★)** Plot regression line with 95% CI band.

```{webr-r}
# your code here
```

**EX10.23 (★★★)** Confidence band vs prediction band — which is wider, why?

**EX10.24 (★★★)** Parallel-slopes lines with CI bands on `pl2`.
...
**EX10.25 (★★★)** With $n \approx 1000$ tiny p-values are easy. Does that mean every effect is meaningful?

**EX10.26 (★★★)** Why does removing `Sport` change the `Sex` coefficient on Height?

**EX10.27 (★★★)** Demonstrate this for two specific sports.

```{webr-r}
# your code here
```

**EX10.28 (★★★)** CI widths at $n \in \{30, 100, \text{full}\}$.

```{webr-r}
# your code here
```

**EX10.29 (★★★)** Extract R², adjusted R², F-statistic. What does F test?

```{webr-r}
# your code here
```

**EX10.30 (★★★)** Why does adjusted R² *decrease* when adding a useless predictor?

**EX10.31 (★★★)** Try to demonstrate Simpson's paradox in `planets` (slope flips sign when grouping is collapsed).

```{webr-r}
# your code here
```

**EX10.32 (★★★)** R² = 0.95 but diagnostics show curvature — trust the inference?

**EX10.33 (★★★)** CI for slope is `(0.001, 0.005)`. Above 0 but tiny — takeaway?

**EX10.34 (★★★)** Open exploration: pick a numerical pair, fit and run inference.

```{webr-r}
# your exploration here
```

**EX10.35 (★★★)** Build a multiple regression on `planets` predicting `eq_temp_k` from ≥2 predictors and write a 2–3 sentence summary of *robustly supported* findings.
