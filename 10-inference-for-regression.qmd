...
a. The probability that the slope is positive
b. The R-squared of the model
c. The exact value of the slope
d. A range of plausible values for the *population* slope, based on the sample
...
a. The sample is very small
b. The data is non-normal
c. The model has many predictors
d. The LINE conditions are met and the sample is reasonably large
...
a. The slope is significant at any $\alpha$
b. **There is strong evidence that the population slope is not zero — but our estimate of the slope (0.5) still carries uncertainty (use a CI to quantify it)**
c. The slope is exactly 0.5 in the population
d. The relationship is causal

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** The point estimate is 0.5, but the population slope could plausibly be anywhere in the slope's CI. The p-value tests whether 0 is plausible, not whether 0.5 is correct.
:::
...
a. Linearity
b. Independence
c. **Equal variance** — the "E" in LINE
d. Normality

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Funnel-shaped residual spread = unequal variance. The "equal variance" assumption requires residuals to have roughly the SAME spread across the range of fitted values. When that fails, the SEs reported by `lm()` (which assume equal variance) are unreliable, and the resulting CIs/tests can be miscalibrated. The simulation-based workflow from §10.3 (bootstrap CIs, permutation tests via `infer`) doesn't lean on equal variance and is a good fallback.
:::

**Q10-6.** A QQ-plot of residuals shows points sharply deviating from the diagonal at both tails. This suggests:

a. **The residuals are not Normal — at the extremes, observed residuals are more extreme than a Normal distribution would predict**
b. Independence is violated
c. Linearity is violated
...
b. QQ plot
c. Histogram of $y$
d. **A plot of residuals against the order in which observations were collected** (looking for repeating patterns or trends across that order)
...
a. $p > 0.05$
b. **$p < 0.05$, because 0 is NOT in the CI**
c. $p = 0.5$
d. Cannot tell

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Two equivalent ways of saying the same thing. If a 95% CI excludes a value, then a two-sided test of "is the parameter equal to that value" rejects at $\alpha = 0.05$.
:::
...
a. The slope you'd get from `lm(total_cup_points ~ aroma)` alone
b. The correlation between `aroma` and `total_cup_points`
c. **The expected change in `total_cup_points` for a one-unit increase in `aroma`, *holding the other predictors (flavor, moisture, continent) fixed***
d. The fraction of variance in `total_cup_points` explained by `aroma`
...
a. **The data don't provide strong evidence that `moisture_percentage` has a non-zero partial effect on `total_cup_points` once `aroma`, `flavor`, and `continent_of_origin` are in the model — its partial slope is consistent with 0**
b. The bootstrap is unreliable for partial slopes
c. `moisture_percentage` and `aroma` are equally important predictors
d. `moisture_percentage` causes no change in cup points
