...
```{r inference-for-regression-fit-lm-alt}
# Fit regression model:
model_1 <- lm(waiting ~ duration, data = old_faithful_2024)
...
```{r inference-for-regression-scatter, eval=FALSE}
...
In @fig-scatt-and-residual we show this residual plot (right plot) alongside the scatterplot for `duration` vs `waiting` (left plot). 
...
```{r fig-scatt-and-residual, fig.alt="Two-panel plot for the Old Faithful data: scatterplot with regression line (left), and corresponding residuals-vs-fitted plot (right).", echo=FALSE, fig.cap="The scatterplot and residual plot for the Old Faithful data.", purl=FALSE, message=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2.3, 4)}
...
##### Linearity of relationship {.unnumbered}
...
This is largely the case for the Old Faithful geyser example with the residuals against the fitted values (`waiting_hat`) shown in the right-plot of @fig-scatt-and-residual. 
...
By contrast, assume now that the scatterplot of `waiting` against `duration` and its associated residual plot are shown in @fig-non-linear. 
...
```{r fig-non-linear, fig.alt="Scatterplot showing data with a clear curved (non-linear) relationship; a straight regression line would not fit well.", echo=FALSE, fig.cap="Example of a non-linear relationship.", fig.height=ifelse(knitr::is_latex_output(), 3.3, 4), message=FALSE, purl=FALSE}
set.seed(76)
...
##### Independence of the error terms and the response {.unnumbered}
...
We show the plot of `residuals` against `date` (time) in @fig-time-plot:

```{r fig-time-plot, fig.alt="Scatterplot of date (x-axis) versus residuals (y-axis) for the Old Faithful data, used to check the Independence condition by looking for time-based patterns.", echo=FALSE, fig.cap="Scatterplot of date (time) vs residuals for the Old Faithful example.", fig.height=ifelse(knitr::is_latex_output(), 3, 4), message=FALSE, purl=FALSE}
old_faithful_2024 |>
...
As discussed in @sec-sample-regression-inference, we can consider this a random sample.
...
##### Normality of the error terms {.unnumbered}
...
Using the residuals as a rough estimate of the error term values, we have seen in the right plot of @fig-scatt-and-residual that sometimes the residuals are positive and other times negative. 
...
```{r inference-for-regression-hist, eval=FALSE}
...
```{r inference-for-regression-plot, eval = FALSE}
fitted_and_residuals |>
...
In @fig-model1residualshist we include both the histogram of the residuals including a normal curve for comparison (left plot) and a QQ-plot (right plot):

```{r fig-model1residualshist, fig.alt="Histogram of regression residuals, roughly symmetric and bell-shaped, supporting the Normality condition.", echo=FALSE, warning=FALSE, fig.cap="Histogram of residuals."}
...
The histogram of the residuals shown in @fig-model1residualshist (left plot) does not appear exactly normal as there are some deviations, such as the highest bin value appearing just to the right of the center. 
...
We compare these results with residuals found by simulation that do not appear to follow normality as shown in @fig-not-normal-residuals.
...
```{r fig-not-normal-residuals, fig.alt="Histogram of regression residuals showing clear right skew, illustrating a violation of the Normality condition.", echo=FALSE, fig.cap="Non-normal residuals.", message=FALSE, purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 1.4, 4)}
set.seed(3)
...
##### Equality or constancy of variance for errors {.unnumbered}
...
In @fig-scatt-and-residual, we showed the scatterplot of residuals against fitted values (right plot). 
We can also produce the scatterplot of residuals against the regressor `duration` in @fig-residual-plot:

```{r fig-residual-plot, fig.alt="Residuals (y-axis) plotted against the regressor (x-axis). A random scatter around zero supports the regression assumptions.", fig.cap="Plot of residuals against the regressor.", message=FALSE, fig.height=ifelse(knitr::is_latex_output(), 1.5, 4)}
...
In @fig-equal-variance-residuals we present an example using simulated data with non-constant variance.

```{r fig-equal-variance-residuals, fig.alt="Residual plot showing a clear funnel shape: residuals fan out as the fitted values increase, illustrating a violation of the Equal Variance (homoscedasticity) condition.", echo=FALSE, fig.cap="Example of clearly non-equal variance.", purl=FALSE, fig.height=ifelse(is_latex_output(), 2, 4)}
old_faithful_2024 |>
...
##### What is the conclusion? {#sec-what-is-the-conclusion .unnumbered}
...
```{r inference-for-regression-conditional-text, echo=FALSE, results="asis"}
if(!is_latex_output()) 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Use the `un_member_states_2024` data frame included in the `moderndive` package with response variable fertility rate (`fertility_rate_2022`) and the regressor human development index (`hdi_2022`). Make sure to omit missing values in `un_member_states_2024`.
...
```{r lc-sol-10-07, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 7))
```
...
```{r lc-sol-10-08, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 8))
```
...
```{r lc-sol-10-09, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 9))
```
...
```{r lc-sol-10-10, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 10))
```

:::

## Simulation-based inference for simple linear regression {#sec-infer-regression}

In this section, we'll use the simulation-based methods you previously learned in @sec-confidence-intervals and @sec-hypothesis-testing to recreate the values in the regression table. 
In particular, we'll use the `infer` package workflow to

-   Construct a 95% confidence interval for the population slope $\beta_1$ using bootstrap resampling with replacement. We did this previously in @sec-bootstrap-process with the almonds data and @sec-case-study-two-prop-ci with the `mythbusters_yawn` data.
-   Conduct a hypothesis test of $H_0: \beta_1 = 0$ versus $H_A: \beta_1 \neq 0$ using a permutation test. We did this previously in @sec-ht-infer with the `spotify_sample` data and @sec-ht-case-study with the `movies_sample` IMDb data.

### Confidence intervals for the population slope using `infer` {#sec-infer-ci-slr}

```{r inference-for-regression-create-n_reps, echo=FALSE}
n_reps <- 1000
```

We'll construct a 95% confidence interval for $\beta_1$ using the `infer` workflow outlined in @sec-infer-workflow. Specifically, we'll first construct the bootstrap distribution for the fitted slope $b_1$ using our single sample of `r n_old_faithful` eruptions:

1.  `specify()` the variables of interest in `old_faithful_2024` with the formula: `waiting ~ duration`.
2.  `generate()` replicates by using `bootstrap` resampling with replacement from the original sample of `r n_old_faithful` eruptions. We generate `` reps = `r n_reps` `` replicates using `type = "bootstrap"`.
...
```{r inference-for-regression-bootstrap, eval=FALSE}
...
```{r inference-for-regression-bootstrap-alt, echo=FALSE, purl=FALSE}
...
Observe how we have `r n_reps` values of the bootstrapped slope $b_1$ in the `stat` column. Let's visualize the `r n_reps` bootstrapped values in @fig-bootstrap-distribution-slope.

```{r fig-bootstrap-distribution-slope, fig.alt="Histogram of bootstrapped sample slopes for a regression model. Roughly bell-shaped and centered at the observed slope.", fig.cap="Bootstrap distribution of slope.", fig.height=ifelse(knitr::is_latex_output(), 2.2, 4)}
visualize(bootstrap_distn_slope)
```

Observe how the bootstrap distribution is roughly bell-shaped. Recall from @sec-revisit-almond-bootstrap that the shape of the bootstrap distribution of $b_1$ closely approximates the shape of the sampling distribution of $b_1$.
...
```{r inference-for-regression-conf-interval}
percentile_ci <- bootstrap_distn_slope |> 
  get_confidence_interval(type = "percentile", level = 0.95)
percentile_ci
```

The resulting percentile-based 95% confidence interval for $\beta_1$ is (`r percentile_ci[[1]]`, `r percentile_ci[[2]]`).

#### Standard error method {.unnumbered}

Since the bootstrap distribution in @fig-bootstrap-distribution-slope appears to be roughly bell-shaped, we can also construct a 95% confidence interval for $\beta_1$ using the standard error method.

In order to do this, we need to first compute the fitted slope $b_1$, which will act as the center of our standard error-based confidence interval. While we saw in the regression table in @tbl-simple-model-part-deux that this was $b_1$ = `r b1`, we can also use the `infer` pipeline with the `generate()` step removed to calculate it:

```{r inference-for-regression-specify}
...
```{r inference-for-regression-assign-se_ci}
...
### Hypothesis test for population slope using `infer` {#sec-infer-hypo-slr}

Let's now conduct a hypothesis test of $H_0: \beta_1 = 0$ vs. $H_A: \beta_1 \neq 0$. We will use the `infer` package, which follows the hypothesis testing paradigm in the "There is only one test" diagram in @fig-htdowney.
...
We construct the null distribution of the fitted slope $b_1$ by performing the steps that follow. Recall from @sec-understanding-ht on terminology, notation, and definitions related to hypothesis testing where we defined the *null distribution*: the sampling distribution of our test statistic $b_1$ assuming the null hypothesis $H_0$ is true.

1.  `specify()` the variables of interest in `old_faithful_2024` with the formula: `waiting ~ duration`.
2.  `hypothesize()` the null hypothesis of `independence`. Recall from @sec-ht-infer that this is an additional step that needs to be added for hypothesis testing.
...
```{r inference-for-regression-null-dist, eval=FALSE}
...
```{r inference-for-regression-null-dist-sized, echo=FALSE, purl=FALSE}
...
Observe the resulting null distribution for the fitted slope $b_1$ in @fig-null-distribution-slope.

```{r fig-null-distribution-slope, fig.alt="Histogram of null-distribution slopes generated by permutation under the null of zero relationship. Centered at zero.", echo=FALSE, fig.cap="Null distribution of slopes.", fig.height=ifelse(knitr::is_latex_output(), 2, 4), purl=FALSE}
...
```{r inference-for-regression-specify-alt}
# Observed slope
...
Let's visualize the $p$-value in the null distribution by comparing it to the observed test statistic of $b_1$ = `r b1` in @fig-p-value-slope. We'll do this by adding a `shade_p_value()` layer to the previous `visualize()` code.

```{r fig-p-value-slope, fig.alt="Same null distribution with the area beyond the observed slope shaded, that area is the p-value for testing H0: slope equals zero.", echo=FALSE, fig.cap="Null distribution and $p$-value.", fig.height=ifelse(knitr::is_latex_output(), 3.8, 4), purl=FALSE}
visualize(null_distn_slope) +
...
```{r inference-for-regression-alt}
null_distn_slope |> 
...
```{r lc-sol-10-11, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 11))
```
...
```{r lc-sol-10-12, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 12))
```
...
```{r lc-sol-10-13, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 13))
```
...
```{r lc-sol-10-14, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 14))
```
...
### The model {#sec-multiple-linear-model}
...
We plan to regress `total_cup_points` (response variable) on the numerical explanatory variables `aroma`, `flavor`, and `moisture_percentage` and the categorical explanatory variable `continent_of_origin` with four categories: `Africa`, `Asia`, `North America`, and `South America`. 
...
```{r inference-for-regression-create-coffee_data}
...
```{r inference-for-regression-alt2}
coffee_data
```

By looking at the fourth row we can tell, for example, that the `total_cup_points` are `r coffee_data[4,"total_cup_points"][[1]]` with `aroma` score equal to `r coffee_data[4,"aroma"][[1]]` points, `flavor` score equal to `r coffee_data[4,"flavor"][[1]]` points, `moisture_percentage` equal to `r coffee_data[4,"moisture_percentage"][[1]]`%, and `r as.character(coffee_data[4,"continent_of_origin"][[1]])` is the `continent_of_origin`. 
We also display the summary for these variables in @tbl-coffee-tidy-summary:

```{r inference-for-regression-demo-code-v2-dup4, eval=FALSE}
coffee_data |>
  tidy_summary()
```

```{r tbl-coffee-tidy-summary, echo=FALSE}
coffee_data |>
  tidy_summary()  |> 
  kbl(
    digits = 3,
...
```{r inference-for-regression-dynamic-text-alt2, echo=FALSE}
# This code is used for dynamic non-static in-line text output purposes
...
```{r fig-coffee-scatter-matrix, fig.alt="Scatterplot matrix (pairs plot) showing all pairwise relationships between the coffee dataset variables, with histograms on the diagonal.", fig.cap="Scatterplot matrix for coffee variables of interest.", fig.height=8.5, purl=FALSE, message=FALSE}
coffee_data |>
  ggpairs()
```
...
```{r inference-for-regression-create-corr_table, echo=FALSE}
# This code is used for dynamic non-static in-line text output purposes
...
### Least squares for multiple regression {#sec-least-squares-multiple}
...
To introduce the factor levels in the linear model, we represent the factor levels using dummy variables as described in @sec-model4interactiontable. 
...
As we did in the simple linear case, we assume that linearity between the response and the regressors holds and apply the linear model described in @sec-multiple-linear-model to each observation in the sample. 
...
The solutions to this problem are reached by using matrices and matrix calculus, and are the regression coefficients introduced in @sec-multiple-regression and @sec-model4interactiontable. 
...
For example, if we choose $b_1$ (the estimator of the partial slope for `aroma`, $\beta_1$), then the expected value is equal to the parameter, $E(b_1) = \beta_1$. 
...
For $b_3$, for example, there are known constants $c_1$, $c_2$, $\dots$, $c_n$ such that $b_3 = \sum_{i=1}^n c_iy_i$.

When using the `lm()` function, all the necessary calculations are done in R. In @tbl-regtable-coffee, for the coffee example, we get:

```{r inference-for-regression-fit-lm-alt2, eval=FALSE}
# Fit regression model:
mod_mult <- lm(
...
```{r tbl-regtable-coffee, echo=FALSE, purl=FALSE}
# Fit regression model:
mod_mult <- lm(
  total_cup_points ~ aroma + flavor + moisture_percentage + continent_of_origin, 
  data = coffee_data)
...
If the linear model is appropriate, we can interpret these coefficients as we did in @sec-model4interactiontable and @sec-model3table. 
...
In addition, observe that `mod_mult` is a model without interactions, similar to the model described in @sec-model4table. The coefficients for factor levels of `continent_of_origin` ($b_{02}$, $b_{03}$, and $b_{04}$) affect only the intercept of the model, based on the category of the observation in question. 
Recall that the factor levels, in order, are `Africa`, `Asia`, `North America`, and `South America`. 
For example, if the fifth observation's continent of origin is South America ($D_{54} = 1$ and $D_{52} = D_{53} = 0$), the regression formula is given by
...
```{r inference-for-regression-select-vars-alt2, eval=FALSE}
coffee_data |>
...
```{r inference-for-regression-select-vars-alt2-dup1, echo=FALSE, purl=FALSE}
...
We discuss in @sec-model-fit-mult how we could determine what is the best subset of regressors to use when many are available. 
...
```{r lc-sol-10-15, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 15))
```
...
```{r lc-sol-10-16, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 16))
```
...
```{r lc-sol-10-17, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 17))
```
...
```{r lc-sol-10-18, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 18))
```

:::

## Theory-based inference for multiple linear regression {#sec-theory-multiple-regression}
...
We illustrate this framework using the coffee example and the R function [`get_regression_table()`](https://moderndive.github.io/moderndive/reference/get_regression_table.html) introduced in @sec-regression-table.
...
The standard errors are shown when applying the [`get_regression_table()`](https://moderndive.github.io/moderndive/reference/get_regression_table.html) function on a regression model. The output for the model `mod_mult` is in @tbl-mult-model-1:


```{r inference-for-regression-demo-code-v2-dup5, eval=FALSE}
get_regression_table(mod_mult)
```

```{r tbl-mult-model-1, echo=FALSE, purl=FALSE}
...
### Model dependency of estimators {#sec-model-dependency}
...
where $\beta_1$, $\beta_2$, and $\beta_3$, are the parameters for the partial slopes of `aroma`, `flavor`, and `moisture_percentage`, respectively. 
Recall that we assume that the parameters $\beta_1$, $\beta_2$, and $\beta_3$ are constants, unknown to us, but constants. 
We use multiple linear regression and find the least-square estimates $b_1$, $b_2$, and $b_3$, respectively. The results using the `coffee_data` are calculated and stored in the object `mod_mult_1` and shown in @tbl-regtable-coffee-mult-1:

```{r inference-for-regression-fit-lm-alt2-dup1, eval=FALSE}
# Fit regression model:
mod_mult_1 <- lm(
...
```{r tbl-regtable-coffee-mult-1, echo=FALSE, purl=FALSE}
# Fit regression model:
mod_mult_1 <- lm(
  total_cup_points ~ aroma + flavor + moisture_percentage, 
  data = coffee_data)
...
The results using the `coffee_data` are calculated and stored in the object `mod_mult_2` and shown in @tbl-regtable-coffee-mult-2:

```{r inference-for-regression-fit-lm-alt2-dup2, eval=FALSE}
# Fit regression model:
mod_mult_2 <- lm(
...
```{r tbl-regtable-coffee-mult-2, echo=FALSE, purl=FALSE}
# Fit regression model:
mod_mult_2 <- lm(
  total_cup_points ~ aroma + moisture_percentage, data = coffee_data)
# Get the coefficients of the model
...
We discuss methods addressing this comparison in @sec-hypo-test-mult-lm. 
...
```{r inference-for-regression-dynamic-text-alt2-dup1, echo=FALSE}
# This code is used for dynamic non-static in-line text output purposes
...
- three numerical regressors (`aroma`, `flavor`, and `moisture_percentage`), 
- one factor (`continent_of_origin`), 
- the confidence level is 95%, 
- the sample size is $n = `r n_coffee`$, 
- the number of regression coefficients is $p = `r p_mult`$: 
- one intercept ($b_0$), 
- three partial slopes for `aroma`, `flavor` and `moisture_percentage` ($b_1, b_2$, and $b_3$), and
...
The 95% confidence interval for the partial slope of `aroma`, using $b_1$, is determined by
...
This time, however, we add the argument `conf.level = 0.98` to get 98% confidence intervals as shown in @tbl-mult-model-2.

```{r inference-for-regression-reg-table-alt, eval=FALSE}
get_regression_table(mod_mult, conf.level = 0.98)
```


```{r tbl-mult-model-2, echo=FALSE, purl=FALSE}
...
### Hypothesis test for a single coefficient {#sec-hypo-test-mult-lm}
...
We emphasize that $\beta_0, \beta_2, \dots, \beta_p$ are given but arbitrary to acknowledge that the test only matters in the context of the appropriate model. 
...
```{r inference-for-regression-demo-code-v2-dup6, eval=FALSE}
get_regression_table(mod_mult_1)
```

```{r tbl-mult-model-3, echo=FALSE, purl=FALSE}
...
@tbl-mult-model-3 provides information for all the coefficients. Observe, in particular, that the test for $\beta_1$ (`aroma`) is also statistically significant but the test for $\beta_3$ (`moisture_percentage`) is not ($p$-value = `r round(mod_mult_table_1[4,5][[1]],2)`). For the latter, the conclusion is that there is not statistical evidence to reject the null hypothesis that this partial slope was zero. In other words, we have not found evidence that adding `moisture_percentage` to a model that already includes `aroma` and `flavor` helps explain changes in the response `total_cup_points`. We can remove the `moisture_percentage` regressor from the model.

### Hypothesis test for model comparison {#sec-hypo-test-mult-lm-1}
...
To get the result of this test in R, we use the R function `anova()` and enter the reduced model followed by the full model with @tbl-mult-model-4 providing information for this test.

```{r inference-for-regression-alt2-dup1, eval=FALSE}
anova(mod_mult_2, mod_mult_1) 
```

```{r tbl-mult-model-4, echo=FALSE, purl=FALSE}
...
The ANOVA test is performed as follows with the output given in @tbl-mult-model-5:

```{r inference-for-regression-demo-code-v2-dup7, eval=FALSE}
anova(mod_mult_1, mod_mult) 
```

```{r tbl-mult-model-5, echo=FALSE, purl=FALSE}
...
### Model fit and diagnostics {#sec-model-fit-mult}
...
So, we create a model only with the relevant regressors called `mod_mult_final`, determine the residuals, and create a plot of residuals against fitted values and a QQ-plot using the `grid.arrange()` function from the `gridExtra` package in @fig-grid-arrange-plot-check:

```{r inference-for-regression-fit-lm-alt2-dup3}
# Fit regression model:
...
```{r fig-grid-arrange-plot-check, fig.alt="Two diagnostic plots side by side: residuals-vs-fitted (left) and QQ-plot of residuals (right) for the multiple regression model.", fig.cap="Residuals vs. fitted values plot and QQ-plot for the multiple regression model.", fig.height=ifelse(knitr::is_latex_output(), 2, 4)}
g1 <- fit_and_res_mult |>
...
```{r lc-sol-10-19, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 19))
```
...
```{r lc-sol-10-20, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 20))
```
...
```{r lc-sol-10-21, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 21))
```
...
```{r lc-sol-10-22, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 22))
```
...
We'll now use the simulation-based methods you previously learned in @sec-confidence-intervals and @sec-hypothesis-testing to compute ranges of plausible values for partial slopes with multiple linear regression. Recall that simulation-based methods provide an alternative to the theory-based methods in that they do not rely on the assumptions of normality or large sample sizes. We'll use the `infer` package as we did with simple linear regression in @sec-infer-regression, but this time using the `fit()` function.

#### Getting the observed fitted model {.unnumbered}

We will revisit using our full model on the `coffee_data` with the factor of `continent_of_origin` and three numerical regressors in `aroma`, `flavor`, and `moisture_percentage`. As we did with hypothesis testing in @sec-hypothesis-testing and in @sec-infer-hypo-slr, we can retrieve the observed statistic. In this case, we get the observed coefficients of the model using the `specify()` function on our full model with formula syntax combined with `fit()`\index{R packages!infer!fit()}:

```{r inference-for-regression-specify-alt2}
...
As we would expect, these values match up with the values of `mod_mult_table` given in the first two columns there in @tbl-mod-mult-table-again:


```{r inference-for-regression-alt2-dup2, eval=FALSE}
mod_mult_table
```

```{r tbl-mod-mult-table-again, echo=FALSE}
mod_mult_table |> 
  kbl(
    digits = 3,
...
#### Bootstrap distribution for the partial slopes {.unnumbered}

We will now find the bootstrap distribution for the partial slopes using the `infer` workflow. Just like in @sec-infer-ci-slr, we are now resampling entire rows of values to construct the bootstrap distribution for the fitted partial slopes using our full sample of `r n_coffee` coffees:
...
```{r inference-for-regression-bootstrap-alt2, eval=FALSE}
coffee_data |>
  specify(
...
```{r inference-for-regression-bootstrap-alt2-dup1, echo=FALSE}
...
```{r inference-for-regression-bootstrap-alt2-dup2, eval=FALSE}
...
```{r inference-for-regression-alt2-dup3, echo=FALSE}
...
```{r fig-boot-distn-slopes, fig.alt="Side-by-side histograms of bootstrap distributions for the partial slopes of multiple predictors in a regression model.", echo=FALSE, out.width="68%", fig.height=12, fig.cap="Bootstrap distributions of partial slopes."}
if(is_latex_output()) {
...
#### Confidence intervals for the partial slopes {.unnumbered}

As we did in @sec-infer-ci-slr, we can construct 95% confidence intervals for the partial slopes. Here, we focus on using the percentile-based method with the `get_confidence_interval()` function and `type = "percentile"` to construct these intervals.

```{r inference-for-regression-conf-interval-alt}
...
We can also visualize these confidence intervals. This is done in @fig-ci-slopes-multiple. 

```{r fig-ci-slopes-multiple, fig.alt="Forest plot of 95% confidence intervals for the partial slope of each predictor in a multiple regression model. Each row is one predictor; horizontal segments are the CI bounds.", fig.cap="95% confidence intervals for the partial slopes.", fig.height=8.5, fig.width=6}
visualize(boot_distribution_mlr) +
...
#### Null distribution for the partial slopes {.unnumbered}
...
```{r inference-for-regression-null-dist-alt}
set.seed(2024)
...
#### Hypothesis tests for the partial slopes {.unnumbered}

We can now conduct hypothesis tests for the partial slopes in multiple linear regression. We can use the permutation test to test the null hypothesis $H_0: \beta_i = 0$ versus the alternative hypothesis $H_A: \beta_i \neq 0$ for each of the partial slopes. Let's use a significance level of $\alpha = 0.05$. We can visualize the $p$-values in the null distribution by comparing them to the observed test statistics. We do this by adding a `shade_p_value()` layer to `visualize()`. 

```{r inference-for-regression-viz-pvalue, eval=FALSE}
visualize(null_distribution_mlr) +
  shade_p_value(obs_stat = observed_fit, direction = "two-sided")
```

```{r inference-for-regression-viz-pvalue-alt, echo=FALSE, fig.show='hide'}
...
```{r fig-shaded-p-values-partial, fig.alt="Histograms showing the null distribution of each partial slope in a multiple regression, with the area beyond the observed slope shaded to indicate the p-value.", echo=FALSE, out.width="55%", fig.height=12, fig.cap="Shaded p-values for the partial slopes in this multiple regression."}
if(is_latex_output()) {
...
From these visualizations in @fig-shaded-p-values-partial, we can surmise that `aroma` and `flavor` are statistically significant, as their observed test statistics fall far to the right of the null distribution. On the other hand, `moisture_percentage` is not statistically significant, as its observed test statistic falls within the null distribution. We can also compute the numerical $p$-values using the `get_p_value()` function.

```{r inference-for-regression-alt2-dup4}
null_distribution_mlr |>
  get_p_value(obs_stat = observed_fit, direction = "two-sided")
```

These results match up with our findings from the visualizations of the shaded $p$-values with the null distribution and in the regression table in @tbl-mult-model-3. We reject the null hypothesis $H_0: \beta_i = 0$ for `aroma` and `flavor`, but fail to reject it for `moisture_percentage`.
...
```{r lc-sol-10-23, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 23))
```
...
```{r lc-sol-10-24, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 24))
```
...
```{r lc-sol-10-25, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 25))
```
...
```{r lc-sol-10-26, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(10, 26))
```
...
Twelve questions to assess your understanding. Several are designed around common misconceptions — read each option carefully before peeking at the answer. The last two anchor the chapter's multiple-regression and simulation-based inference material (§ 10.4–10.6) on the coffee-quality model.

**Q10-1.** What does a confidence interval for a regression slope $\beta_1$ tell you?

a. The probability that the population slope is positive
b. The proportion of variation in $y$ explained by the model
c. The exact value of the population slope
d. A range of plausible values for the *population* slope

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** Same logic as a CI for a mean: a range likely to contain the true population value, given the sample data.
:::

**Q10-2.** Which of the LINE conditions can be assessed using a *residuals vs. fitted values* plot?

a. Normality of the residuals
b. Independence of the observations
c. Linearity and equal variance
d. Normality and independence

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** A random "cloud" of points (no curvature, no funnel shape) supports linearity and equal variance. Normality is checked with a QQ-plot; independence is mostly a study-design question.
:::

**Q10-3.** A theory-based *t*-test and a simulation-based permutation test for a regression slope often yield similar p-values when:

a. The sample size is very small and the residuals are heavily skewed
b. The residuals are strongly non-normal with heavy tails
c. The model has many strongly correlated predictors
d. The LINE conditions hold and the sample is reasonably large

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** When the assumptions hold and $n$ is reasonable, theory-based and simulation-based methods usually agree closely. They diverge most when assumptions fail, which is exactly when simulation-based methods earn their keep.
:::

**Q10-4.** A regression model has slope = 0.5 and p-value = 0.001. The MOST precise conclusion:

a. The slope is statistically significant at any significance level $\alpha$
b. There is strong evidence that the population slope is not zero
c. The slope is exactly 0.5 in the population
d. The relationship between the two variables is causal

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Our estimate of the slope still carries uncertainty: the point estimate is 0.5, but the population slope could plausibly be anywhere in the slope's CI, which is the tool for quantifying that uncertainty. The p-value tests whether 0 is plausible, not whether 0.5 is correct.
:::

**Q10-5.** A residual plot shows residuals fanning OUT (small at low fitted values, large at high fitted values). This violates which LINE condition?
...
::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Funnel-shaped residual spread = unequal variance, the "E" in LINE. The "equal variance" assumption requires residuals to have roughly the SAME spread across the range of fitted values. When that fails, the SEs reported by `lm()` (which assume equal variance) are unreliable, and the resulting CIs/tests can be miscalibrated. The simulation-based workflow from §10.3 (bootstrap CIs, permutation tests via `infer`) doesn't lean on equal variance and is a good fallback.
:::

**Q10-6.** A QQ-plot of residuals shows points sharply deviating from the diagonal at both tails. This suggests:

a. The residuals are not Normal
b. Independence is violated
c. The linearity condition is violated
d. Equal variance is violated

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** A QQ-plot lines up *observed quantiles* against *Normal quantiles*. When the tails of the cloud diverge from the diagonal, the actual residuals are more spread out at the extremes than a Normal distribution would predict; the "N" assumption is wobbly. Mild deviations are usually fine; extreme ones can make `lm()`'s theory-based CIs and $p$-values too aggressive (too narrow, too small) and the simulation-based workflow becomes the more honest choice.
:::

**Q10-7.** When checking the *Independence* assumption, the most common diagnostic is:

a. A plot of residuals against the fitted values
b. A QQ-plot of the residuals against Normal quantiles
c. A histogram of the response variable $y$ with a Normal curve overlaid
d. A plot of residuals against the order of data collection

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** Independence is largely about *study design*: were the observations sampled independently, or do consecutive points share something (same person measured twice, same time period, same lab tech)? When the data has a natural ordering (by time, by sample number, by location) plotting residuals against that order can reveal repeating patterns or trends that suggest successive observations carry information about each other. If residuals look random with respect to order, the assumption is plausible.
:::

**Q10-8.** A 95% CI for a slope is $(0.2, 0.8)$. What can you conclude about the p-value for $H_0: \beta_1 = 0$?
...
::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Zero is *not* in the interval $(0.2, 0.8)$, so the test rejects at $\alpha = 0.05$. Two equivalent ways of saying the same thing. If a 95% CI excludes a value, then a two-sided test of "is the parameter equal to that value" rejects at $\alpha = 0.05$.
:::

**Q10-9.** You bootstrap a regression slope 1000 times and get a roughly normal distribution. The standard error of the slope is best estimated by:

a. The mean of the bootstrap slopes
b. The standard deviation of the bootstrap slopes
c. The original observed slope from the full sample
d. The range of the bootstrap slopes

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Standard error = standard deviation of the sampling distribution of the statistic. The bootstrap distribution approximates the sampling distribution, so its SD approximates the SE.
:::

**Q10-10.** A theory-based t-test gives $p = 0.002$ for a regression slope. A simulation-based permutation test on the same model gives $p = 0.005$. The level of agreement is:

a. Expected when the LINE conditions are met and $n$ is reasonable
b. A sign that the fitted regression model is misspecified
c. A bug; the two methods should give identical p-values on the same data
d. Evidence that the LINE conditions are violated

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** When assumptions hold, the two approaches converge; small gaps like this one come from finite simulation reps and minor approximations. Differences of this magnitude (especially when reps are limited) are normal Monte Carlo variation. Big disagreements would be a red flag, usually that LINE is violated. Which approach to trust then depends on *which* condition broke: the bootstrap is assumption-*lighter* about Normality and equal variance, so it's the safer choice there, but it is not assumption-*free*. Both methods rest on Independence, and if that fails neither result is trustworthy, resampling a dependent sample simply reproduces the dependence.
:::

**Q10-11.** Section 10.4 fits `lm(total_cup_points ~ aroma + flavor + moisture_percentage + continent_of_origin)` on the coffee data. The coefficient on `aroma` (its **partial slope**) tells you:

a. The slope you'd get from fitting `lm(total_cup_points ~ aroma)` on its own, with no other predictors
b. The correlation between `aroma` and `total_cup_points` in the sample
c. The change in `total_cup_points` per one-unit increase in `aroma` *with the other predictors held fixed*
d. The fraction of the variance in `total_cup_points` explained by `aroma` alone

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** That "holding the others fixed" qualifier is the defining feature of a partial slope and the reason multiple regression is more than a stack of simple regressions. The marginal slope from (a) would absorb whatever overlapping signal `aroma` shares with the other predictors, when the predictors are themselves correlated, the marginal and partial slopes can differ noticeably. (b) and (d) describe different summaries entirely; the partial slope is neither a correlation nor a variance share.
:::

**Q10-12.** In Section 10.6 you build a bootstrap distribution for the coffee model's partial slopes using `specify(...) |> generate(reps = 1000, type = "bootstrap") |> fit()`. The 95 % percentile CI for the `moisture_percentage` slope **includes zero**, while the CIs for `aroma` and `flavor` do not. The most defensible interpretation is:

a. There is no strong evidence of a non-zero partial slope for `moisture_percentage`
b. The bootstrap is unreliable for estimating partial slopes in a multiple regression
c. `moisture_percentage` and `aroma` are equally important predictors here
d. `moisture_percentage` has no effect on `total_cup_points`

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** "CI includes 0" is the simulation-based analog of "fail to reject $H_0: \beta_j = 0$"; once `aroma`, `flavor`, and `continent_of_origin` are in the model, `moisture_percentage`'s partial slope is *consistent* with 0. The data are consistent with no partial effect, but that doesn't *prove* the effect is zero. The partial framing matters: even if `moisture_percentage` correlates with cup points on its own, once the other predictors are in the model it may add little extra information. The theory-based CI from `get_regression_table()` reaches the same conclusion under LINE; the bootstrap CI gets there without invoking LINE. (d) overstates "fail to reject" into "no effect"; (b) is wrong; the bootstrap CI here is *more* trustworthy than the theory CI if LINE is questionable.
:::

::: {.callout-tip title="Chapter cheatsheet"}

| Approach | Pattern | What it gives you |
|---|---|---|
| Theory-based CI / *t*-test | `lm(y ~ x, data = df) |> get_regression_table()` | Coefficients with standard errors, 95% CIs, and *p*-values from the *t*-distribution |
| Bootstrap CI for slope | `df |> specify(y ~ x) |> generate(reps, type = "bootstrap") |> calculate(stat = "slope")` then `get_ci()` | Simulation-based CI for the slope $\beta_1$ |
| Permutation test for slope | `... |> specify(y ~ x) |> hypothesize(null = "independence") |> generate(reps, type = "permute") |> calculate(stat = "slope")` then `get_p_value()` | Simulation-based p-value for $H_0: \beta_1 = 0$ |
| Diagnostic plots (LINE conditions) | `gridExtra::grid.arrange(residuals_vs_fitted, qq_plot, ncol = 2)` | Visually check Linearity, Equal variance, and Normality |

:::

## Exercises {.unnumbered #sec-inf-reg-exercises}

The end-of-chapter exercises use `planets` (`exoplanetdata`) and `olympic_athletes` (`olympicAthletes`). Solutions are available to instructors separately.
...
# Pre-define `vball` — the volleyball-athlete subset used throughout ch10.
# Many exercises (e.g. 10, 48-49, 51-55, 58-59) reference it without redefining.
# TEMP-WASM-SHIM: define volleyball_athletes / curling_athletes from olympic_athletes until
# r-universe rebuilds the olympicAthletes wasm (these were added in v0.5.2). REMOVE once synced.
volleyball_athletes <- olympic_athletes |> filter(sport == "Volleyball")
curling_athletes    <- olympic_athletes |> filter(sport == "Curling")

vball <- volleyball_athletes |>
  filter(!is.na(height), !is.na(weight))

# vball is the COMPLETE catalog (a population). EX 10.1 draws the chapter's
# seeded 100-athlete sample and fits the estimation thread's model on it;
# later exercises (e.g. 2-9, 21-22, 24, 38-40, 44-45, 47, 50) reuse both.
set.seed(76)
vball_samp <- vball |> slice_sample(n = 100)
model_weight_height <- lm(weight ~ height, data = vball_samp)
```
...
## Conclusion {#sec-inference-conclusion}


### Summary of statistical inference

We've finished the last two scenarios, which we re-display in @tbl-table-ch10.

```{r tbl-table-ch10, echo=FALSE, message=FALSE, purl=FALSE}
...
Armed with the regression modeling techniques you learned in @sec-regression and @sec-multiple-regression, your understanding of sampling for inference in @sec-sampling, and the tools for statistical inference like confidence intervals and hypothesis tests in @sec-confidence-intervals and @sec-hypothesis-testing, you're now equipped to study the significance of relationships between variables in a wide array of data! Many of the ideas presented here can be extended into multiple regression and other more advanced modeling techniques.

### Additional resources

```{r inference-for-regression-conditional-text-dup1, echo=FALSE, results="asis", purl=FALSE}
if (is_latex_output()) {
