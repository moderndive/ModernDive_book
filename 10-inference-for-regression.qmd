...
```{r inference-for-regression-load-internal, message=FALSE, echo=FALSE}
...
```{r inference-for-regression-create-simple_model, echo=FALSE}
...
```{r simple-model-part-deux, echo=FALSE}
get_regression_table(model_1) |>
  kbl(
...
```{r residual-example, echo=FALSE, fig.cap="Example of observed value, fitted value, and residual.", message=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2.2, 4)}
...
```{r scatt-and-residual, echo=FALSE, fig.cap="The scatterplot and residual plot for the Old Faithful data.", message=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2.3, 4)}
...
```{r non-linear, echo=FALSE, fig.cap="Example of a non-linear relationship.", fig.height=ifelse(knitr::is_latex_output(), 3.3, 4), message=FALSE}
set.seed(76)
...
```{r time-plot, echo=FALSE, fig.cap="Scatterplot of date (time) vs residuals for the Old Faithful example.", fig.height=ifelse(knitr::is_latex_output(), 3, 4), message=FALSE}
old_faithful_2024 |>
...
```{r not-normal-residuals, echo=FALSE, fig.cap="Non-normal residuals.", message=FALSE, fig.height=ifelse(knitr::is_latex_output(), 1.4, 4)}
set.seed(3)
...
```{r equal-variance-residuals, echo=FALSE, fig.cap="Example of clearly non-equal variance.", fig.height=ifelse(is_latex_output(), 2, 4)}
old_faithful_2024 |>
...
```{r inference-for-regression-bootstrap-alt, echo=FALSE}
...
```{r inference-for-regression-null-dist-sized, echo=FALSE}
...
```{r null-distribution-slope, echo=FALSE, fig.cap="Null distribution of slopes.", fig.height=ifelse(knitr::is_latex_output(), 2, 4)}
...
```{r p-value-slope, echo=FALSE, fig.cap="Null distribution and $p$-value.", fig.height=ifelse(knitr::is_latex_output(), 3.8, 4)}
visualize(null_distn_slope) +
...
```{r coffee-scatter-matrix, fig.cap="Scatterplot matrix for coffee variables of interest.", fig.height=8.5, message=FALSE}
coffee_data |>
  ggpairs()
```
...
```{r inference-for-regression-select-vars-alt2-dup1, echo=FALSE}
...
```{r regtable-coffee-mult-1, echo=FALSE}
# Fit regression model:
mod_mult_1 <- lm(
  total_cup_points ~ aroma + flavor + moisture_percentage, 
  data = coffee_data)
...
```{r regtable-coffee-mult-2, echo=FALSE}
# Fit regression model:
mod_mult_2 <- lm(
  total_cup_points ~ aroma + moisture_percentage, data = coffee_data)
# Get the coefficients of the model
...
```{r inference-for-regression-arrange, grid-arrange-plot-check, fig.cap="Residuals vs. fitted values plot and QQ-plot for the multiple regression model.", fig.height=ifelse(knitr::is_latex_output(), 2, 4)}
g1 <- fit_and_res_mult |>
...
if(is_latex_output()) {
  knitr::knitr::include_graphics("images/boot_mlr_viz.png")
} else {
  boot_mlr_viz
}
```
...
if(is_latex_output()) {
  knitr::knitr::include_graphics("images/mlr_pvalue_viz.png")
} else {
  mlr_pvalue_viz
}
```
...
```{r table-ch10, echo=FALSE, message=FALSE}
...
```{r inference-for-regression-conditional-text-dup1, echo=FALSE, results="asis"}
if (is_latex_output()) {
...
```{r inference-for-regression-alt2-dup5, echo=FALSE, results="asis"}
