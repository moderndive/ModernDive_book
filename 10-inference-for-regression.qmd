...
b. A range of plausible values for the *population* slope, based on the sample
c. The exact value of the slope
d. The R-squared of the model

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Same logic as a CI for a mean: a range likely to contain the true population value, given the sample data.
:::

**Q10-2.** Which of the LINE conditions can be assessed using a *residuals vs. fitted values* plot?

a. Independence
b. Linearity and equal variance
c. Normality of residuals
d. None of the above

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** A random "cloud" of points (no curvature, no funnel shape) supports linearity and equal variance. Normality is checked with a QQ-plot; independence is mostly a study-design question.
:::
...
b. The LINE conditions are met and the sample is reasonably large
c. The model has many predictors
d. The data is non-normal

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** When the assumptions hold and $n$ is reasonable, theory-based and simulation-based methods usually agree closely. They diverge most when assumptions fail — which is exactly when simulation-based methods earn their keep.
:::

**Q10-4.** A regression model has slope = 0.5 and p-value = 0.001. The MOST precise conclusion:

a. The slope is exactly 0.5 in the population
...
c. The slope is significant at any $\alpha$
...
a. Linearity is violated
b. **The residuals are not Normal — at the extremes, observed residuals are more extreme than a Normal distribution would predict**
c. Equal variance is violated
d. Independence is violated

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** A QQ-plot lines up *observed quantiles* against *Normal quantiles*. When the tails of the cloud diverge from the diagonal, the actual residuals are more spread out at the extremes than a Normal distribution would predict — the "N" assumption is wobbly. Mild deviations are usually fine; extreme ones can make `lm()`'s theory-based CIs and $p$-values too aggressive (too narrow, too small) and the simulation-based workflow becomes the more honest choice.
:::
...
b. **The expected change in `total_cup_points` for a one-unit increase in `aroma`, *holding the other predictors (flavor, moisture, continent) fixed***
c. The correlation between `aroma` and `total_cup_points`
