...
In order to keep things simple, we will only use five different types of graphics in this book, each with a commonly given name.  We term these "five named graphs" the **5NG**:
...
Suggestion to add hint: For example, think about think about the number of rows in each dataset? OR think about the number/flights of types in each dataset.
...
The solution leading to the ‘weather’ dataset can potentially throw a student off-guard as there has so far been no mention of the weather dataset and had been focusing only on the flights. Additionally, later on in the chapter, students are shown how to View(weather)...which implies the assumption that they have notlooked at the dataset at all, and are not expected to have viewedit before.
**Suggestion:**
1.Hint: Look at some other datasets we have loaded in this chapter using the `View` function. 
2.Introduce students to the weather dataset earlier on in the chapter before asking this question. 
...
**Jitter versus transparency**

The differences have been clearly stated but just thought *it may be really helpful to briefly mention when one method may be more effective in the other.* 
...
Answer in learning check does not mention that they are months. Rather, it talks about how numbers 1-12 have been used as categorical variable... which does not really answer the question.
*Suggestion:* An answer such as “they correspond to months” would be much simpler. And then you can start talking about how the number have been turned to categorical variables.
...
Answer dived right into how faceting using days would not be ideal.
*Suggestion:* Start with “It would not work if we used individual days rather than months as we would have 365 facets to look at and...”
...
I love the very detailed explanation of the solution but it may be more information than needed for a solution. 

*Suggestion:* 
1. Rather than also talking about the consistent August temperature that has the least temperature, maybe make this a separate question or split the question above into Part(i) and part(ii) sections?
2. Put the explanation about checking spread of data using boxplots just before asking the questions. 
...
From the solution given, does that mean there is no solution until we get to Chapter 5? 
