...
But to really bring home what boxplots show, let's focus only on the month of November's `r weather %>% filter(month == 11) %>% nrow()` temperature recordings and plot these points on top of the boxplot for `month` of 11 in Figure \@ref(fig:monthtempbox2). 

```{r monthtempbox2, echo=FALSE, fig.cap="November boxplot with points", fig.height=3.7}
...
1. 25% of points fall below the bottom edge of the box which is the first quartile of `r quartiles[1] %>% round(3)` degrees farenheit (2.2 degrees celsisus). In other words 25% of observations were colder than `r quartiles[1] %>% round(3)`.
