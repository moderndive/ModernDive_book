...
* Explore the `weather` data by typing `View(weather)` in the console. 
...
We can now better associate ranges of temperatures to each of the bins. We can also vary the color of the bars by setting the `fill` argument. Type `colors()` in the console to see all `r colors() %>% length()` possible choice of colors!
