...
* A *Comma Separated Values* `.csv` \index{CSV file} file.  You can think of a `.csv` file as a bare-bones spreadsheet where:
...
Let's read in the exact same data saved in Excel format, but this time via RStudio's graphical interface instead of via the R console. First download the Excel file `dem_score.xlsx` by going <a href="https://moderndive.com/data/dem_score.xlsx" download>here</a>, then
...
```{r read-excel, echo=FALSE, fig.cap="Reading in an Excel file to R."}
include_graphics("images/read_excel.png")
```

After clicking on the "Import" \index{RStudio!import data} button on the bottom right RStudio, RStudio will save this spreadsheet's data in a data frame called `dem_score` and display its contents in the spreadsheet viewer. Furthermore, note in the bottom right of the above image there exists a "Code Preview": you can copy and paste this code to reload your data again later automatically instead of repeating the above manual point-and-click process.
...
What does it mean for your data to be "tidy"? While "tidy" has a clear English meaning of "organized", "tidy" in the context of data science using R means that your data follows a standardized format. We will follow Hadley Wickham's \index{Wickham, Hadley} definition of *tidy data* \index{tidy data} here [@tidy]:
...
Recall the `nycflights13` package with data about all domestic flights departing from New York City in 2013 that we introduced in Section \@ref(nycflights13) and used extensively in Chapter \@ref(viz) on data visualization and Chapter \@ref(wrangling) on data wrangling. Let's revisit the `flights` data frame by running `View(flights)`. We saw that `flights` has a rectangular shape with each of its `r comma(nrow(flights))` rows corresponding to a flight and each of its `r ncol(flights)` columns corresponding to different characteristics/measurements of each flight. This matches exactly with our definition of "tidy" data from above.
...
* `planes`: construction information about each of `r comma(nrow(nycflights13::planes))` planes used. i.e. the observational unit is an aircraft.
...
We can now create the plot to show how the democracy score of Guatemala changed from 1952 to 1992 using a \index{ggplot2!geom\_line()} `geom_line()`:
...
If you want to learn more about using the `readr` \index{R packages!readr!cheatsheet} and `tidyr` \index{R packages!tidyr!cheatsheet} package, we suggest you that you check out RStudio's "Data Import" cheatsheet. You can access this cheatsheet by going to RStudio's [cheatsheet page](https://www.rstudio.com/resources/cheatsheets/) and searching for "Data Import Cheat Sheet".

```{r import-cheatsheet, echo=FALSE, fig.cap="Data Import cheatsheet."}
