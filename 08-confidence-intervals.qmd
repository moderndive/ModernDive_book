...
a. If we repeated the procedure many times, about 95% of the resulting intervals would contain the true parameter
...
a. Less precision (more uncertainty)
b. More precision
c. A larger sample
...
c. Automatically raises the confidence level
d. Doubles the CI width
...
a. Take fewer samples
b. **Decrease the confidence level** (e.g., to 90%)
c. You can't narrow it
...
a. You need more data
b. Group A is bigger than Group B
c. The two means are equal
d. **0 is a plausible value for the difference — the data are consistent with "no difference between the groups"**
...
a. Coincidence
b. Bootstrap distributions are always normal
c. **The bootstrap distribution mimics the unknown sampling distribution of the statistic** — its spread approximates the standard error
...
a. A wider interval always
b. The same answer always
c. **Sample stat $\pm$ 1.96 $\times$ $\text{SE}_{\text{boot}}$**, where $\text{SE}_{\text{boot}}$ is the standard deviation of the bootstrap distribution
d. The minimum and maximum of the bootstrap

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** SE method assumes the bootstrap distribution is normal-shaped and uses the symmetric interval $\bar{x} \pm 1.96 \cdot \text{SE}$. Percentile method makes no shape assumption — better when the bootstrap distribution is skewed.
:::

**Q8-9.** A correct, plain-English interpretation of "95% CI for $\mu$ is $(15.2, 18.7)$" is:

a. The sample mean is 95% accurate
b. 95% of sample values are between 15.2 and 18.7
c. There's a 95% chance $\mu$ is in (15.2, 18.7)
d. **We are 95% confident the true population mean lies between 15.2 and 18.7**, where "confident" refers to the long-run reliability of the procedure
...
a. **The data strongly suggest male athletes are taller than female athletes, with the typical gap somewhere between about 10 and 11 cm**
b. There's a 5% chance the gap is negative
c. 95% of male athletes are 10–11 cm taller than the female mean
d. The two means are equal
...
b. Yawning is *definitely* not contagious
c. **The data are *consistent* with there being no difference between the two groups — but the CI is wide enough that a meaningful difference (in either direction) hasn't been ruled out**
