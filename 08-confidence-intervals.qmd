...
```{r confidence-intervals-load-packages, echo=FALSE, message=FALSE}
library(dplyr)
library(moderndive)
library(tidyverse)
library(knitr)
...
```{r confidence-intervals-compute-mean, echo=FALSE}
...
```{r confidence-intervals-load-internal, message=FALSE, echo=FALSE}
...
```{r confidence-intervals-compute-mean-sized, echo=FALSE}
# This code is used for dynamic non-static in-line text output purposes
...
```{r sample-mean-100-with-normal-redraw, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 1.9, 4), fig.cap="The distribution of the sample mean."}
...
```{r normal-curves, echo=FALSE, fig.cap="Three normal distributions.", fig.height=ifelse(knitr::is_latex_output(), 1, 4)}
all_points <- tibble(
...
```{r confidence-intervals-conditional-text, echo=FALSE, results="asis"}
if(!is_latex_output()) 
...
```{r confidence-intervals-conditional-text-dup1, echo=FALSE, results="asis"}
if(!is_latex_output()) 
...
```{r normal-curve-1, echo=FALSE, fig.cap="Normal density curve for the sample mean weight of almonds.", out.width="90%", warning=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
...
```{r normal-curve-2, echo=FALSE, fig.cap="Is the population mean in the interval?", out.width="90%", warning=FALSE}
...
```{r t-curve-1, echo=FALSE, fig.cap="The standard normal and two t-distributions.", fig.height=ifelse(knitr::is_latex_output(), 2.5, 4)}
...
```{r confidence-intervals-conditional-text-v2, echo=FALSE, results="asis"}
if(!is_latex_output()) 
...
```{r almond-mean-cis, fig.cap="(ref:almond-mean-ci)", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 5, 5)}
set.seed(202)
...
```{r confidence-intervals-v26, echo=FALSE}
round(qnorm(0.95), 3)
```
...
```{r confidence-intervals-v29, echo=FALSE}
round(qnorm(0.9), 3)
```
...
**Step 2**: Mix the bag contents, draw one almond, weigh it, and record the weight as seen in Figure \@ref(fig:confidence-intervals-tactile-resampling-alt).

```{r confidence-intervals-tactile-resampling-alt, echo=FALSE, fig.cap="Step 2: Weighing one almond at random.", fig.show="hold", out.width="30%"}
knitr::include_graphics("images/sampling/almonds/one-almond.png")
```

**Step 3**: Put the almond back into the bag! In other words, replace it as seen in Figure \@ref(fig:confidence-intervals-tactile-resampling-v4). 

```{r confidence-intervals-tactile-resampling-v4, echo=FALSE, fig.cap="Step 3: Replacing almond.", fig.show="hold", out.width="50%"}
...
boot_sample |> 
  summarize(mean_weight = mean(weight))
```
```{r confidence-intervals-assign-resample_mean, echo=FALSE}
...
```{r origandresample, echo=FALSE, fig.cap="(ref:compare-plots)", fig.height=ifelse(knitr::is_latex_output(), 4.2, 4)}
...
```{r resampling-35, fig.cap="Distribution of 35 sample means from 35 bootstrap samples."}
...
```
```{r confidence-intervals-compute-mean-v11, echo=FALSE}
...
```{r infer-specify, out.width="40%", echo=FALSE, fig.cap="Diagram of the specify() verb."}
knitr::knitr::include_graphics("images/flowcharts/infer/specify.png")
```
...
```{r infer-generate, out.width="40%", echo=FALSE, fig.cap="Diagram of generate() replicates."}
knitr::knitr::include_graphics("images/flowcharts/infer/generate.png")
```
...
```{r confidence-intervals-bootstrap-alt, echo=FALSE}
...
```{r confidence-intervals-bootstrap-alt2, eval=FALSE}
...
```{r infer-calculate, out.width="50%", echo=FALSE, fig.cap="Diagram of calculate() summary statistics."}
knitr::knitr::include_graphics("images/flowcharts/infer/calculate.png")
```
...
```{r confidence-intervals-bootstrap-v5, echo=FALSE}
...
```{r confidence-intervals-compute-mean-v14, eval=FALSE}
# infer workflow:                   # Original workflow:
almonds_sample_100 |>                  almonds_sample_100 |> 
  specify(response = weight) |>        rep_sample_n(size = 100, replace = TRUE, 
  generate(reps = 1000) |>                          reps = 1000) |>              
...
```{r infer-visualize, out.width="60%", echo=FALSE, fig.cap="Diagram of visualize() results."}
knitr::knitr::include_graphics("images/flowcharts/infer/visualize.png")
```
...
```{r bootstrap-distribution-infer, fig.height=ifelse(is_latex_output(), 1.7, 4) , fig.cap="Bootstrap distribution."}
...
```{r confidence-intervals-hist-alt, eval=FALSE}
# infer workflow:                    # Original workflow:
...
```{r infer-workflow-ci, out.width="80%", echo=FALSE, fig.cap="infer package workflow for confidence intervals."}
knitr::knitr::include_graphics("images/flowcharts/infer/ci_diagram.png")
```
...
```{r percentile-ci-viz, echo=FALSE, fig.cap="(ref:perc-ci-viz)", fig.height=ifelse(knitr::is_latex_output(), 2.8, 4)}
...
```{r se-ci-viz, echo=FALSE, fig.show="hold", fig.cap="(ref:se-viz)", fig.height=ifelse(knitr::is_latex_output(), 2, 4)}
if (knitr::is_html_output()) {
  visualize(bootstrap_means) +
    shade_confidence_interval(endpoints = standard_error_ci)
} else {
...
```{r confidence-intervals-dynamic-text, echo=FALSE}
# This code is used for dynamic non-static in-line text output purposes
n_participants <- 50L
```
...
```{r confidence-intervals-bootstrap-v7, echo=FALSE}
...
```{r confidence-intervals-bootstrap-v10, echo=FALSE}
...
```{r bootstrap-distribution-mythbusters, echo=FALSE, fig.cap="Bootstrap distribution.", fig.height=ifelse(knitr::is_latex_output(), 2, 4)}
...
```
```{r confidence-intervals-assign-myth_ci_percentile, include=FALSE}
# Load common setup (image functions, knitr, etc.)
source("R/common_setup.R")
...
```{r bootstrap-distribution-mythbusters-CI, echo=FALSE, fig.cap="Two 95\\% confidence intervals: percentile method (black) and standard error method (grey).", fig.height=2.5}
visualize(bootstrap_distribution_yawning) +
  ggtitle("") +
  shade_confidence_interval(
...
```{r confidence-intervals-conditional-text-v2-dup1, echo=FALSE, results="asis"}
