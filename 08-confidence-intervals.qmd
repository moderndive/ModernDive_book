...
**Q10.** When you bootstrap a confidence interval, you resample your data **with replacement**. Why?

a. R requires it
b. **Sampling without replacement would just give you back your original sample every time (only the row order changes) — with replacement is what makes each bootstrap resample a different "what if we had sampled from this population again?" scenario**
c. With replacement is faster
d. The two give the same answer

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** The bootstrap mimics the original sampling process. We don't have access to the population, only to our one sample — so each bootstrap "resample" is built by drawing rows of our sample **with replacement**, the same size as the original. Without replacement, you'd always get back exactly the same set of rows (just shuffled), and the bootstrap distribution would collapse to a single point. With replacement, some rows appear multiple times and others not at all, which is what gives the resamples their useful variation.
:::
