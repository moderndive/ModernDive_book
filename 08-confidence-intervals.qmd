...
```{r fig-confidence-intervals-tactile-resampling-v4, fig.alt="Photograph showing the weighed almond being placed back in the bowl (replacement step) before drawing the next bootstrap sample.", echo=FALSE, fig.cap="Step 3: Replacing almond.", fig.show="hold", purl=FALSE, out.width="50%"}
...
a. There is a 95% probability that the population parameter lies in the interval
b. 95% of the data values fall in the interval
c. If we repeated the procedure many times, about 95% of the resulting intervals would contain the true parameter
d. The sample mean has a 95% chance of being correct

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** The 95% refers to the long-run reliability of the *procedure*, not the probability for any single computed interval. The parameter is fixed; the interval is random.
:::

**Q2.** In bootstrapping, you generate replicate samples by:

a. Drawing without replacement from the population
b. Drawing with replacement from the original sample
c. Drawing without replacement from a different population
...
a. More precision
b. Less precision (more uncertainty)
c. A larger sample
d. A higher confidence level always

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Wider = more uncertain about where the parameter is. To narrow a CI, lower the confidence level (e.g., 90% instead of 99%) or collect more data.
:::

**Q4.** A 99% CI for the same data is **wider** than a 95% CI because:

a. **Higher confidence requires casting a wider net to be more likely to capture the true parameter**
b. 99% means more data
c. Wider intervals are always better
d. The formula is fundamentally different

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** Confidence level is the trade-off lever: higher confidence ⇒ wider interval, all else equal. To narrow a 99% CI without sacrificing confidence, you'd need more data.
:::

**Q5.** To **narrow** a 95% confidence interval **without changing the data**, you can:

a. Increase the confidence level
b. **Decrease the confidence level** (e.g., to 90%)
c. Take fewer samples
d. You can't narrow it
...
b. **The CI contains 0, so there is no statistically significant difference at $\alpha = 0.05$**
c. Group A is bigger than Group B
d. You need more data

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** A CI for a difference that includes 0 is consistent with "no difference" at the corresponding $\alpha$. Doesn't prove the means are equal; just no evidence they differ.
:::

**Q7.** A bootstrap distribution has mean $\approx$ sample mean and looks roughly normal. Why does this work for inference?

a. Coincidence
b. **The bootstrap distribution mimics the unknown sampling distribution of the statistic** — its spread approximates the standard error
c. Bootstrap distributions are always normal
d. The CLT requires normality

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Bootstrapping's key insight: the variability you'd see across many real samples can be approximated by the variability across many resamples-with-replacement of your one sample. So the bootstrap distribution stands in for the (unobservable) true sampling distribution.
:::

**Q8.** The percentile-method 95% CI is the (2.5, 97.5) percentiles of the bootstrap distribution. The standard-error method gives:

a. The same answer always
b. **Sample stat $\pm$ 1.96 $\times$ $\text{SE}_{\text{boot}}$**, where $\text{SE}_{\text{boot}}$ is the standard deviation of the bootstrap distribution
c. The minimum and maximum of the bootstrap
d. A wider interval always

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** SE method assumes the bootstrap distribution is normal-shaped and uses the symmetric interval $\bar{x} \pm 1.96 \cdot \text{SE}$. Percentile method makes no shape assumption — better when the bootstrap distribution is skewed.
:::

**Q9.** A correct, plain-English interpretation of "95% CI for $\mu$ is $(15.2, 18.7)$" is:

a. There's a 95% chance $\mu$ is in (15.2, 18.7)
b. **We are 95% confident the true population mean lies between 15.2 and 18.7**, where "confident" refers to the long-run reliability of the procedure
c. 95% of sample values are between 15.2 and 18.7
d. The sample mean is 95% accurate

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** "Confident" here is a frequentist hedge — about the procedure's long-run behavior, not about any particular interval. The parameter is fixed; this specific interval either contains it or doesn't.
:::

**Q10.** A confidence interval for a *mean* and a confidence interval for an *individual prediction* differ because:

a. They're identical
b. **An individual-prediction interval is wider — it accounts for the spread of individuals around the line, not just the uncertainty about the mean's location**
c. Prediction intervals are always narrower
d. Only the formula's units differ

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** A CI for a mean captures uncertainty about *the average*. A prediction interval for an individual must also account for the spread *around* that average — so it's wider. Conflating the two is a common student error.
:::
...
| 4. get the CI | `get_ci(level = 0.95, type = "percentile")` | also `type = "se"` for std-error method |
...
**EX8.2 (★★)** Recall $\mu \approx$ population-mean elevation from @sec-sampling-exercises. Does your CI contain $\mu$? Why isn't this a guarantee?
...
### Bootstrap CIs for a mean {.unnumbered}
...
### Bootstrap CI for a proportion {.unnumbered}
...
### Infer workflow practice {.unnumbered}
...
### Two-sample CIs (mean and proportion) {.unnumbered}

**EX8.16 (★★★)** Sample $n = 100$ each from male and female `olympic_athletes` rows in `Sport == "Athletics"` (drop `NA` heights). Build a bootstrap CI for the **difference in mean `Height`** ($\mu_M - \mu_F$).
...
**EX8.17 (★★★)** Does this CI contain 0? What does that imply?

**EX8.18 (★★★)** Repeat for **mean `imdb_rating`** of *Rick Steves' Europe* episodes filmed in **Italy vs France**. Use the full `episodes` data (no resampling).

```{webr-r}
# your code here
```

**EX8.19 (★★★)** Difference in proportions: did Bob Ross paint a `mountain` more often than a `cabin`? Use the `bob_ross` dataset (already CRAN). Compute a 95% bootstrap CI for $p_\text{mountain} - p_\text{cabin}$ — treat each episode as an observation.
...
**EX8.20 (★★)** Why does **wider sample** mean **narrower CI** but **higher confidence level** mean **wider CI**? Address each in one sentence.

**EX8.21 (★★★)** Suppose you triple your sample size. Does the CI necessarily contain $\mu$ now? Why or why not?

**EX8.22 (★★★)** A peer says "95% CI means 95% of values fall in this range." What's wrong with that, and what's the correct interpretation?

**EX8.23 (★★★)** Demonstrate the chapter's "long-run" interpretation: take **100 different samples** of size 50 from `volcanoes`, build a CI for each, count how many contain $\mu$. Plot results.

```{webr-r}
# your code here
```

**EX8.24 (★★★)** Why does the bootstrap "work" — what's the conceptual link between *resampling from the sample* and *sampling from the population*?

**EX8.25 (★★★)** When *not* to use the bootstrap: name two situations where it fails or is misleading.

**EX8.26 (★★★)** Compute a CI for the **median** elevation. Why is this *easier* with the bootstrap than with theory?

```{webr-r}
# your code here
```

**EX8.27 (★★★)** Build a CI for $\mu$ at sample sizes $n \in \{20, 50, 200\}$. Plot the three CIs as horizontal bars. Visualize the precision-vs-sample-size tradeoff.

```{webr-r}
# your code here
```

**EX8.28 (★★★)** A 99% CI for `mean(elevation)` in `volcanoes` is wider than a 95% CI. Compute both and report the *width difference*. By how much (in meters) does insisting on 99% confidence cost you?

```{webr-r}
# your code here
```

**EX8.29 (★★★)** A non-statistician colleague asks "OK, but what does **95%** even mean?" Write a 2-sentence non-jargon answer using volcanoes as the example.

**EX8.30 (★★★)** What information would you need to *check* whether the t-CI assumptions hold for your basketball Height CI?

**EX8.31 (★★★)** When the population is heavily skewed AND $n$ is small, the theory-based and bootstrap CIs may *disagree*. Demonstrate using $n = 10$ from `volcanoes$elevation`.

```{webr-r}
# your code here
```

**EX8.32 (★★★)** Open exploration: pick any numerical or proportion-style quantity from one of the three datasets. Build a CI for it (theory or bootstrap), interpret in plain language, and write a 1–2 sentence finding.
