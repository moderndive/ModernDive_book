...
```{r fig-resampling-35, fig.alt="Histogram of 35 sample means, each computed from a different bootstrap sample. Roughly bell-shaped but rough due to small number of resamples.", fig.cap="Distribution of 35 sample means from 35 bootstrap samples."}
...
```{r confidence-intervals-compute-mean-v9}
...
```{r confidence-intervals-compute-mean-v10, echo=-1}
set.seed(20)
...
The data frame `boot_means` contains `r n_virtual_resample` sample mean weights. Each is calculated from a different bootstrap sample and visualized in @fig-one-thousand-sample-means.

```{r fig-one-thousand-sample-means, fig.alt="Histogram of 1000 bootstrap sample means of almond weights. Smooth bell-shaped distribution centered at the original sample mean.", message=FALSE, fig.cap="Histogram of 1000 bootstrap sample mean weights of almonds.", fig.height=ifelse(knitr::is_latex_output(), 4, 4)}
ggplot(boot_means, aes(x = mean_weight)) +
  geom_histogram(binwidth = 0.01, color = "white") +
  labs(x = "sample mean weight in grams")
```
...
Observe also that the bootstrap distribution itself can approximate the *sampling distribution* of the sample mean, a concept we studied in @sec-sampling where we took multiple samples from the population. The key difference here is that we resample from a single sample, the original sample, not from the entire population.

By inspecting the histogram in @fig-one-thousand-sample-means, the bell-shape is apparent. We can also approximate the center and the spread of this distribution by computing the mean and the standard deviation of these `r n_virtual_resample` bootstrap sample means:

```{r confidence-intervals-mean-sd-v2-dup3}
boot_means |> 
  summarize(mean_of_means = mean(mean_weight),
            sd_of_means = sd(mean_weight))
```
```{r confidence-intervals-compute-mean-v11, echo=FALSE, purl=FALSE}
...
```{r lc-sol-08-10, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(8, 10))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Looking at the bootstrap distribution for the sample mean in @fig-one-thousand-sample-means, between what two values would you say *most* values lie?

```{r lc-sol-08-11, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(8, 11))
```
...
```{r lc-sol-08-12, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(8, 12))
```
...
```{r lc-sol-08-13, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(8, 13))
```

:::

### Confidence intervals and the bootstrap: original workflow {#sec-bootstrap-process}
...
First, we use `rep_sample_n()` to resample from the original sample `almonds_sample_100` of `r num_almonds_sample` almonds. We set  ``size = `r num_almonds_sample` `` to generate bootstrap samples of the same size as the original sample, and we resample with replacement by setting `replace = TRUE`. We create `r n_virtual_resample` bootstrap samples by setting ``reps = `r n_virtual_resample` ``:

```{r confidence-intervals-virtual-sample-sized, results='hide'}
almonds_sample_100 |> 
  rep_sample_n(size = 100, replace = TRUE, reps = 1000)
```
...
```{r confidence-intervals-compute-mean-v12, results='hide'}
almonds_sample_100 |> 
  rep_sample_n(size = 100, replace = TRUE, reps = 1000) |> 
  summarize(mean_weight = mean(weight))
```
...
### The `infer` package workflow: {#sec-infer-workflow}

The `infer` package is an R package for statistical inference. It makes efficient use of the `|>` pipe operator we introduced in @sec-piping to spell out the sequence of steps necessary to perform statistical inference in a "tidy" and transparent fashion.\index{operators!pipe} Just as the `dplyr` package provides functions with verb-like names to perform data wrangling, the `infer` package provides functions with intuitive verb-like names to perform statistical inference, such as constructing confidence intervals or performing hypothesis testing. We have discussed the theory-based implementation of the former in @sec-CI-general and we introduce the latter in @sec-hypothesis-testing.
...
```{r confidence-intervals-demo-code-v2-dup3, results='hide'}
almonds_sample_100 |> 
  summarize(stat = mean(weight))
```

If we want to use `infer` instead, we use the functions `specify()` and `calculate()`\index{R packages!infer!observed statistic shortcut} as shown:

```{r confidence-intervals-specify, results='hide'}
almonds_sample_100 |> 
...
- First, the `infer` verb names better align with the overall simulation-based framework you need to understand to construct confidence intervals and to conduct hypothesis tests (in @sec-hypothesis-testing). We will see flowchart diagrams of this framework in the upcoming @fig-infer-workflow-ci and in @sec-hypothesis-testing with @fig-htdowney.

- Second, you can transition seamlessly between confidence intervals and hypothesis testing with minimal changes to your code. This becomes apparent in @sec-comparing-infer-workflows when we compare the `infer` code for both of these inferential methods.

- Third, the `infer` workflow is much simpler for conducting inference when you have *more than one variable*. We introduce *two-sample* inference\index{two-sample inference} where the sample data is collected from two groups, such as in @sec-case-study-two-prop-ci where we study the contagiousness of yawning and in @sec-ht-activity where we compare the popularity of music genres. Then in @sec-infer-regression, we see situations of *inference for regression* using the regression models you fit in @sec-regression. 
...
```{r fig-infer-specify, fig.alt="Diagram of the infer package specify() verb: takes a data frame and identifies which columns are the response and explanatory variables.", out.width="40%", echo=FALSE, fig.cap="Diagram of the specify() verb.", purl=FALSE}
...
```{r confidence-intervals-demo-code-v2-dup4}
almonds_sample_100 |> 
  specify(response = weight)
```

Notice how the data itself does not change, but the `Response: weight (numeric)` *meta-data* does\index{meta-data}. This is similar to how the `group_by()` verb from `dplyr` doesn't change the data, but only adds "grouping" meta-data, as we saw in @sec-groupby.

We can also specify which variables are the focus of the study by introducing a `formula = y ~ x` in `specify()`. This is the same formula notation you saw in @sec-regression and @sec-multiple-regression on regression models: the response variable `y` is separated from the explanatory variable `x` by a `~` ("tilde"). The following use of `specify()` with the `formula` argument yields the same result seen previously:

```{r confidence-intervals-demo-code-v2-dup5, results='hide'}
almonds_sample_100 |> 
...
In cases where inference is focused on a single sample, as it is the almonds' weights example, either specification works. In the examples we present in later sections, the `formula` specification is simpler and more flexible. In particular, this comes up in the upcoming @sec-case-study-two-prop-ci on comparing two proportions and @sec-infer-regression on inference for regression.

#### 2. `generate` replicates {.unnumbered}


```{r fig-infer-generate, fig.alt="Diagram of the infer package generate() verb: produces multiple replicate samples (bootstrap or permutation) from the original data.", out.width="40%", echo=FALSE, fig.cap="Diagram of generate() replicates.", purl=FALSE}
knitr::include_graphics("images/flowcharts/infer/generate.png")
```

After we `specify()` the variables of interest, we pipe the results into the `generate()` function to generate replicates. This is the function that produces the bootstrap samples or performs the similar resampling process a large number of times, based on the variable(s) specified previously, as shown in @fig-infer-generate. Recall we did this `r n_virtual_resample` times.
...
The second argument `type` determines the type of computer simulation used. Setting this to `type = "bootstrap"` produces bootstrap samples using resampling with replacement. We present different options for `type` in @sec-hypothesis-testing. 

```{r confidence-intervals-bootstrap, results='hide'}
almonds_sample_100 |> 
  specify(response = weight) |> 
  generate(reps = 1000, type = "bootstrap")
```

```{r confidence-intervals-bootstrap-alt, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-bootstrap-alt2, eval=FALSE, purl=FALSE}
...
#### 3. `calculate` summary statistics {.unnumbered}

```{r fig-infer-calculate, fig.alt="Diagram of the infer package calculate() verb: computes a summary statistic from each replicate sample.", out.width="50%", echo=FALSE, fig.cap="Diagram of calculate() summary statistics.", purl=FALSE}
knitr::include_graphics("images/flowcharts/infer/calculate.png")
```

After we `generate()` `r n_virtual_resample` bootstrap samples, we want to summarize each of them, for example, by calculating the sample mean of each one of them. As @fig-infer-calculate shows, the `calculate()` \index{R packages!infer!calculate()} function does this.
...
```{r confidence-intervals-bootstrap-v4, results='hide'}
...
```{r confidence-intervals-bootstrap-v5, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-compute-mean-v14, eval=FALSE, purl=FALSE}
# infer workflow:                   # Original workflow:
almonds_sample_100 |>                  almonds_sample_100 |> 
  specify(response = weight) |>        rep_sample_n(size = 100, replace = TRUE, 
  generate(reps = 1000) |>                          reps = 1000) |>              
...
```{r fig-infer-visualize, fig.alt="Diagram of the infer package visualize() verb: plots the resulting distribution of statistics as a histogram.", out.width="60%", echo=FALSE, fig.cap="Diagram of visualize() results.", purl=FALSE}
knitr::include_graphics("images/flowcharts/infer/visualize.png")
```

The `visualize()` \index{R packages!infer!visualize()} verb provides a quick way to visualize the bootstrap distribution as a histogram of the numerical `stat` variable's values as shown in @fig-bootstrap-distribution-infer. The pipeline of the main `infer` verbs used for exploring bootstrap distribution results is shown in @fig-infer-visualize.  

```{r fig-bootstrap-distribution-infer, fig.alt="Bell-shaped histogram of bootstrap sample statistics produced by the infer workflow.", fig.height=ifelse(is_latex_output(), 1.7, 4) , fig.cap="Bootstrap distribution.", purl=FALSE}
visualize(bootstrap_means)
```

**Comparing with original workflow**: In fact, `visualize()` is a *wrapper function* for the `ggplot()` function that uses a `geom_histogram()` layer. Recall that we illustrated the concept of a wrapper function in @fig-moderndive-figure-wrapper in @sec-model1table.

```{r confidence-intervals-hist-alt, eval=FALSE, purl=FALSE}
# infer workflow:                    # Original workflow:
visualize(bootstrap_means)           ggplot(bootstrap_means, aes(x = stat)) +
                                        geom_histogram()
```

The `visualize()` function can take many other arguments to customize the plot further. In later sections, we take advantage of this flexibility. In addition, it works with helper functions to add shading of the histogram values corresponding to the confidence interval values.
We have introduced the different elements of the `infer` workflow for constructing a bootstrap distribution and visualizing it. A summary of these steps is presented in @fig-infer-workflow-ci.

```{r fig-infer-workflow-ci, fig.alt="Flowchart of the infer package confidence-interval workflow: specify → generate → calculate → get_ci → visualize.", out.width="80%", echo=FALSE, fig.cap="infer package workflow for confidence intervals.", purl=FALSE}
...
### Confidence intervals using bootstrap samples with `infer` {#sec-conf-int-infer}
...
#### Percentile method {#sec-percentile-method-infer .unnumbered}

Recall that in @sec-infer-workflow we have generated `r n_virtual_resample` bootstrap samples and stored them in data frame `bootstrap_means`:

```{r confidence-intervals-v56}
bootstrap_means
```
...
```{r confidence-intervals-conf-interval}
...
```{r confidence-intervals-viz-dist, echo=TRUE, fig.show='hide'}
visualize(bootstrap_means) + 
  shade_confidence_interval(endpoints = percentile_ci)
```


```{r fig-percentile-ci-viz, fig.alt="Bootstrap distribution histogram with the central 95% region shaded, illustrating the percentile-method confidence interval bounds.", echo=FALSE, fig.cap="Percentile method 95% confidence interval shaded corresponding to potential values.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2.8, 4)}
...
Observe in @fig-percentile-ci-viz that 95% of the sample means stored in the `stat` variable in `bootstrap_means` fall between the two endpoints marked with the darker lines, with 2.5% of the sample means to the left of the shaded area and 2.5% of the sample means to the right. You also have the option to change the colors of the shading using the `color` and `fill` arguments. 
...
```{r confidence-intervals-viz-ci, echo=TRUE, fig.show='hide'}
visualize(bootstrap_means) + 
...
#### Standard error method {#sec-se-infer .unnumbered}


In @sec-CI-general we introduced theory-based confidence intervals. We show that a 95% confidence interval can be constructed as
...
```{r confidence-intervals-mean-sd-v2-dup4}
...
```{r confidence-intervals-compute-mean-v15}
almonds_sample_100 |>
...
```{r confidence-intervals-specify-v4}
x_bar <- almonds_sample_100 |> 
  specify(response = weight) |> 
  calculate(stat = "mean")
x_bar
...
```{r confidence-intervals-assign-standard_error_ci}
...
If we would like to visualize the interval (`r standard_error_ci[["lower_ci"]] |> round(2)`, `r standard_error_ci[["upper_ci"]] |> round(2)`), we can once again pipe the `bootstrap_means` data frame into the `visualize()` function and add a `shade_confidence_interval()` layer to our plot. We set the `endpoints` argument to be `standard_error_ci`. The resulting standard-error method based on a 95% confidence interval for $\mu$ can be seen in @fig-se-ci-viz.


```{r confidence-intervals-viz-dist-alt, echo=TRUE, fig.show='hide'}
visualize(bootstrap_means) + 
  shade_confidence_interval(endpoints = standard_error_ci)
```

```{r fig-se-ci-viz, fig.alt="Bootstrap distribution histogram with a 95% confidence interval marked using the standard-error method (mean plus or minus 1.96 times SE).", echo=FALSE, fig.show="hold", fig.cap="Standard-error method 95% confidence interval.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 4)}
if (knitr::is_html_output()) {
  visualize(bootstrap_means) +
    shade_confidence_interval(endpoints = standard_error_ci)
} else {
...
<!--
#### Percentile-$t$ method {#sec-infer-percentile-t .unnumbered}
...
<!--
#### Bias-corrected and accelerated, BCa, method {#sec-bca .unnumbered}
-->
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Construct a 95% confidence interval for the *median* weight of *all* almonds with the percentile method. Is it appropriate to also use the standard-error method?

```{r lc-sol-08-14, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(8, 14))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What are the advantages of using `infer` for building confidence intervals?

```{r lc-sol-08-15, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(8, 15))
```
...
```{r lc-sol-08-16, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(8, 16))
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** "Which function denotes the variables of interest for inference?"
...
```{r lc-sol-08-17, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(8, 17))
```
...
```{r lc-sol-08-18, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(8, 18))
```

:::

## Additional remarks about the bootstrap {#sec-boot-remarks}
...
@sec-case-study-two-prop-ci if you want to go over directly another
...
### Confidence intervals and rate of convergence {#sec-rate-convergence}

We want to compare how the bootstrap performs when building confidence intervals 
with respect to the theory-based approach discussed in @sec-theory-based-CI. 
...
The bootstrap percentile method discussed in @sec-conf-int-infer, in the case 
...
@sec-theory-based-CI. This problem may be fixed if the
sample size is large, but, depending on how extreme the population distribution 
...
Second, in this chapter, we have studied confidence intervals for the population 
mean $\mu$, because it is a fundamental quantity and it is the foundation for 
other cases. However, building confidence intervals for other parameters using 
the theory-based approach (for example, for the median, the first quartile, 
...
is no formula for the standard error and alternative methods have to be used to 
estimate it. This can create an additional source of bias. When bias is present, 
the confidence intervals created using the theory-based approach in @sec-CI-general could be suspect, even completely useless.

The bootstrap percentile method is not affected directly by the second and third 
limitations. It can be implemented for any other parameter beyond the population 
mean, as long as all the information needed can be extracted from each bootstrap 
sample. On the other hand, the first limitation listed above can also affect the
...
## Case study: is yawning contagious? {#sec-case-study-two-prop-ci}
...
```{r confidence-intervals-v65}
mythbusters_yawn
```

```{r confidence-intervals-dynamic-text, echo=FALSE, purl=FALSE}
# This code is used for dynamic non-static in-line text output purposes
n_participants <- 50L
```
...
Recall that you learned about treatment and response variables in @sec-correlation-is-not-causation in our discussion on confounding variables. \index{variables!treatment}\index{variables!response}

Let's use some data wrangling to calculate counts of the four possible outcomes:

```{r confidence-intervals-grouped-summary}
mythbusters_yawn |> 
  group_by(group, yawn) |> 
...
Just like with our sampling bowl, the population parameter here will involve proportions. However, in this case, it will be the *difference in population proportions* $p_{seed} - p_{control}$, where $p_{seed}$ is the proportion of *all* Americans who if exposed to yawning will yawn themselves, and $p_{control}$ is the proportion of *all* Americans who if not exposed to yawning still yawn themselves. Correspondingly, the point estimate/sample statistic based on the *Mythbusters*' sample of participants will be the *difference in sample proportions* $\widehat{p}_{seed} - \widehat{p}_{control}$. Let's extend @tbl-table-ch8-c of scenarios of sampling for inference to include our latest scenario. 

```{r tbl-table-ch8-c, echo=FALSE, message=FALSE, purl=FALSE}
...
$$
\widehat{p}_{seed} - \widehat{p}_{control} = \frac{10}{34} - \frac{4}{16} = 0.04411765 \approx 4.4\%
$$
...
```{r confidence-intervals-specify-v5}
mythbusters_yawn |> 
...
Our next step is to perform *bootstrap resampling with replacement* like we did with the almonds in the activity in @sec-revisit-almond-bootstrap. We saw how it works with both a single variable in computing bootstrap means in @sec-bootstrap-process, but we haven't yet worked with bootstrapping involving multiple variables. 
...
```{r confidence-intervals-create-first_six_rows}
...
```{r confidence-intervals-v70, echo=FALSE}
set.seed(22)
```

```{r confidence-intervals-v71}
first_six_rows |> 
...
```{r confidence-intervals-bootstrap-v6, results='hide'}
mythbusters_yawn |> 
  specify(formula = yawn ~ group, success = "yes") |> 
  generate(reps = 1000, type = "bootstrap")
```

```{r confidence-intervals-bootstrap-v7, echo=FALSE, purl=FALSE}
...
```{r confidence-intervals-bootstrap-v8, results='hide'}
mythbusters_yawn |> 
  specify(formula = yawn ~ group, success = "yes") |> 
  generate(reps = 1000, type = "bootstrap") |> 
...
```{r confidence-intervals-bootstrap-v9, results='hide'}
...
```{r confidence-intervals-bootstrap-v10, echo=FALSE, purl=FALSE}
...
In @fig-bootstrap-distribution-mythbusters we `visualize()` the resulting bootstrap resampling distribution. Let's also add a vertical line at 0 by adding a `geom_vline()` layer. 

```{r fig-bootstrap-distribution-mythbusters, fig.alt="Bootstrap distribution histogram for the difference in proportions of yawners between the seed and no-seed groups in the MythBusters yawning experiment.", echo=FALSE, fig.cap="Bootstrap distribution.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 4)}
...
```{r confidence-intervals-conf-interval-alt2}
bootstrap_distribution_yawning |> 
  get_confidence_interval(type = "percentile", level = 0.95)
```
```{r confidence-intervals-assign-myth_ci_percentile, include=FALSE, purl=FALSE}
...
```{r confidence-intervals-bootstrap-v11}
...
```{r confidence-intervals-conf-interval-v5}
...
Let's visualize both confidence intervals in @fig-bootstrap-distribution-mythbusters-CI, with the percentile method interval marked with black lines and the standard-error method marked with grey lines. Observe that they are both similar to each other. 

```{r fig-bootstrap-distribution-mythbusters-CI, fig.alt="The same MythBusters bootstrap distribution with two 95% confidence intervals overlaid: percentile method (black) and standard-error method (grey), both clearly contain zero.", echo=FALSE, fig.cap="Two 95\\% confidence intervals: percentile method (black) and standard error method (grey).", fig.height=2.5, purl=FALSE}
visualize(bootstrap_distribution_yawning) +
  ggtitle("") +
  shade_confidence_interval(
...
Eleven questions to assess your understanding. Several are designed around common misconceptions — read each option carefully before peeking at the answer. The last one anchors the framework in this chapter's two-proportion case study (§ 8.4).

**Q8-1.** What does it mean to construct a 95% confidence interval?

a. About 95% of intervals built this way would contain the true parameter
b. There is a 95% probability that the population parameter lies in the interval
c. 95% of the data values fall in the interval
d. The sample mean has a 95% chance of being correct

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** The 95% refers to the long-run reliability of the *procedure*, not the probability for any single computed interval. The parameter is fixed; the interval is random.
:::

**Q8-2.** In bootstrapping, you generate replicate samples by:

a. Generating random numbers from a normal distribution
b. Drawing with replacement from the original sample
c. Drawing without replacement from the population
d. Drawing without replacement from a different population

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Bootstrap = resample *with replacement* from your one observed sample. This simulates "what would another sample have looked like?" without needing the actual population.
:::

**Q8-3.** A wider confidence interval generally means:

a. Less precision
b. More precision
c. A larger sample
d. A higher confidence level always

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** Wider = more uncertain about where the parameter is. To narrow a CI, lower the confidence level (e.g., 90% instead of 99%) or collect more data.
:::

**Q8-4.** As a rough rule, **doubling** your sample size (with everything else unchanged) typically:

a. Has no effect on CI width
b. Narrows the CI by about a factor of $\sqrt{2}$
c. Automatically raises the confidence level to 99%
d. Doubles the width of the CI

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Standard error scales like $1/\sqrt{n}$, so doubling $n$ shrinks the SE, and roughly the CI half-width, by a factor of $\sqrt{2} \approx 1.4$. To **halve** the CI width you'd need *four times* the data. This is the practical reason "collect more data" is the answer to "I need a tighter estimate"; it works, but with diminishing returns.
:::

**Q8-5.** To **narrow** a 95% confidence interval **without changing the data**, you can:

a. Take fewer bootstrap resamples
b. Decrease the confidence level
c. Nothing can narrow it without new data
d. Increase the confidence level

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Lowering confidence narrows the CI, but you trade away long-run reliability. Be honest about this trade-off when you choose a confidence level.
:::

**Q8-6.** A 95% CI for the difference in two group means is $(-2, 5)$. What can you conclude about the difference?

a. You need more data before concluding anything
b. Group A's mean is bigger than Group B's
c. The two population means are equal
d. 0 is a plausible value for the difference

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** A CI captures the range of plausible values for the parameter. When 0 sits inside the CI, "no difference" is one of the plausible explanations, so we can't rule it out. This doesn't *prove* the means are equal; it just says the data don't give us evidence to claim they differ.
:::

**Q8-7.** A bootstrap distribution has mean $\approx$ sample mean and looks roughly normal. Why does this work for inference?

a. It is a coincidence of this dataset
b. Bootstrap distributions are always normal in shape
c. It mimics the unknown sampling distribution
d. The CLT requires the data to be normal

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Bootstrapping's key insight: the variability you'd see across many real samples can be approximated by the variability across many resamples-with-replacement of your one sample. So the bootstrap distribution stands in for the (unobservable) true sampling distribution.
:::

**Q8-8.** The percentile-method 95% CI is the (2.5, 97.5) percentiles of the bootstrap distribution. The standard-error method gives:

a. A wider interval in every situation
b. Exactly the same answer every time
c. Sample stat $\pm$ 1.96 $\times$ $\text{SE}_{\text{boot}}$
d. The smallest and largest values of the bootstrap distribution

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** Here $\text{SE}_{\text{boot}}$ is the standard deviation of the bootstrap distribution. The SE method assumes that distribution is normal-shaped and uses the symmetric interval $\bar{x} \pm 1.96 \cdot \text{SE}$. Percentile method makes no shape assumption; better when the bootstrap distribution is skewed.
:::

**Q8-9.** A correct, plain-English interpretation of "95% CI for $\mu$ is $(15.2, 18.7)$" is:

a. The sample mean is 95% accurate as an estimate
b. 95% of the sampled values fall between 15.2 and 18.7
c. There is a 95% chance that $\mu$ lies between 15.2 and 18.7
d. We are 95% confident that $\mu$ lies between 15.2 and 18.7

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** "Confident" here is a frequentist hedge; about the procedure's long-run behavior, not about any particular interval. The parameter is fixed; this specific interval either contains it or doesn't.
:::

**Q8-10.** A 95% bootstrap CI for the **difference** in mean heights ($\mu_M - \mu_F$) between male and female Olympic athletes in athletics is $(10.2, 10.9)$ cm. What does this tell you?

a. The mean gap $\mu_M - \mu_F$ plausibly lies between about 10 and 11 cm
b. There is a 5% chance the true gap is negative
c. 95% of individual male athletes are 10–11 cm taller than the female mean
d. The two population means are equal

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** When an entire CI for a *difference* sits strictly above zero (or strictly below zero), it strongly suggests a real difference in the parameter, with the CI giving the plausible range for the *size* of the difference. Compare to a CI like $(-2, 5)$ from Q8-6 that *includes* zero; there, "no difference" was one of the plausible explanations. Option (c) confuses a CI for the mean difference with a CI for individual observations; option (b) misreads the 95%.
:::

**Q8-11.** In Section 8.4's *Mythbusters* yawning case study, the 95 % bootstrap CI for the difference in *population* yawning proportions $p_{\text{seed}} - p_{\text{control}}$ contains zero. What is the appropriate takeaway?

a. The experiment must have been conducted incorrectly
b. Yawning is *definitely* not contagious after all
c. The data are *consistent* with no difference between the groups
d. The bootstrap CI is unreliable for proportions from small samples

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** A CI that includes 0 means 0 is *one* of the plausible values for the population difference, but so is every other value in the interval. With only 50 participants split unevenly between the seed and control groups, the CI is *wide*; a true difference of several percentage points (in either direction) would not be detected by this sample. The right writeup is "this experiment did not provide strong evidence that yawning is contagious" — not "yawning is not contagious." (b) overstates the conclusion; (a) and (d) are unsupported by anything in the data.
:::

::: {.callout-tip title="Chapter cheatsheet"}

The `infer` workflow for confidence intervals — chain these with `|>`:

| Step | Function | Quick example |
|---|---|---|
| 1. specify | `specify(response = var)` | `specify(response = age)` |
| 2. generate | `generate(reps = 1000, type = "bootstrap")` | (with replacement) |
| 3. calculate | `calculate(stat = "mean")` | other options: `"prop"`, `"diff in means"`, etc. |
| 4. get the CI | `get_confidence_interval(level = 0.95, type = "percentile")` | for the std-error method: `type = "se", point_estimate = x_bar` |
| 5. visualize | `visualize() + shade_ci(endpoints = ci)` | overlays CI on the bootstrap distribution |

:::

## Exercises {.unnumbered #sec-CI-exercises}

The end-of-chapter exercises mix three datasets:

* `volcanoes` (~1,200 Holocene volcanoes — `elevation`, `primary_volcano_type`, `country`).
* `episodes` from `steves` (159 episodes — `imdb_rating`, `primary_country`).
* `olympic_athletes` (`height`, `sex`, `sport`).
...
# TEMP-WASM-SHIM: define volleyball_athletes from olympic_athletes until r-universe rebuilds
# the olympicAthletes wasm (added v0.5.2). REMOVE once synced.
...
## Summary and final remarks {#sec-summary-CI}

### Additional resources

```{r confidence-intervals-conditional-text-v2-dup1, echo=FALSE, results="asis", purl=FALSE}
...
Now that we've equipped ourselves with confidence intervals, in @sec-hypothesis-testing we'll cover the other common tool for statistical inference: hypothesis testing. Just like confidence intervals, hypothesis tests are used to infer about a population using a sample. However, we'll see that the framework for making such inferences is slightly different. 
