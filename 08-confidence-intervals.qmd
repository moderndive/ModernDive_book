...
a. Doubles the CI width
b. **Narrows the CI by about a factor of $\sqrt{2}$** — standard error shrinks like $1/\sqrt{n}$
c. Has no effect on CI width
d. Automatically raises the confidence level
...
b. **0 is a plausible value for the difference — the data are consistent with "no difference between the groups"**
c. Group A is bigger than Group B
d. You need more data

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** A CI captures the range of plausible values for the parameter. When 0 sits inside the CI, "no difference" is one of the plausible explanations, so we can't rule it out. This doesn't *prove* the means are equal — it just says the data don't give us evidence to claim they differ.
:::
...
a. The two means are equal
b. **The data strongly suggest male athletes are taller than female athletes, with the typical gap somewhere between about 15 and 17 cm**
c. 95% of male athletes are 15–17 cm taller than the female mean
d. There's a 5% chance the gap is negative

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** When an entire CI for a *difference* sits strictly above zero (or strictly below zero), it strongly suggests a real difference in the parameter, with the CI giving the plausible range for the *size* of the difference. Compare to a CI like $(-2, 5)$ from Q6 that *includes* zero — there, "no difference" was one of the plausible explanations. Option (c) confuses a CI for the mean difference with a CI for individual observations; option (d) misreads the 95%.
:::
...
library(readr)
# olympicAthletes, steves, volcanoes, exoplanets are GitHub-only; read their datasets
# from the v2 raw mirror. Exercises here reference olympic_athletes, episodes, planets,
# and volcanoes.
...
episodes <- read_csv(
  "https://raw.githubusercontent.com/moderndive/ModernDive_book/v2/data/steves_episodes.csv.gz",
  show_col_types = FALSE
)
planets <- read_csv(
