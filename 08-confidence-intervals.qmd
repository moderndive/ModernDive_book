...
```{r sample-mean-100-with-normal-redraw, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 1.9, 4), fig.cap="The distribution of the sample mean.", purl=FALSE}
...
The added `r if(knitr::is_html_output()) "red"` smooth curve is the density curve for the normal distribution with the appropriate expected value and standard error calculated from the sample distribution. 
The `r ifelse(knitr::is_html_output(), "red", "left")` dot represents the population mean $\mu$, the unknown parameter we are trying to estimate. The `r ifelse(knitr::is_html_output(), "blue", "right")` right is the sample mean $\overline{x} = `r xbar`$ from the random sample stored in `almonds_sample_100`.

In real-life applications, a sample mean is taken from a sample, but the distribution of the population and the population mean are unknown, so the location of the `r ifelse(knitr::is_html_output(), "blue", "right")` dot with respect to the `r ifelse(knitr::is_html_output(), "red", "left")` dot is also unknown. However, if we construct an interval centered on the `r ifelse(knitr::is_html_output(), "blue", "right")` dot, as long as it is wide enough the interval will contain the `r ifelse(knitr::is_html_output(), "red", "left")` dot.
...
```{r normal-curves, echo=FALSE, fig.cap="Three normal distributions.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 1, 4)}
all_points <- tibble(
...
```{r normal-curve-shaded-1a, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), .9, 4), fig.width=3, fig.cap="Normal area within one standard deviation."}
...
```{r normal-curve-shaded-2a, echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 0.9, 4), fig.width=3, fig.cap="Normal area within two standard deviations."}
ggplot(data = data.frame(x = c(-4, 4)), aes(x)) +
  stat_function(fun = dnorm, args = list(mean = 0, sd = 1)) +
...
```{r confidence-intervals-create-colors, echo=FALSE, fig.cap="Normal density curve for the sample mean weight of almonds.", purl=FALSE, out.width="90%", warning=FALSE, fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
...
The horizontal axis (X-axis) represents the sample means that we can determine from all the possible random samples of 100 almonds. The `r ifelse(knitr::is_html_output(), "red", "left")` dot represents the expected value of the sampling distribution, $\mu = 3.64$, located on the X-axis at the center of the distribution. The density curve's height can be thought of as how likely those sample means are to be observed. For example, it is more likely to get a random sample with a sample mean around $`r mu`$ grams (which corresponds to the highest point of the curve) than it is to get a sample with a sample mean of around $3.5$ grams, since the curve's height is almost zero at that value. The `r ifelse(knitr::is_html_output(), "blue", "right")` dot is the sample mean from our sample of 100 almonds, $\overline{x} = `r sample_mean`$ grams. It is located `r deviance` grams above the population mean weight. How far is `r deviance` grams? It is helpful to express this distance in standardized values: 
...
- a $t$ density curve for a t distribution with 2 degrees of freedom, in `r ifelse(knitr::is_latex_output(), "dotted", "dotted blue")`, and
- a $t$ density curve for a t distribution with 10 degrees of freedom, in `r ifelse(knitr::is_latex_output(), "dashed", "dashed red")`.


```{r confidence-intervals-create-p1, echo=FALSE, fig.cap="The standard normal and two t-distributions.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2.5, 4)}
...
Observe how the $t$ density curve in `r ifelse(knitr::is_latex_output(), "dashed", "dashed red")` ($t$ with 10 degrees of freedom) gets closer to the standard normal density curve, or $z$-curve, in `r ifelse(knitr::is_latex_output(), "solid", "solid black")`, than the $t$ curve in `r ifelse(knitr::is_latex_output(), "dotted", "dotted blue")` ($t$ with 2 degrees of freedom). The greater the number of degrees of freedom, the closer the $t$ density curve is from the $z$ curve. This change makes our calculations slightly different.
...
```{r almond-mean-cis, fig.cap="(ref:almond-mean-ci)", echo=FALSE, fig.height=ifelse(knitr::is_latex_output(), 5, 5), purl=FALSE}
set.seed(202)
...
```{r normal-curve-shaded-3a, echo=FALSE, fig.cap="Normal curve with the shaded middle area being 0.95", fig.height=ifelse(knitr::is_latex_output(), 1.5, 4), fig.width=3}
ggplot(data = data.frame(x = c(-4, 4)), aes(x)) +
  stat_function(fun = dnorm, args = list(mean = 0, sd = 1)) +
...
```{r confidence-intervals-tactile-resampling-alt, echo=FALSE, fig.cap="Step 2: Weighing one almond at random.", fig.show="hold", purl=FALSE, out.width="30%"}
...
```{r confidence-intervals-tactile-resampling-v4, echo=FALSE, fig.cap="Step 3: Replacing almond.", fig.show="hold", purl=FALSE, out.width="50%"}
...
```{r origandresample, echo=FALSE, fig.cap="(ref:compare-plots)", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 4.2, 4)}
...
```{r confidence-intervals-hist-white-border, fig.cap="Distribution of 35 sample means from 35 bootstrap samples."}
...
```{r one-thousand-sample-means, message=FALSE, fig.cap="Histogram of 1000 bootstrap sample mean weights of almonds.", fig.height=ifelse(knitr::is_latex_output(), 4, 4)}
ggplot(boot_means, aes(x = mean_weight)) +
  geom_histogram(binwidth = 0.01, color = "white") +
  labs(x = "sample mean weight in grams")
```
...
```{r infer-specify, out.width="40%", echo=FALSE, fig.cap="Diagram of the specify() verb.", purl=FALSE}
...
In cases where inference is focused on a single sample, as it is the almonds' weights example, either specification works. In the examples we present in later sections, the `formula` specification is simpler and more flexible. In particular, this comes up in the upcoming Section \@ref(case-study-two-prop-ci) on comparing two proportions and Section \@ref(infer-regression) on inference for regression.

#### 2. `generate` replicates {.unnumbered}


```{r infer-generate, out.width="40%", echo=FALSE, fig.cap="Diagram of generate() replicates.", purl=FALSE}
...
```{r infer-calculate, out.width="50%", echo=FALSE, fig.cap="Diagram of calculate() summary statistics.", purl=FALSE}
knitr::include_graphics("images/flowcharts/infer/calculate.png")
```

After we `generate()` `r n_virtual_resample` bootstrap samples, we want to summarize each of them, for example, by calculating the sample mean of each one of them. As Figure \@ref(fig:infer-calculate) shows, the `calculate()` \index{R packages!infer!calculate()} function does this.
...
```{r bootstrap-distribution-infer, fig.height=ifelse(knitr::is_latex_output(), 1.7, 4) , fig.cap="Bootstrap distribution.", purl=FALSE}
...
We have introduced the different elements of the `infer` workflow for constructing a bootstrap distribution and visualizing it. A summary of these steps is presented in Figure \@ref(fig:infer-workflow-ci).

```{r infer-workflow-ci, out.width="80%", echo=FALSE, fig.cap="infer package workflow for confidence intervals.", purl=FALSE}
...
```{r percentile-ci-viz, echo=FALSE, fig.cap="(ref:perc-ci-viz)", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2.8, 4)}
...
```{r se-ci-viz, echo=FALSE, fig.show="hold", fig.cap="(ref:se-viz)", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 4)}
if (knitr::is_html_output()) {
  visualize(bootstrap_means) +
    shade_confidence_interval(endpoints = standard_error_ci)
} else {
...
the confidence intervals created using the theory-based approach in Subsection 
\@ref(CI-general) could be suspect, even completely useless.
...
sampling_scenarios |>
  ...
  kable_styling(
    font_size = ifelse(knitr::is_latex_output(), 8, 16),
...
```{r bootstrap-distribution-mythbusters, echo=FALSE, fig.cap="Bootstrap distribution.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 2, 4)}
...
```{r bootstrap-distribution-mythbusters-CI, echo=FALSE, fig.cap="Two 95\\% confidence intervals: percentile method (black) and standard error method (grey).", fig.height=2.5, purl=FALSE}
visualize(bootstrap_distribution_yawning) +
  ggtitle("") +
  shade_confidence_interval(
