...
You'll often encounter issues with missing values `NA`. In fact, an entire branch of the field of statistics deals with missing data. It is not good practice to include a `na.rm = TRUE` in your summary commands by default; you should attempt to run them without this argument. Why? You should be aware of missing values in your data and consider the impact on the analysis if you ignore these values. In other words, `na.rm = TRUE` should only be used when necessary.
