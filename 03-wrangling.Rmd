...
```{r wrangling-view-btv_sea_flights_fall-2, eval=FALSE}
btv_sea_flights_fall <- flights |> 
  filter(origin == "JFK", (dest == "BTV" | dest == "SEA"), month >= 10)
View(btv_sea_flights_fall)
```
...
```{r wrangling-mean-and-sd-3, eval=FALSE}
summary_windspeed <- weather |>   
  summarize(mean = mean(wind_speed, na.rm = TRUE)) |> 
...
```{r wrangling-summary-by-month-2}
...
```{r wrangling-summary-by-month-3}
...
```{r wrangling-summary-by-month-4}
...
```{r wrangling-select-vars-2, eval=FALSE}
...
```{r wrangling-arrange-desc-3, eval=FALSE}
named_dests |> 
...
```{r wrangling-conditional-text-2, results="asis", echo=FALSE, purl=FALSE}
if(is_latex_output()){
...
```{r wrangling-conditional-text-3, echo=FALSE, results="asis"}
if(!is_latex_output()) 
