...
1. The starting value `x` will be a data frame. For example, the \index{R packages!nycflights22} `flights` data frame we explored in Section \@ref(nycflights).
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Why doesn't the following code work?  Run the code line-by-line instead of all at once, and then look at the data.  In other words, run `summary_windspeed <- weather %>% summarize(mean = mean(wind_speed, na.rm = TRUE))` first.
...
We can modify our code above to look at the average temperature and its spread instead of wind speed too, keeping the `na.rm = TRUE` set just in case any missing values are stored in the `temp` column:
...
We see that LaGuardia (`"LGA"`) had the most flights departing in 2022 followed by `"JFK"` and lastly by Newark (`"EWR"`). Note there is a subtle but important difference between `sum()` and `n()`; while `sum()` returns the sum of a numerical variable, `n()` returns a count of the number of rows/observations. 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What code would be required to get the mean and standard deviation temperature for each day in 2022 for NYC?
...
The flight in the first row departed 113 minutes late but arrived 92 minutes late, so its "gained time in the air" is a gain of 9 minutes, hence its `gain` is 113 - 92 = 21. On the other hand, the flight in the fourth row departed a little late (`dep_delay` of 12) but arrived 14 minutes late (`arr_delay` of 14), so its "gained time in the air" is $12 - 14 = -2$ minutes, hence its `gain` is $-2$ or a `loss` of 2 minutes.
...
We see for example that the average gain is 90 minutes, while the largest is +90 minutes and the largest negative gain (or loss) at -222 minutes! However, this code would take some time to type out in practice. We'll see later on in Subsection \@ref(model1EDA) that there is a much more succinct way to compute a variety of common summary statistics: using the `skim()` function from the `skimr` package.
...
Suppose we are interested in determining the most frequent destination airports for all domestic flights departing from New York City in 2022:
...
Say instead you are interested in the destinations of all domestic flights departing NYC in 2022, and you ask yourself questions like: "What cities are these airports in?", or "Is `"ORD"` Orlando?", or "Where is `"FLL"`?".
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What surprises you about the top 10 destinations from NYC in 2022?
...
flights_cols <- nycflights22::flights %>%
  ncol() 
```

We've seen that the `flights` data frame in the `nycflights22` package contains `r flights_cols` different variables. You can identify the names of these `r flights_cols` variables by running the `glimpse()` function from the `dplyr` package:
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Create a new data frame that shows the top 5 airports with the largest arrival delays from NYC in 2022.
...
Using the datasets included in the `nycflights22` package, compute the available seat miles for each airline sorted in descending order. After completing all the necessary data wrangling steps, the resulting data frame should have 16 rows (one for each airline) and 2 columns (airline name and available seat miles). Here are some hints:
...
In the online [Appendix C](https://moderndive.com/C-appendixC.html), we provide a page of data wrangling "tips and tricks" consisting of the most common data wrangling questions we've encountered in student projects (shout out to [Dr. Jenny Smetzer](https://www.scsparkscience.org/fellow/jennifer-smetzer/) for her work setting this up!):
...
In the current version of RStudio in late 2019, you can access this cheatsheet by going to the RStudio Menu Bar -> Help -> Cheatsheets -> "Data Transformation with dplyr." `r if(is_html_output()) "You can see a preview in the figure below."`
