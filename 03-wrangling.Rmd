...
Observe that there are `r nrow(by_origin_monthly)` rows to `by_origin_monthly` because there are 12 months for 3 airports (`EWR`, `JFK`, and `LGA`). 
...
We've seen that the `flights` data frame in the `nycflights13` package contains `r ncol(nycflights13::flights)` different variables. You can identify the names of these `r ncol(nycflights13::flights)` variables by running the `glimpse()` function from the `dplyr` package:

```{r, eval=FALSE}
glimpse(flights)
```

However, say you only need two of these `r ncol(nycflights13::flights)` variables, say `carrier` and `flight`. You can `select()` \index{dplyr!select()} these two variables:
