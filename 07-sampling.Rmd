...
```{r sampling-compute-mean-2, echo=-c(1, 2)}
...
```{r sampling-virtual-sample-2, echo=-1}
set.seed(76)
...
```{r sampling-compute-mean-3, echo=-1}
set.seed(76)
...
```{r sampling-hist-2, echo=TRUE, fig.show='hide'}
...
```{r sampling-compute-mean-4, echo=-1}
set.seed(76)
virtual_prop_red <- bowl |> 
...
```{r sampling-hist-3, echo=TRUE, fig.show='hide'}
ggplot(virtual_prop_red, aes(x = prop_red)) +
...
```{r sampling-conditional-text-2, echo=FALSE, results="asis", purl=FALSE}
if(!is_latex_output()) 
...
```{r sampling-compute-mean-5, eval=FALSE}
...
```{r sampling-compute-mean-7, echo=TRUE, results='hide'}
virtual_prop_red_50 |> 
...
```{r sampling-compute-mean-8, echo=FALSE, purl=FALSE}
...
```{r sampling-hist-4, echo=TRUE, fig.show='hide'}
...
```{r sampling-mean-virtual-1000, echo=FALSE, fig.cap="The distribution of 1000 means based on 1000 random samples of size 25.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 1.5, 4)}
virtual_mean_weight <- virtual_samples_almonds |> 
  summarize(mean_weight = mean(weight))
...
```{r sampling-compute-mean-12, eval=FALSE}
# Segment 1: sample size = 25 ------------------------------
...
```{r sampling-mean-and-sd-4, results='hide'}
# n = 25
virtual_mean_weight_25 |> 
...
```{r sampling-hist-5, echo=TRUE, fig.show='hide'}
...
```
```{r samplingdistribution-virtual-diff-1000, echo=FALSE, fig.cap="The distribution of 1000 differences in sample proportions based on 1000 random samples of size 50 from the first bowl and 1000 random samples of size 60 from the second bowl.", purl=FALSE, fig.height=ifelse(knitr::is_latex_output(), 5.5, 7),}
...
```{r sampling-conditional-text-3, echo=FALSE, results='asis', purl=FALSE}
if (is_latex_output())
...
```{r sampling-conditional-text-4, echo=FALSE, results="asis", purl=FALSE}
if (is_latex_output()) {
...
```{r sampling-48, echo=FALSE, purl=FALSE, results="asis"}
