...
1. A **population parameter** is a numerical summary quantity about the population that is unknown, but you wish you knew. For example, when this quantity is a mean, the population parameter of interest is the *population mean*. This is mathematically denoted with the Greek letter $\mu$ pronounced "mu" (we'll see a sampling activity involving means in the upcoming Section \@ref(resampling-tactile)). 
1. A **census** is an exhaustive enumeration or counting of all $N$ individuals or observations in the population in order to compute the population parameter's value *exactly*. As the number $N$ of individuals or observations in our population increases, conducting a census gets more expensive in terms of time, energy, and money. 

So in our sampling activities, the (study) **population** is the collection of $N$ = `r num_balls` identically sized red and white balls in the bowl shown in Figure \@ref(fig:sampling-exercise-1). Recall also that we virtually represented the actual bowl in the data frame `bowl`:

```{r}
bowl
```

The **population parameter** in this case is the proportion of the bowl's balls that are red. Whenever we're interested in a proportion of a some population, the population parameter has a specific name: the *population propotion* which is denoted mathematically with the letter $p$. As we'll see in Table \@ref(tab:table-ch8), depending on whether we're interested in proportions or means, the specific name of the **population parameter** in question will differ.

Lastly, in our case a **census** corresponds to counting the number of balls out of $N$ = `r num_balls` that are red. Using the results of this census, we can compute the **population proportion** $p$ that are red *exactly*. 

At this point, you might be now asking yourself: "Wait. I understand that performing a census on the actual bowl would take a long time. But can't we conduct a 'virtual' census using the virtual `bowl`?" You are absolutely correct! In fact, we matched the contents of `bowl` with the actual bowl not by doing a census, but by reading the "contents" written on the box the bowl came in! 

Let's conduct a "virtual" census by using the same `dplyr` verbs you used earlier to count the number of balls that are red:
...
Since `r num_red` of the `r num_balls` are red, the proportion is `r num_red`/`r num_balls` = `r prop_red` = `r percent_red`. So we know the value of the **population parameter**: in our case, the *population proportion* $p$ is equal to `r prop_red`.

At this point, you might be further asking yourself: "If we had a way of finding out that the proportion of the bowl's balls that are red is `r percent_red`, then why did we do any sampling?" Great question! This is because the sampling activities is this chapter are a *simulation* of how sampling is done in real-life! We're using this simulation to study:

1. The effect of *sampling variation* on our estimates.
1. The effect of sample size on *sampling variation*.

As we'll see in the next Section \@ref(sampling-case-study) on polls, in a real-life sampling situation not only will the population size $N$ be very large making a census expensive, but also we won't know what $N$ is! For now however, we press on with our terminology and notation.

The second set of terms and notation relating to **samples**:

1. **Sampling** is the act of collecting a sample from the population when we don't have the means to perform a census. We mathematically denote the sample's size using lower case $n$, as opposed to upper case $N$ which denotes the population's size. Typically the sample size $n$ is much smaller than the population size $N$.  Thus sampling is a much cheaper alternative than performing a census. 
1. A **point estimate** AKA **sample statistic** is a summary statistic computed from a sample that *estimates* the unknown *population parameter**.

So in our sampling activities, we conducted **sampling** using shovels with 25, 50, and 100 slots to extract samples of size $n$ = 25, $n$ = 50, and $n$ = 100. For example, recall that in order to take a virtual sample of size 50 from the virtual `bowl`, we used:
...
Using this sample, we generated an *estimate* of the proportion of the bowl's balls that are red `prop_red`
...
The value of `prop_red` is the **point estimate** of the **population parameter**, here the *population proportion* $p$. We mathematically denote the sample proportion using $\widehat{p}$, where the "hat" on top of the $p$ indicates that it is an *estimate*.

The third set of terms relate to **sampling methodology**, or in other words the way the samples were gathered. You'll see here and throughout the rest of your book, that the *way* you gather your samples directly influences the sample's quality:

1. A sample is said to be a **representative** sample if it roughly *looks like* the population. In other words, are the sample's characteristics a good representation of the population's characteristics? 
1. We say a sample is **generalizable** if any results based on the sample can generalize to the population. In other words, does the value of the point estimate *generalize* to the population? 
1. We say **biased sampling** occurs if certain individuals or observations in a population have a higher chance of being included in a sample than others. We say a sampling procedure is *unbiased* if every observation in a population had an equal chance of being sampled. One way to ensure that a sample is *unbiased* is by using **random sampling**

So in our sampling activities, we say a sample of $n$ balls extracted using our shovel is **representative** if it's contents "roughly resemble" the contents of the bowl. If so, then the proportion of the shovel's balls that are red can **generalize** to the proportion of the bowl's $N$ = `r num_balls` balls that are red. Or expressed different, the estimate $\widehat{p}$ is a "good guess" of $p$. 

Since (1) all the balls were of the same size and (2) we **randomly** mixed all the balls before sampling, each ball had an equal chance of being sampled. Thus our sample was **unbiased**. Compare this to a situation where we put the white balls in the bowl first, then the red balls, but then sampled without mixing the bowl first. This could lead to a **bias** towards one color of ball being sampled over the other.

The fourth and final terms relates to the goal of sampling:

1. To *infer* the value of a **population parameter** using **sampling** instead of a **census** is called **statistical inference**.
...
* the contents of the shovel are an **unbiased representation** of the contents of the bowl's `r num_balls` balls, thus
* any result based on the shovel's balls can **generalize** to the bowl, thus
* the sample proportion $\widehat{p}$ of the $n=50$ balls in the shovel that are red is a **"good guess"** of the population proportion $p$ of the $N$ = `r num_balls` balls that are red, thus
* instead of manually going over all `r num_balls` balls in the bowl, we can **infer** about the bowl using the shovel. 
...
What was the value of the population proportion $p$ of the $N$ = `r num_balls` balls in the actual bowl that were red? There were `r num_red` red balls, for a proportion red of `r num_red`/`r num_balls` = `r prop_red` = `r percent_red`! How do we know this? Did the authors do an exhaustive count of all the balls? No! They were listed in the contents of the box that the bowl came in! Hence we were able to make the contents of the virtual `bowl` match the tactile bowl:
