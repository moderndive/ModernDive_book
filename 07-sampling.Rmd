...
percent_red_chr <- (100*prop_red) %>% str_c("%")
```

Observe that `bowl` has `r nrow(bowl)` rows, telling us that the bowl contains `r nrow(bowl)` equally sized balls. The first variable `ball_ID` is used as an *identification variable* as discussed in Subsection \@ref(identification-vs-measurement-variables); none of the balls in the actual bowl are marked with numbers. The second variable `color` indicates whether a particular virtual ball is red or white. View the contents of the bowl in RStudio's data viewer and scroll through the contents to convince yourself that `bowl` is indeed a virtual analog of the actual bowl in Figure \@ref(fig:sampling-exercise-1).
...
Third, a **census** is an exhaustive enumeration or counting of all $N$ individuals or observations in the population in order to compute the population parameter's value *exactly*. In our sampling activity, this would correspond to counting the number of balls out of $N$ = `r nrow(bowl)` that are red and computing the *population proportion* $p$ that are red *exactly*. When the number $N$ of individuals or observations in our population is large as was the case with our bowl, a census can be quite expensive in terms of time, energy, and money. 
...
* the contents of the shovel are an **unbiased representation** of the contents of the bowl's `r nrow(bowl)` balls, thus
* any result based on the shovel's balls can **generalize** to the bowl, thus
* the sample proportion $\widehat{p}$ of the $n=50$ balls in the shovel that are red is a **"good guess"** of the population proportion $p$ of the $N=`r nrow(bowl)`$ balls that are red, thus
* instead of manually going over all `r nrow(bowl)` balls in the bowl, we can **infer** about the bowl using the shovel. 
...
What was the value of the population proportion $p$ of the $N$ = `r nrow(bowl)` balls in the actual bowl that were red? There were `r num_red` red balls, for a proportion red of `r num_red`/`r nrow(bowl)` = `r prop_red` = `r percent_red_chr`! How do we know this? Did the authors do an exhaustive count of all the balls? No! They were listed in the contents of the box that the bowl came in! Hence we were able to make the contents of the virtual `bowl` match the tactile bowl:
...
* Bowl: $N$ = `r nrow(bowl)` identically sized red and white balls
...
* Bowl: Manually going over all $N$ = `r nrow(bowl)` balls and exactly computing the population proportion $p$ of the balls that are red.
