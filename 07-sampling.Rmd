...
To close this chapter, we'll generalize the previous "sampling from a bowl" exercise to other sampling scenarios, present an important theoretical result known as the *Central Limit Theorem*.
...
Observe that for every row where `color == "red"`, the Boolean (AKA logical)  value `TRUE` is returned and for every row where `color` is not equal to `"red"`, the Boolean `FALSE` is returned.
