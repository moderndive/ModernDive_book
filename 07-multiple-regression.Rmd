...
$$
\begin{align}
\text{Interaction model} &: \widehat{y} = \widehat{\text{score}} = b_0 + b_{\mbox{age}} \cdot \mbox{age} + b_{\mbox{male}} \cdot \mathbb{1}_{\mbox{is male}}(x) + b_{\mbox{age,male}} \cdot \mbox{age} \cdot \mathbb{1}_{\mbox{is male}}\\
\text{Parallel slopes model} &: \widehat{y} = \widehat{\text{score}} = b_0 + b_{\mbox{age}} \cdot \mbox{age} + b_{\mbox{male}} \cdot \mathbb{1}_{\mbox{is male}}(x)
\end{align}
$$
...
So while in the Figure \@ref(fig:numxcatx-comparison) it appears this additional complexity is warranted as evidenced by the clearly different slopes in the interaction model, in Section \@ref(model-selection) on model selection below we'll present data where the case for using an interaction model isn't as strong.
