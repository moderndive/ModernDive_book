...
Before interpreting the finding let's clarify what a 95% confidence interval is. The graphic below from @isrs2014 shows us that if we repeated a confidence interval process many times each with a new sample, we would expect about 95% of them to actually contain the population parameter of interest.  This parameter is marked with a dotted vertical line.  We can see that only one confidence interval does not overlap with this value.  (The one marked in red.)  Therefore 24 in 25 (96%), which is quite close to our 95% **reliability**, do include the population parameter. It is the reliability  of the process highlighted below that is what we are 95% confident in, not the particular interval.
...
```{r}
Based on the sample data and 95% confidence interval bootstrapping techniques, we conclude that the 
true mean rating of **ALL** IMDB ratings is between `r ciq_mean_rating$lower` and `r ciq_mean_rating$upper`.
```

This is similar to how in hypothesis testing we use the test to draw a conclusion. When generating confidence intervals we use the interval to draw a conclusion.
...
We see here that the population mean does fall in our range of plausible values generated from the bootstrapped samples. But it's possible it might not have. Either way, we would conclude that the interval contained the value. The higher the confidence level selected in the procedure the more often we will be correct in that conclusion.
...
We can, therefore, say that the population mean rating for romance movies is between `r round(lower, 3)` and `r round(upper, 3)` points higher than for that of action movies using a 95% confidence interval procedure.

If one wishes to use the confidence interval in a way analogous to a hypothesis test then, the important thing to check here is whether 0 is contained in the confidence interval.  If it is, it is plausible that the difference in the two population means between the two groups is 0. This means that the null hypothesis is plausible.  The results of the hypothesis test and the confidence interval should match as they do here.  We rejected the null hypothesis with hypothesis testing and we have evidence here that the mean rating for romance movies is higher than for action movies.

We include this confidence interval and hypothesis testing parallel so that you can see the similarities but it's important to note that the confidence interval is primarily to be used for a different kind of a conclusion that's much more powerful than a significance test. It provides upper and lower bounds of the effect and can easily be compared to a confidence interval generated in another experiment whereas a significance test cannot. So, while the parallel is available between a confidence interval and a significance test it's important to note that the confidence interval is much more flexible.
