...
The `library` function here loads the R package `nycflights13` into the current R environment in which you are working.  The `data(flights)` loads in the `flights` dataset that is stored in the `nycflights13` package.  Note that you'll get an error if you try to load this package in and it hasn't been donwloaded and installed.  You can ensure it is installed by running the code below:
