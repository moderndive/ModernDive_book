...
We saw in Chapter \@ref(viz) that organizing data in this tidy way makes it easy for us to produce graphics.  We can simply specify what variable/column we would like on one axis, what variable we'd like on the other axis, and what type of plot we'd like to make.  We can also do things such as changing the color by another variable or changing the size of our points by a fourth variable given this tidy data set.

Furthermore, in Chapter \@ref(viz), we hinted at some ways to summarize and manipulate data to suit your needs.  This chapter expands on this by giving a variety of examples using what we call the *Five Main Verbs* (5MV) in the `dplyr` package [@R-dplyr].  There are more advanced operations than just these five and you'll see some examples of this near the end of the chapter.  

At various points we prompt you to use the `View()` command to inspect a particular data frame.  Feel free to do so whenever; you should get into the habit of doing this for *any* data frame you work with.
...
We can calculate the standard deviation and mean of the temperature variable `temp` in the `weather` data frame of `nycflights13` in one step using the `summarize` function in `dplyr` (See \@ref(appendixA) ):
...
Values can missing missing for many reasons. Perhaps the data was collected but someone forgot to enter it? Perhaps the data was not collected at all because it was too difficult? Perhaps there was an erroneous value that someone entered that has been correct to read as missing? You'll often encounter issues with missing values `NA`. In fact, an entire branch of the field of statistics deals with missing data. It is not good practice to include a `na.rm = TRUE` in your summary commands by default; you should attempt to run them without this argument. Why?  You should be aware of missing values in your data and consider the impact on the analysis if you ignore these values. In other words, `na.rm = TRUE` should only be used when necessary.
...
It is important to note that `group_by` doesn't change the data frame. It sets *meta-data* (data about the data), specifically the group structure of the data. It is only after we apply the `summarize` function that the data frame changes. If we would like to remove this group structure meta-data, we can pipe the resulting data frame into the `ungroup()` function.
...
**`r paste0("(LC", chap, ".", (lc - 2), ")")`** Recreate `by_monthly_origin`, but instead of grouping via `group_by(origin, month)`, group variables in a different order `group_by(month, origin)`. What differs in the resulting data set?
```
