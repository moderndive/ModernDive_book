...
\begin{verbatim}
## [1] "By cismay on Wednesday, November 16, 2016 11:54:46 PST"
\end{verbatim}
...
\addcontentsline{toc}{chapter}{(PART) Data Exploration}
...
{\centering \includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-59-1} 

}

\caption[Histogram of gain variable]{Histogram of gain variable}\label{fig:unnamed-chunk-59}
\end{figure}
...
\addcontentsline{toc}{chapter}{(PART) Inference}
...
\caption[Barplot of number of heads in simulation - needs tweaking]{Barplot of number of heads in simulation - needs tweaking}\label{fig:unnamed-chunk-77}
\end{figure}
...
{\centering \includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-81-1} 

}

\caption[Barplot of heads with p-value highlighted]{Barplot of heads with p-value highlighted}\label{fig:unnamed-chunk-81}
\end{figure}
...
\caption[Population ratings histogram]{Population ratings histogram}\label{fig:unnamed-chunk-84}
\end{figure}
...
\caption[Sample ratings histogram]{Sample ratings histogram}\label{fig:unnamed-chunk-86}
\end{figure}
...
\caption[Bootstrapped means histogram]{Bootstrapped means histogram}\label{fig:unnamed-chunk-92}
\end{figure}
...
An R script file of all R code used in this chapter is available
\href{http://ismayc.github.io/moderndiver-book/06-inference_basics.R}{here}.
...
\caption[Rating vs genre in the population]{Rating vs genre in the population}\label{fig:unnamed-chunk-100}
\end{figure}
...
\caption[Faceted histogram of genre vs rating]{Faceted histogram of genre vs rating}\label{fig:unnamed-chunk-101}
\end{figure}
...
\StringTok{  }\KeywordTok{resample}\NormalTok{(}\DecValTok{34}\NormalTok{, }\DataTypeTok{replace =} \OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\caption[Genre vs rating for our sample]{Genre vs rating for our sample}\label{fig:unnamed-chunk-103}
\end{figure}
...
\caption[Genre vs rating for our sample as faceted histogram]{Genre vs rating for our sample as faceted histogram}\label{fig:unnamed-chunk-104}
\end{figure}
...
##     genre     mean  std_dev
##     <chr>    <dbl>    <dbl>
## 1  Action 4.500000 1.360515
## 2 Romance 6.284615 1.104420
\end{verbatim}
...
\(\bar{x}_r - \bar{x}_a = 6.2846154 - 4.5 = 1.7846154\). We will now
proceed similarly to how we conducted hypothesis tests in Chapter
...
\caption[Simulated differences in means histogram]{Simulated differences in means histogram}\label{fig:unnamed-chunk-110}
\end{figure}

Remember that we are interested in seeing where our observed sample mean
difference of 1.7846154 falls on this null/randomization distribution.
...
\caption[Shaded histogram to show p-value]{Shaded histogram to show p-value}\label{fig:unnamed-chunk-111}
\end{figure}

You may initially think there is an error here, but remember that the
observed difference in means was 1.7846154. It falls far outside the
...
\caption[Histogram with vertical lines corresponding to observed statistic]{Histogram with vertical lines corresponding to observed statistic}\label{fig:unnamed-chunk-112}
\end{figure}
...
\textbf{(LC7.16)} Using the definition of ``\(p\)-value'' in Chapter
...
\textbf{(LC7.17)} What is the value of the \(p\)-value for the
hypothesis test comparing the mean rating of romance to action movies?

\textbf{(LC7.18)} Do the results of the hypothesis test match up with
...
An R script file of all R code used in this chapter is available
\href{http://ismayc.github.io/moderndiver-book/07-hypo_tests.R}{here}.

\section{What's to come?}\label{whats-to-come-4}

This chapter examined how to conclude whether two sample statistics are
statistically different using hypothesis testing. This is the same thing
as trying to conclude if the difference in sample statistics is
statistically different from zero. We will see that this value of zero
plays an important role in confidence intervals as well. We'll also see
in Chapter \ref{ci} the relationship between confidence intervals and
two-sided hypothesis tests as we worked with in this chapter.
...
\caption[Simulated shuffled sample means histogram]{Simulated shuffled sample means histogram}\label{fig:unnamed-chunk-114}
\end{figure}
...
mean rating for romance movies is between 1.721 and 1.848 points higher
...
\addcontentsline{toc}{chapter}{(PART) Conclusion}
...
\caption[Number of flights departing NYC in 2013 by airline - Descending numbers]{Number of flights departing NYC in 2013 by airline - Descending numbers}\label{fig:unnamed-chunk-122}
\end{figure}
...
\begin{center}\includegraphics[width=1\linewidth]{ismaykim_files/figure-latex/unnamed-chunk-124-1} \end{center}
