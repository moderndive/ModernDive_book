...
would like to receive updates on this book and other similar projects,
please fill out \href{https://goo.gl/forms/IxiwBeEnk72NxMMx2}{this
Google Form} with your name, institution, and email address.
...
\begin{verbatim}
## [1] "By Chester on Thursday, January 05, 2017 20:12:17 EST"
\end{verbatim}
...
York City in 2013 and profiles of OKCupid users in San Francisco. By
focusing on just two large data sources, it is our hope that you'll be
able to see how each of the chapters is interconnected. You'll see how
the data being tidy leads into data visualization and manipulation and
how those concepts tie into inference and regression.
...
{\centering \includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-13-1} 

}

\caption[Strip Plot of Hourly Temperature Recordings from NYC in 2013]{Strip Plot of Hourly Temperature Recordings from NYC in 2013}\label{fig:unnamed-chunk-13}
\end{figure}
...
\caption[ggplot backdrop]{ggplot backdrop}\label{fig:unnamed-chunk-14}
\end{figure}
...
\caption[Histogram of Hourly Temperature Recordings from NYC in 2013]{Histogram of Hourly Temperature Recordings from NYC in 2013}\label{fig:unnamed-chunk-15}
\end{figure}
...
\caption[Histogram of Hourly Temperature Recordings from NYC in 2013 - 60 Bins]{Histogram of Hourly Temperature Recordings from NYC in 2013 - 60 Bins}\label{fig:unnamed-chunk-16}
\end{figure}
...
\caption[Histogram of Hourly Temperature Recordings from NYC in 2013 - Binwidth = 10]{Histogram of Hourly Temperature Recordings from NYC in 2013 - Binwidth = 10}\label{fig:unnamed-chunk-17}
\end{figure}
...
\caption[The only good pie chart]{The only good pie chart}\label{fig:unnamed-chunk-24}
\end{figure}
...
{\centering \includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-26-1} 

}

\caption[Stacked barplot comparing the number of flights by carrier and airport]{Stacked barplot comparing the number of flights by carrier and airport}\label{fig:unnamed-chunk-26}
\end{figure}
...
\caption[Side-by-side barplot comparing the number of flights by carrier and airport]{Side-by-side barplot comparing the number of flights by carrier and airport}\label{fig:unnamed-chunk-27}
\end{figure}
...
\caption[Faceted barplot comparing the number of flights by carrier and airport]{Faceted barplot comparing the number of flights by carrier and airport}\label{fig:unnamed-chunk-28}
\end{figure}
...
\caption[Jittered delay scatterplot]{Jittered delay scatterplot}\label{fig:unnamed-chunk-30}
\end{figure}
...
\caption[Hard to read scatterplot]{Hard to read scatterplot}\label{fig:unnamed-chunk-31}
\end{figure}
...
customization but also keeping with a consistent framework that allows
...
\textbf{(LC5.1)} What are some ways to select all three of the
...
\texttt{shiny} R package below. You can specify the sample size you'd
like to work with (100 is chosen by default) and then generate a random
sample. You then can see the mean of this generated sample plotted in
the bottom visualization. Repeating this process many times, you can
start to see the shape of the sampling distribution take form.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(knitr)}
\KeywordTok{include_app}\NormalTok{(}\StringTok{"http://ismay.shinyapps.io/okcupidheights/"}\NormalTok{, }
  \DataTypeTok{height =} \StringTok{"1300px"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \href{http://ismay.shinyapps.io/okcupidheights/}{\includegraphics[width=1\linewidth]{ismaykim_files/figure-latex/shiny-1} }

}

\caption[Sampling distribution app at <http://ismay.shinyapps.io/okcupidheights/>]{Sampling distribution app at <http://ismay.shinyapps.io/okcupidheights/>}\label{fig:shiny}
\end{figure}
...
{\centering \includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-77-1} 

}

\caption[Histogram of number of heads in simulation - needs tweaking]{Histogram of number of heads in simulation - needs tweaking}\label{fig:unnamed-chunk-77}
\end{figure}
...
{\centering \includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-78-1} 

}

\caption[Barplot of number of heads in simulation]{Barplot of number of heads in simulation}\label{fig:unnamed-chunk-78}
\end{figure}
...
for the general frameworks, but require conditions to be met for their
results to be valid. The general frameworks using randomization,
simulation, and bootstrapping do not hold the same sorts of restrictions
and further advance computational thinking, which is one big reason for
their emphasis here.
...
\begin{center}\includegraphics[width=\textwidth]{images/errors} \end{center}
...
\textbf{(LC7.1)} Reproduce the table above, but for a hypothesis test,
instead of the one provided for a criminal trial.

\begin{center}\rule{0.5\linewidth}{\linethickness}\end{center}

\section{Basic Logic of Hypothesis
...
\textbf{(LC7.2)} What is wrong about saying ``The defendant is
innocent.'' based on the US system of criminal trials?

\textbf{(LC7.3)} What is the purpose of hypothesis testing?

\textbf{(LC7.4)} What are some flaws with hypothesis testing? How could
we alleviate them?

\begin{center}\rule{0.5\linewidth}{\linethickness}\end{center}

\section{Revisiting the Lady Tasting Tea
example}\label{revisiting-the-lady-tasting-tea-example}
...
Let's assume as we did in Chapter \ref{infer-basics}, that the lady is
correct in determining whether milk was added first in 9 out of 10
trials. Our data, therefore, may look something like
...
flips in Chapter \ref{infer-basics} and we've stored these values in the
\texttt{heads} variable in the \texttt{simGuesses} data frame:
...
\begin{center}\includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-85-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{simGuesses %>%}\StringTok{ }\KeywordTok{filter}\NormalTok{(heads >=}\StringTok{ }\DecValTok{9}\NormalTok{)    }
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-88-1} 

}

\caption[Barplot of heads with p-value highlighted]{Barplot of heads with p-value highlighted}\label{fig:unnamed-chunk-88}
\end{figure}
...
\textbf{(LC7.5)} What is meant by ``pseudo-random number generation?''

\textbf{(LC7.6)} How can simulation be used to help us address the
question of whether or not an observed result is statistically
significant?

\textbf{(LC7.7)} In Chapter \ref{viz}, we noted that barplots should be
...
An important and time-saving skill is to \textbf{ALWAYS} do exploratory
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(dplyr)}
\NormalTok{bos_sfo %>%}\StringTok{ }\KeywordTok{group_by}\NormalTok{(dest) %>%}\StringTok{ }
...
\textbf{(LC7.8)} Could we make the same type of immediate conclusion
...
\begin{center}\includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-91-1} \end{center}
...
diagram to the Lady Tasting Tea example presented in Chapter
\ref{infer-basics}. We'll see in Chapter \ref{ci} how we can provide a
range of possible values for an unknown population parameter instead of
just running a Yes/No decision from a hypothesis test.
...
\textbf{(LC9.2)} Why are the different genre variables stored as binary
variables (1s and 0s) instead of just listing the \texttt{genre} as a
column of values like ``Action'', ``Comedy'', etc.?

\textbf{(LC9.3)} What complications could come above with us excluding
...
\caption[Rating vs genre in the population]{Rating vs genre in the population}\label{fig:unnamed-chunk-96}
\end{figure}
...
\caption[Faceted histogram of genre vs rating]{Faceted histogram of genre vs rating}\label{fig:unnamed-chunk-97}
\end{figure}
...
\textbf{(LC9.4)} Define the relevant parameters here in terms of the
...
\caption[Genre vs rating for our sample]{Genre vs rating for our sample}\label{fig:unnamed-chunk-99}
\end{figure}
...
\caption[Genre vs rating for our sample as faceted histogram]{Genre vs rating for our sample as faceted histogram}\label{fig:unnamed-chunk-100}
\end{figure}
...
\textbf{(LC9.5)} What single value could we change to improve the
...
\textbf{(LC9.6)} Why did we not specify \texttt{na.rm\ =\ TRUE} here as
...
\textbf{(LC9.7)} Why might the standard deviation provide some insight
...
\ref{infer-basics} using simulation. Our goal is figure out a random
process with which to simulate the null hypothesis being true. In
Chapter \ref{infer-basics}, we used flipping of a fair coin as the
random process we were simulating with the null hypothesis being true
(\(H_0: \pi = 0.5\)).
...
equal values of sample sizes, we split the deck in half. (You should be
thinking about how this process is similar to what was done with
\textbf{bootstrapping} in Chapter \ref{infer-basics}.)
...
\textbf{(LC9.8)} How would the tactile shuffling of index cards change
if we had different samples of say 20 action movies and 60 romance
movies? Describe each step that would change.

\textbf{(LC9.9)} Why are we taking the difference in the means of the
...
\caption[Simulated differences in means histogram]{Simulated differences in means histogram}\label{fig:unnamed-chunk-106}
\end{figure}
...
\caption[Shaded histogram to show p-value]{Shaded histogram to show p-value}\label{fig:unnamed-chunk-107}
\end{figure}
...
\caption[Histogram with vertical lines corresponding to observed statistic]{Histogram with vertical lines corresponding to observed statistic}\label{fig:unnamed-chunk-108}
\end{figure}
...
\textbf{(LC9.10)} Conduct the same analysis comparing action movies
...
\textbf{(LC9.11)} What conclusions can you make from viewing the faceted
histogram looking at \texttt{rating} versus \texttt{genre} that you
couldn't see when looking at the boxplot?

\textbf{(LC9.12)} Describe in a paragraph how we used Allen Downey's
diagram to conclude if a statistical difference existed between mean
movie ratings for action and romance movies.

\textbf{(LC9.13)} Why are we relatively confident that the distributions
of the sample ratings will be good approximations of the population
distributions of ratings for the two genres?

\textbf{(LC9.14)} Using the definition of ``\(p\)-value'' in Chapter
\ref{infer-basics}, write in words what the \(p\)-value represents for
the hypothesis test above comparing the mean rating of romance to action
movies.

\textbf{(LC9.15)} What is the value of the \(p\)-value for the
hypothesis test comparing the mean rating of romance to action movies?

\textbf{(LC9.16)} Do the results of the hypothesis test match up with
...
\caption[Population ratings histogram]{Population ratings histogram}\label{fig:unnamed-chunk-111}
\end{figure}
...
\textbf{(LC9.17)} Why was a histogram chosen as the plot to make for the
\texttt{rating} variable above?

\textbf{(LC9.18)} Why does the shape of the \texttt{rating} histogram
...
{\centering \includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-113-1} 

}

\caption[Sample ratings histogram]{Sample ratings histogram}\label{fig:unnamed-chunk-113}
\end{figure}
...
\textbf{(LC9.21)} What was the purpose of assuming we didn't have access
...
\textbf{(LC9.22)} Reproduce the bootstrapping above using a sample of
size 50 instead of 25. What changes do you see?

\textbf{(LC9.23)} Reproduce the bootstrapping above using a sample of
size 5 instead of 25. What changes do you see?

\textbf{(LC9.24)} How does the sample size affect the analysis?

\textbf{(LC9.25)} Why must bootstrap samples be the same size as the
original sample?
...
\textbf{(LC9.26)} Check to see whether the difference in population mean
...
\textbf{(LC9.27)} Why do you think many scientific fields are shifting
to preferring inclusion of confidence intervals in articles over just
\(p\)-values and hypothesis tests?

\textbf{(LC9.28)} Why is 95\% related to a value of 2 in the margin of
error? What would approximate values be for 90\% and for 99\%?

\textbf{(LC9.29)} Why is a 95\% confidence interval wider than a 90\%
confidence interval? Explain by using a concrete example from everyday
life about what is meant by ``confidence.''

\textbf{(LC9.30)} How would confidence intervals correspond to one-sided
hypothesis tests?

\textbf{(LC9.31)} There is a relationship between the significance level
...
An R script file of all R code used in this chapter is available
\href{http://ismayc.github.io/moderndiver-book/08-conf_intervals.R}{here}.
...
{\centering \includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-131-1} 

}

\caption[Number of flights departing NYC in 2013 by airline - Descending numbers]{Number of flights departing NYC in 2013 by airline - Descending numbers}\label{fig:unnamed-chunk-131}
\end{figure}
...
\begin{center}\includegraphics[width=1\linewidth]{ismaykim_files/figure-latex/unnamed-chunk-133-1} \end{center}
