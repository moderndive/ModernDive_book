...
\addlinespace
rmarkdown &  & 1.4.0.9001 & 2017-04-18 & Github (rstudio/rmarkdown@f2788b8)\\
...
Book was last updated by Chester on Sunday, April 23, 2017 18:58:41 PDT.
...
A \emph{statistic} is a calculation based on one or more variables
measured in the sample. Statistics are usually denoted by lower case
...
\begin{center}\includegraphics[width=\textwidth]{ismaykim_files/figure-latex/do-plot-1} \end{center}
...
\begin{center}\includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-87-1} \end{center}
...
\caption[Barplot of heads with p-value highlighted]{Barplot of heads with p-value highlighted}\label{fig:unnamed-chunk-89}
\end{figure}
...
\caption[Rating vs genre in the population]{Rating vs genre in the population}\label{fig:unnamed-chunk-93}
\end{figure}
...
\StringTok{  }\KeywordTok{sample_n}\NormalTok{(}\DecValTok{34}\NormalTok{) %>%}\StringTok{ }
\StringTok{  }\KeywordTok{ungroup}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\textbf{Note} the addition of the \texttt{ungroup()} function here. This
will be useful shortly in allowing us to shuffle the values of
...
\caption[Genre vs rating for our sample]{Genre vs rating for our sample}\label{fig:unnamed-chunk-95}
\end{figure}
...
\caption[Genre vs rating for our sample as faceted histogram]{Genre vs rating for our sample as faceted histogram}\label{fig:unnamed-chunk-96}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mean_ratings <-}\StringTok{ }\NormalTok{movies_genre_sample %>%}\StringTok{ }
\StringTok{  }\KeywordTok{group_by}\NormalTok{(genre) %>%}
\StringTok{  }\KeywordTok{summarize}\NormalTok{(}\DataTypeTok{mean =} \KeywordTok{mean}\NormalTok{(rating))}
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(mosaic)}
\NormalTok{shuffled_ratings <-}\StringTok{ }\CommentTok{#movies_trimmed %>%}
\StringTok{  }\NormalTok{movies_genre_sample %>%}\StringTok{ }
...
\KeywordTok{head}\NormalTok{(rand_distn, }\DecValTok{10}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-103-1} 

}

\caption[Simulated differences in means histogram]{Simulated differences in means histogram}\label{fig:unnamed-chunk-103}
\end{figure}

\subsection{The p-value}\label{the-p-value-1}

Remember that we are interested in seeing where our observed sample mean
difference of 0.8294 falls on this null/randomization distribution. We
are interested in simply a difference here so ``more extreme''
...
\caption[Shaded histogram to show p-value]{Shaded histogram to show p-value}\label{fig:unnamed-chunk-104}
\end{figure}

Remember that the observed difference in means was 0.8294. We have
shaded green all values at or above that value and also shaded green
those values at or below its negative value (since this is a two-tailed
test). We can add a vertical line to represent both the observed
difference and its negative instead. To better estimate how large the
\(p\)-value will be, we also increase the number of bins to 100 here
from 20:
...
\caption[Histogram with vertical lines corresponding to observed statistic]{Histogram with vertical lines corresponding to observed statistic}\label{fig:unnamed-chunk-105}
\end{figure}

At this point, it is important to take a guess as to what the
\(p\)-value may be. We can see that there are only a few shuffled
...
2\%, or maybe 3\%, but certainly not 30\% or more. **You'll find
yourself getting very strange results if you've messed up the signs in
your calculation of the \(p\)-value so you should always check first
that you have a range of reasonable values after looking at the
histogram for the \(p\)-value. Lastly, we calculate the \(p\)-value
directly using \texttt{dplyr}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(pvalue_movies <-}\StringTok{ }\NormalTok{rand_distn %>%}
\StringTok{  }\KeywordTok{filter}\NormalTok{(}\KeywordTok{abs}\NormalTok{(diffmean) >=}\StringTok{ }\NormalTok{obs_diff) %>%}
\StringTok{  }\KeywordTok{nrow}\NormalTok{() /}\StringTok{ }\KeywordTok{nrow}\NormalTok{(rand_distn))}
\end{Highlighting}
\end{Shaded}
...
We have around 1.32\% of values as extreme or more extreme than our
observed effect in both directions. Assuming we are using a 5\%
...
that are at or above 2.5522 or at or below -2.5522.
...
mean rating for romance movies is between 0.159 and 1.5 points higher
...
\StringTok{  }\KeywordTok{geom_point}\NormalTok{() +}
\StringTok{  }\KeywordTok{geom_smooth}\NormalTok{(}\DataTypeTok{method =} \StringTok{"lm"}\NormalTok{, }\DataTypeTok{se =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{color =} \StringTok{"red"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rand_slope_distn <-}\StringTok{ }\NormalTok{mosaic::}\KeywordTok{do}\NormalTok{(}\DecValTok{5000}\NormalTok{) *}
...
\KeywordTok{names}\NormalTok{(rand_slope_distn)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-171-1} \end{center}
...
\begin{center}\includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-174-1} \end{center}
...
\begin{center}\includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-176-1} \end{center}
...
{\centering \includegraphics[width=\textwidth]{ismaykim_files/figure-latex/unnamed-chunk-183-1} 

}

\caption[Number of flights departing NYC in 2013 by airline - Descending numbers]{Number of flights departing NYC in 2013 by airline - Descending numbers}\label{fig:unnamed-chunk-183}
\end{figure}
...
\begin{center}\includegraphics[width=1\linewidth]{ismaykim_files/figure-latex/unnamed-chunk-185-1} \end{center}
