...
```{r regression-conditional-text, echo=FALSE, results="asis"}
if (is_latex_output()) {
...
```{r regression-load-internal, message=FALSE, echo=FALSE}
...
```{r regression-dynamic-text, echo=FALSE}
...
```{r correlation1, echo=FALSE, fig.cap="Nine different correlation coefficients.", fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
...
```{r regression-summarize-v2, echo=FALSE}
cor_ch5 <- UN_data_ch5 |>
  summarize(correlation = cor(fert_rate, life_exp,
                              use = "complete.obs")) |>
  round(3) |>
...
```
```{r regression-lm-fertility-alt, echo=FALSE}
...
```{r numxplot4, echo=FALSE, fig.cap="Example of observed value, fitted value, and residual.", fig.height=ifelse(knitr::is_latex_output(), 3, 4), message=FALSE}
...
```{r regression-points-alt, echo=FALSE}
set.seed(76)
regression_points <- get_regression_points(demographics_model)
regression_points |>
...
```{r moderndive-figure-wrapper, echo=FALSE, fig.cap="The concept of a wrapper function.", out.width="50%"}
...
```{r regression-create-life_exp_worldwide, echo=FALSE}
...
```{r regression-assign-median_life_exp, echo=FALSE}
# This code is used for dynamic non-static in-line text output purposes
...
```{r catxplot0b, echo=FALSE, fig.cap="Life expectancy in 2022 by continent (faceted).", fig.height=ifelse(knitr::is_latex_output(), 4, 4)}
...
```{r regression-create-life_exp_model, echo=FALSE}
# This code is used for dynamic non-static in-line text output purposes
...
```{r regression-create-afghanistan_life_e, echo=FALSE}
...
```{r moderndive-figure-causal-graph-2, echo=FALSE, fig.cap="Does sleeping with shoes on cause headaches?", out.width="60%"}
...
```{r moderndive-figure-causal-graph, echo=FALSE, out.width="30%", fig.cap="Causal graph."}
...
```{r best-fitting-line, fig.height=ifelse(knitr::is_latex_output(), 5.4, 7), echo=FALSE, fig.cap="Example of observed value, fitted value, and residual.", message=FALSE}
# First residual
demographics_model <- lm(fert_rate ~ life_exp, data = UN_data_ch5)
...
```{r three-lines, fig.cap="Regression line and two others.", out.width="60%", echo=FALSE, message=FALSE}
example <- tibble(
  x = c(0, 0.5, 1),
  y = c(2, 1, 3)
)
...
```
```{r regpoints-augment, echo=FALSE, message=FALSE, warning=FALSE}
library(broom)
library(janitor)
demographics_model |>
  augment() |>
...
```{r regression-conditional-text-dup1, echo=FALSE, results="asis"}
if (is_latex_output()) {
...
```{r regression-conditional-text-v2, echo=FALSE, results="asis"}
...
```{r guess-the-correlation, echo=FALSE, fig.cap="(ref:guess-corr)", out.width="70%"}
