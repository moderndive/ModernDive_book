...
**Q10.** In `lm(weight ~ sex, data = vball)` (a regression with one categorical predictor, levels `F` and `M`), the **fitted value** $\widehat{\text{weight}}$ for any *male* volleyball player equals:
...
# Pre-define `vball` — the volleyball-athlete subset that this chapter's
