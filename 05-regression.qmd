---
description: &desc "Fit and interpret a simple linear regression model with lm(): read its slope and intercept, and compute fitted values and residuals."
open-graph:
  description: *desc
twitter-card:
  description: *desc
...
library(moderndive)
UN_data_ch5 <- un_member_states_2024 |>
  filter(!is.na(life_expectancy_2022), !is.na(fertility_rate_2022))
glimpse(UN_data_ch5)
```
:::
...
```{webr-r}
gapminder2022 <- un_member_states_2024 |>
  select(country, continent, life_exp = life_expectancy_2022) |>
  na.omit()
ggplot(gapminder2022, aes(x = continent, y = life_exp)) +
  geom_boxplot() +
  labs(x = "Continent", y = "Life expectancy",
...
```{r regression-scatter-lifeexp, fig.alt="Scatterplot of life expectancy against Human Development Index across UN member states. Strong positive nonlinear association — countries with higher HDI tend to have substantially higher life expectancy, with the relationship flattening at the top end.", fig.height=1.5}
...
```{r regression-scatter, fig.alt="Scatterplot of fertility rate against Human Development Index across UN member states. Strong negative association — countries with higher HDI generally have lower fertility rates.", fig.height=1.5}
ggplot(data = un_member_states_2024,
...
a. $80 + 76 = 156$
b. **$80 - 76 = 4$** (observed minus fitted)
c. $76 - 80 = -4$ (fitted minus observed)
d. $80 / 76 \approx 1.05$
...
a. Maximize the slope
b. Minimize the sum of the **residuals** ($\sum (y_i - \widehat{y}_i)$)
c. **Minimize the sum of the *squared* residuals** ($\sum (y_i - \widehat{y}_i)^2$)
d. Make every residual exactly 0

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** That's where the name comes from: *least* (smallest possible) *squares* (of the residuals). Why square? Squaring (i) makes negative and positive residuals contribute equally and (ii) penalizes large residuals more than small ones, pulling the line toward the bulk of the data rather than letting one outlier dominate. Option (b) is wrong because positive and negative residuals would cancel; option (d) is impossible unless every point already lies on a single line.
:::
...
# Pre-define `bball` — the basketball-athlete subset that this chapter's
# exercises reference throughout. EX 5.1 asks the student to construct it;
# pre-defining here keeps later exercises runnable even when a student
# starts mid-list.
bball <- olympic_athletes |>
  filter(sport == "Basketball", !is.na(height), !is.na(weight))

# Pre-define `UN_data_ch5` — chapter narrative shows the build, but
# downstream {webr-r} cells reference it directly. Mirrors the canonical
# {r} definition in @sec-model1EDA.
UN_data_ch5 <- un_member_states_2024 |>
  select(iso,
         life_exp  = life_expectancy_2022,
         fert_rate = fertility_rate_2022,
