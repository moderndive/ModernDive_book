...
**Q1.** In the model `lm(fert_rate ~ life_exp, data = UN_data_ch5)`, what does the *slope* coefficient represent?

a. The expected `fert_rate` when `life_exp` is 0
b. **The expected change in `fert_rate` for a one-unit increase in `life_exp`**
c. The total variance in `fert_rate`
d. The correlation between the two variables

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Slope = expected change in $y$ per one-unit change in $x$: here, the expected change in `fert_rate` for each additional year of `life_exp`. The intercept (option a) is the predicted $y$ when $x = 0$.
:::
...
**Q4.** The chapter's regression `lm(fert_rate ~ life_exp, data = UN_data_ch5)` produces an intercept of about 12.6. Should you interpret this as "a country with a life expectancy of 0 years is predicted to have a fertility rate of about 12.6"?

a. Yes — that's exactly what the intercept means, and it's a useful summary here
b. Mathematically yes, but the interpretation is **not meaningful in context** because no real country has `life_exp = 0`; you're extrapolating outside the data
c. No — intercepts are meaningless
d. The intercept is wrong; refit the model

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** The intercept has a *practical* interpretation only when $x = 0$ falls within (or near) the observed range of $x$. A life expectancy of 0 is impossible — the observed values run from roughly 54 to 86 years — so the math is fine, but the line just isn't valid that far from the data.
:::
...
**Q10.** In `lm(weight ~ sex, data = vball)` (a regression with one categorical predictor, levels `F` and `M`; `vball` is a volleyball-athlete dataset you'll work with in Chapter 10), the **fitted value** $\widehat{\text{weight}}$ for any *male* volleyball player equals:

a. The overall mean weight across all `vball` players
b. **The mean weight among male players in `vball`** (with one categorical predictor, there's no $x$ to slide along — each group's prediction is just that group's mean)
c. Zero
