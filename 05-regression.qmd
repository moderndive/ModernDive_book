...
```{r lc-inline-setup-05, include=FALSE, purl=FALSE}
...
In particular, when the two variables are numerical, we can compute the \index{correlation (coefficient)} *correlation coefficient*. Generally speaking, *coefficients* are quantitative expressions of a specific phenomenon. A *correlation coefficient* measures the *strength of the linear relationship between two numerical variables*. Its value goes from -1 to 1 where:
...
```{r lc-sol-05-01, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 1))
```
...
```{r lc-sol-05-02, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 2))
```
...
```{r lc-sol-05-03, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 3))
```
...
```{r lc-sol-05-04, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 4))
```
...
```{r lc-sol-05-05, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 5))
```
...
```{r lc-sol-05-06, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 6))
```
...
```{r lc-sol-05-07, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 7))
```
...
UN_data_ch5 |>
  ...
  kbl(
#    digits = 4,
    caption = "Data for the 21st country out of 181",
    booktabs = TRUE,
    linesep = ""
  ) |>
  kable_styling(
...
```{r lc-sol-05-08, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 8))
```
...
```{r lc-sol-05-09, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 9))
```
...
```{r lc-sol-05-10, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 10))
```
...
1. Differences within continents: How does life expectancy vary within the world's six continents? For example, is the spread of life expectancy among the countries of Africa larger than the spread of life expectancy among the countries of Asia?
...
1. `continent`: A categorical variable with six levels. Here "levels" correspond to the possible categories: Africa, Asia, Europe, North America, Oceania, and South America. This is the explanatory variable $x$ of interest.
...
- `group`: Breaks down a categorical variable into its unique levels. For this variable, the groups correspond to Africa, Asia, Europe, North America, Oceania, and South America. 
...
```{r lc-sol-05-11, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 11))
```
...
```{r lc-sol-05-12, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 12))
```
...
```{r lc-sol-05-13, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 13))
```
...
```{r lc-sol-05-14, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 14))
```
...
regression_points |>
  ...
  kbl(
    digits = 3,
    caption = "Regression points (Sample of 6 out of 188 countries)",
    booktabs = TRUE,
    linesep = ""
  ) |>
  kable_styling(
...
Observe in @tbl-model2-residuals that `life_exp_hat` contains the fitted values $\widehat{y}$ = $\widehat{\text{life exp}}$. If you look closely, there are only 6 possible values for `life_exp_hat`. These correspond to the six mean life expectancies for the 6 continents that we displayed in @tbl-continent-mean-life-expectancies and computed using the regression coefficient values.
...
```{r lc-sol-05-15, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 15))
```
...
```{r lc-sol-05-16, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 16))
```
...
```{r lc-sol-05-17, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 17))
```
...
```{r lc-sol-05-18, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 18))
```
...
Looking at both of the scatterplots above, we see a strong positive association between the Human Development Index (HDI) and life expectancy, one that curves and flattens out among the countries with the highest HDI values, as well as a strong negative correlation between the Human Development Index (HDI) and fertility rates. The correlation coefficients between the Human Development Index (HDI) and life expectancy, as well as the Human Development Index (HDI) and fertility rates, are also given. These findings as well as some additional understanding of socioeconomic factors suggest that the Human Development Index (HDI) is a confounding variable in the relationship between life expectancy and fertility rates.     
...
```{r lc-sol-05-19, echo=FALSE, results='asis', purl=FALSE}
cat(lc_solution(5, 19))
```
...
a. The expected `fert_rate` when `life_exp` is 0
b. The total variance in `fert_rate` that is explained by `life_exp`
c. The expected change in `fert_rate` per extra year of `life_exp`
d. The correlation between `fert_rate` and `life_exp`
...
a. The slope of the least-squares fitted line
b. The sum of squared errors across all the data points
c. The R-squared value reported by the model
d. The vertical gap from a point to the fitted line
...
b. The variables have a perfect linear relationship
...
a. Yes, that's exactly what the intercept means, and it's a useful summary here
b. The intercept is wrong; the model needs refitting
c. No; an intercept never has a meaningful interpretation
d. Mathematically yes; not meaningful, it extrapolates outside the data
...
a. $76 - 80 = -4$ (fitted minus observed)
...
a. Weight increases 2.5 kg on average per additional cm of height
b. Height *causes* an increase in weight
c. Every tall person weighs exactly 2.5 kg more than every short person
d. Weight equals 2.5 times height
...
b. $r$ measures only *linear* association
c. Linear regression always gives $r = 0$ for U-shapes
d. You need to collect more data
...
a. Make every residual exactly equal to 0
b. Minimize the sum of the **signed residuals** ($\sum (y_i - \widehat{y}_i)$)
c. Maximize the slope of the fitted line
d. **Minimize the sum of *squared* residuals** ($\sum (y_i - \widehat{y}_i)^2$)
...
a. The mean weight among male players in `vball`
b. Zero, since `M` is not the baseline level
c. The overall mean weight across all `vball` players
...
# Pre-define `planets_lite` — the complete-case subset that this chapter's
...
# Pre-define the models and subsets that later exercises build on
# (EX 5.6 constructs model_mass_radius; EX 5.12/5.13 construct pl2 and
# model_mass_method), for the same start-mid-list reason.
model_mass_radius <- lm(mass_earth ~ radius_earth, data = planets_lite)
pl2 <- planets_lite |>
  filter(discovery_method %in% c("Transit", "Radial Velocity")) |>
  mutate(discovery_method = factor(discovery_method))
model_mass_method <- lm(mass_earth ~ discovery_method, data = pl2)
