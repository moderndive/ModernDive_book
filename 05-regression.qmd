...
### Observed/fitted values and residuals {.unnumbered}
...
**EX5.12 (★★)** Use the model to **predict** the weight of a basketball athlete who is 200 cm tall. Show your math (intercept + slope × 200) and verify with `predict()`.
...
**EX5.20 (★★★)** Plot residuals against fitted values for `model_b`. Do you see any obvious pattern (curvature, fan-shape) that would suggest mis-specification?
...
### Correlation is not necessarily causation {.unnumbered}
...
### `get_regression_*()` companion functions {.unnumbered}
