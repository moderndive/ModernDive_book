...
We have introduced data visualization in @sec-viz, data wrangling in @sec-wrangling, and data importing and "tidy" data in @sec-tidy. In this chapter, we work with **regression**, a method that helps us study the relationship between an *outcome variable* or *response* and one or more *explanatory variables* or *regressors*.
...
In @sec-regression, we introduce linear regression with only one regressor. In @sec-model1, the explanatory variable is numerical. This scenario is known as *simple linear regression*. In @sec-model2, the explanatory variable is categorical.

In @sec-multiple-regression on multiple regression, we extend these ideas and work with models with two explanatory variables.  In @sec-model4, we work with two numerical explanatory variables. In @sec-model3, we work with one numerical and one categorical explanatory variable and study the model with and without interactions.

In @sec-inference-for-regression on inference for regression, we revisit the regression models and analyze the results using *statistical inference*, a method discussed in @sec-sampling, @sec-confidence-intervals, and @sec-hypothesis-testing on sampling, bootstrapping and confidence intervals, and hypothesis testing and $p$-values, respectively. The focus there is also be on using linear regression for prediction instead of explanation.
...
### Needed packages {.unnumbered #sec-reg-packages}
...
1. The `tidyverse` "umbrella" [@R-tidyverse] package. Recall from our discussion in @sec-tidyverse-package that loading the `tidyverse` package by running `library(tidyverse)` loads the following commonly used data science packages all at once:
...
You can do this by using RStudio's spreadsheet viewer or by using the `glimpse()` function as introduced in @sec-exploredataframes on exploring data frames:

```{r regression-glimpse-UN_data_ch5}
glimpse(UN_data_ch5)
```

Observe that ``Rows: `r n_demo_ch5` `` indicates that there are `r n_demo_ch5` rows/observations in `UN_data_ch5` after filtering out the missing values, where each row corresponds to one observed country/member state. It is important to note that the *observational unit* \index{observational unit} is an individual country. Recall from @sec-exploredataframes that the observational unit is the "type of thing" that is being measured by our variables.
...
An alternative way to look at the raw data values is by choosing a random sample of the rows in `UN_data_ch5` by piping it into the `slice_sample()` \index{R packages!dplyr!slice\_sample()} function from the `dplyr` package. Here we set the `n` argument to be ``r sample_size``, indicating that we want a random sample of `r sample_size` rows. We display the results in @tbl-random-countries. Note that due to the random nature of the sampling, you will likely end up with a different subset of `r sample_size` rows.
...
```{r tbl-random-countries, echo=FALSE, purl=FALSE}
UN_data_ch5 |> 
...
We have looked at the raw values in our `UN_data_ch5` data frame and got a preliminary sense of the data. We can now compute summary statistics. We start by computing the mean and median of our numerical outcome variable `fert_rate` and our numerical explanatory variable `life_exp`. We do this by using the `summarize()` function from `dplyr` along with the `mean()` and `median()` summary functions we saw in @sec-summarize.
...
@fig-correlation1 gives examples of nine different correlation coefficient values for hypothetical numerical variables $x$ and $y$. 

```{r fig-correlation1, fig.alt="Nine small scatterplots arranged in a 3x3 grid, each labeled with a correlation coefficient from -1 to +1. The visible cloud-of-points shape changes from tightly increasing (r near +1) to a circular blob (r near 0) to tightly decreasing (r near -1).", echo=FALSE, fig.cap="Nine different correlation coefficients.", fig.height=ifelse(knitr::is_latex_output(), 3, 4), purl=FALSE}
...
We build on the scatterplot in @fig-numxplot1 by adding a "best-fitting" line: of all possible lines we can draw on this scatterplot, it is the line that "best" fits through the cloud of points. We do this by adding a new `geom_smooth(method = "lm", se = FALSE)` layer to the `ggplot()` code that created the scatterplot in @fig-numxplot1. The `method = "lm"` argument sets the line to be a "**l**inear **m**odel." The `se = FALSE` \index{R packages!ggplot2!geom\_smooth()} argument suppresses _standard error_ uncertainty bars. (We'll define the concept of _standard error_ later in @sec-sampling-variation.)

```{r fig-numxplot3, fig.alt="Same scatterplot of fertility rate vs life expectancy, with a downward-sloping straight regression line overlaid through the cloud of points.", fig.cap="Scatterplot of life expectancy and fertility rate with regression line.", message=FALSE, fig.height=ifelse(knitr::is_latex_output(), 4, 5)}
...
However, when defining a regression line like the one in @fig-numxplot3, we use slightly different notation: the equation of the regression line is $\widehat{y} = b_0 + b_1 \cdot x$ \index{regression!equation of a line}. The intercept coefficient is $b_0$, so $b_0$ is the value of $\widehat{y}$ when $x = 0$. The slope coefficient for $x$ is $b_1$, i.e., the increase in $\widehat{y}$ for every increase of one in $x$. Why do we put a "hat" on top of the $y$? It's a form of notation commonly used in regression to indicate that we have a \index{regression!fitted value} "fitted value," or the value of $y$ on the regression line for a given $x$ value as discussed further in @sec-model1points.

We know that the regression line in @fig-numxplot3 has a negative slope $b_1$ corresponding to our explanatory $x$ variable `life_exp`. Why? Because as countries tend to have higher `life_exp` values, they tend to have lower `fert_rate` values. However, what is the numerical value of the slope $b_1$? What about the intercept $b_0$?  We do not compute these two values by hand, but rather we use a computer!
...
We first focus on interpreting the regression coefficients, and later revisit the code that produced it. The coefficients are the intercept $b_0 = `r demo_line[1]`$ and the slope $b_1 = `r demo_line[2]`$ for `life_exp`. Thus the equation of the regression line in @fig-numxplot3 follows:
...
The intercept $b_0$ = `r demo_line[1]` is the average fertility rate $\widehat{y}$ = $\widehat{\text{fertility}\_\text{rate}}$ for those countries that had a `life_exp` of 0. Or in graphical terms, where the line intersects the $y$ axis for $x$ = 0. Note, however, that while the \index{regression!equation of a line!intercept} intercept of the regression line has a mathematical interpretation, it has no *practical* interpretation here, since observing a `life_exp` of 0 is impossible. Furthermore, looking at the scatterplot with the regression line in @fig-numxplot3, no countries had a life expectancy anywhere near 0.
...
We only state that there is an *associated* increase and not necessarily a *causal* increase. Perhaps it may not be that higher life expectancies directly cause lower fertility rates. Instead, wealthier countries could tend to have stronger educational backgrounds, improved health, a higher standard of living, and have lower fertility rates, while at the same time these wealthy countries also tend to have higher life expectancies. Just because two variables are strongly associated, it does not necessarily mean that one causes the other. This is summed up in the often-quoted phrase, "correlation is not necessarily causation."  We discuss this idea further in @sec-correlation-is-not-causation.
...
Now that we have learned how to compute the equation for the regression line in @fig-numxplot3 using the model coefficient values and how to interpret the resulting intercept and slope, we revisit the code that generated these coefficients:
...
* The combination of `y ~ x` is called a *model formula*. (Note the order of `y` and `x`.) In our case, the model formula is `fert_rate ~ life_exp`. We saw such model formulas earlier with the [`get_correlation()`](https://moderndive.github.io/moderndive/reference/get_correlation.html) function in @sec-model1EDA.
...
We just saw how to get the value of the intercept and the slope of a regression line from the output of the `coef()` function. Now instead say we want information on individual observations. For example, we focus on the 21st of the `r n_demo_ch5` countries in the `UN_data_ch5` data frame in @tbl-country-21. This corresponds to the UN member state of Bosnia and Herzegovina (BIH).

```{r tbl-country-21, echo=FALSE, purl=FALSE}
...
What is the value $\widehat{y}$ on the regression line corresponding to this country's `life_exp` value of `r x`? In @fig-numxplot4 we mark three values corresponding to these results for Bosnia and Herzegovina and give their statistical names:
...
```{r fig-numxplot4, fig.alt="Annotated scatterplot showing one observed point, its corresponding fitted value on the regression line directly below it, and the vertical residual segment connecting them.", echo=FALSE, fig.cap="Example of observed value, fitted value, and residual.", fig.height=ifelse(knitr::is_latex_output(), 3, 4), message=FALSE, purl=FALSE}
...
```{r tbl-regression-points-1, echo=FALSE, purl=FALSE}
set.seed(76)
regression_points <- get_regression_points(demographics_model)
regression_points |>
...
This function is an example of what is known in computer programming as a *wrapper function*. \index{functions!wrapper} It takes other pre-existing functions and "wraps" them into a single function that hides its inner workings. This concept is illustrated in @fig-moderndive-figure-wrapper.

```{r fig-moderndive-figure-wrapper, fig.alt="Schematic of a wrapper function: an outer function box wraps an inner function box, taking the same inputs and producing the same outputs but with simpler syntax.", echo=FALSE, fig.cap="The concept of a wrapper function.", out.width="50%", purl=FALSE}
include_graphics("images/shutterstock/wrapper_function.png")
```

So all you need to worry about is what the inputs look like and what the outputs look like; you leave all the other details "under the hood of the car." In our regression modeling example, the `get_regression_points()` function takes a saved `lm()` linear regression model as input and returns a data frame of the regression predictions as output. If you are interested in learning more about the `get_regression_points()` function's inner workings, check out @sec-underthehood.

We inspect the individual columns and match them with the elements of @fig-numxplot4:
...
Just as we did for the 21st country in the `UN_data_ch5` dataset (in the first row of the table), we repeat the calculations for the 24th country (in the fourth row of @tbl-regression-points-1). This corresponds to the country of Brunei (BRN):
...
If you like, you can skip ahead to @sec-leastsquares to learn about the processes behind what makes "best-fitting" regression lines. As a primer, a "best-fitting" line refers to the line that minimizes the *sum of squared residuals* out of all possible lines we can draw through the points. In @sec-model2, we'll discuss another common scenario of having a categorical explanatory variable and a numerical outcome variable.
...
To answer such questions, we use an updated version of the `gapminder` data frame we visualized in @fig-gapminder in @sec-gapminder on the grammar of graphics. This updated `un_member_states_2024` data we saw earlier in this chapter. It is included in the `moderndive` \index{R packages!moderndive} package and has international development statistics such as life expectancy, GDP per capita, and population for `r n_countries` countries for years near 2024. We use this data for basic regression again, but now using an explanatory variable $x$ that is categorical, as opposed to the numerical explanatory variable model we used in the previous @sec-model1:
...
When the explanatory variable $x$ is categorical, the concept of a "best-fitting" regression line is a little different than the one we saw previously in @sec-model1 where the explanatory variable $x$ was numerical. We study these differences shortly in @sec-model2table, but first we conduct an exploratory data analysis.
...
We perform the first common step in an exploratory data analysis: looking at the raw data values. You can do this by using RStudio's spreadsheet viewer or by using the `glimpse()` command as introduced in @sec-exploredataframes on exploring data frames:
...
We next look at a random sample of three out of the `r gapminder2022_rows` countries in @tbl-model2-data-preview. 

```{r regression-alt2, eval=FALSE}
gapminder2022 |> sample_n(size = 3)
```
```{r tbl-model2-data-preview, echo=FALSE, purl=FALSE}
gapminder2022 |>
  sample_n(3) |>
  kbl(
    digits = 3,
...
We can answer this question by performing the last of the three common steps in an exploratory data analysis: creating data visualizations. We visualize the distribution of our outcome variable $y$ = `life_exp` in @fig-lifeexp2022hist.

```{r fig-lifeexp2022hist, fig.alt="Left-skewed histogram of country-level life expectancy in 2022, with most countries between 70 and 80 years and a long left tail of countries with lower life expectancy.", echo=TRUE, fig.cap="Histogram of life expectancy in 2022.", fig.height=ifelse(knitr::is_latex_output(), 3, 4)}
...
```{r fig-catxplot0b, fig.alt="Faceted strip-plot of life expectancy by continent: one panel per continent (Africa, Americas, Asia, Europe, Oceania), each showing the spread of country life-expectancy values within that continent.", echo=FALSE, fig.cap="Life expectancy in 2022 by continent (faceted).", fig.height=ifelse(knitr::is_latex_output(), 4, 4), purl=FALSE}
...
```{r fig-catxplot1, fig.alt="Side-by-side boxplots of country life expectancy (y-axis) by continent (x-axis). Africa has the lowest median and widest spread; Europe and Oceania have the highest medians.", fig.cap="Life expectancy in 2022 by continent (boxplot).", fig.height=ifelse(knitr::is_latex_output(), 2.5, 4)}
...
```
```{r tbl-catxplot0, echo=FALSE, purl=FALSE}
life_exp_by_continent |>
  kbl(
    digits = 3,
...
Observe the order of the second column `median` life expectancy: Africa is lowest, Europe the highest, and the others have similar medians between Africa and Europe. This ordering corresponds to the ordering of the solid black lines inside the boxes in our side-by-side boxplot in @fig-catxplot1. 
...
We now turn our attention to the values in the third column `mean`. Using Africa's mean life expectancy of `r intercept` as a *baseline for comparison*, we start making comparisons to the mean life expectancies of the other four continents and put these values in @tbl-continent-mean-life-expectancies, which we'll revisit later on in this section.
...
```{r tbl-continent-mean-life-expectancies, echo=FALSE, purl=FALSE}
gapminder2022 |>
  group_by(continent) |>
...
In @sec-model1table we introduced simple linear regression, which involves modeling the relationship between a numerical outcome variable $y$ and a numerical explanatory variable $x$. In our life expectancy example, we now instead have a categorical explanatory variable `continent`. Our model will not yield a "best-fitting" regression line like in @fig-numxplot3, but rather *offsets* relative to a baseline for comparison.\index{offset}

As we did in @sec-model1table when studying the relationship between fertility rates and life expectancy, we output the regression coefficients for this model. Recall that this is done in two steps:
...
1. `continentAsia` corresponds to countries in Asia and the value +`r offset_asia` is the same difference in mean life expectancy relative to Africa we displayed in @tbl-continent-mean-life-expectancies. In other words, the mean life expectancy of countries in Asia is $`r intercept` + `r offset_asia` = `r mean_asia`$.
1. `continentEurope` corresponds to countries in Europe and the value +`r offset_europe` is the same difference in mean life expectancy relative to Africa we displayed in @tbl-continent-mean-life-expectancies. In other words, the mean life expectancy of countries in Europe is $`r intercept` + `r offset_europe` = `r mean_europe`$.
...
which is the mean life expectancy for countries in Asia of `r mean_asia` years in @tbl-continent-mean-life-expectancies. Note the "offset from the baseline for comparison" is +`r offset_asia` years. 
...
which is the mean life expectancy for South American countries of `r mean_south_america` years in @tbl-continent-mean-life-expectancies. The "offset from the baseline for comparison" here is +`r offset_asia` years.
...
Recall in @sec-model1points, we defined the following three concepts:
...
```{r regression-reg-points-alt, eval=FALSE}
get_regression_points(life_exp_model, ID = "country")
```
```{r tbl-model2-residuals, echo=FALSE, purl=FALSE}
...
```{r regression-create-afghanistan_life_e, echo=FALSE, purl=FALSE}
...
## Related topics {#sec-reg-related-topics}

### Correlation is not necessarily causation {#sec-correlation-is-not-causation}

Throughout this chapter we have been cautious when interpreting regression slope coefficients. We always discussed the "associated" effect of an explanatory variable $x$ on an outcome variable $y$. For example, our statement from @sec-model1table that "for every increase of 1 unit in `life_exp`, there is an *associated* decrease of on average `r abs(demo_line[2])` units of `fert_rate`." We include the term "associated" to be extra careful not to suggest we are making a *causal* statement. So while `life_exp` is negatively correlated with `fert_rate`, we can't necessarily make any statements about life expectancy's direct causal effect on fertility rates without more information. 

Here is another example depicted in @fig-moderndive-figure-causal-graph-2: a not-so-great medical doctor goes through medical records and finds that patients who slept with their shoes on tended to wake up more with headaches. So this doctor declares, "Sleeping with shoes on causes headaches!"

```{r fig-moderndive-figure-causal-graph-2, fig.alt="Cartoon causal-graph diagram suggesting sleeping with shoes on might appear to cause headaches, with arrows hinting at a third variable (drinking the night before) as the actual cause of both.", echo=FALSE, fig.cap="Does sleeping with shoes on cause headaches?", out.width="60%", purl=FALSE}
include_graphics("images/shutterstock/shoes_headache.png")
```

However, there is a good chance that if someone is sleeping with their shoes on, it is potentially because they are intoxicated from alcohol. Furthermore, higher levels of drinking leads to more hangovers, and hence more headaches. The amount of alcohol consumption here is what's known as a *confounding/lurking* variable\index{confounding variable}. It "lurks" behind the scenes, confounding the causal relationship (if any) of "sleeping with shoes on" with "waking up with a headache." We can summarize this in @fig-moderndive-figure-causal-graph with a *causal graph* where Y is a *response* variable and X is a *treatment* variable whose causal effect we are interested in. \index{variables!treatment}

```{r fig-moderndive-figure-causal-graph, fig.alt="Generic causal graph: arrows from a treatment variable, a confounding variable, and an outcome variable, illustrating how confounding can produce spurious associations.", echo=FALSE, out.width="30%", fig.cap="Causal graph.", purl=FALSE}
include_graphics("images/flowcharts/flowchart.009-cropped.png")
```

To study the relationship between Y and X, we could use a regression model where the outcome variable is set to Y and the explanatory variable is set to be X, as you've been doing throughout this chapter. However, @fig-moderndive-figure-causal-graph also includes a third variable with arrows pointing at both X and Y:
...
```{r regression-scatter-lifeexp, fig.height=1.5}
...
```{r regression-scatter, fig.height=1.5}
ggplot(data = un_member_states_2024, 
...
Regression lines are also known as "best-fitting" lines. But what do we mean by "best"? We unpack the criteria that is used in regression to determine "best." Recall @fig-numxplot4, where for Bosnia and Herzegovina we marked the *observed value* $y$ with a circle, the *fitted value* $\widehat{y}$ with a square, and the *residual* $y - \widehat{y}$ with an arrow. We re-display @fig-numxplot4 in the top-left plot of @fig-best-fitting-line in addition to three more arbitrarily chosen countries:

```{r fig-best-fitting-line, fig.alt="Annotated scatterplot showing the line of best fit, with vertical residual segments drawn from each observed point to the line — emphasizing that the fitted line minimizes the sum of squared vertical distances.", fig.height=ifelse(knitr::is_latex_output(), 5.4, 7), echo=FALSE, fig.cap="Example of observed value, fitted value, and residual.", purl=FALSE, message=FALSE}
# First residual
demographics_model <- lm(fert_rate ~ life_exp, data = UN_data_ch5)
...
```{r regression-create-four_countries, include=FALSE}
...
1. The country of Bosnia and Herzegovina had a life expectancy $x = `r bosnia |> pull(life_exp)`$ and fertility rate $y = `r bosnia |> pull(fert_rate)`$. The residual in this case is $`r bosnia |> pull(fert_rate)` - `r bosnia |> pull(fert_rate_hat)` = `r bosnia |> pull(residual)`$, which we mark with an `r if_else(!is_latex_output(), "blue", "")` arrow in the top-left plot.
1. The addition of Chad, which had a life expectancy $x = `r chad |> pull(life_exp)`$ and fertility rate $y = `r chad |> pull(fert_rate)`$. The residual in this case is $`r chad |> pull(fert_rate)` - `r chad |> pull(fert_rate_hat)` = `r chad |> pull(residual)`$, which we mark with a new `r if_else(!is_latex_output(), "blue", "")` arrow in the top-right plot.
...
```{r regression-create-demographics_model, eval=FALSE}
...
```{r regression-create-demographics_model-alt, echo=FALSE}
# Fit regression model:
demographics_model <- lm(fert_rate ~ life_exp, data = UN_data_ch5)
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Note in @fig-three-lines there are 3 points marked with dots and:
...
```{r fig-three-lines, fig.alt="Same scatterplot of fertility rate vs life expectancy, but with three candidate lines overlaid: the actual best-fit regression line plus two other lines that fit visibly worse.", fig.cap="Regression line and two others.", out.width="60%", echo=FALSE, purl=FALSE, message=FALSE}
example <- tibble(
  x = c(0, 0.5, 1),
  y = c(2, 1, 3)
)
...
```
```{r tbl-regpoints-augment, echo=FALSE, purl=FALSE, message=FALSE, warning=FALSE}
library(broom)
library(janitor)
demographics_model |>
  augment() |>
...
a. The expected `life_exp` when `fert_rate` is 0
b. The expected change in `life_exp` for a one-unit increase in `fert_rate`
c. The total variance in `life_exp`
d. The correlation between the two variables

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Slope = expected change in $y$ per one-unit change in $x$. The intercept (option a) is the predicted $y$ when $x = 0$.
:::

**Q2.** A *residual* is:

a. The slope of the fitted line
b. The squared error of the model
c. The vertical distance from an observed point to the fitted line: $y_i - \hat{y}_i$
d. The R-squared value

::: {.callout-tip collapse="true" title="Show answer"}
**(c)** $e_i = y_i - \hat{y}_i$. Small residuals (in aggregate) suggest the model fits well.
:::

**Q3.** A correlation coefficient of $r = -0.78$ between two variables means:

a. The relationship is causal
b. There is a strong, *negative* linear relationship
c. The variables are unrelated
d. The variables have a perfect linear relationship

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Negative sign = inverse relationship; $|r|$ close to 1 = strong. Causation requires a randomized experiment or careful confounding adjustment, not just a strong correlation.
:::
...
a. Yes — that's exactly what the intercept means
b. Mathematically yes, but the interpretation is **not meaningful in context** because no real country has `fert_rate = 0`; you're extrapolating outside the data
c. No — intercepts are meaningless
d. The intercept is wrong; refit the model

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Intercept interpretation only matters when $x = 0$ falls within (or near) the observed range of $x$. Negative life expectancy is nonsensical, but the math is fine — the line just isn't valid that far from the data. Centering predictors is one fix.
:::

**Q5.** A regression model has $R^2 = 0.85$. This means:

a. 85% of the variance in $y$ is explained by $x$
b. The model is 85% accurate
c. The p-value is 0.85
d. The slope is 0.85

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** $R^2$ is the proportion of variance in $y$ that the model accounts for. It is **not** an "accuracy" or probability. A high $R^2$ doesn't mean predictions are correct — only that the line follows the cloud of points well.
:::

**Q6.** A regression has slope = 2.5 for `weight ~ height` (kg ~ cm). The correct interpretation:

a. For every 1 cm taller, weight is 2.5 kg more **on average**
b. Tall people weigh exactly 2.5 kg more than short people
c. Height *causes* weight
d. Weight is 2.5 times height

::: {.callout-tip collapse="true" title="Show answer"}
**(a)** "On average" is critical. The slope describes a population-level *average* relationship — individual people vary around the line. Causation (c) requires more than a regression: an experiment or careful confounder adjustment.
:::
...
::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Correlation only captures linear association. It says nothing about causation, doesn't equal the slope value (slopes depend on units; $r$ is unitless), and doesn't imply normality.
:::

**Q8.** A scatterplot shows a clear U-shaped pattern. You compute the correlation and get $r \approx 0$. Why is this misleading?

a. The data is wrong
b. $r$ only captures **linear** relationships; the U is a strong **non-linear** relationship that $r$ misses entirely
c. Linear regression always gives $r = 0$ for U-shapes
d. You need more data
...
**Q9.** After fitting `lm(y ~ x)`, the output shows `slope = 1.2, p-value = 0.001`. This p-value tests:

a. Whether the model is correct
b. Whether the slope differs from zero ($H_0: \beta_1 = 0$)
c. Whether $y$ predicts $x$
d. Whether the data is normal

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** The standard p-value next to a coefficient tests whether that coefficient could plausibly be zero. A small p-value is evidence against $\beta = 0$; it doesn't validate the model overall or check assumptions.
:::

**Q10.** Least squares regression chooses the line that minimizes:

a. The sum of the residuals
b. The sum of the *squared* residuals
c. The sum of the absolute residuals
d. The maximum residual

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Squaring penalizes large deviations more than small ones and produces a unique closed-form solution. The sum of residuals (a) is always zero for any line through the data's centroid, so it can't fit. Sum of absolute deviations (c) is "least absolute deviations" regression — a different (more robust) method.
:::
...
|---|---|---|
| `lm(y ~ x, data = df)` | Fit a simple linear regression | `lm(life_exp ~ fert_rate, data = UN_data_ch5)` |
| `get_regression_table(model)` | Tidy table of coefficients, std. errors, CIs, p-values | `get_regression_table(model)` |
| `get_regression_points(model)` | Tidy table of fitted values + residuals per observation | `get_regression_points(model)` |
| `cor(x, y, use = "complete.obs")` | Pearson correlation coefficient | `cor(UN_data_ch5$fert_rate, UN_data_ch5$life_exp, use = "complete.obs")` |
| `geom_smooth(method = "lm", se = FALSE)` | Overlay a fitted regression line on a `ggplot` | `... + geom_smooth(method = "lm", se = FALSE)` |

:::

## Exercises {.unnumbered #sec-reg-exercises}

The end-of-chapter exercises use `olympic_athletes` (314,749 athlete-rows × 15 columns) from the [`olympicAthletes`](https://github.com/moderndive/olympicAthletes) package. The natural numerical pair is **Height** (cm) and **Weight** (kg); the natural categorical predictor is **Sex** (or `Season`, or `Sport`). Solutions are available to instructors separately.
...
### Simple linear regression: one numerical predictor {.unnumbered}
...
**EX5.9 (★★)** Use `get_regression_points(model_b)` to inspect fitted values and residuals. Print the first 5.

```{webr-r}
# your code here
```

**EX5.10 (★★)** Use `get_regression_summaries(model_b)` and report **R²**. Interpret it in one sentence (in context).

```{webr-r}
get_regression_summaries(model_b)
```

**EX5.11 (★★)** Use the model to **predict** the weight of a basketball athlete who is 200 cm tall. Show your math (intercept + slope × 200) and verify with `predict()`.

```{webr-r}
# your code here
```

**EX5.12 (★★★)** Plot residuals against fitted values. Do you see any obvious pattern (curvature, fan-shape) that would suggest mis-specification?

```{webr-r}
# your code here
```

**EX5.13 (★★★)** Repeat EX5.6 for `Sport == "Gymnastics"` and call the model `model_g`. Compare the two slopes — does Weight rise *faster* or *slower* with Height in gymnastics vs basketball?

```{webr-r}
# your code here
```

### Simple linear regression: one categorical predictor {.unnumbered}

**EX5.14 (★★)** On `bball`, build side-by-side boxplots of `Height` by `Sex`. Which sex has the higher median height?

```{webr-r}
ggplot(bball, aes(x = Sex, y = Height)) + geom_boxplot()
```

**EX5.15 (★★)** Fit `model_sex <- lm(Height ~ Sex, data = bball)` and print `get_regression_table()`. Which sex level is the **baseline** (i.e., absorbed into the intercept)?

```{webr-r}
# your code here
```

**EX5.16 (★★)** Interpret the **intercept** of `model_sex` in one sentence — what does it represent?

**EX5.17 (★★)** Interpret the **`SexM`** coefficient — what does it represent? Connect it to a difference of group means.

**EX5.18 (★★★)** Compute the two group means directly with `dplyr` (`group_by(Sex) |> summarize(mean(Height))`). Confirm that intercept = mean of baseline, and intercept + slope = mean of the other group.

```{webr-r}
# your code here
```

**EX5.19 (★★★)** On the **full** `olympic_athletes` (drop missing `Height`), fit `lm(Height ~ Season)`. Which Season has taller athletes on average?

```{webr-r}
# your code here
```

**EX5.20 (★★★)** Filter to the **5 most-populous sports** (most athlete-rows). Fit `lm(Height ~ Sport)` and read the table — each non-baseline sport gets its own coefficient. Interpret one of them.

```{webr-r}
# your code here
```

### Residuals, diagnostics, and model fit {.unnumbered}

**EX5.21 (★★★)** Build a histogram of residuals from `model_b`. Are they roughly symmetric and centered at zero?

```{webr-r}
# your code here
```

**EX5.22 (★★★)** Plot a residuals vs Height (the predictor) chart. Does the residual spread *grow* with Height (heteroscedasticity)?
...
**EX5.29 (★★★)** Pick *two sports* and fit a regression `Weight ~ Height` to each separately. Compare the slopes — what does that imply about pooling them in a single model?

```{webr-r}
# your code here
```

**EX5.30 (★★★)** Why does the *baseline* level matter for interpreting categorical regression — but **not** for the model's predictions?

**EX5.31 (★★★)** Name two situations in which a *linear* regression model is a poor choice.

**EX5.32 (★★★)** Build a categorical regression of your choosing (one categorical predictor with at least 3 levels), interpret the intercept and one slope coefficient, and write a 1-sentence headline finding.

```{webr-r}
# your code here
```

**EX5.33 (★★)** A regression call `lm(Weight ~ Height, data = olympic_athletes)` will succeed even though many rows have `NA` in `Height` or `Weight`. What does R do silently?

```{webr-r}
# your code here
```

**EX5.34 (★★★)** Name two questions about Olympic athletes that this dataset *cannot* answer with regression alone. For each, what extra data would help?

**EX5.35 (★★★)** Open exploration: pick any sport, fit a regression that tests a question you find interesting, interpret the coefficients, and write your finding as a one-sentence headline.

```{webr-r}
# your exploration here
```

## Conclusion {#sec-reg-conclusion}

### Additional resources {#sec-additional-resources-basic-regression}

```{r regression-conditional-text-dup1, echo=FALSE, results="asis", purl=FALSE}
if (is_latex_output()) {
...
```{r regression-conditional-text-v2, echo=FALSE, purl=FALSE, results="asis"}
...
As we suggested in @sec-model1EDA, interpreting coefficients that are not close to the extreme values of -1, 0, and 1 can be somewhat subjective. To help develop your sense of correlation coefficients, we suggest you play the 80s-style video game called, "Guess the Correlation," at <http://guessthecorrelation.com/> previewed in @fig-guess-the-correlation.


```{r fig-guess-the-correlation, fig.alt="Screenshot preview of the Guess the Correlation web game, where players see a scatterplot and try to guess the correlation coefficient.", echo=FALSE, fig.cap="Preview of \"Guess the Correlation\" game.", purl=FALSE, out.width="70%", purl=FALSE}
