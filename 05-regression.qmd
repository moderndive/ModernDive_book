...
b. The total variance in `fert_rate`
c. **The expected change in `fert_rate` for a one-unit increase in `life_exp`**
...
d. The vertical distance from an observed point to the fitted line: $y_i - \hat{y}_i$
...
b. The intercept is wrong; refit the model
c. No — intercepts are meaningless
d. Mathematically yes, but the interpretation is **not meaningful in context** because no real country has `life_exp = 0`; you're extrapolating outside the data
...
b. Height *causes* weight
c. Tall people weigh exactly 2.5 kg more than short people
...
a. Make every residual exactly 0
b. Minimize the sum of the **residuals** ($\sum (y_i - \widehat{y}_i)$)
c. Maximize the slope
d. **Minimize the sum of the *squared* residuals** ($\sum (y_i - \widehat{y}_i)^2$)
...
a. **The mean weight among male players in `vball`** (with one categorical predictor, there's no $x$ to slide along — each group's prediction is just that group's mean)
b. Zero
