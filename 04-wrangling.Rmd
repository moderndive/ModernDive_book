...
So to join these two data frames so that we can identify the destination cities for example, our `inner_join()` operation will use the `by = c("dest" = "faa")` \index{dplyr!inner_join()!by} argument, which allows us to join two data frames where the key variable has a different name:
