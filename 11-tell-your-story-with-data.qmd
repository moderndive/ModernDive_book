...
```{r moderndive-figure-conclusion, echo=FALSE, out.width="100%", fig.cap="(ref:finalflowchart)"}
...
```{r pipeline-figure-conclusion, fig.cap="Data/science pipeline.", out.width="80%", echo=FALSE}
...
```{r tell-your-story-with-data-load-internal, message=FALSE, echo=FALSE}
...
```
```{r tell-your-story-with-data-glimpse-house_prices-alt, echo=FALSE}
glimpse(house_prices)
```
...
```{r house-prices-viz, echo=FALSE, message=FALSE, fig.cap="Exploratory visualizations of Seattle house prices data.", fig.height=ifelse(knitr::is_latex_output(), 4.8, 5)}
...
```{r log10-price-viz, echo=FALSE, message=FALSE, fig.cap="House price before and after log10 transformation.", fig.height=ifelse(knitr::is_latex_output(), 2.3, 4)}
p1 <- ggplot(house_prices, aes(x = price)) +
  geom_histogram(color = "white") +
  labs(x = "price (USD)", title = "House price: Before")
...
```
```{r log10-size-viz, echo=FALSE, message=FALSE, fig.cap="House size before and after log10 transformation.", fig.height=ifelse(knitr::is_latex_output(), 2.3, 4)}
...
ggplot(house_prices, 
  ...
  labs(y = "log10 price", 
       ...
       title = "House prices in Seattle")
```
```{r house-price-parallel-slopes, echo=FALSE, message=FALSE, fig.cap="Interaction and parallel slopes models."}
interaction <-
  ggplot(
    house_prices,
    aes(x = log10_size, y = log10_price, col = condition)
...
```{r house-price-interaction-2, echo=FALSE, message=FALSE, fig.cap="Faceted plot of interaction model."}
...
```{r tell-your-story-with-data-create-intercept, echo=FALSE}
...
```{r tell-your-story-with-data-dynamic-text, echo=FALSE}
# This code is used for dynamic non-static in-line text output purposes
ex_condition <- 5L
ex_size <- 1900L
...
```{r house-price-interaction-3, echo=FALSE, message=FALSE, fig.cap="Interaction model with prediction.", fig.height=ifelse(knitr::is_latex_output(), 5, 5)}
...
get_regression_table(price_interaction)
```
```{r price-inter-table-again, echo=FALSE}
price_interaction <- lm(log10_price ~ log10_size * condition,
                        data = house_prices
)
get_regression_table(price_interaction) |>
...
if(is_latex_output())
  knitr::knitr::include_graphics("images/null_housing_shaded.png")
```
...
```{r tell-your-story-with-data-v35, echo=FALSE, results="asis"}
