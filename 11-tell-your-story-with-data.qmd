...
a. Look at the average price of all houses in the data
b. **Use `predict(model, newdata = ...)` with a one-row data frame for the new house** — back-transform the predicted `log10_price` with `10^prediction` to get dollars
c. Read the slope from the regression table and multiply by 2,000
d. Take the maximum predicted price across all listings

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** `predict()` is the right tool for applying a fitted model to *new* inputs. Pass a small data frame with the same column names as the model's predictors and `predict()` returns one prediction per row. Because the response is `log10_price`, you back-transform: `10^prediction` gives a price in dollars. Adding `interval = "prediction"` returns prediction intervals around each estimate so you can quote a *range* to the stakeholder rather than a single point estimate — much more honest about the spread of plausible prices.
:::
...
library(fivethirtyeight)
library(readr)
# olympicAthletes, steves, exoplanets, volcanoes are GitHub-only; read the four
# capstone datasets from the v2 raw mirror.
