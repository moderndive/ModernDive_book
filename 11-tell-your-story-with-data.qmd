...
b. Taking the logarithm (`log10()`)
c. Subtracting the mean
d. Multiplying by 10

::: {.callout-tip collapse="true" title="Show answer"}
**(b)** Log transformation compresses large values relative to small ones, often producing a more symmetric distribution that's easier to model.
:::
...
b. To meet a regulatory requirement
c. To make the report longer
d. To accurately represent how much the estimate could vary across samples
...
a. Take the log of every variable
b. Read the data dictionary and explore the variables visually
c. Skip exploratory analysis to save time
d. Always remove all rows with any missing values
...
a. The relationship cannot be interpreted
b. Price increases by 50%
c. For every 1-square-foot increase in size, log10(price) increases by 0.5
d. **For every 10× increase in size, log10(price) increases by 0.5 — i.e., price multiplies by $10^{0.5} \approx 3.16$**

::: {.callout-tip collapse="true" title="Show answer"}
**(d)** With *both* variables on a log scale, slopes describe **multiplicative** changes: a 10× increase in $x$ predicts a $10^{\beta}$× change in $y$. This is the elasticity interpretation common in economics.
:::
...
a. To make the slope clearer
b. To increase plot file size
c. **So the audience can see the spread of individual points around the line — the line alone hides how tight or loose the relationship really is**
d. To make the plot look fancier
...
a. **Plug `log10(2000)` into the regression equation for that condition group — intercept plus slope times `log10(size)`, using the coefficients from the regression table — then back-transform with `10^` to get dollars**
b. Look at the average price of all houses in the data
c. Take the maximum predicted price across all listings
d. Read the slope from the regression table and multiply by 2,000
