...
```{r moderndive-figure-conclusion, echo=FALSE, out.width="100%", fig.cap="(ref:finalflowchart)", purl=FALSE}
...
```{r pipeline-figure-conclusion, fig.cap="Data/science pipeline.", out.width="80%", echo=FALSE, purl = FALSE}
...
house_prices |> 
  ...
  kable_styling(
    font_size = ifelse(knitr::is_latex_output(), 7.8, 16),
...
```{r house-prices-viz, echo=FALSE, message=FALSE, fig.cap="Exploratory visualizations of Seattle house prices data.", fig.height=ifelse(knitr::is_latex_output(), 4.8, 5), purl=FALSE}
...
```{r log10-price-viz, echo=FALSE, message=FALSE, fig.cap="House price before and after log10 transformation.", fig.height=ifelse(knitr::is_latex_output(), 2.3, 4), purl=FALSE}
p1 <- ggplot(house_prices, aes(x = price)) +
  geom_histogram(color = "white") +
  labs(x = "price (USD)", title = "House price: Before")
...
```
```{r log10-size-viz, echo=FALSE, message=FALSE, fig.cap="House size before and after log10 transformation.", fig.height=ifelse(knitr::is_latex_output(), 2.3, 4), purl=FALSE}
...
```{r tell-your-story-with-data-facet-viz, echo=FALSE, message=FALSE, fig.cap="Faceted plot of interaction model.", purl=FALSE}
...
```{r tell-your-story-with-data-create-colors, echo=FALSE, message=FALSE, fig.cap="Interaction model with prediction.", fig.height=ifelse(knitr::is_latex_output(), 5, 5), purl=FALSE}
...
get_regression_table(price_interaction)
```
```{r price-inter-table-again, echo=FALSE, purl=FALSE}
price_interaction <- lm(log10_price ~ log10_size * condition,
                        data = house_prices
)
get_regression_table(price_interaction) |>
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Interpret the results of the other rows in terms of inference in the `get_regression_table(price_interaction)` output in Table \@ref(tab:seattle-interaction) that we did not interpret in Subsection \@ref(house-prices-inference-for-regression).

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Create, visualize, and interpret confidence intervals using both theory-based and simulation-based approaches to mirror the hypothesis testing done in Subsection \@ref(house-prices-inference-for-regression).
...
```{r us-births, fig.cap="Number of births in the US in 1999.", fig.height=ifelse(knitr::is_latex_output(), 6.4, 7)}
...
In Figure \@ref(fig:us-births), we see a big dip occurring just before January 1st, 2000, most likely due to the holiday season. However, what about the large spike of over 14,000 births occurring just before October 1st, 1999? What could be the reason for this anomalously high spike? 
...
if(!knitr::is_latex_output()){
  cat("# (APPENDIX) Appendix {.unnumbered}")
} else {
  cat("\\appendix")
}
```
