...
Since both the outcome variable `score` and the explanatory variable `bty_avg` are numerical, we can compute summary statistics about them such as the mean, median, and standard deviation. Let’s take `evals_ch6` and select only the two variables of interest for now. However, let's instead pipe this into the `skim()` function from the `skimr` package. What this function does it quickly "skim" the data and return the following summary information about each variable. 
...
- `ordered`: Reporting whether the variable is "ordinal", in this case not. 
