...
In order to define a line like the one in Figure \@ref(fig:numxplot4), we need two quantities called *coefficients*: an intercept coefficient (the value of $y$ at $x=0$) and a slope coefficient. What are the intercept and slope coefficients of the above blue regression line? Let's get these values by outputting something called the *linear regression table* using the `get_regression_table()` function that we've included in the `moderndive` R package:
