...
## 5RM#1: Understanding Teacher Evaluations {#regressionmodel1}
...
First, let's look at a random sample of 5 out of the 463 instructors:
...
1. A numerical outcome variable $y$, in this case `score`
...
We start off by looking at the raw values. You can do this by running `View(evals)` in the console to pop-up the spreadsheet viewer, but let's use the `glimpse()`
...
First, let’s look at a random sample of 5 out of the `r nrow(gapminder2007)` countries:
...
Let's look at the raw data values. Either by bringing up RStudio's spreadsheet Viewer:
...
* The median life expectancy of the Americas is roughly 20 years greater than the baseline of Africa
* The median life expectancy of Asia is also roughly 20 years greater
* The median life expectancy of Europe and Oceania are even more different than the baseline of Africa
...
## 5RM#4: Teacher Evaluations Part II
