...
### Professor evaluations {#profevals}
...
## One numerical x: Professor evaluations {#onenumerical}
...
Let's fit a simple linear regression using the command `get_regression_table(y ~ x, data)` below:
...
```{r, eval=FALSE}
point_by_point_info <- get_regression_points(score ~ bty_avg, data = evals)
point_by_point_info
```
```{r, echo=FALSE}
set.seed(76)
...
* **residual**: the residual $y - \widehat{y}$. One example is the length of the blue arrow
...
ggplot(point_by_point_info, aes(x = bty_avg, y = residual)) +
  geom_point() +
...
ggplot(point_by_point_info, aes(x = residual)) +
...
temp <- evals
temp$`Ideal` <- rnorm(nrow(temp), 0, sd = sd(point_by_point_info$residual))
temp$`Less than ideal` <-
  rnorm(nrow(temp), 0, sd = sd(point_by_point_info$residual))^2
...
## One categorical x: Worldwide life expectancy
...
* `get_regression_table(lifeExp ~ continent, data=gapminder2007)` returns a regression table where
...
```{r, eval=FALSE}
point_by_point_info <- get_regression_points(lifeExp ~ continent, data = gapminder2007)
point_by_point_info
```
...
* We then only select the `lifeExp`, `continent`, `.fitted`, and  `residual` columns.
...
ggplot(point_by_point_info, aes(x = continent, y = residual)) +
...
1. Explanatory/predictor variable $x$ being income in $1000 units
...
## One numerical x and one categorical x: Professor evaluations
...
* $y$: outcome variable of teacher evaluation score
...
```{r numxcatxplot1, warning=FALSE, fig.cap="Teacher evaluation scores at UT Austin by gender: Jittered"}
...
```{r numxcatxplot2, echo=FALSE, warning=FALSE, fig.cap="Teacher evaluation scores at UT Austin by gender: same slope"}
...
point_by_point_info <- get_regression_points(score ~ age * gender, data = evals)
point_by_point_info
```
...
ggplot(point_by_point_info, aes(x = age, y = residual)) +
  geom_point() +
...
## How does `get_regression_table()` work?