...
Where do these different labels of outcome, dependent, predictor variable root in? Regression, in its simplest form, can be viewed in two ways:
...
One way to measure the association between two numerical variables is measuring their correlation.  In fact, the **correlation coefficient** measures the degree to which points formed by two numerical variables make a line.
...
We now unpack one possible criteria for a line to be a "best fitting line" to a set of points. Let's choose an arbitrary point on the graph and label it the color blue:
