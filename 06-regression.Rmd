...
ggplot(Credit, aes(x=Income, y=Balance, col=limit_bracket)) +
  geom_point() +
  geom_smooth(method="lm", se=FALSE) +
  labs(x="Income (in $1000)", y="Credit card balance (in $)", color="Credit limit\nbracket")
```
...
ggplot(biopics, aes(x=subject_sex, y=box_office)) +
  ...
  scale_y_log10() + 
  geom_boxplot() +
  labs(x="Subject sex", y="Box office revenue (log10-scale)", title="Person of color?")
```
...
```{r, eval=FALSE}
lm(box_office ~ person_of_color * subject_sex, data=biopics) %>% 
  tidy()
```
```{r, echo=FALSE}
lm(box_office ~ person_of_color * subject_sex, data=biopics) %>% 
