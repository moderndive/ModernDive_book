...
```{r regression-compute-mean-2, echo=FALSE}
UN_data_ch5 |>
  summarize(mean_life_exp = mean(life_exp), 
            mean_fert_rate = mean(fert_rate),
            median_life_exp = median(life_exp), 
...
```{r regression-select-vars-2, echo=FALSE}
UN_data_ch5 |> 
  select(fert_rate, life_exp) |> 
  tidy_summary() |> 
  kbl() |>
...
```{r regression-summarize-2, echo=FALSE, purl=FALSE}
cor_ch5 <- UN_data_ch5 |>
  summarize(correlation = cor(fert_rate, life_exp,
                              use = "complete.obs")) |>
  round(3) |>
...
```
```{r regression-lm-fertility-2, echo=FALSE, purl=FALSE}
...
```{r regression-lm-fertility-3, eval=FALSE}
# Fit regression model:
demographics_model <- lm(fert_rate ~ life_exp, data = UN_data_ch5)
# Get regression coefficients:
coef(demographics_model)
...
```{r regression-select-vars-3, eval=FALSE}
...
```{r regression-dynamic-text-2, echo=FALSE, purl=FALSE}
# This code is used for dynamic non-static in-line text output purposes
...
```{r regression-reg-points-2, eval=FALSE}
...
```{r regression-create-demographics_model-2, echo=FALSE}
# Fit regression model:
demographics_model <- lm(fert_rate ~ life_exp, data = UN_data_ch5)
...
```{r regression-load-packages-2, eval=FALSE}
library(broom)
library(janitor)
demographics_model |>
  augment() |>
...
```{r regression-conditional-text-2, echo=FALSE, results="asis", purl=FALSE}
if (is_latex_output()) {
...
```{r regression-conditional-text-3, echo=FALSE, purl=FALSE, results="asis"}
