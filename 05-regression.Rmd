...
The data on the `r nrow(moderndive::evals)` courses at UT Austin can be found in the `evals` data frame included in the `moderndive` package. However, to keep things simple, let's `select()` only the subset of the variables we'll consider in this chapter, and save this data in a new data frame called `evals_ch5`:
...
Observe that ``Observations: `r nrow(evals_ch5)` `` indicates that there are `r nrow(evals_ch5)` rows/observations in `evals_ch5`, where each row corresponds to one observed course at UT Austin. It is important to note that the *observational unit* \index{observational unit} is an individual course and not an individual instructor. Recall from Subsection \@ref(exploredataframes) that the observational unit is the "type of thing" that is being measured by our variables. Since instructors teach more than one course in an academic year, the same instructor will appear more than once in the data. Hence there are fewer than `r nrow(evals_ch5)` unique instructors being represented in `evals_ch5`. We'll revisit this idea in Section \@ref(regression-conditions), when we talk about the "independence assumption" for inference for regression.
...
1. `ID`: An identification variable used to distinguish between the 1 through `r nrow(evals_ch5)` courses in the dataset.
...
Now say we want to compute both the fitted value $\widehat{y} = b_0 + b_1 \cdot x$ and the residual $y - \widehat{y}$ for *all* `r nrow(evals_ch5)` courses in the study. Recall that each course corresponds to one of the `r nrow(evals_ch5)` rows in the `evals_ch5` data frame and also one of the `r nrow(evals_ch5)` points in the regression plot in Figure \@ref(fig:numxplot4).

We could repeat the previous calculations we performed by hand `r nrow(evals_ch5)` times, but that would be tedious and time consuming. Instead, let's do this using a computer with the `get_regression_points()` function. Just like the `get_regression_table()` function, the `get_regression_points()` function is a "wrapper" function. However, this function returns a different output. Let's apply the `get_regression_points()` function to `score_model`, which is where we saved our `lm()` model in the previous section. In Table \@ref(tab:regression-points-1) we present the results of only the 21st through 24th courses for brevity's sake.
...
* `score` = `r regression_points$score[24]` is the observed teaching `score` $y$ for this course's instructor.
* `bty_avg` = `r regression_points$bty_avg[24] %>% format(round(2), nsmall = 2)` is the value of the explanatory variable `bty_avg` $x$ for this course's instructor.
* `score_hat` = `r regression_points$score_hat[24] %>% format(round(2), nsmall = 2)` = `r evals_line[1]` + `r evals_line[2]` $\cdot$ `r regression_points$bty_avg[24] %>% format(round(2), nsmall = 2)` is the fitted value $\widehat{y}$ on the regression line for this course's instructor.
* `residual` = `r regression_points$residual[24]` =  `r regression_points$score[24]` - `r regression_points$score_hat[24] %>% format(round(2), nsmall = 2)` is the value of the residual for this instructor. In other words, the model's fitted value was off by `r regression_points$residual[24]` teaching score units for this course's instructor.
...
To answer such questions, we'll use the `gapminder` data frame included in the `gapminder` \index{R packages!gapminder!gapminder data frame} package. This dataset has international development statistics such as life expectancy, GDP per capita, and population for `r gapminder %>% select(country) %>% n_distinct()` countries for 5-year intervals between 1952 and 2007. Recall we visualized some of this data in Figure \@ref(fig:gapminder) in Subsection \@ref(gapminder) on the grammar of graphics.
...
The data on the `r gapminder %>% select(country) %>% n_distinct()` countries can be found in the `gapminder` data frame included in the `gapminder` package. However, to keep things simple, let's `filter()` for only those observations/rows corresponding to the year 2007. Additionally, let's `select()` only the subset of the variables we'll consider in this chapter. We'll save this data in a new data frame called `gapminder2007`:
...
Observe that ``Observations: `r nrow(gapminder2007)` `` indicates that there are `r nrow(gapminder2007)` rows/observations in `gapminder2007`, where each row corresponds to one country. In other words, the *observational unit* is an individual country. Furthermore, observe that the variable `continent` is of type `<fct>`, which stands for _factor_, which is R's way of encoding categorical variables.
...
Now say we repeated this process of computing residuals for all `r nrow(evals_ch5)` courses' instructors, then we squared all the residuals, and then we summed them. We call this quantity the *sum of squared residuals*\index{sum of squared residuals}; it is a measure of the _lack of fit_ of a model. Larger values of the sum of squared residuals indicate a bigger lack of fit. This corresponds to a worse fitting model.
...
Furthermore, of all possible lines we can draw through the cloud of `r nrow(evals_ch5)` points, the regression line minimizes this value. In other words, the regression and its corresponding fitted values $\widehat{y}$ minimizes the sum of the squared residuals:
