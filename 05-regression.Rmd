...
1. For **prediction** when we want to determine, based on the observed values of the regressors what will the value of the response before this value has been observed. We are not concerned about how all the regressors relate and interact with one another or with the response, we simply want as good predictions as possible.

As and illustration, assume that we want to study lung cancer and its relationship with potential risk factors such as smoking habits, age, and socioeconomic status. 
The response is lung cancer and the regressors are the risk factors. 
If we use regression for explanation, we may want to determine whether reducing smoking consumption has a real effect in the probability of getting lung cancer, or by how much this probability changes if an individual reduces the smoking consumption by half. 
This information may help targeting smokers of a specific age group with advertising for smoking cessation programs. 
On the other hand, if we use a regression for prediction, we would like to determine, as accurately as possible, the probability that a given individual will contract cancer based on the data collected about this individual in terms of cigarettes smoked per day, age, and socioeconomic status. 
In this book, we use regression for explanation.
...
Linear regression is easy to understand but it is also the foundation for almost any other type of regression or related method.
...
In Chapter \@ref(multiple-regression) on multiple regression, we extend these ideas and work with models with two explanatory variables.  In Section \@ref(model4), we work with two numerical explanatory variables. In Section \@ref(model3), we work with one numerical and one categorical explanatory variable and study model with and without interactions.
...
We begin with regression with a single explanatory variable, \index{regression!basic}. We also introduce the *correlation coefficient*, discuss about "correlation versus causation," and determine whether the model *fits* the data observed.
...
UN_data_ch5 <- un_member_states_2024 |>
  select(iso, life_exp = life_expectancy_2022, 
...
A full description of all the variables included in `un_member_states_2024` can be found by reading the associated help file (run `?un_member_states_2024` in the console). However, we fully describe only the `r UN_data_ch5 |> ncol()` variables we selected in `UN_data_ch5`:
...
Typing out all these summary statistic functions in `summarize()` would be long and tedious. Instead, we use the convenient `skim()` function from the `skimr` \index{R packages!skimr!skim()} package. This function takes in a data frame, "skims" it, and returns commonly used summary statistics. We take our `UN_data_ch5` data frame, `select()` only the outcome and explanatory variables `fert_rate` and `life_exp`, and pipe them into the `skim()` function:
...
1 fert_rate          0             1  2.50 1.15  1.1  1.6  2    3.2  6.6 ▇▃▂▁▁
2 life_exp         0             1 73.6  6.80 53.6 69.4 75.1 78.3 86.4 ▁▂▅▇▃
```

For the numerical variables `fert_rate` and `life_exp` it returns:
...
fert_skim_df <- skim_df |> 
  filter(skim_variable == "fert_rate") |> 
  select(-skim_type)
life_skim_df <- skim_df |>
  filter(skim_variable == "life_exp") |>
  select(-skim_type)
```
...
* Square: The *fitted value* $\widehat{y}$ is the value on the regression line for $x$ = `life_exp` = `r x`. This value is computed using the intercept and slope in the previous regression table: 
...
life_exp_24 <- regression_points$life_exp[24] |> format(round(2), nsmall = 2)
fert_rate_hat_24 <- regression_points$fert_rate_hat[24] |> format(round(2), nsmall = 2)
...
<!--
Lastly, you might be wondering what the remaining five columns in Table \@ref(tab:regtable) are: `std_error`, `statistic`, `p_value`, `lower_ci` and `upper_ci`. They are the _standard error_, _test statistic_, _p-value_, _lower 95% confidence interval bound_, and _upper 95% confidence interval bound_. They tell us about both the *statistical significance* and *practical significance* of our results. This is loosely the "meaningfulness" of our results from a statistical perspective. We put aside these ideas for now and revisit them in Chapter \@ref(inference-for-regression) on (statistical) inference for regression. We'll do this after we've had a chance to cover standard errors in Chapter \@ref(sampling), confidence intervals in Chapter \@ref(confidence-intervals), and hypothesis testing and $p$-values in Chapter \@ref(hypothesis-testing).
-->
...
A full description of all the variables included in `un_member_states_2024` can be found by reading the associated help file (run `?un_member_states_2024` in the console). However, we fully describe only the `r ncol(un_member_states_2024)` variables we selected in `gapminder2022`:
...
We look at a random sample of five out of the `r gapminder2022_rows` countries in Table \@ref(tab:model2-data-preview). 
...
Note that random sampling will likely produce a different subset of 5 rows for you than what's shown. Now that we have looked at the raw values in our `gapminder2022` data frame and got a sense of the data, we move on to computing summary statistics. We once again apply the `skim()` function from the `skimr` package. Recall from our previous EDA that this function takes in a data frame, "skims" it, and returns commonly used summary statistics. We take our `gapminder2022` data frame, `select()` only the outcome and explanatory variables `life_exp` and `continent`, and pipe them into the `skim()` function:
...
── Variable type: numeric ───────────────────────────────────────────────────────────────────────────────────────────────────────────────
  skim_variable        n_missing complete_rate mean   sd   p0  p25  p50  p75 p100 hist 
1 life_exp         0             1 73.8 6.93 53.6 69.4 75.2 78.4 89.6 ▁▃▆▇▂
```
...
```{r, echo=FALSE, purl=FALSE}
# This code is used for dynamic non-static in-line text output purposes
median_life_exp <- life_exp_worldwide$median |> round(2)
mean_life_exp <- life_exp_worldwide$mean |> round(2)
```
...
We compute the median and mean life expectancy for each continent with a little more data wrangling and display the results in Table \@ref(tab:catxplot0).
...
We use our data wrangling tools from Chapter \@ref(wrangling) to compute the sum of squared residuals exactly:
