...
1. The `moderndive` \index{R packages!moderndive} package of datasets and functions for tidyverse-friendly introductory linear regression as well as a data frame summary function.
...
Looking at this output, we can see how the values of both variables distribute. For example, the median fertility rate was `r fert_summary_df$median[1]`, whereas the median life expectancy was `r life_summary_df$median[1]` years. Furthermore, the middle 50% of fertility rates was between `r fert_summary_df$Q1[[1]]` and `r fert_summary_df$Q3[[1]]` (the first and third quartiles), whereas the middle 50% of life expectancies falls within `r life_summary_df$Q1[[1]]` to `r life_summary_df$Q3[[1]]`.
...
<!-- For regression table output. Move to Chapter 10 since it more closely aligns with inferential techniques than exploratory ones -->
...
- `group`: Breaks down a categorical variable into its unique levels. For this variable, it corresponding to Africa, Asia, North and South America, Europe, and Oceania. 
