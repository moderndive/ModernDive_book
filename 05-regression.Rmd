(ref:moderndivepart) Data Modeling via `moderndive`

```{r echo=FALSE, results="asis"}
if(knitr::is_latex_output()){
  cat("# (PART) (ref:moderndivepart) {-}")
} else {
  cat("# (PART) Data Modeling via moderndive {-} ")
}
```
...
(Note that for formatting purposes, the inline histogram that is usually printed with `skim()` has been removed. This removal can be done by running `skim_with(numeric = list(hist = NULL))` prior to using the `skim()` function as well.)
...
```{r numxplot1, warning=FALSE, echo=FALSE, fig.cap="Instructor evaluation scores at UT Austin.", fig.height=2.5}
# Define orange box
margin_x <- 0.15
margin_y <- 0.075
box <- tibble(
