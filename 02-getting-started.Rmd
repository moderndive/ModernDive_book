...
* R is like a car's engine. \index{R}
* RStudio is like a car's dashboard. \index{RStudio}
...
* Conditionals: \index{conditionals}
    ...
    + Logical operators: `&` representing "and", `|` representing "or". Ex: `(2 + 1 == 3) & (2 + 1 == 4)` returns `FALSE` while `(2 + 1 == 3) | (2 + 1 == 4)` returns `TRUE`. \index{logical operators}
...
* **Errors**: \index{R!errors} When the red text is a legitimate error, it will be prefaced with "Error in…" and try to explain what went wrong. Generally when there's an error, the code will not run. For example, as shown in Subsection \@ref(package-use) below if you see `Error in ggplot(...) : could not find function "ggplot"`, it means that the `ggplot()` function is not accessible because the package was not loaded with `library(ggplot2)`, and thus you cannot use it.
...
If after running the above code, a blinking cursor returns next to the `>` "prompt" sign, it means you were successful and the `ggplot2` package is now loaded and ready to use. If however, you get a red "error message" that reads... \index{R package!loading error}
...
Lastly, the `$` operator \index{operators!$} allows us to extract and then explore a single variable within a data frame. For example, run the following in your console
...
<center>
![](images/getting-used-to-R.png){ height=3.5in }
</center>
